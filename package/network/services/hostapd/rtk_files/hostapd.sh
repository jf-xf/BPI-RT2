...
hostapd_append_wpa_key_mgmt() {
	...
	rtk_hostapd_append_wpa_key_mgmt "$ifname"
}
...
hostapd_set_bss_options() {
	...
	json_get_vars \
		...
		multi_ap multi_ap_backhaul_ssid multi_ap_backhaul_key mbo skip_inactivity_poll \
	...
	set_default mbo 0
	[ -n "$mbo" ] && append bss_conf "mbo=$mbo" "$N"
	...
	if [ "$wpa" -ge "1" ]; then
		...
		hostapd_append_wpa_key_mgmt "$ifname"
		[ -n "$wpa_key_mgmt" ] && append bss_conf "wpa_key_mgmt=$wpa_key_mgmt" "$N"
		if [[ "$wpa_key_mgmt" = *"DPP"* ]]; then
			# dpp_controller pkhash should be the hashed peer bootstrap key
			# for now this value is hard-coded as it is unnecessary to get the actual value
			local dpp_controller="ipaddr=127.0.0.1 pkhash=$pkhash"
			append bss_conf "dpp_controller=$dpp_controller" "$N"

			local dpp_conn=`cat "/tmp/dpp_${ifname}_conn"`
			if [ -n "$dpp_conn" ]; then
					append bss_conf "dpp_connector=$dpp_conn" "$N"
			fi

			local dpp_netaccess_key=`cat "/tmp/dpp_netaccess_key"`
			if [ -n "$dpp_netaccess_key" ]; then
					append bss_conf "dpp_netaccesskey=$dpp_netaccess_key" "$N"
			fi

			local dpp_csign=`cat "/tmp/dpp_csign_key"`
			if [ -n "$dpp_csign" ]; then
					append bss_conf "dpp_csign=$dpp_csign" "$N"
			fi
		fi
	fi
...
wpa_supplicant_prepare_interface() {
	...
	json_get_vars mode wds multi_ap sae_pwe
	...
	local sae_pwe_str=
	if [ $auth_type == "sae" -a "$sae_pwe" -gt 0 ]; then
		sae_pwe_str="sae_pwe=$sae_pwe"
	fi
...
update_config=1
config_methods="virtual_display keypad physical_push_button virtual_push_button"
EOF
	return 0
}
...
wpa_supplicant_add_network() {
	...
	local dpp_conn=`cat "/tmp/dpp_${ifname}_conn"`
	if [ -n "$dpp_conn" ]; then
		append network_data "dpp_connector=\"$dpp_conn\"" "$N$T"
	fi

	local dpp_netaccess_key=`cat "/tmp/dpp_netaccess_key"`
	if [ -n "$dpp_netaccess_key" ]; then
		append network_data "dpp_netaccesskey=$dpp_netaccess_key" "$N$T"
	fi

	local dpp_csign=`cat "/tmp/dpp_csign_key"`
	if [ -n "$dpp_csign" ]; then
		append network_data "dpp_csign=$dpp_csign" "$N$T"
	fi

	local dpp_pp_key=`cat "/tmp/dpp_pp_key"`
	if [ -n "$dpp_pp_key" ]; then
		append network_data "dpp_pp_key=$dpp_pp_key" "$N$T"
	fi
