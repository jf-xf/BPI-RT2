...
WPS_LED_TIMER_FILE="/etc/wpa_supplicant_wps_timer.sh"
...
SYSTEM_WPS_STOP="SYSTEM-WPS-STOP"
...
$WPS_EVENT_FAIL)
	...
	;;
$SYSTEM_WPS_STOP)
	if  [ -f $WPS_LED_INTERFACE ]; then
		wps_reset
		rm $WPS_LED_INTERFACE
		flag=$LED_WSC_TIMEOUT
	else
		exit 0
	fi
...
;'
if [ $flag == $LED_WSC_SUCCESS ]; then
	if [ -z "$SYSTEMD_USOCK_SERVER" ]; then
		. /usr/share/libubox/jshn.sh
		json_init
		json_add_string wlan_inf "$1"
		ubus call $UBUS_OBJ_NAME do_wlan_wpas_wps_success "$(json_dump)"
	else
		sysconf send_unix_sock_message $SYSTEMD_USOCK_SERVER do_wlan_wpas_wps_success $1
	fi
fi
...
if [ -z "$SYSTEMD_USOCK_SERVER" ]; then
	...
	ubus call $UBUS_OBJ_NAME do_wlan_store_wpas_wps_status "$(json_dump)"
else
	sysconf send_unix_sock_message $SYSTEMD_USOCK_SERVER do_wlan_store_wpas_wps_status $1 $2
fi
'
