...
ppp_generic_init_config() {
	proto_config_add_boolean pppdbg
	...
	proto_config_add_string ip4table2
}

ppp_generic_setup() {
	local config="$1"; shift
	local localip
	local opt_file=/etc/ppp/options.$config
	file_reset $opt_file
	...
	json_get_var ip4table ip4table
	json_get_var ip4table2 ip4table2
	...
	xappend $opt_file nodetach
	xappend $opt_file ipparam "$config"
	xappend $opt_file ifname "$pppname"
	[ -z $localip ] || xappend $opt_file $localip:
	[ -z $lcp_interval ] || xappend $opt_file lcp-echo-interval $lcp_interval
	[ -z $lcp_failure ] || xappend $opt_file lcp-echo-failure $lcp_failure
	[ -z $lcp_adaptive ] || xappend $opt_file $lcp_adaptive
	[ -z $no_ipv4 ] || xappend $opt_file noip
	[ -z $ipv6 ] || xappend $opt_file +ipv6
	[ -z $no_ipv6 ] || xappend $opt_file noipv6
	if [ -n "$demand" ]; then
		# remote LL address required for demand-dialling for IPv6 only mode
		[ -z $no_ipv4 ] || xappend $opt_file "ipv6 ,::123"
		if [ "$ip4table" = "main" -o "$ip4table2" = "main" ]; then
			xappend $opt_file defaultroute
		else
			xappend $opt_file nodefaultroute
		fi
	else
		xappend $opt_file nodefaultroute
	fi
	xappend $opt_file usepeerdns
	[ -z $persist ] || xappend $opt_file $persist
	[ -z $maxfail ] || xappend $opt_file maxfail $maxfail
	[ -z $holdoff ] || xappend $opt_file holdoff $holdoff
	case $rtk_ppp_authmode in
		pap)
			xappend $opt_file refuse-chap
			xappend $opt_file refuse-mschap
			xappend $opt_file refuse-mschap-v2
			xappend $opt_file refuse-eap
			xappend $opt_file "nomppe"
		;;
		chap)
			xappend $opt_file refuse-pap
			xappend $opt_file refuse-mschap
			xappend $opt_file refuse-mschap-v2
			xappend $opt_file refuse-eap
			xappend $opt_file "nomppe"
		...
		mschap)
			...
			xappend $opt_file refuse-mschap-v2
			xappend $opt_file refuse-eap
			[ -z $rtk_ppp_mppe ] || xappend $opt_file $rtk_ppp_mppe
		;;
		mschapv2)
			xappend $opt_file refuse-pap
			xappend $opt_file refuse-chap
	...
	[ $pppdbg -eq 1 ] && xappend $opt_file debug dump
	[ -z $username ] || xappend $opt_file user $username
	[ -z $password ] || xappend $opt_file password $password
	[ -z $connect ] || xappend $opt_file connect $connect
	[ -z $disconnect ] || xappend $opt_file disconnect $disconnect
	[ -z $ipv4 ] || xappend $opt_file ip-up-script /lib/netifd/ppp-up
	[ -z $ipv4 ] || xappend $opt_file ip-down-script /lib/netifd/ppp-down
	[ -z $ipv6 ] || xappend $opt_file ipv6-up-script /lib/netifd/ppp6-up
	[ -z $ipv6 ] || xappend $opt_file ipv6-down-script /lib/netifd/ppp-down
	[ -z $mtu ] || xappend $opt_file mtu $mtu
	[ -z $mtu ] || xappend $opt_file mru $mtu
