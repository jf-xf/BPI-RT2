...
proto_dhcp_init_config() {
	...
	proto_config_add_array 'reqopts:list(string)'
...
proto_dhcp_setup() {
	...
	json_get_vars ipaddr hostname clientid vendorid broadcast norelease defaultreqopts iface6rd delegate zone6rd zone mtu6rd customroutes classlessroute

	local opt dhcpopts
	json_for_each_item proto_dhcp_add_reqopts reqopts dhcpopts
