...
int8
_phy_826x_is_8261(uint32 unit, rtk_port_t port)
{
    int32   ret = RT_ERR_OK;
    uint32  phyData;
...
int32
_phy_826x_eeep_enable_set(uint32 unit, rtk_port_t port, rtk_enable_t enable)
{
    int32   ret = RT_ERR_OK;
    uint32  phyData = 0;
...
int32
_phy_826x_eeep_enable_get(uint32 unit, rtk_port_t port, rtk_enable_t *pEnable)
{
    int32   ret = RT_ERR_OK;
    uint32  phyData = 0;
...
_phy_826x_synce_init(uint32 unit, rtk_port_t port)
{
    int32   ret = RT_ERR_OK;
    uint32  phyData = 0;
    uint32  is_rtl8261 = _phy_826x_is_8261(unit, port);
...
_phy_826x_synce_recovery_phy_set(uint32 unit, rtk_port_t port, uint32 synce_id, uint32 phy)
{
    ...
    if (_phy_826x_is_8261(unit, port) == TRUE)
...
phy_826x_rtctResult_get(uint32 unit, rtk_port_t port, rtk_rtctResult_t *pRtctResult)
{
    ...
    cable_factor = rtl826x_info[unit]->cable_factor[port];
    /* use default cable_factor */
    if (cable_factor == 0)
    {
...
int32
_phy_826x_preemphasis_mode(uint32 unit, rtk_port_t port, uint32 *mode)
{
    int32   ret = 0;
    uint32  phyData1 = 0, phyData2 = 0, phyData3 = 0;

    if ((ret = _phy_826x_serdes_reg_read(unit, port, 31, 9, &phyData1)) != RT_ERR_OK)
        return ret;
    if ((ret = phy_common_general_reg_mmd_get(unit, port, PHY_MMD_VEND1, 0xC1, &phyData2)) != RT_ERR_OK)
        return ret;
    phyData2 &= BIT_4;
    if ((ret = phy_common_general_reg_mmd_get(unit, port, PHY_MMD_VEND1, 0xC2, &phyData3)) != RT_ERR_OK)
        return ret;
    phyData3 &= 0x1F;

    if ((phyData1 == 0x0d40) && (phyData2 == 0))
    {
        *mode = 0;
    }
    else if ((phyData1 == 0x0d40) && (phyData2 == BIT_4) && (phyData3 == 0x1A))
    {
        *mode = 1;
    }
    else if ((phyData1 == 0x0ac0) && (phyData2 == 0))
    {
        *mode = 2;
    }
    else if ((phyData1 == 0x0b40) && (phyData2 == 0))
    {
        *mode = 3;
    }
    else if ((phyData1 == 0x0240) && (phyData2 == 0))
    {
        *mode = 4;
    }
    else
...
_phy_826x_preemphasis_set(uint32 unit, rtk_port_t port, uint32 param)
{
    ...
    uint32  sdsReg[5] = {46, 52, 44, 40, 36};
    uint32  i = 0;
    ...
    ret = _phy_826x_preemphasis_mode(unit, port, &i);
    if (ret == RT_ERR_OK && i < 5)
    {
        if ((ret = _phy_826x_serdes_reg_read(unit, port, sdsReg[i], 7, &phyDataH)) != RT_ERR_OK)
            return ret;
        if ((ret = _phy_826x_serdes_reg_read(unit, port, sdsReg[i], 6, &phyDataL)) != RT_ERR_OK)
            return ret;

        phyDataH = (phyDataH & 0x3) |  paramH;
        phyDataL = (phyDataL & 0x3) |  paramL;

        if ((ret = _phy_826x_serdes_reg_write(unit, port, sdsReg[i], 7, phyDataH)) != RT_ERR_OK)
            return ret;
        if ((ret = _phy_826x_serdes_reg_write(unit, port, sdsReg[i], 6, phyDataL)) != RT_ERR_OK)
            return ret;
    }
    else
    {
...
_phy_826x_preemphasis_get(uint32 unit, rtk_port_t port, uint32 *pParam)
{
    ...
    if ((ret = _phy_826x_serdes_reg_read(unit, port, 46, 7, &phyDataH)) != RT_ERR_OK)
        return ret;
    if ((ret = _phy_826x_serdes_reg_read(unit, port, 46, 6, &phyDataL)) != RT_ERR_OK)
        return ret;
...
phy_826x_ctrl_set(uint32 unit, rtk_port_t port, rtk_phy_ctrl_t ctrl_type, uint32 value)
{
    ...
    switch (ctrl_type)
    {
        ...
        case RTK_PHY_CTRL_RTCT_CABLE_FACTOR:
            rtl826x_info[unit]->cable_factor[port] = value;
            break;
...
phy_826x_ctrl_get(uint32 unit, rtk_port_t port, rtk_phy_ctrl_t ctrl_type, uint32 *pValue)
{
    ...
    switch (ctrl_type)
    {
        ...
        case RTK_PHY_CTRL_RTCT_CABLE_FACTOR:
            *pValue = rtl826x_info[unit]->cable_factor[port];
            break;
