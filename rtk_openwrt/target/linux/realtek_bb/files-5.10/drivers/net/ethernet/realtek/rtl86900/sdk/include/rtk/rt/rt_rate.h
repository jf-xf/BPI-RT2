...
{
    /* share meter can be indexed by storm filtering control, host policer, flow meter, acl meter, software meter */
    RT_METER_TYPE_STORM,    /* storm filtering control for specific type of traffic */
    RT_METER_TYPE_HOST,     /* host plicer and logging for a specific host (ex: MAC address) */
    RT_METER_TYPE_FLOW,     /* flow meter */
    RT_METER_TYPE_ACL,      /* acl meter */
    RT_METER_TYPE_SW,       /* software meter for ARP request, specific dos, and SYN only */
    RT_METER_TYPE_END
} rt_rate_meter_type_t;

typedef enum rt_rate_meter_mode_e
{
    RT_METER_MODE_BIT_RATE = 0, /* byte mode */
    RT_METER_MODE_PACKET_RATE,  /* packet mode */
    RT_METER_MODE_END
