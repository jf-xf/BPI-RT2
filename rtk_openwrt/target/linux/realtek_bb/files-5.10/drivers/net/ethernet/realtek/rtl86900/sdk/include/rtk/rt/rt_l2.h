 ...
 *           (1) MAC learning limit
 *           (2) Forward action of new MAC address
 *           (3) Unicast address entry operation
 *           (4) Forward action of port moving
 *           (5) IVL/SVL mode
 *
 */
...
typedef struct rt_l2_ucastAddr_s
{
    rt_mac_t       mac;
    rt_port_t      port;
    uint32         age;         /*unit:second, the field is only valid for dynamic entry*/
    rt_enable_t    staticFlag;  /*1=> static entry, 0=> dynamic entry*/
    rt_enable_t    filterFlag;  /*1=> the mac is allowed to forward, 0=> the mac is blocked*/
    uint32         vid;
}rt_l2_ucastAddr_t;
 ...
 *      RT_ERR_L2_ENTRY_NOTFOUND - specified entry not found
 * Note:
 *      If the unicast mac address existed in LUT, it will return the port and vid where
 ...
 *      None
 * Output:
 *      pIvlEn  - IVL enable, 1=> independent VLAN(IVL) mode, 0=> share VLAN(SVL) mode
 * Return:
 *      RT_ERR_OK           - OK
 *      RT_ERR_FAILED       - Failed
 *      RT_ERR_NULL_POINTER - input parameter may be null pointer
 ...
 *      Set L2 IVL SVL mode
 * Input:
 *      ivlEn  - IVL enable ,1=> independent VLAN(IVL) mode, 0=> share VLAN(SVL) mode
 * Output:
 *      None
 * Return:
 *      RT_ERR_OK           - OK
