...
#define RTL8277C_CLS_RULE_DEFAULT_RESERVED_NUM 32
#define RTL8277C_CLS_RULE_SYSTEM_RESERVED_NUM 128
#define RTL8277C_CLS_RULE_NOT_IN_USED -1
#define RTL8277C_CLS_RULE_PORT_NOT_USED -1
#define RTL8277C_CLS_RULE_PORT_IGNORE -2
#define RTL8277C_CLS_DEFAULT_CTAG_TPID 0x8100
#define RTL8277C_CLS_DEFAULT_STAG_TPID 0x88a8
#define RTL8277C_CLS_XLATEVID_NOT_USED 0x00000000
#define RTL8277C_CLS_XLATEVID_ONLY_ONE 0x00000001
#define RTL8277C_CLS_XLATEVID_TWO_MORE 0x00000002
#define RTL8277C_CLS_XLATEVID_UNTAG    0xFFFFFFFD
#define RTL8277C_CLS_XLATEVID_INDEX_SHIFT_BASE 100
#define RTL8277C_CLS_DSCP_PORT_INDEX_BASE 100

#define DEFAULT_DEVICE_ID 0
#define RTL8277C_CLS_SID_MAX 256
#define RTL8277C_CLS_PORT_TYPE_PPTP 1
#define RTL8277C_CLS_PORT_TYPE_VEIP 0



typedef struct dal_8277c_cls_rule_index_s
{
    int32 index;
    rt_cls_dir_t dir;
    uint32 ruleIndex;
...
typedef struct {
    ...
    struct list_head list;
#endif
} dal_8277c_cls_vlan_rule_index_t;

typedef struct
{
    unsigned int vid;
    unsigned int xlateMode;
} dal_8277c_cls_vlan_xlate_info_t;
...
dal_8277c_cls_rule_index_t *clsRuleUsage;
#if defined(CONFIG_SDK_KERNEL_LINUX)
...
rt_ponmisc_sidInfo_t clsSidInfo[RTL8277C_CLS_SID_MAX];

static void _8277c_cls_rule_adjust(void);
static int32 _8277c_cls_rule_set(rt_cls_rule_t *pClsRule);



static int32 _8277c_cls_ethertype_encode_get(uint16 ethertype,uint8 *encode)
{
    int i;
...
static int32 _8277c_cls_rule_index_get(uint32 ruleIndex, rt_cls_dir_t dir,int *index, int32 port,uint32 rulePriority, int isReversedRule)
{
    int i=0,j=0,min,max;
    int ruleMax = HAL_CLASSIFY_ENTRY_MAX();
    dal_8277c_cls_rule_index_t writeData, tempData;
    memset(&writeData,0,sizeof(dal_8277c_cls_rule_index_t));
    memset(&tempData,0,sizeof(dal_8277c_cls_rule_index_t));
    ...
    if(isReversedRule)
    {
        min = RTL8277C_CLS_RULE_DEFAULT_RESERVED_NUM;
        max = RTL8277C_CLS_RULE_SYSTEM_RESERVED_NUM;
    }
    else
    {
        min = RTL8277C_CLS_RULE_SYSTEM_RESERVED_NUM;
        max = ruleMax;
    }

    for(i=min;i<max;i++)
    {
        if((RTL8277C_CLS_RULE_NOT_IN_USED != clsRuleUsage[i].index) && (clsRuleUsage[i].ruleIndex == ruleIndex) && (clsRuleUsage[i].dir == dir) && (port == clsRuleUsage[i].port))
        {
    ...
    for(i=min;i<max;i++)
    {
        ...
        if(RTL8277C_CLS_RULE_NOT_IN_USED != clsRuleUsage[i].index && clsRuleUsage[i].rulePriority < rulePriority)
        {
            writeData.dir = dir;
            ...
            *index = i;
            for(j=i;j<max;j++)
            {
                if(RTL8277C_CLS_RULE_NOT_IN_USED == clsRuleUsage[j].index )
                {
                ...
                else
                {
                    ...
                    memcpy(&writeData,&tempData,sizeof(dal_8277c_cls_rule_index_t));
                }
            }
            _8277c_cls_rule_adjust();
            if(j == max)
               return RT_ERR_FAILED;
            else
                return RT_ERR_OK;
        }
        if(RTL8277C_CLS_RULE_NOT_IN_USED == clsRuleUsage[i].index)
        {
...
static int32 _8277c_cls_rule_index_find(uint32 ruleIndex, rt_cls_dir_t dir,int *index, int32 port,int isReversedRule)
{
    int i=0,min,max;
    int ruleMax = HAL_CLASSIFY_ENTRY_MAX();
    ...
    for(i=min;i<max;i++)
    {
        if((RTL8277C_CLS_RULE_NOT_IN_USED != clsRuleUsage[i].index) && (clsRuleUsage[i].ruleIndex == ruleIndex) && (clsRuleUsage[i].dir == dir) )
        {
            if(port == RTL8277C_CLS_RULE_PORT_IGNORE || port == clsRuleUsage[i].port)
            {
                *index=clsRuleUsage[i].index;
                return RT_ERR_OK;
            }
...
static int32 _8277c_cls_rule_index_del(uint32 index)
{
    int i=0,min,max;
    int ruleMax = HAL_CLASSIFY_ENTRY_MAX();
    ...
    clsRuleUsage[index].index = RTL8277C_CLS_RULE_NOT_IN_USED;
    clsRuleUsage[index].port=RTL8277C_CLS_RULE_PORT_NOT_USED;
...
static void _8277c_cls_tpid_set(uint16 tpid)
{
...
static void _8277c_cls_rule_adjust(void)
{
    ...
    dal_8277c_cls_rule_db_t *pEntryData= NULL;

    list_for_each_safe(pEntry, pTmpEntry, &dsRuleHead)
    {
        pEntryData = list_entry(pEntry, dal_8277c_cls_rule_db_t, list);
        _8277c_cls_rule_set(&(pEntryData->rule));
    }
...
static int32 _8277c_cls_action_set(aal_l2_cf_rule_t *pEntry, rt_cls_rule_t *pClsRule)
{
    int8 isCsTag = 0;
    ...
    if(doubleCtag == 1 && filterTag == 1 && pClsRule->filterRule.innerTagTpid == RTL8277C_CLS_DEFAULT_CTAG_TPID)
    {
    ...
    switch(pClsRule->outerTagAct.tagAction)
    {
        ...
        case RT_CLS_TAG_TAGGING:
            isCsTag=1;
            if(pClsRule->outerTagAct.assignedTpid == RTL8277C_CLS_DEFAULT_STAG_TPID)
            {
            ...
            else
            {
                ...
                _8277c_cls_tpid_set(pClsRule->outerTagAct.assignedTpid);
            }
            break;
        case RT_CLS_TAG_REMOVE:
            processTag--;
            if(pClsRule->filterRule.outerTagTpid == RTL8277C_CLS_DEFAULT_CTAG_TPID && pClsRule->filterRule.innerTagTpid != RTL8277C_CLS_DEFAULT_CTAG_TPID)
            {
    ...
    switch(pClsRule->innerTagAct.tagAction)
    {
        case RT_CLS_TAG_TRANSPARENT:
            break;
        case RT_CLS_TAG_TAGGING:
            if(pClsRule->innerTagAct.assignedTpid == RTL8277C_CLS_DEFAULT_CTAG_TPID)
            {
                if(pClsRule->filterRule.innerTagTpid == RTL8277C_CLS_DEFAULT_CTAG_TPID)
                {
                    isCsTag=1;
                    pEntry->action.cs_act = AAL_L2_CF_ACTION_CS_ACT_ADD_TAG_TPID_B;
                    _8277c_cls_tpid_set(pClsRule->innerTagAct.assignedTpid);
                }
                else
                {
                    pEntry->action.cact = AAL_L2_CF_ACTION_C_ACT_ADD_CTAG;
            ...
            {
                isCsTag=1;
                if(pClsRule->innerTagAct.assignedTpid == RTL8277C_CLS_DEFAULT_STAG_TPID)
                {
                    pEntry->action.cs_act = AAL_L2_CF_ACTION_CS_ACT_ADD_TAG_STAG_TPID;
                }
                else
        ...
        case RT_CLS_TAG_MODIFY:
            ...
            if(doubleCtag == 1 && filterTag == 1 && pClsRule->filterRule.innerTagTpid == RTL8277C_CLS_DEFAULT_CTAG_TPID && pClsRule->innerTagAct.assignedTpid == RTL8277C_CLS_DEFAULT_CTAG_TPID && pClsRule->outerTagAct.tagAction == RT_CLS_TAG_TRANSPARENT)
                pEntry->action.cs_act = AAL_L2_CF_ACTION_CS_ACT_DELETE_TAG;
            break;
        case RT_CLS_TAG_REMOVE:
            if(processTag == 1)
...
static int32 _8277c_cls_filter_set(aal_l2_cf_rule_t *pEntry, rt_cls_rule_t *pClsRule)
{
    int16 innerVlan=0;
    ...
    if(RT_CLS_FILTER_OUTER_TAGIf_BIT & pClsRule->filterRule.filterMask && pClsRule->filterRule.outerTagIf == 1)
    {
        ...
        else
        {
            ...
            if(pClsRule->filterRule.outerTagTpid != RTL8277C_CLS_DEFAULT_STAG_TPID && pClsRule->filterRule.outerTagTpid == RTL8277C_CLS_DEFAULT_CTAG_TPID)
            {
            ...
            if(doubleCtag == 0 && pClsRule->filterRule.outerTagTpid == RTL8277C_CLS_DEFAULT_CTAG_TPID)
            {
                doubleCtag = 1;
    ...
    if(RT_CLS_FILTER_INNER_TAGIf_BIT & pClsRule->filterRule.filterMask && pClsRule->filterRule.innerTagIf == 1)
    {
        ...
        else
        {
            stag=1;
            /*change default stag tpid*/
            if(pClsRule->filterRule.innerTagTpid != RTL8277C_CLS_DEFAULT_STAG_TPID)
            {
                memset(&ilpbCfgMask,0,sizeof(aal_ilpb_cfg_msk_t));
                ilpbCfgMask.s.s_tpid_match = 1;
                for(portIndex = 0; portIndex < RTK_MAX_NUM_OF_PORTS; portIndex++)
    ...
    {
        if(RT_ERR_OK != _8277c_cls_ethertype_encode_get(pClsRule->filterRule.etherType, &ethertypeEncode))
        {
...
static void _8277c_cls_vlan_aggregate_cf_delAll(void)
{
#if defined(CONFIG_SDK_KERNEL_LINUX)
    int32 index;
    struct list_head *cur_next = NULL;
    struct list_head *cur_tmp = NULL;
    dal_8277c_cls_vlan_rule_index_t *cur  = NULL;
    int32 ret=RT_ERR_FAILED;

    list_for_each_safe(cur_next, cur_tmp, &vlanRuleIndexHead)
    {
        cur = list_entry(cur_next, dal_8277c_cls_vlan_rule_index_t, list);

        if(_8277c_cls_rule_index_find(cur->index+RTL8277C_CLS_XLATEVID_INDEX_SHIFT_BASE, RT_CLS_DIR_DS, &index,RTL8277C_CLS_RULE_PORT_IGNORE, 1) == RT_ERR_OK)
        {
            ...
            ret = _8277c_cls_rule_index_del(index);
            if(ret != RT_ERR_OK)
            {
...
static void _8277c_cls_vlan_aggregate_fill_outerVID(unsigned int *pOuterVID, rt_cls_rule_t *pDsVlanRule)
{
    ...
    {
        *pOuterVID = RTL8277C_CLS_XLATEVID_UNTAG;
    }
    else
    {
        *pOuterVID = RTL8277C_CLS_XLATEVID_NOT_USED;
    }
}

static void _8277c_cls_vlan_aggregate_fill_xlateVID(dal_8277c_cls_vlan_xlate_info_t *pXlateVID,
                                              rt_cls_rule_t *pDsVlanRule,
                                              unsigned int outerVID,
                                              unsigned int uniMask)
{
    uint32 xlate_cvid = RTL8277C_CLS_XLATEVID_UNTAG;
    uint32 port;
    ...
    if((pDsVlanRule->filterRule.filterMask & RT_CLS_FILTER_OUTER_TAGIf_BIT) && pDsVlanRule->filterRule.outerTagIf == 1)
    {
        ...
        {
            xlate_cvid = RTL8277C_CLS_XLATEVID_UNTAG;
        }
    ...
    for(port = 0; port < RTK_MAX_NUM_OF_PORTS; port++)
    {
        if (uniMask & (0x0001 << port))
        {
            ...
            if (pXlateVID[port].xlateMode == RTL8277C_CLS_XLATEVID_NOT_USED)
            {
                pXlateVID[port].xlateMode = RTL8277C_CLS_XLATEVID_ONLY_ONE;
                pXlateVID[port].vid = xlate_cvid;
            }
            else
            {
                pXlateVID[port].xlateMode = RTL8277C_CLS_XLATEVID_TWO_MORE;

                /* if more than one VID, we record untag first, than smallest VID */
                if ( (xlate_cvid == RTL8277C_CLS_XLATEVID_UNTAG) || (pXlateVID[port].vid > xlate_cvid) )
                    pXlateVID[port].vid = xlate_cvid;
            }
        }
    }
...
static int _8277c_cls_is_same_rule_by_ds_vlan_filter(
                                               rt_cls_rule_t *pBR,
                                               rt_cls_rule_t *pL2)
{
...
static int32 _8277c_cls_vlan_aggregate_cf_set(rt_cls_rule_t *pClsRule,int portIndex)
{
    ...
    ret = _8277c_cls_filter_set(&entry,pClsRule);
    if(ret != RT_ERR_OK)
    {
    ...
    ret = _8277c_cls_action_set(&entry,pClsRule);
    if(ret != RT_ERR_OK)
    {
    ...
    ret = _8277c_cls_rule_index_get(pClsRule->index+RTL8277C_CLS_XLATEVID_INDEX_SHIFT_BASE, pClsRule->direction, &index, portIndex, 0, 1);
    if(ret != RT_ERR_OK)
    {
...
static int _8277c_cls_l2_vlan_aggregate(void)
{
#if defined(CONFIG_SDK_KERNEL_LINUX)
    struct list_head *cur_next = NULL;
    struct list_head *cur_tmp = NULL;
    dal_8277c_cls_rule_db_t *cur = NULL;
    struct list_head *tar_next = NULL;
    struct list_head *tar_tmp = NULL;
    dal_8277c_cls_rule_db_t *tar = NULL;
    uint32 aggrUniMask;
    uint32 setupAggVlanCf;
    int32 ret;
    uint32 outerVID;
    dal_8277c_cls_vlan_xlate_info_t pXlateVID[RTK_MAX_NUM_OF_PORTS];
    uint32 i;
    dal_8277c_cls_vlan_rule_index_t *pVlanRuleIndexEntry;

    /* Delete all exist L2 Aggregated VLAN CF rule & clear database*/
    _8277c_cls_vlan_aggregate_cf_delAll();
    /* Serach all service flow to find out any aggregated service */
    list_for_each_safe(cur_next, cur_tmp, &dsRuleHead)
    {
        cur = list_entry(cur_next, dal_8277c_cls_rule_db_t, list);
        ...
        {
            pXlateVID[i].xlateMode = RTL8277C_CLS_XLATEVID_NOT_USED;
            pXlateVID[i].vid = 0;
        }
        _8277c_cls_vlan_aggregate_fill_outerVID(&outerVID, &cur->rule);
        _8277c_cls_vlan_aggregate_fill_xlateVID(pXlateVID, &cur->rule, outerVID, cur->rule.ingressPortMask);
        list_for_each_safe(tar_next, tar_tmp, &dsRuleHead)
        {
            tar = list_entry(tar_next, dal_8277c_cls_rule_db_t, list);
            ...
            {
                if (_8277c_cls_is_same_rule_by_ds_vlan_filter( &cur->rule, &tar->rule) == TRUE)
                {
                    /* Same DS VLAN Filter */
                    ...
                    if (cur->rule.index < tar->rule.index )
                    {
                        ...
                        _8277c_cls_vlan_aggregate_fill_xlateVID(pXlateVID, &(tar->rule), outerVID, tar->rule.ingressPortMask);
                    }
                    else
                    {
        ...
        if (setupAggVlanCf == TRUE)
        {
            ...
            if ((ret = _8277c_cls_vlan_aggregate_cf_set(  &(cur->rule), i)) != RT_ERR_OK)
                printk("omci_set_vlan_aggregate_cf Fail, ret = 0x%X", ret);
            pVlanRuleIndexEntry=kmalloc(sizeof(dal_8277c_cls_vlan_rule_index_t), GFP_KERNEL);
            ...
            for(i = 0; i < RTK_MAX_NUM_OF_PORTS; i++)
            {
                if(HAL_IS_CPU_PORT(i))
                    continue;
                if (pXlateVID[i].xlateMode != RTL8277C_CLS_XLATEVID_NOT_USED)
                {
                    if ((ret = _8277c_cls_vlan_aggregate_cf_set(  &(cur->rule), i)) != RT_ERR_OK)
                        printk("omci_set_vlan_aggregate_cf Fail, ret = 0x%X", ret);
                    pVlanRuleIndexEntry=kmalloc(sizeof(dal_8277c_cls_vlan_rule_index_t), GFP_KERNEL);
...
static int32 _8277c_cls_rule_set(rt_cls_rule_t *pClsRule)
{
    ...
    int32 ret=RT_ERR_OK;
    dal_8277c_cls_rule_db_t *pNewDsRule=NULL;
        ...
        if(pClsRule->ingressPortMask & (1 << portIndex))
        {
            ...
            if(RT_CLS_FILTER_DSCP_BIT & pClsRule->filterRule.filterMask)
            {
                for(dscp=0;dscp<=RTK_VALUE_OF_DSCP_MAX;dscp++)
                {
                    ...
                    {
                        ret = _8277c_cls_rule_index_get(pClsRule->index, pClsRule->direction, &index, (portIndex+1)*RTL8277C_CLS_DSCP_PORT_INDEX_BASE + dscp, pClsRule->rulePriority, 0);
                        if(ret != RT_ERR_OK)
                        {
            ...
            {
                ret = _8277c_cls_rule_index_get(pClsRule->index, pClsRule->direction, &index, portIndex, pClsRule->rulePriority, 0);
                if(ret != RT_ERR_OK)
                {
                    printk(KERN_ERR"[%s] %d Get cls rule index fail ret:%d ruleIndex:%d dir:%d"
                        ,__FUNCTION__, __LINE__, ret, pClsRule->index, pClsRule->direction);
    ...
    if(pClsRule->direction == RT_CLS_DIR_DS && pClsRule->ingressPortMask == 0)
    {
        ...
        ret = _8277c_cls_rule_index_get(pClsRule->index, pClsRule->direction, &index, 0, pClsRule->rulePriority, 0);
        if(ret != RT_ERR_OK)
        {
            printk(KERN_ERR"[%s] %d Get cls rule index fail ret:%d ruleIndex:%d dir:%d"
                ,__FUNCTION__, __LINE__, ret, pClsRule->index, pClsRule->direction);
    ...
    if(pClsRule->direction == RT_CLS_DIR_DS)
    {
        ...
        ret = _8277c_cls_rule_index_get(pClsRule->index, pClsRule->direction, &index, RTL8277C_CLS_RULE_PORT_IGNORE, pClsRule->rulePriority, 0);
...
int32 dal_rt_rtl8277c_cls_init(void)
{
    ...
    aal_ilpb_cfg_msk_t ilpbCfgMask;
#if defined(CONFIG_SDK_KERNEL_LINUX)
    clsRuleUsage=(dal_8277c_cls_rule_index_t *)kzalloc(sizeof(dal_8277c_cls_rule_index_t)*ruleMax, GFP_KERNEL);
#endif
    memset(clsRuleUsage,0,sizeof(dal_8277c_cls_rule_index_t)*ruleMax);
    for(i=0;i<ruleMax;i++)
    {
        clsRuleUsage[i].index = RTL8277C_CLS_RULE_NOT_IN_USED;
        clsRuleUsage[i].port = RTL8277C_CLS_RULE_PORT_NOT_USED;
    }
#if defined(CONFIG_SDK_KERNEL_LINUX)
...
#endif
    memset(&clsSidInfo, 0, sizeof(rt_ponmisc_sidInfo_t)*RTL8277C_CLS_SID_MAX);
    ...
    portFwdPort[RTL8277C_CLS_EXT_WAN_PORT] = AAL_LPORT_L3_WAN;
    ...
    aal_port_ilpb_cfg_set(0, RTL8277C_CLS_EXT_WAN_PORT, ilpbCfgMask, &ilpbCfg);
...
#endif
    {
        ...
        dal_rt_rtl8277c_cls_veipPortRule_set(port, ENABLED);
    }

    /* Rule 10/11: PON VEIP port default rule*/
    portRuleIndex[HAL_GET_PON_PORT()] = index++;
    portRuleIndex[RTL8277C_CLS_EXT_WAN_PORT] = index++;
    dal_rt_rtl8277c_cls_veipPortRule_set(HAL_GET_PON_PORT(), ENABLED);
    ...
    if(index > RTL8277C_CLS_RULE_DEFAULT_RESERVED_NUM)
    {
        printk("Default reserve rule need %d rules, but only reserve %d rules\n",index-1,RTL8277C_CLS_RULE_DEFAULT_RESERVED_NUM);
    }
...
int32 dal_rt_rtl8277c_cls_veipPortRule_set(rtk_port_t port, rtk_enable_t state)
{
    aal_l2_cf_rule_t ruleCfg;
    ...
    if(state == ENABLED)
    {
        ...
        portType[port] = RTL8277C_CLS_PORT_TYPE_VEIP;
    }
    else
    {
        aal_l2_cf_rule_delete(DEFAULT_DEVICE_ID, portRuleIndex[port]);
        portType[port] = RTL8277C_CLS_PORT_TYPE_PPTP;
    }
    ...
    if(port == HAL_GET_PON_PORT())
    {
        if(state == ENABLED)
        {
            ...
            ruleCfg.key.data_bits.valid = 1;
            ruleCfg.key.care_bits.lspid = 0x3f;
            ruleCfg.key.data_bits.lspid = RTL8277C_CLS_EXT_WAN_PORT;
            ruleCfg.action.fwd_act = AAL_L2_CF_ACTION_FWD_ACT_LDPID;
            ruleCfg.action.cf_fwd_id = portFwdPort[RTL8277C_CLS_EXT_WAN_PORT];
            ruleCfg.action.cf_cos_act = AAL_L2_CF_ACTION_COS_ACT_ASSIGN;
            ruleCfg.action.cf_cos = 0;
            aal_l2_cf_rule_add(DEFAULT_DEVICE_ID, portRuleIndex[RTL8277C_CLS_EXT_WAN_PORT] ,&ruleCfg);
            portType[RTL8277C_CLS_EXT_WAN_PORT] = RTL8277C_CLS_PORT_TYPE_VEIP;
        }
        else
        {
            aal_l2_cf_rule_delete(DEFAULT_DEVICE_ID, portRuleIndex[RTL8277C_CLS_EXT_WAN_PORT]);
            portType[RTL8277C_CLS_EXT_WAN_PORT] = RTL8277C_CLS_PORT_TYPE_PPTP;
        }
        return RT_ERR_OK;
    }
    ...
    for(i=0;i<RTK_MAX_NUM_OF_PORTS;i++)
    {
        if(portType[i] == RTL8277C_CLS_PORT_TYPE_PPTP)
        {
            hasPptp = 1;
            break;
        }
...
int32 dal_rt_rtl8277c_cls_rule_add(rt_cls_rule_t *pClsRule)
{
    int32 ret=RT_ERR_FAILED;
    dal_8277c_cls_rule_db_t *pNewDsRule=NULL;
    ...
    _8277c_cls_rule_set(pClsRule);

    /* VLAN aggregation */
    if(pClsRule->direction == RT_CLS_DIR_DS)
    {
#if defined(CONFIG_SDK_KERNEL_LINUX)
        pNewDsRule = kmalloc(sizeof(dal_8277c_cls_rule_db_t), GFP_KERNEL);
#endif
        if (!pNewDsRule)
            return RT_ERR_FAILED;
...
int32 dal_rt_rtl8277c_cls_rule_delete(uint32 ruleIndex)
{
    int32 index;
    int32   ret = RT_ERR_OK;
#if defined(CONFIG_SDK_KERNEL_LINUX)
    ...
    while(_8277c_cls_rule_index_find(ruleIndex, RT_CLS_DIR_DS, &index,RTL8277C_CLS_RULE_PORT_IGNORE, 0) == RT_ERR_OK)
    {
        ret = aal_l2_cf_rule_delete(DEFAULT_DEVICE_ID, index);
    ...
    while(_8277c_cls_rule_index_find(ruleIndex, RT_CLS_DIR_US, &index,RTL8277C_CLS_RULE_PORT_IGNORE, 0) == RT_ERR_OK)
    {
        ret = aal_l2_cf_rule_delete(DEFAULT_DEVICE_ID, index);
...
extern int32 dal_rt_rtl8277c_cls_rule_get(rt_cls_get_op_t op, uint32 index, rt_cls_dir_t dir, rt_cls_rule_info_t *pClsRuleInfo)
{
    ...
    {
#if defined(CONFIG_SDK_KERNEL_LINUX)
        pEntryData = list_first_entry_or_null(pHead, dal_8277c_cls_rule_db_t, list);
#else
        pEntryData = NULL;
#endif
    }
    ...
    for(i=RTL8277C_CLS_RULE_SYSTEM_RESERVED_NUM;i<ruleMax;i++)
    {
        if((RTL8277C_CLS_RULE_NOT_IN_USED != clsRuleUsage[i].index) && (clsRuleUsage[i].ruleIndex == pEntryData->rule.index) && (clsRuleUsage[i].dir == dir) )
        {
...
int32 dal_rt_rtl8277c_cls_pon_sid_set(uint16 sid,rt_ponmisc_sidInfo_t sidInfo)
{
    /* check Init status */
    RT_INIT_CHK(cls_init);

    if(sid >= RTL8277C_CLS_SID_MAX)
        return RT_ERR_FAILED;
...
int32 dal_rt_rtl8277c_cls_fwdPort_set(rt_port_t port, rt_port_t fwdPort)
{
    int32   ret = RT_ERR_OK;
    aal_ilpb_cfg_t ilpbCfg;
    aal_ilpb_cfg_msk_t ilpbCfgMask;
    ...
    aal_port_ilpb_cfg_set(0, port, ilpbCfgMask, &ilpbCfg);
    if(portType[port] == RTL8277C_CLS_PORT_TYPE_VEIP)
        ret = dal_rt_rtl8277c_cls_veipPortRule_set(port, ENABLED);
