...
{
    RT_DAEQSA_DENY = 0, /*Deny for L2 DMAC is the same as SMAC*/
    RT_LAND_DENY, /*Deny for SIP is the same as DIP*/
    RT_BLAT_DENY, /*Deny for TCP/UDP SPORT is the same as DPORT*/
    RT_SYNFIN_DENY,
    RT_XMA_DENY, /*Deny for TCP packets while sequence number is zero and FIN,URG,PSH bits are set*/
    RT_NULLSCAN_DENY, /*Deny for TCP packets while sequence number is zero and all contorl bits are zeros*/
    RT_SYN_SPORTL1024_DENY,/*Deny for TCP SYN packets with source port less than 1024*/
    RT_TCPHDR_MIN_CHECK,
    RT_TCP_FRAG_OFF_MIN_CHECK, /*Deny for the Fragment_Offset=1 in any fragment of  a fragmented IP datagram carrying part of TCP data*/
    RT_ICMP_FRAG_PKTS_DENY, /*eny for ICMPv4/ICMPv6 data unit carried in a fragmented IP datagram*/
    RT_POD_DENY, /*Deny for Ping of Death attack with IP/IP6 packets*/
    RT_UDPDOMB_DENY,
    RT_SYNWITHDATA_DENY,/*Deny for IP length > IP header + TCP header length while SYN flag is set 1*/
    RT_SYNFLOOD_DENY, /*Deny for TCP SIN flood over number of packets in time unit*/
    RT_FINFLOOD_DENY, /*Deny for TCP FIN flood over number of packets in time unit*/
    RT_ICMPFLOOD_DENY, /*Deny for ICMP flood over number of packets in time unit*/ 
    RT_ATTACK_TYPE_END
} rt_sec_attackType_t;
 ...
 * Note:
 *      Must initialize security module before calling any security APIs.
 */
extern int32
rt_sec_init(void);
 ...
 * Description:
 *      Per port get attack prevention config state
 * Input:
 *      port    - port id
 * Output:
 ...
 * Description:
 *      Per port set attack prevention config state
 * Input:
 *      port   - port id.
 ...
 * Description:
 *      Get flood threshold with numbers of packets per time unit
 * Input:
 *      None
 * Output:
 ...
 * Description:
 *      Set flood threshold with numbers of packets per time unit
 * Input:
