...
{
    aal_xgpon_bwmp_alloc_id_cfg_msk_t mask;
    aal_xgpon_bwmp_alloc_id_cfg_t alloc_id_cfg={0};
    aal_gpon_tcont_cfg_msk_t gpon_mask;
    aal_gpon_tcont_cfg_t gpon_alloc_id_cfg={0};
...
_ca8277b_gpon_aal_dsGem_set(unsigned int dsFlowId, const rt_gpon_dsFlow_t *pDsFlow, unsigned short *pGemIdx)
{
    ...
    if(CA_PON_MODE == ONU_PON_MAC_GPON)
    {
        ...
        unsigned char aes_en = 0; //no SW AES involved
...
dal_rt_ca8277b_gpon_serialNumber_set(rt_gpon_serialNumber_t *pSN)
{
    ...
    /* function body */
    memcpy(&serial_number[0], &pSN->vendor[0], RT_GPON_VENDOR_ID_LEN);
    memcpy(&serial_number[RT_GPON_VENDOR_ID_LEN], &pSN->specific[0], RT_GPON_VENDOR_SPECIFIC_LEN);
...
{
    int32 ret = RT_ERR_OK;
    aal_xgpon_usbg_xgtc_ctrl_config_t xgtc_ctrl={0};
    ...
    switch(attributeType)
    {
        ...
        case RT_GPON_ATTRIBUTE_TYPE_SD_THRESHOLD:
            if(CA_PON_MODE == ONU_PON_MAC_GPON)
            {
                aal_gpon_sf_degrd_alrm_thrsd_cfg_t th_cfg={0};
