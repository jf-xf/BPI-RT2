...
dal_ca8277c_ponmac_linkState_get(rtk_ponmac_mode_t mode, uint32 *pSd, uint32 *pSync)
{
    ca_status_t  ret = CA_E_OK;
    aal_pon_opt_status_t    opt_stas;
	ca_uint32_t             alarm_bits;
	aal_xgpon_dsfp_ds_sync_t  dsync;
	...
	switch(CA_PON_MODE) 
	{
		...
		case ONU_PON_MAC_XGSPON:
		case ONU_PON_MAC_NGPON2:
			if((ret = aal_xgpon_dsfp_sync_state_get(0, &dsync)) != CA_E_OK)
			{
...
dal_rtl8277c_ponmac_queue_del(rtk_ponmac_queue_t *pQueue)
{
    ...
    uint16 phySchId = 0;
    aal_puc_btc_cfg_msk_t btc_cfg_msk;
    aal_puc_btc_cfg_t     btc_cfg;
    int32 i;
    uint32 en_que_num;

    RT_DBG(LOG_DEBUG, (MOD_DAL|MOD_PONMAC),"%s",__FUNCTION__);
    RTL8277C_LOG_DBG("%s %d pQueue->schedulerId=%d pQueue->queueId=%d\n",__FUNCTION__,__LINE__,pQueue->schedulerId,pQueue->queueId);
...
//RTK_VOQ_VALID_SUPPORT
    ...
    /* before flush, for gpon mode set schmode to packet mode to avoid flush error */
    if(CA_PON_MODE == ONU_PON_MAC_GPON)
    {
        memset(&btc_cfg, 0, sizeof(aal_puc_btc_cfg_t));
        btc_cfg_msk.u32 = 0;
        btc_cfg_msk.s.schmode = 1;
        btc_cfg.schmode = AAL_PUC_BTC_SCHMODE_PACKET;
        aal_puc_btc_cfg_set(0,btc_cfg_msk,&btc_cfg);
    ...
    if (CA_PON_MODE == ONU_PON_MAC_GPON)
    {
        ...
        {
            aal_puc_port_voq_map_get(0, phySchId, i, &puc_queue_cfg);
            if(puc_queue_cfg.queue_ena == 1)
                en_que_num++;
        }
    ...
    if (CA_PON_MODE == ONU_PON_MAC_GPON)
    {
        /* after flush, for gpon mode set schmode to fragment mode */
        btc_cfg.schmode = AAL_PUC_BTC_SCHMODE_FRAGMENT;
        aal_puc_btc_cfg_set(0,btc_cfg_msk,&btc_cfg);
    }
...
dal_rtl8277c_ponmac_queue_add(rtk_ponmac_queue_t *pQueue, rtk_ponmac_queueCfg_t *pQueueCfg)
{
    ...
    RTL8277C_LOG_DBG("%s %d pQueue->schedulerId=%d pQueue->queueId=%d pQueueCfg->type=%d pQueueCfg->cir=%d pQueueCfg->pir=%d pQueueCfg->weight=%d pQueueCfg->type=%d\n"
        ,__FUNCTION__,__LINE__,pQueue->schedulerId,pQueue->queueId,pQueueCfg->type,pQueueCfg->cir,pQueueCfg->pir,pQueueCfg->weight,pQueueCfg->type);
