...
uint8_t ldpid_to_pon_port_map_offset = 0;
char cpuport_init = 0;
...
asic_ret_t rtk_8277c_flow_default_action_update(uint32_t profileIdx, rtk_8277c_asic_flow_defAct_type_t act)
{
	...
	{
		aal_hash_profile_defAct_update(profileIdx, rtk_8277c_db.flow_defAct_idx[RTK_8277C_FLOW_DEFACT_TYPE_TRAP_WITH_METER], rtk_8277c_db.flow_defAct_idx[RTK_8277C_FLOW_DEFACT_TYPE_TRAP_WITH_METER]);
	}
	else
	{
...
asic_ret_t rtk_asic_flow_default_action_meter_update(uint32_t profileIdx, uint32_t flow_meter_idx)
{
	...
	aal_entry_get_by_idx(AAL_TABLE_HASH_DEFAULT_ACTION, rtk_8277c_db.flow_defAct_idx[RTK_ASIC_FLOW_DEFACT_TYPE_TRAP_WITH_METER],  &default_action);
	...
	aal_entry_set(AAL_TABLE_HASH_DEFAULT_ACTION, rtk_8277c_db.flow_defAct_idx[RTK_ASIC_FLOW_DEFACT_TYPE_TRAP_WITH_METER],  &default_action);
...
{
	ldpid_to_pon_port_map_offset = offset;
...
asic_ret_t _rtk_8277c_asic_flow_action_gen(rtk_8277c_asic_pathValue_t in_path, void* flow_key, void* flow_act, aal_hash_action_t *hash_action, uint32_t profileIdx, uint32_t tupleIdx)
{
	...
	if(in_path == FB_PATH_12)
	{
		...
		{	// CN2 mode[1]
			...
			hash_action->dpid_vld = TRUE;
			if(pPath12_act->tcont_id - ldpid_to_pon_port_map_offset <= 15) 	//	to wan & ldpid is 0x20~0x2f => dq
				hash_action->deepq = 1;
			else																				// to wan & ldpid is 0x30~0x3F => !dq
				hash_action->deepq = 0;
	...
	else if(in_path == FB_PATH_34)
	{
		...
		{	// CN2 mode[1]
			hash_action->mc = TRUE; // workaround to prevent gem id 0x1f in mcgid to identify as blackhole
			hash_action->dpid_vld = TRUE;
			if(pPath34_act->tcont_id - ldpid_to_pon_port_map_offset <= 15) 	//	to wan & ldpid is 0x20~0x2f => dq
				hash_action->deepq = 1;
			else																				// to wan & ldpid is 0x30~0x3F => !dq
				hash_action->deepq = 0;
	...
	else if(in_path == FB_PATH_5)
	{
		...
		{	// CN2 mode[1]
			hash_action->mc = TRUE; // workaround to prevent gem id 0x1f in mcgid to identify as blackhole
			hash_action->dpid_vld = TRUE;
			if(pPath5_act->tcont_id - ldpid_to_pon_port_map_offset <= 15)		//	to wan & ldpid is 0x20~0x2f => dq
				hash_action->deepq = 1;
			else																				// to wan & ldpid is 0x30~0x3F => !dq
				hash_action->deepq = 0;
...
uint32_t rtk_8277c_flow_hash_profile_init(uint32 actGrpMsk_flow,uint32 actGrpMsk_type1, rtk_8277c_flow_key_mask_t flowKeyMask)
{
	...
	******************************************************/
	...
	hash_mask.lspid = mask;	// sel lspid instead of o_lspid because of TM bandwidth limitation.
