 ...
 * Data Declaration
 */
static dal_mapper_t dal_luna_g3_mapper =
{
    0,//Kem TBD
    ._init = NULL,
 ...
 */
int32 dal_luna_g3_init(void)
{
    return RT_ERR_OK;
} /* end of dal_luna_g3_init */
 ...
 * Note:
 */
dal_mapper_t *dal_luna_g3_mapper_get(void)
{
    return &dal_luna_g3_mapper;
} /* end of dal_luna_g3_mapper_get */



void dal_luna_g3_mapper_register(dal_mapper_t *pMapper)
{
    int32 ret;

    memcpy(&dal_luna_g3_mapper,pMapper,sizeof(dal_mapper_t));

    /*initial function*/
    if((ret = dal_luna_g3_mapper.switch_init()) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_SEC), "");
    }

    if((ret = dal_luna_g3_mapper.l2_init()) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_SEC), "");
    }

    if((ret = dal_luna_g3_mapper.vlan_init()) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_SEC), "");
    }

    if((ret = dal_luna_g3_mapper.stat_init()) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_SEC), "");
    }

    if((ret = dal_luna_g3_mapper.rate_init()) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_SEC), "");
    }

    if((ret = dal_luna_g3_mapper.qos_init()) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_SEC), "");
    }

    if((ret = dal_luna_g3_mapper.intr_init()) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_SEC), "");
    }

    if((ret = dal_luna_g3_mapper.port_init()) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_SEC), "");
    }

    if((ret = dal_luna_g3_mapper.sec_init()) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_SEC), "");
    }

    if((ret = dal_luna_g3_mapper.trap_init()) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_SEC), "");
    }
}

EXPORT_SYMBOL(dal_luna_g3_mapper_register);