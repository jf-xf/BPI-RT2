...
int32 _phy_rtl826xb_patch(uint32 unit, uint8 port, uint8 portOffset, uint8 bcast)
{
    ...
    if ((ret = phy_rtl826xb_patch_op(unit, port, portOffset, RTK_HWPATCH_OP_TOP, 0xff, 90, 18, 15, 0, RTL8264B_MAIN_VER)) != RT_ERR_OK)
        return ret;
    //PP_TOPReg w $PHYID 90 19 ,(0x2d3)
    if ((ret = phy_rtl826xb_patch_op(unit, port, portOffset, RTK_HWPATCH_OP_TOP, 0xff, 90, 19, 15, 0, RTL8264B_SW_VER)) != RT_ERR_OK)
        return ret;
    //PP_TOPReg w $PHYID 90 20 ,(0x2d4)
    if ((ret = phy_rtl826xb_patch_op(unit, port, portOffset, RTK_HWPATCH_OP_TOP, 0xff, 90, 21, 15, 0, RTL8264B_TOP_VER)) != RT_ERR_OK)
        return ret;
    //PP_TOPReg w $PHYID 90 21 ,(0x2d5)
    if ((ret = phy_rtl826xb_patch_op(unit, port, portOffset, RTK_HWPATCH_OP_TOP, 0xff, 90, 21, 15, 0, RTL8264B_AFEFW_VER)) != RT_ERR_OK)
        return ret;
    ...
    if ((ret = _phy_rtl826xb_patch_process(unit, port, portOffset, rtl8264B_patch_fwpr_conf, sizeof(rtl8264B_patch_fwpr_conf), &cnt, bcast))!= RT_ERR_OK)
    {
    ...
    if ((ret = _phy_rtl826xb_patch_process(unit, port, portOffset, rtl8264B_patch_fwlm_conf, sizeof(rtl8264B_patch_fwlm_conf), &cnt, bcast))!= RT_ERR_OK)
    {
    ...
    if ((ret = phy_rtl826xb_patch_op(unit, port, portOffset, RTK_HWPATCH_OP_PHY, 0xff, 0x00, 0xA438, 15, 0, RTL8264B_FW_VER)) != RT_ERR_OK)
        return ret;

    if ((ret = _phy_rtl826xb_patch_process(unit, port, portOffset, rtl8264B_patch_afe_conf, sizeof(rtl8264B_patch_afe_conf), &cnt, bcast)) != RT_ERR_OK)
    {
    ...
    if ((ret = _phy_rtl826xb_patch_process(unit, port, portOffset, rtl8264B_patch_top_conf, sizeof(rtl8264B_patch_top_conf), &cnt, bcast))!= RT_ERR_OK)
    {
    ...
    if ((ret = _phy_rtl826xb_patch_process(unit, port, portOffset, rtl8264B_patch_sds_conf, sizeof(rtl8264B_patch_sds_conf), &cnt, bcast))!= RT_ERR_OK)
    {
