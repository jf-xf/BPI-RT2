 ...
 */
int32 dal_rtl8198f_init(void)
{
    return RT_ERR_OK;
} /* end of dal_rtl8198f_init */



/* Function Name:
 *      dal_rtl8198f_mapper_get
 * Description:
 ...
 * Note:
 */
dal_mapper_t *dal_rtl8198f_mapper_get(void)
{
    return &dal_rtl8198f_mapper;
} /* end of dal_rtl8198f_mapper_get */



void dal_rtl8198f_mapper_register(dal_mapper_t *pMapper)
{
    int32 ret;

    memcpy(&dal_rtl8198f_mapper,pMapper,sizeof(dal_mapper_t));

    /*initial function*/
    if((ret = dal_rtl8198f_mapper.switch_init()) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_SEC), "");
    }

    if((ret = dal_rtl8198f_mapper.l2_init()) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_SEC), "");
    }

    if((ret = dal_rtl8198f_mapper.vlan_init()) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_SEC), "");
    }

    if((ret = dal_rtl8198f_mapper.stat_init()) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_SEC), "");
    }

    if((ret = dal_rtl8198f_mapper.rate_init()) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_SEC), "");
    }

    if((ret = dal_rtl8198f_mapper.qos_init()) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_SEC), "");
    }

    if((ret = dal_rtl8198f_mapper.intr_init()) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_SEC), "");
    }

    if((ret = dal_rtl8198f_mapper.port_init()) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_SEC), "");
    }

    if((ret = dal_rtl8198f_mapper.sec_init()) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_SEC), "");
    }

    if((ret = dal_rtl8198f_mapper.trap_init()) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_SEC), "");
    }
}

EXPORT_SYMBOL(dal_rtl8198f_mapper_register);
