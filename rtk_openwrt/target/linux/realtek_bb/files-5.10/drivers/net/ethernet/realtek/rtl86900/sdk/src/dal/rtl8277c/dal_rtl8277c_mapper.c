...
static dal_mapper_t dal_rtl8277c_mapper =
{
    ...
    .port_serdesModeSpeed_set                   = NULL,
    .port_serdesModeSpeed_get                   = NULL,
 ...
 */
int32 dal_rtl8277c_init(void)
{
    return RT_ERR_OK;
} /* end of dal_rtl8277c_init */



/* Function Name:
 *      dal_rtl8277c_mapper_get
 * Description:
 ...
 * Note:
 */
dal_mapper_t *dal_rtl8277c_mapper_get(void)
{
    return &dal_rtl8277c_mapper;
} /* end of dal_rtl8277c_mapper_get */



void dal_rtl8277c_mapper_register(dal_mapper_t *pMapper)
{
    //int32 ret;

    memcpy(&dal_rtl8277c_mapper,pMapper,sizeof(dal_mapper_t));

#if 0 //Before dal change from CA api to AAL, not open it.
    /*initial function*/
    if((ret = dal_rtl8277c_mapper.switch_init()) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_SEC), "");
    }

    if((ret = dal_rtl8277c_mapper.l2_init()) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_SEC), "");
    }

    if((ret = dal_rtl8277c_mapper.vlan_init()) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_SEC), "");
    }

    if((ret = dal_rtl8277c_mapper.stat_init()) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_SEC), "");
    }

    if((ret = dal_rtl8277c_mapper.rate_init()) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_SEC), "");
    }

    if((ret = dal_rtl8277c_mapper.qos_init()) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_SEC), "");
    }

    if((ret = dal_rtl8277c_mapper.intr_init()) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_SEC), "");
    }

    if((ret = dal_rtl8277c_mapper.port_init()) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_SEC), "");
    }

    if((ret = dal_rtl8277c_mapper.sec_init()) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_SEC), "");
    }

    if((ret = dal_rtl8277c_mapper.epon_init()) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_SEC), "");
    }

    if((ret = dal_rtl8277c_mapper.trap_init()) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_SEC), "");
    }

    if((ret = dal_rtl8277c_mapper.i2c_init(0)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_SEC), "");
    }

    if((ret = dal_rtl8277c_mapper.led_init()) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_SEC), "");
    }

    if((ret = dal_rtl8277c_mapper.gpon_init()) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_SEC), "");
    }

    if((ret = dal_rtl8277c_mapper.mirror_init()) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_SEC), "");
    }

    if((ret = dal_rtl8277c_mapper.ponmac_init()) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_SEC), "");
    }

    if((ret = dal_rtl8277c_mapper.gpio_init()) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_SEC), "");
    }
    ...
    if((ret = dal_rtl8277c_mapper.rt_ponmisc_init()) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_SEC), "");
    }
#endif
...
void dal_rtl8277c_mapper_unregister(void)
{
    memset(&dal_rtl8277c_mapper,0,sizeof(dal_mapper_t));
}
#if defined(CONFIG_SDK_KERNEL_LINUX)
EXPORT_SYMBOL(dal_rtl8277c_mapper_register);
EXPORT_SYMBOL(dal_rtl8277c_mapper_unregister);
#endif
