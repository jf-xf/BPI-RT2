...
int _rtk_rg_ftp_parse_addressString(unsigned char * pData, ipaddr_t * pIpAddr)
{
	char *endCP;
...
static int _rtk_rg_ftp_process(int direct, unsigned char * pData, unsigned int dataLen,rtk_rg_alg_connection_t * pConn)
{
	...
	if(direct==NAPT_DIRECTION_OUTBOUND)
	{
		...
		if(*((unsigned int *)pData) == htonl(ALG_FTP_PORT_STR))
		{
			...
			newPort=_rtk_rg_extPort_get(TRUE, pConn->tuple.internalIp.ip, portNum, portNum);
		...
		else if(*((unsigned int *)pData) == htonl(ALG_FTP_EPRT_STR))		//Server In WAN's ACTIVE MODE
		{
			unsigned int ipver;
			char *strptr,*tokptr,*ipptr,*portptr;
			char sepliter;
			//DEBUG("len is %d, l4offset is %d, headerlen is %d",dataLen,pPktInfo->l4Offset, pPktInfo->headerLen);
			//DEBUG("POST_FUNCTION outbound: i am in _rtk_rg_algRegFunc_TCP_FTP!! data len is %d dport is %d",dataLen,pPktInfo->dport);
			//DEBUG("DATA is:");
			...
			if (tokptr==NULL)return 0;

			sepliter=strptr[0];

			//WARNING("strptr=%s strptr[0]=%c sepliter=%c",strptr,strptr[0],sepliter);

			strptr++;

			tokptr = strsep(&strptr,&sepliter);
			if (tokptr==NULL)return 0;

			ipver = simple_strtol(tokptr, NULL, 0);

			if(ipver!=ALG_FTP_EPRT_IPVERSION_V4){
				ALG("[FTP] do not support ip version %d EPRT command..",ipver);
				return 0;
			}

			//WARNING("ipver = %d ",ipver);

			ipptr = strsep(&strptr,&sepliter);

			//_rtk_rg_ftp_parse_addressString(ipptr, &ipAddr);

			//WARNING("ipptr=%s number of it is 0x%x",ipptr,ipAddr);

			portptr = strsep(&strptr,&sepliter);

			portNum = simple_strtol(portptr, NULL, 0);

			//WARNING("portptr=%s number of it is %d",portptr,portNum);

			newPort=_rtk_rg_extPort_get(TRUE, pConn->tuple.internalIp.ip, portNum, portNum);
			if(newPort==FAIL) return SUCCESS;

			gwIPAddr=pConn->tuple.extIp.ip;			
			snprintf(portString,sizeof(portString),"EPRT %c%d%c%d.%d.%d.%d%c%d%c\r\n",
							sepliter,
							ipver,
							sepliter,
							(gwIPAddr>>24)&0xff,
...
int _rtk_rg_server_in_wan_passive_mode_naptPriority_assign(unsigned char *pPktInfo, rtk_rg_alg_connection_t * pConn){
	...
	if((pPktHdr->tagif&IPV4_TAGIF)&&((pPktHdr->tagif&TCP_TAGIF)||(pPktHdr->tagif&UDP_TAGIF))){//FTP only translated by TCP or UDP.
		if(pPktHdr->pL4Payload!=NULL && (*((unsigned int *)pPktHdr->pL4Payload) == htonl(ALG_FTP_PASV_RESP_STR))){ //The PASV request packet start with 227(ASCii) in payload.
			...
			_rtk_rg_ftp_parse_addressAndPortString(strchr(pPktHdr->pL4Payload,'(')+1,&ipAddr,&portNum); //bypass string before "("
			...
			if(rg_db.naptOut[pPktHdr->naptOutboundIndx].priValid==ENABLED){
				...
				napt_filter.ingress_dest_ipv4_addr = ipAddr;
				napt_filter.ingress_dest_l4_port = portNum;
				...
				ALG("FTP ALG add outbound naptFilter[%d](DIP=0x%x + dport=%d => priority=%d, one shot) for data flow (The priority is based on napt[%d])",index,pConn->tuple.remoteIp,pConn->tuple.remotePort,rg_db.naptOut[pPktHdr->naptOutboundIndx].priValue,pPktHdr->naptOutboundIndx);
			}
			...
			if(rg_db.naptIn[pPktHdr->naptrInboundIndx].priValid==ENABLED){
				...
				napt_filter.ingress_src_ipv4_addr = ipAddr;
				napt_filter.ingress_src_l4_port = portNum;
				napt_filter.action_fields = ASSIGN_NAPT_PRIORITY_BIT;
				napt_filter.assign_priority = rg_db.naptIn[pPktHdr->naptrInboundIndx].priValue;//follow control flow.
				napt_filter.ruleType = RTK_RG_NAPT_FILTER_ONE_SHOT;
				assert_ok((pf.rtk_rg_naptFilterAndQos_add)(&index,&napt_filter));
				ALG("FTP ALG add inbound naptFilter[%d](SIP=0x%x + sport=%d => priority=%d, one shot) for data flow (The priority is based on naptr[%d])",index,pConn->tuple.remoteIp,pConn->tuple.remotePort,rg_db.naptIn[pPktHdr->naptrInboundIndx].priValue,pPktHdr->naptrInboundIndx);
			}
		}
	}
#endif
