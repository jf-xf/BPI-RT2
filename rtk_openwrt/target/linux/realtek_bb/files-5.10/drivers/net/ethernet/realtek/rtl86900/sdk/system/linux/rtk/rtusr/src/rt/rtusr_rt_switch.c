...
/* Function Name:
  *      rt_switch_thermal_get
  * Description:
  *      Get soc thermal value
  * Input:
  *      none
  * Output:
  *       pThermalIntger  - pointer to thermal value for integer part
  *       pThermalDecimal - pointer to thermal value for Decimal part
  * Return:
  *      RT_ERR_OK           - OK
  ...
  */
int32
rt_switch_thermal_get(int32 *pThermalIntger, int32 *pThermalDecimal)
{
    rtdrv_rt_switchCfg_t switch_cfg;

    /* parameter check */
    RT_PARAM_CHK((NULL == pThermalIntger), RT_ERR_NULL_POINTER);
    RT_PARAM_CHK((NULL == pThermalDecimal), RT_ERR_NULL_POINTER);

    /* function body */
    GETSOCKOPT(RTDRV_RT_SWITCH_THERMAL_GET, &switch_cfg, rtdrv_rt_switchCfg_t, 1);
    osal_memcpy(pThermalIntger, &switch_cfg.thermalIntger, sizeof(int32));
    osal_memcpy(pThermalDecimal, &switch_cfg.thermalDecimal, sizeof(int32));

    return RT_ERR_OK;
}   /* end of rtk_switch_thermal_get */
