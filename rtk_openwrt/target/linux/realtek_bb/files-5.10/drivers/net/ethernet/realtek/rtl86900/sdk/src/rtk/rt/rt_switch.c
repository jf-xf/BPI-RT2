...
static ssize_t rt_rate_v_idx_write(struct file *file, const char __user *buffer, size_t count, loff_t *ppos)
{
...
rt_switch_thermal_get(int32 *pThermalIntger, int32 *pThermalDecimal)
{
    ...
    /* function body */
    if (NULL == RT_MAPPER->switch_thermal_get)
        return RT_ERR_DRIVER_NOT_FOUND;
    RTK_API_LOCK();
    ret = RT_MAPPER->switch_thermal_get(pThermalIntger, pThermalDecimal);
    RTK_API_UNLOCK();
    return ret;
