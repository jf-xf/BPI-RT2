...
int32
_phy_8218d_intrCommonStatus_get(uint32 unit, uint32 port, rtk_phy_intrStatusVal_t *pStatus)
{
    uint32 data = 0;
    uint32 blk = 0;
    ...
    pStatus->statusValue |= (data & 0x4) ? (1 << RTK_PHY_INTR_STATUS_COMMON_NWAY_NEXT_PAGE_RECV) : (0);
    pStatus->statusValue |= (data & 0x8) ? (1 << RTK_PHY_INTR_STATUS_COMMON_AN_COMPLETE) : (0);
    pStatus->statusValue |= (data & 0x10) ? (1 << RTK_PHY_INTR_STATUS_COMMON_LINK_CHANGE) : (0);
    pStatus->statusValue |= (data & 0x200) ? (1 << RTK_PHY_INTR_STATUS_COMMON_ALDPS_STATE_CHANGE) : (0);
...
phy_8218d_intr_init(uint32 unit, rtk_port_t port, rtk_phy_intr_t phyIntr)
{
    ...
    /*
     * Common part init.
     * Enable PHY Interrupt pin, bit0=1
     * Set PHY Interrupt Pin as Open drain Mode, bit1=0
     */
