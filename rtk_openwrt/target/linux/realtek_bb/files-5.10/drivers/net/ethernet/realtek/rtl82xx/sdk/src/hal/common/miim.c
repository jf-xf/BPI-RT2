...
phy_ptpRefTime_set(uint32 unit, rtk_port_t port, rtk_time_timeStamp_t timeStamp)
{
    ...
    MIIM_SEM_LOCK(unit);
    ret = (pHalCtrl->pPhy_ctrl[port]->pPhydrv->fPhydrv_ptpRefTime_set(unit, port, timeStamp));
    MIIM_SEM_UNLOCK(unit);
...
phy_ptpRefTimeAdjust_set(uint32 unit, rtk_port_t port, uint32 sign, rtk_time_timeStamp_t timeStamp)
{
    ...
    MIIM_SEM_LOCK(unit);
    ret = (pHalCtrl->pPhy_ctrl[port]->pPhydrv->fPhydrv_ptpRefTimeAdjust_set(unit, port, sign, timeStamp));
    MIIM_SEM_UNLOCK(unit);
