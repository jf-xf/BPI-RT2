...
int32 _dal_rtl8277c_rate_glb_db_dump(void)
{
    rtk_port_t port;
    uint32 queue, idx;
...
static int32 _dal_rtl8277c_agr_flow_profile_id_get(rtk_rate_storm_group_t stormType, ca_uint32_t *pProfile_id)
{
...
// assign igmp/mld to use agr flow profile id 5
    ...
    RT_PARAM_CHK((stormType != STORM_GROUP_DHCP && \
        stormType != STORM_GROUP_ARP && stormType != STORM_GROUP_IGMP_MLD), RT_ERR_FAILED);

    if (stormType == STORM_GROUP_DHCP)
        *pProfile_id = CA_AAL_MAX_AGR_FLOW_ID;

    if (stormType == STORM_GROUP_ARP)
        *pProfile_id = CA_AAL_MAX_AGR_FLOW_ID - 1;

    if (stormType == STORM_GROUP_IGMP_MLD)
            *pProfile_id = CA_AAL_MAX_AGR_FLOW_ID - 2;

    return RT_ERR_OK;
}

static int32 _dal_rtl8277c_meter_cfg_get(uint32 index, meter_cfg_t *pMeter)
{
    ...
    HAL_SCAN_ALL_PORT(port)
    {
        ...
        if (index == stormCfg[port].arp_flow_id)
        {
            ...
            RT_ERR_CHK((rt_error_common_t)_dal_rtl8277c_agr_flow_profile_id_get(STORM_GROUP_ARP, &agr_pol_id), ret);
        ...
        if (index == stormCfg[port].dhcp_flow_id)
        {
            ...
            RT_ERR_CHK((rt_error_common_t)_dal_rtl8277c_agr_flow_profile_id_get(STORM_GROUP_DHCP, &agr_pol_id), ret);
        ...
        if (index == stormCfg[port].igmp_mld_flow_id)
        {
            ...
            RT_ERR_CHK((rt_error_common_t)_dal_rtl8277c_agr_flow_profile_id_get(STORM_GROUP_IGMP_MLD, &agr_pol_id), ret);
...
int32 _dal_rtl8277c_use_ca_agr_flow_set(rtk_rate_storm_group_t stormType, ca_uint16_t meterEntryIdx)
{
    rt_error_common_t ret = RT_ERR_OK;
    ...
    RT_ERR_CHK((rt_error_common_t)_dal_rtl8277c_agr_flow_profile_id_get(stormType, &agr_pol_id), ret);
...
int32 _dal_rtl8277c_use_ca_specpkt_ctrl_set(rtk_port_t port, rtk_rate_storm_group_t stormType, rtk_enable_t enable)
{
    rt_error_common_t ret = RT_ERR_OK;
    ...
    config.grp_flow_en = 1;

    config.flowid = (agr_pol_id  << 9);
    ...
    // fdb limit fwd permit to avoid FE drop
...
int32 _dal_rtl8277c_use_ca_l2_flood_set(rtk_port_t port,rtk_rate_storm_group_t stormType,rtk_enable_t enable,ca_uint16_t flow_id)
{
    rt_error_common_t ret = RT_ERR_OK;
    aal_l2_te_policer_cfg_msk_t pol_msk;
    aal_l2_te_policer_cfg_t pol_cfg;
...
int32 _dal_rtl8277c_check_meter_change_set(uint32 meterIndex)
{
    int32 ret = RT_ERR_OK;
    uint32  port,queue;
    ...
    HAL_SCAN_ALL_PORT(port)
    {
        ...
        {
            RT_ERR_CHK(_dal_rtl8277c_use_ca_l2_flood_set(port,STORM_GROUP_UNKNOWN_UNICAST,stormCfg[port].uuc_enable,stormCfg[port].uuc_flow_id), ret);
        }

        if(meterIndex == stormCfg[port].bc_flow_id)
        {
            RT_ERR_CHK(_dal_rtl8277c_use_ca_l2_flood_set(port,STORM_GROUP_BROADCAST,stormCfg[port].bc_enable,stormCfg[port].bc_flow_id), ret);
        }

        if ( meterIndex == stormCfg[port].arp_flow_id)
        {
            RT_ERR_CHK(_dal_rtl8277c_use_ca_agr_flow_set(STORM_GROUP_ARP, stormCfg[port].arp_flow_id), ret);
            RT_ERR_CHK(_dal_rtl8277c_use_ca_specpkt_ctrl_set(port, STORM_GROUP_ARP, stormCfg[port].arp_enable), ret);
        ...
        {
            RT_ERR_CHK(_dal_rtl8277c_use_ca_agr_flow_set(STORM_GROUP_DHCP, stormCfg[port].dhcp_flow_id), ret);
            RT_ERR_CHK(_dal_rtl8277c_use_ca_specpkt_ctrl_set(port, STORM_GROUP_DHCP, stormCfg[port].dhcp_enable), ret);
        ...
        {
            RT_ERR_CHK(_dal_rtl8277c_use_ca_agr_flow_set(STORM_GROUP_IGMP_MLD, stormCfg[port].igmp_mld_flow_id), ret);
            RT_ERR_CHK(_dal_rtl8277c_use_ca_specpkt_ctrl_set(port, STORM_GROUP_IGMP_MLD, stormCfg[port].igmp_mld_enable), ret);
...
dal_rtl8277c_rate_init(void)
{
    ...
    HAL_SCAN_ALL_ETH_PORT(port)
    {
        dal_rtl8277c_rate_portIgrBandwidthCtrlRate_set(port,RTL8277C_INGRESS_RATE_MAX);
        dal_rtl8277c_rate_portEgrBandwidthCtrlRate_set(port,RTL8277C_EGRESS_RATE_MAX);
    }
    ...
    return RT_ERR_OK;
} /* end of dal_rtl8277c_rate_init */

/* Function Name:
 *      dal_rtl8277c_rate_portIgrBandwidthCtrlRate_get
 * Description:
 ...
 */
int32
dal_rtl8277c_rate_portIgrBandwidthCtrlRate_get(rtk_port_t port, uint32 *pRate)
{
    rt_error_common_t ret = RT_ERR_OK;
    ca_port_id_t port_id;
    aal_l2_te_policer_cfg_t     cfg;
    ...
    if(cir >= RTL8277C_INGRESS_RATE_MAX)
        *pRate = RTL8277C_INGRESS_RATE_MAX;
    else
        *pRate = cir;

    return RT_ERR_OK;
} /* end of dal_rtl8277c_rate_portIgrBandwidthCtrlRate_get */

/* Function Name:
 *      dal_rtl8277c_rate_portIgrBandwidthCtrlRate_set
 * Description:
 ...
 */
int32
dal_rtl8277c_rate_portIgrBandwidthCtrlRate_set(rtk_port_t port, uint32 rate)
{
    rt_error_common_t ret = RT_ERR_OK;
    ca_port_id_t port_id;
    ...
    if (port_type == CA_PORT_TYPE_L3)   /* WAN */
    {
        ...
        if(rate >= RTL8277C_INGRESS_RATE_MAX)
        {
            cir = RTL8277C_INGRESS_RATE_MAX;
            pir = RTL8277C_INGRESS_RATE_MAX;
    ...
    return RT_ERR_OK;
} /* end of dal_rtl8277c_rate_portIgrBandwidthCtrlRate_set */

/* Function Name:
 *      dal_rtl8277c_rate_portEgrBandwidthCtrlRate_get
 * Description:
 ...
 */
int32
dal_rtl8277c_rate_portEgrBandwidthCtrlRate_get(rtk_port_t port, uint32 *pRate)
{
    rt_error_common_t ret = RT_ERR_OK;
    ca_port_id_t port_id;
    aal_l2_te_shaper_tbc_cfg_t     profile;
    ...
    case CA_PORT_TYPE_L3:   /* WAN */
        map_port += CA_AAL_L3_TE_NI_PORT_OFFSET;
        RT_ERR_CHK ((rt_error_common_t)aal_l3_te_shaper_port_tbc_get(0, map_port, &l3_profile), ret);
    ...
    if(enable == 0 || rate >= RTL8277C_EGRESS_RATE_MAX)
    {
        *pRate = RTL8277C_EGRESS_RATE_MAX;
    }
    else
    {
        *pRate = rate;
    ...
    return RT_ERR_OK;
} /* end of dal_rtl8277c_rate_portEgrBandwidthCtrlRate_get */

/* Function Name:
 *      dal_rtl8277c_rate_portEgrBandwidthCtrlRate_set
 * Description:
 ...
 */
int32
dal_rtl8277c_rate_portEgrBandwidthCtrlRate_set(rtk_port_t port, uint32 rate)
{
    rt_error_common_t ret = RT_ERR_OK;
    ca_port_id_t port_id;
    ...
    if(rate >= RTL8277C_EGRESS_RATE_MAX)
    {
        enable = 0;
        egr_rate = RTL8277C_EGRESS_RATE_MAX;
    }
    else
    {
        enable = 1;
    ...
    return RT_ERR_OK;
} /* end of dal_rtl8277c_rate_portEgrBandwidthCtrlRate_set */

/* Function Name:
 *      dal_rtl8277c_rate_stormControlRate_get
 * Description:
 ...
 */
int32
dal_rtl8277c_rate_stormControlMeterIdx_get(
    rtk_port_t              port,
    ...
    uint32                  *pIndex)
{
    ...
    return RT_ERR_OK;
} /* end of dal_rtl8277c_rate_stormControlMeterIdx_get */

/* Function Name:
 *      dal_rtl8277c_rate_stormControlMeterIdx_set
 * Description:
    ...
    uint32                  index)
{
    ...
    switch(stormType)
    {
        case STORM_GROUP_UNKNOWN_UNICAST:
            ...
            {
                RT_ERR_CHK(_dal_rtl8277c_use_ca_l2_flood_set(port,stormType,stormCfg[port].uuc_enable,index), ret);
            }
        ...
        case STORM_GROUP_BROADCAST:
            ...
            {
                RT_ERR_CHK(_dal_rtl8277c_use_ca_l2_flood_set(port,stormType,stormCfg[port].bc_enable,index), ret) ;
            }
        ...
        case STORM_GROUP_DHCP:
            ...
            {
                RT_ERR_CHK(_dal_rtl8277c_use_ca_agr_flow_set(stormType, index), ret);
                RT_ERR_CHK(_dal_rtl8277c_use_ca_specpkt_ctrl_set(port, stormType, stormCfg[port].dhcp_enable), ret);
            }
        ...
        case STORM_GROUP_ARP:
            ...
            {
                RT_ERR_CHK(_dal_rtl8277c_use_ca_agr_flow_set(stormType, index), ret);
                RT_ERR_CHK(_dal_rtl8277c_use_ca_specpkt_ctrl_set(port, stormType, stormCfg[port].arp_enable), ret);
            }
        ...
        case STORM_GROUP_IGMP_MLD:
            ...
            {
                RT_ERR_CHK(_dal_rtl8277c_use_ca_agr_flow_set(stormType, index), ret);
                RT_ERR_CHK(_dal_rtl8277c_use_ca_specpkt_ctrl_set(port, stormType, stormCfg[port].igmp_mld_enable), ret);
            }
    ...
    return RT_ERR_OK;
} /* end of dal_rtl8277c_rate_stormControlMeterIdx_set */

/* Function Name:
 *      dal_rtl8277c_rate_stormControlPortEnable_get
 * Description:
    ...
    rtk_enable_t            *pEnable)
{
    ...
    return RT_ERR_OK;
} /* end of dal_rtl8277c_rate_stormControlPortEnable_get */

/* Function Name:
 *      dal_rtl8277c_rate_stormControlPortEnable_set
 * Description:
    ...
    rtk_enable_t            enable)
{
    ...
    switch(stormType)
    {
        case STORM_GROUP_UNKNOWN_UNICAST:
            ...
            {
                RT_ERR_CHK(_dal_rtl8277c_use_ca_l2_flood_set(port,stormType,enable,stormCfg[port].uuc_flow_id), ret);
            }
        ...
        case STORM_GROUP_BROADCAST:
            ...
            {
                RT_ERR_CHK(_dal_rtl8277c_use_ca_l2_flood_set(port,stormType,enable,stormCfg[port].bc_flow_id), ret) ;
            }
        ...
        case STORM_GROUP_DHCP: /* only support global configuration of policer agr flow  */
            ...
            {
                RT_ERR_CHK(_dal_rtl8277c_use_ca_specpkt_ctrl_set(port, stormType, enable), ret);
            }
    ...
    return RT_ERR_OK;
} /* end of dal_rtl8277c_rate_stormControlPortEnable_set */

/* Function Name:
 *      dal_rtl8277c_rate_shareMeter_set
 * Description:
    ...
    rtk_enable_t ifgInclude )
{
    ...
    RT_PARAM_CHK(((RTL8277C_METER_RATE_MAX) < rate), RT_ERR_INPUT);
    RT_PARAM_CHK((RTK_ENABLE_END <= ifgInclude), RT_ERR_INPUT);

    if(rate >= RTL8277C_METER_RATE_MAX) //Disable
    {
        meterEntry[index].mode = CA_AAL_POLICER_TYPE_NONE;
        meterEntry[index].pps = 0;
        meterEntry[index].cir = 0;
        meterEntry[index].pir = 0;
        meterEntry[index].cbs = RTL8277C_METER_BUCKETSIZE_MAX;
        meterEntry[index].pbs = RTL8277C_METER_BUCKETSIZE_MAX;
    }
    else
    {
        // get original meter cfg
        RT_ERR_CHK(_dal_rtl8277c_meter_cfg_get(index, &(meterEntry[index])), ret);
    ...
    RT_ERR_CHK(_dal_rtl8277c_check_meter_change_set(index), ret);

    return RT_ERR_OK;
} /* end of dal_rtl8277c_rate_shareMeter_set */


/* Function Name:
 *      dal_rtl8277c_rate_shareMeter_get
 * Description:
    ...
    rtk_enable_t* pIfgInclude )
{
    ...
    *pRate = ((CA_AAL_POLICER_TYPE_NONE == meterEntry[index].mode) ? RTL8277C_METER_RATE_MAX : meterEntry[index].cir);
    *pIfgInclude = ((meterEntry[index].ipg == 0) ? DISABLED :  ENABLED);
    return RT_ERR_OK;
} /* end of dal_rtl8277c_rate_shareMeter_get */

/* Function Name:
 *      dal_rtl8277c_rate_shareMeterMode_set
 * Description:
 *      Set meter mode
 * Input:
 *      index     - shared meter index
    ...
    rtk_rate_metet_mode_t meterMode )
{
    ...
    meterEntry[index].pps = (meterMode == METER_MODE_PACKET_RATE) ? 1 : 0 ;/* True - PPS mode, false - BPS mode */

    RT_ERR_CHK(_dal_rtl8277c_check_meter_change_set(index), ret);


    return RT_ERR_OK;
}   /* end of dal_rtl8277c_rate_shareMeterMode_set */

/* Function Name:
 *      dal_rtl8277c_rate_shareMeterMode_get
 * Description:
 *      Set meter mode
 * Input:
 *      index     - shared meter index
    ...
    rtk_rate_metet_mode_t* pMeterMode)
{
   ...
   *pMeterMode = ((meterEntry[index].pps == 1) ?  METER_MODE_PACKET_RATE : METER_MODE_BIT_RATE); /* True - PPS mode, false - BPS mode */
    ...
    return RT_ERR_OK;
}   /* end of dal_rtl8277c_rate_shareMeterMode_get */

/* Function Name:
 *      dal_rtl8277c_rate_shareMeterBucket_set
 * Description:
    ...
    uint32 bucketSize )
{
    ...
    /* parameter check */
    RT_PARAM_CHK((HAL_MAX_NUM_OF_METERING() <= index), RT_ERR_INPUT);
    RT_PARAM_CHK((RTL8277C_METER_BUCKETSIZE_MAX < bucketSize), RT_ERR_INPUT);
    ...
    return RT_ERR_OK;
} /* end of dal_rtl8277c_rate_shareMeterBucket_set */


/* Function Name:
 *      dal_rtl8277c_rate_shareMeterBucket_get
 * Description:
    ...
    uint32* pBucketSize )
{
    ...
    *pBucketSize = meterEntry[index].cbs;
    ...
    return RT_ERR_OK;
} /* end of dal_rtl8277c_rate_shareMeterBucket_get */
