...
typedef enum
{
	GEM_PORT_ENC_DISABLE = 0, /*  No encryption */
	GEM_PORT_ENC_UC_BOTH_DIRECTION = 1, /* Unicast payload encryption in both directions */
	GEM_PORT_ENC_BC_MC = 2, /* Broadcast,multicast encryption  */
	GEM_PORT_ENC_UC_DS_ONLY = 3 /* Unicast encryption and downstream only */
}OMCI_GEM_PORT_EN_KEYRING_e;
...
typedef struct
{
    ...
    OMCI_GEM_PORT_EN_KEYRING_e encKeyRing;
} OMCI_GEM_FLOW_ts;
...
typedef enum {
    ...
    // PON TAG
    OMCI_IOCTL_PONTAG_GET,

    // OMCI_IK
    OMCI_IOCTL_OMCI_IK_SET,
...
#define OMCIDRV_BASE_CTL RTDRV_END_OFFSET
#define OMCIDRV_GET_MAX OMCIDRV_BASE_CTL+OMCIDRV_CMD_END

#define OMCIDRV_GPON_EXTMSG_GET OMCIDRV_BASE_CTL+OMCIDRV_CMD_GET
