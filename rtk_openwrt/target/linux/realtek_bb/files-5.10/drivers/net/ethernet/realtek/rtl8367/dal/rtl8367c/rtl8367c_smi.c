 ...
 */
#define MDC_MDIO_WRITE(preamableLength, phyID, regID, data) aal_mdio_write(0, 1, phyID, regID, 0, (ca_uint16_t)data)
...
#define MDC_MDIO_READ(preamableLength, phyID, regID, pData) \
        do { \
                aal_mdio_read(0, 1, phyID, regID, 0, (ca_uint16_t *)pData); \
                *rData &= 0xFFFF; \
        } while(0)
...
rtksw_int32 rtl8367c_smi_read(rtksw_uint32 mAddrs, rtksw_uint32 *rData)
{
#if (!defined(MDC_MDIO_OPERATION) && !defined(SPI_OPERATION))
...
rtksw_int32 rtl8367c_smi_write(rtksw_uint32 mAddrs, rtksw_uint32 rData)
{
#if (!defined(MDC_MDIO_OPERATION) && !defined(SPI_OPERATION))
    rtksw_int8 con;
    rtksw_uint32 ACK;
