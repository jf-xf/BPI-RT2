 ...
 */
int32 dal_rtl9607f_init(void)
{
    return RT_ERR_OK;
} /* end of dal_rtl9607f_init */



/* Function Name:
 *      dal_rtl9607f_mapper_get
 * Description:
 ...
 * Note:
 */
dal_mapper_t *dal_rtl9607f_mapper_get(void)
{
    return &dal_rtl9607f_mapper;
} /* end of dal_rtl9607f_mapper_get */



void dal_rtl9607f_mapper_register(dal_mapper_t *pMapper)
{
    int32 ret;

    memcpy(&dal_rtl9607f_mapper,pMapper,sizeof(dal_mapper_t));

#if 0 //Before dal change from CA api to AAL, not open it.
    /*initial function*/
    if((ret = dal_rtl9607f_mapper.switch_init()) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_SEC), "");
    }

    if((ret = dal_rtl9607f_mapper.l2_init()) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_SEC), "");
    }

    if((ret = dal_rtl9607f_mapper.vlan_init()) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_SEC), "");
    }

    if((ret = dal_rtl9607f_mapper.stat_init()) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_SEC), "");
    }

    if((ret = dal_rtl9607f_mapper.rate_init()) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_SEC), "");
    }

    if((ret = dal_rtl9607f_mapper.qos_init()) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_SEC), "");
    }

    if((ret = dal_rtl9607f_mapper.intr_init()) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_SEC), "");
    }

    if((ret = dal_rtl9607f_mapper.port_init()) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_SEC), "");
    }

    if((ret = dal_rtl9607f_mapper.sec_init()) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_SEC), "");
    }

    if((ret = dal_rtl9607f_mapper.epon_init()) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_SEC), "");
    }

    if((ret = dal_rtl9607f_mapper.trap_init()) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_SEC), "");
    }

    if((ret = dal_rtl9607f_mapper.i2c_init(0)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_SEC), "");
    }

    if((ret = dal_rtl9607f_mapper.led_init()) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_SEC), "");
    }

    if((ret = dal_rtl9607f_mapper.gpon_init()) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_SEC), "");
    }

    if((ret = dal_rtl9607f_mapper.mirror_init()) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_SEC), "");
    }

    if((ret = dal_rtl9607f_mapper.ponmac_init()) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_SEC), "");
    }

    if((ret = dal_rtl9607f_mapper.gpio_init()) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_SEC), "");
    }
    ...
    if((ret = dal_rtl9607f_mapper.rt_ponmisc_init()) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_SEC), "");
    }
#endif
...
void dal_rtl9607f_mapper_unregister(void)
{
    memset(&dal_rtl9607f_mapper,0,sizeof(dal_mapper_t));
}
#if defined(CONFIG_SDK_KERNEL_LINUX)
EXPORT_SYMBOL(dal_rtl9607f_mapper_register);
EXPORT_SYMBOL(dal_rtl9607f_mapper_unregister);
#endif
