...
/* For NGPON2 XFP optical module */
static uint32 rt_transceiver_addrs_xfp[][2]={
    {148,16},
    {196,16},
    {168,16},
    {96,2},
...
rt_ponmisc_transceiver_get(rt_transceiver_parameter_type_t type, rt_transceiver_data_t *pData)
{
    ...
    if(ponMode==RT_NGPON2_MODE)
    {   /* For NGPON2 XFP optical module */
        for (idx = rt_transceiver_addrs_xfp[type][0]; idx < rt_transceiver_addrs_xfp[type][0]+rt_transceiver_addrs_xfp[type][1]; idx++)
        {
            ret = rt_i2c_read(TRANSCEIVER_PORT, devID, idx, &(transceiver_data[idx-rt_transceiver_addrs_xfp[type][0]]));
            pData->buf[idx-rt_transceiver_addrs_xfp[type][0]]=transceiver_data[idx-rt_transceiver_addrs_xfp[type][0]];
        }
    }
    else
    {
