...
typedef struct dal_rtl9602c_phy_ocp_data_s
{
    uint16  addr;
    uint16  data;
} dal_rtl9602c_phy_ocp_data_t;
...
static dal_rtl9602c_phy_ocp_data_t voltagePatchArray[] = {
    {0xa436, 0xA012},
    {0xa438, 0x0000},
    {0xa436, 0xA014},
    {0xa438, 0x2c04},
...
static int32
_dal_rtl9602c_switch_phyVoltage(void)
{
    int32 ret;
    uint32 i, port, cnt, patchSize;
    uint16 value;
    dal_rtl9602c_phy_ocp_data_t *patchArray;

    RT_LOG(LOG_DEBUG, (MOD_SWITCH|MOD_DAL), "switch phyVoltage start\n");

    /* Only port 1 need patch this */
    port = 1;

    /* Patch start */
    if((ret = rtl9602c_ocpInterPhy_read(port, 0xb820, &value)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_SWITCH|MOD_DAL), "");
        return ret;
    }
    value |= 0x0010;
    if((ret = rtl9602c_ocpInterPhy_write(port, 0xb820, value)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_SWITCH|MOD_DAL), "");
        return ret;
    }
    ...
    while(cnt > 0)
    {
        if((ret = rtl9602c_ocpInterPhy_read(port, 0xb800, &value)) != RT_ERR_OK)
        {
            RT_ERR(ret, (MOD_SWITCH|MOD_DAL), "");
            return ret;
        }
    ...
    value = 0x8028;
    if((ret = rtl9602c_ocpInterPhy_write(port, 0xA436, value)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_SWITCH|MOD_DAL), "");
        return ret;
    }
    value = 0x8500;
    if((ret = rtl9602c_ocpInterPhy_write(port, 0xA438, value)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_SWITCH|MOD_DAL), "");
        return ret;
    }
    ...
    patchSize = sizeof(voltagePatchArray)/sizeof(dal_rtl9602c_phy_ocp_data_t);
    for(i=0 ; i<patchSize ; i++)
    {
        if((ret=rtl9602c_ocpInterPhy_write(port,
                              patchArray[i].addr,
                              patchArray[i].data))!=RT_ERR_OK)
        {
            RT_ERR(ret, (MOD_SWITCH|MOD_DAL), "");
    ...
    if((ret = rtl9602c_ocpInterPhy_read(port, 0xa432, &value)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_SWITCH|MOD_DAL), "");
        return ret;
    }
    value &= ~(1<<6);
    if((ret = rtl9602c_ocpInterPhy_write(port, 0xa432, value)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_SWITCH|MOD_DAL), "");
        return ret;
    }
...
#else
    /* voltage patch - common_mode_patch_220908 */
    if((ret=_dal_rtl9602c_switch_phyVoltage())!=RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_SWITCH|MOD_DAL), "");
        return ret;
    }    
