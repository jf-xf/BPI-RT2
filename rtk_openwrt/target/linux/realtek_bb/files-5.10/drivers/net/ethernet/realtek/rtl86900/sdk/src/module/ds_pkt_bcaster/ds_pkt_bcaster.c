...
static ssize_t trac_cnt_write_proc(
        struct file *file, const char *buffer, size_t count, loff_t *ppos)
{
    uint64  tmpTracCnt;
    uint8   tmpBuf[16];
    ...
    if (buffer && !copy_from_user(tmpBuf, buffer, len))
    {
        tmpTracCnt = simple_strtoull(tmpBuf, NULL, 0);

        g_traceCnt = (tmpTracCnt > UINT_MAX) ? UINT_MAX : tmpTracCnt;
