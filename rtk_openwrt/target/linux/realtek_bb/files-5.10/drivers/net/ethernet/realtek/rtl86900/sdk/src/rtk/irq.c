...
switch_interrupt_bh(uint32 data)
{
    ...
    /* Use LINUX enable IRQ function to place restore all of switch IMR*/
    enable_irq(switch_dev.irq);
#endif
    return;
}
...
#if defined(CONFIG_SDK_KERNEL_LINUX)
    /* Use LINUX disable IRQ function to replace disable all of switch IMR*/
    disable_irq_nosync(switch_dev.irq);
#endif
