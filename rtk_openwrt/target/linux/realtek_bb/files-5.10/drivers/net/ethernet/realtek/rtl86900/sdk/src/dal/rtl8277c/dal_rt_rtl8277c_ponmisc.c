 ...
 * Symbol Definition
 */
#define RTL8277C_MAX_TCONT_LLID  (32)
#define RTL8277C_TCONT_START_IDX (1) /*T-CONT 0 for OMCC use, data start from index 1 */
#define RTL8277C_LLID_START_IDX (0) /*LLID start from index 0 */    
    
    /*For physical T-CONT LLID 16~23(0x30~0x37),physical and logic mapping table for T-CONT/LLID  dependents  on the deep queue setting of LAN port*/
#define RTL8277C_TCONT_LLID_DQ_CHECK_START_IDX (16)
#define RTL8277C_TCONT_LLID_DQ_CHECK_END_IDX (RTL8277C_TCONT_LLID_DQ_CHECK_START_IDX+CFG_ID_RTK_PHY_PORT_TO_DEEP_Q_LEN)



#define RTL8277C_PUC_BP_NUM  (192)

/*
 * Data Declaration
 */
static uint32 ponmisc_init = INIT_NOT_COMPLETED; 
static uint16 gTcont_llid_logic2PhyMap[RTL8277C_MAX_TCONT_LLID]= {0};
...
static int _ponmisc_mode_set(ca_uint8_t pon_mode)
{
    ...
    if(CA_PON_MODE_IS_ETHERNET(pon_mode))
    {
        ...
        dal_rtl8277c_stat_port_reset(HAL_GET_PON_PORT());
    }
...
static int32
_dal_rt_rtl8277c_ponmisc_tcont_llid_logic2phy_init(ca_uint32_t pon_mode)
{
    ...
    portStartIdx = CA_PON_MODE_IS_GPON_FAMILY(pon_mode)?RTL8277C_TCONT_START_IDX:RTL8277C_LLID_START_IDX;


    /*For GPON,Logic T-CONT 0~30=> physical T-CONT 1~31
      For EPON,Logic LLID 0~31=> physical LLID  0~31*/
    for(portId=portStartIdx;portId< RTL8277C_MAX_TCONT_LLID; portId++)
    {
    ...
    for(portId=0;portId< CFG_ID_RTK_PHY_PORT_TO_DEEP_Q_LEN; portId++)
    {
        aal_port_physical_port_to_dq_get(0, portId, &phy_port_to_deep_q[portId]);
        if(phy_port_to_deep_q[portId] == 0)
            gTcont_llid_logic2PhyMap[logicId++] = portId+RTL8277C_TCONT_LLID_DQ_CHECK_START_IDX;
    ...
    for(phyId=RTL8277C_TCONT_LLID_DQ_CHECK_END_IDX;phyId< RTL8277C_MAX_TCONT_LLID; phyId++)
    {
...
dal_rt_rtl8277c_ponmisc_init(void)
{
    ...
    if(pon_mode < ONU_PON_MAC_MAX)
    {
        _dal_rt_rtl8277c_ponmisc_tcont_llid_logic2phy_init(pon_mode);
    }

    ponmisc_init = INIT_COMPLETED;

    return ret;
}/* end of dal_rt_rtl8277c_ponmisc_init */
...
dal_rt_rtl8277c_ponmisc_mode_get(rt_ponmisc_ponMode_t *pPonMode,rt_ponmisc_ponSpeed_t *pPonSpeed)
{
    ...
    switch(CA_PON_MODE)
    {
        ...
        case ONU_PON_MAC_ETHERNET_USXGMII_5G:
        case ONU_PON_MAC_ETHERNET_USXGMII_2G5:
        case ONU_PON_MAC_ETHERNET_USXGMII_1G:
        case ONU_PON_MAC_ETHERNET_USXGMII_100M:
        case ONU_PON_MAC_ETHERNET_USXGMII_10M:
    ...
    return ret;
}/* end of dal_rt_rtl8277c_ponmisc_mode_get */
...
dal_rt_rtl8277c_ponmisc_mode_set(rt_ponmisc_ponMode_t ponMode, rt_ponmisc_ponSpeed_t ponSpeed)
{
    ...
    cur_pon_mode = aal_pon_mac_mode_get(0);
    if((ONU_PON_MAC_MAX > cur_pon_mode)&&(ponMode != PON_MODE_DISABLE))
    {
        return RT_ERR_INPUT;
    }
    ...
    if(ONU_PON_MAC_MAX > cur_pon_mode)
    {
        ...
        {
            dal_rt_rtl8277c_epon_deinit(cur_pon_mode);
        }
        else if(CA_PON_MODE_IS_GPON_FAMILY(cur_pon_mode))
        {
            dal_rt_rtl8277c_gpon_deinit(cur_pon_mode);
        }
    ...
    else if(ponMode == RT_USXGMII_MODE)
    {
        switch(ponSpeed)
        {
            case RT_10G_SPEED:
                ...
                break;
            case RT_5G_SPEED:
                ponmode = ONU_PON_MAC_ETHERNET_USXGMII_5G;
                break;
            case RT_2DOT5G_SPEED:
                ponmode = ONU_PON_MAC_ETHERNET_USXGMII_2G5;
                break;
            case RT_1G_SPEED:
                ponmode = ONU_PON_MAC_ETHERNET_USXGMII_1G;
                break;
            case RT_100M_SPEED:
                ponmode = ONU_PON_MAC_ETHERNET_USXGMII_100M;
                break;
            case RT_10M_SPEED:
                ponmode = ONU_PON_MAC_ETHERNET_USXGMII_10M;
                break;
            default:
                return RT_ERR_FEATURE_NOT_SUPPORTED;
         }
    ...
    _dal_rt_rtl8277c_ponmisc_tcont_llid_logic2phy_init(ponmode);

    return ret;
}/* end of dal_rt_rtl8277c_ponmisc_mode_set */
...
dal_rt_rtl8277c_ponmisc_ponQueue_mode_get(rt_ponmisc_ponQueueMode_t *pPonQueueMode)
{
    ...
    return ret;
}/* end of dal_rt_rtl8277c_ponmisc_ponQueue_mode_get */
...
dal_rt_rtl8277c_ponmisc_tcont_llid_logic2phy_get(uint16 logicId, uint16 *pPhyId)
{
    ...
    /* parameter check */
    RT_PARAM_CHK((logicId >= RTL8277C_MAX_TCONT_LLID), RT_ERR_PORT_ID);
...
dal_rt_rtl8277c_ponmisc_forcePRBS_set(rt_ponmisc_prbs_t prbsCfg)
{
    ...
    if(prbsCfg == RT_PONMISC_PRBS_OFF)
    {
        ...
        prbs_rx_enabled = psds_gbox.prbs_rx_en;

        memset(&psds_gbox, 0, sizeof(aal_psds_gbox_ctrl_t));
        memset(&psds_gbox_mask, 0, sizeof(psds_gbox_mask));

        psds_gbox_mask.s.lao_en = 1;
        psds_gbox_mask.s.prbs_tx_en = 1;
        psds_gbox.lao_en = 0;
        psds_gbox.prbs_tx_en = 0;

        if(prbs_rx_enabled==0)
        {
