...
static void *_rtk_rg_memoryAlignAlloc(uint32 alignedSize, uint32 size)
{
	...
	rgpro_db.ddrMemAlloc = &FLOWDATAPOOL;		// get from static data buffer of cache driver
...
{
	uint32 len = 0U;//ntohs(pip->ip_len) - ((pip->ip_vhl & 0xf) << 2);
	uint32 csum = 0U;
	uint16 tmp[2] = {0};
	uint16 orgcsum=0U;

	memcpy(tmp,&pip->ip_src,sizeof(pip->ip_src));
	csum = _rtk_rg_cksum((uint16 *)tmp, 8U, 0U);
	bzero(tmp,sizeof(tmp));

	tmp[0] = htons(17);	// 0x11: UDP
	csum = _rtk_rg_cksum((uint16 *)tmp, 2U, csum);
	tmp[0] = htons(len);
	csum = _rtk_rg_cksum((uint16 *)tmp, 2U, csum);
	orgcsum = pudp->uh_sum;
	pudp->uh_sum = 0U;
	...
	if(csum == 0xffffU)	csum = 0U;		//FIXME:luke
...
uint32 _rtk_rg_flowEntryValidBit_get(uint32 entryIdx)
{
	...
	if(rgpro_db.fbMode == FB_MODE_4K)
	{
		...
		return (rgpro_db.flowEntryValidBits[entryIdx>>5]>>(entryIdx&0x1fU)) & 0x1U;
	}else
	{
...
int32 _rtk_rg_flowEntryAvailableIdx_get(u32 baseEntIdx)
{
	...
	if(!found){
		if((fbMode == FB_MODE_4K)){
			...
			for(i=0U; i<FLOWBASED_TABLESIZE_FLOWTCAM; i++){
				isValid = _rtk_rg_flowEntryValidBit_get(baseEntIdx);
				if(isValid == 0U){
					found = TRUE;
					break;
...
int32 _rtk_rg_flowEntryConflictEgrCPUPortChecking(void *pFlowData, uint32 s1EntryIdx)
{
	...
	u8 way = 0U, i = 0U, found = FALSE, conflict = FALSE;
	...
	if((pP2Data->in_path == 0U) && (pP2Data->in_multiple_act == 1U))
	{
		DEBUG("Conflict checking for path2");
		if(((pP2Data->out_portmask & cpupmask)==0U) && (pP2Data->out_ext_portmask_idx==0U))
			return SUCCESS;
		pP2Data = (rtk_rg_asic_path2_entry_t *)pFlowData;
		pP4Data = NULL;
		pP1Data = (rtk_rg_asic_path1_entry_t *)&flowData;
	}else if((pP2Data->in_path == 1U) && (pP2Data->in_multiple_act == 1U))
	{
	...
	if(rgpro_db.fbMode == FB_MODE_4K) way = 4U;
	else way = 1U;

	DEBUG("Conflict checking, search step 1 entry in #%d", s1EntryIdx);
	
	for(i=0U; i<way; i++, s1EntryIdx++){
		
		if(_rtk_rg_flowEntryValidBit_get(s1EntryIdx) == 0U)	continue;
	...
	if(!found){
		if((rgpro_db.fbMode == FB_MODE_4K)){
			...
			for(i=0U; i<FLOWBASED_TABLESIZE_FLOWTCAM; i++, s1EntryIdx++){
				if(_rtk_rg_flowEntryValidBit_get(s1EntryIdx) == 0U)	continue;
...
{	
	...
	rtk_fc_ext_dma_cache_wback_inv((unsigned long)rgpro_db.ddrMemBase+memOffset, sizeof(rtk_rg_asic_path1_entry_t));
#else
...
uint32 _rtk_rg_flowEntryReadFromDDR(uint32 entryIdx, void *pFlowData)
{
	// Note: reg.FBA32==0, memory align 4 bytes. (Now 28 bytes per entry)
	//		reg.FBA32==1, memory align 32 bytes.
	uint32 memOffset = (entryIdx << 5);		//entryIdx * sizeof(rtk_rg_asic_path1_entry_t)
...
uint32 _rtk_rg_flowHashPreProcessPort(uint16 port, uint32 cf_ptn){
	...
	uint32 shiftdir = (cf_ptn >> 19) & 0x1U;
	uint32 shiftbits = (cf_ptn >> 16) & 0x7U;
	uint32 xoroperatedvalue = cf_ptn & 0xffffU;
	uint32 port_tmp = 0U;
...
uint32 _rtk_rg_flowHashPreProcessIP(uint32 ip, uint32 cf_ptn){
	uint32 shiftdir = (cf_ptn >> 23) & 0x1U;
	uint32 shiftbits = (cf_ptn >> 20) & 0x7U;
	uint32 xoroperatedvalue = cf_ptn & 0xfffffU;
	uint32 msb_12bits = ip & 0xfff00000U;
	uint32 lsb_20bits = ip & 0x000fffffU;
	uint32 ip_tmp = 0U;
	ip &= 0xfffffU;
	//DEBUG("Hash - preprocess: shift dir: %d, bits %d, xor value = 0x%x", shiftdir, shiftbits, xoroperatedvalue);
	if(shiftdir){	// shift left
	...
	return (msb_12bits |( (ip_tmp ^ xoroperatedvalue) & 0xfffffU));	// sip, dip: (msb12bits + 20 bits)
...
uint32 _rtk_rg_flowHashPath34ExtraItem_get(void *pFlowData, uint16 igrSVID, uint16 igrCVID, uint16 lutDaIdx_gmacChk)
{
	...
	if(isMulticast){
		...
		if(enabled == DISABLED) extraItem |= (igrSVID<<12); 
		else extraItem &= 0x000fffU;
		
		ASSERT_EQ(rtk_rg_asic_globalState_get(FB_GLOBAL_PATH34_MC_SKIP_CVID, &enabled), RT_ERR_RG_OK);
		if(enabled == DISABLED) extraItem |= igrCVID;
		else extraItem &= 0xfff000U;
	}else{
...
uint32 _rtk_rg_flowHashIndexStep1_get(uint16 param1, uint16 param2, uint32 param3, uint32 param4, uint32 extraItem){
	uint32 sum1=0U, sum2=0U, sum=0U, sum_nk=0U, hashIdx=0U;
	...
	sum1 = ((sip&0xfffffU)  + (sip>>20) + (dip&0xfffffU) + (dip>>20) + sport + dport) & 0x7fffffU;		// sum1[22:0]
	sum2 = ((sum1&0xfffffU) + (sum1>>20)) & 0x1fffffU;						// sum2[20:0]
	sum	  = ((sum2&0xfffffU) + (sum2>>20)) & 0x1fffffU;						// sum[20:0]
	...
	if(ASIC_CHIP_ID==RTL9603CVD_CHIP_ID)
	{
		...
		case FB_MODE_1K:
			...
			sum_nk = ((sum&0xffU) + ((sum>>8)&0xffU) + ((sum>>16)&0x1fU))&0xffU;		// sum_4k[7:0]
			hashIdx = sum_nk ^ (extraItem&0xffU) ^ ((extraItem>>8)&0xffU) ^ ((extraItem>>16)&0xffU);
			hashIdx = hashIdx<<2;												// Get base entry index
			break;
		case FB_MODE_8K:
			// 8k mode: 1-way (13 bits index)
			sum1 = (sum&0x1fffU) + ((sum>>13)&0xffU);
			sum2 = (sum1&0x3ffU) ^ ((sum1>>10)&0x7U);
			sum_nk = (sum1&0x1c00U) | (sum2&0x3ffU);							// sum_8k[12:0]
			hashIdx = sum_nk ^ (extraItem&0x1fffU) ^ ((extraItem>>13)&0x7ffU);
			break;
		case FB_MODE_16K:
			// 16k mode: 1-way (14 bits index)
			sum1 = (sum&0x3fffU) + ((sum>>14)&0x7fU);
			sum2 = (sum1&0x3ffU) ^ ((sum1>>10)&0xfU);
			sum_nk = (sum1&0x3c00U) | (sum2&0x3ffU);							// sum_16k[13:0]
			hashIdx = sum_nk ^ (extraItem&0x3fffU) ^ ((extraItem>>14)&0x3ffU);
			break;
		case FB_MODE_32K:
			// 32k mode: 1-way (15 bits index)
			sum1 = (sum&0x7fffU) + ((sum>>15)&0x3fU);
			sum2 = (sum1&0x3ffU) ^ ((sum1>>10)&0x1fU);
			sum_nk = (sum1&0x7c00U) | (sum2&0x3ffU);							// sum_32k[14:0]
			hashIdx = sum_nk ^ (extraItem&0x7fffU) ^ ((extraItem>>15)&0x1ffU);
			break;
		}
	}
	else	// not 9603CVD
	{
		...
		case FB_MODE_4K:
			...
			sum_nk = ((sum&0x3ffU) + ((sum>>10)&0x3ffU) + ((sum>>20)&0x1U))&0x3ffU;		// sum_4k[9:0]
			hashIdx = sum_nk ^ (extraItem&0x3ffU) ^ ((extraItem>>10)&0x3ffU) ^ ((extraItem>>20)&0xfU);
			hashIdx = hashIdx<<2;												// Get base entry index
			break;
		case FB_MODE_8K:
			// 8k mode: 1-way (13 bits index)
			sum1 = (sum&0x1fffU) + ((sum>>13)&0xffU);
			sum2 = (sum1&0xfffU) ^ ((sum1>>12)&0x1U);
			sum_nk = (sum1&0x1000U) | (sum2&0xfffU);							// sum_8k[12:0]
			hashIdx = sum_nk ^ (extraItem&0x1fffU) ^ ((extraItem>>13)&0x7ffU);
			break;
		case FB_MODE_16K:
			// 16k mode: 1-way (14 bits index)
			sum1 = (sum&0x3fffU) + ((sum>>14)&0x7fU);
			sum2 = (sum1&0xfffU) ^ ((sum1>>12)&0x3U);
			sum_nk = (sum1&0x3000U) | (sum2&0xfffU);							// sum_16k[13:0]
			hashIdx = sum_nk ^ (extraItem&0x3fffU) ^ ((extraItem>>14)&0x3ffU);
			break;
		case FB_MODE_32K:
			// 32k mode: 1-way (15 bits index)
			sum1 = (sum&0x7fffU) + ((sum>>15)&0x3fU);
			sum2 = (sum1&0xfffU) ^ ((sum1>>12)&0x7U);
			sum_nk = (sum1&0x7000U) | (sum2&0xfffU);							// sum_32k[14:0]
			hashIdx = sum_nk ^ (extraItem&0x7fffU) ^ ((extraItem>>15)&0x1ffU);
			break;
		}
	}
...
uint32 _rtk_rg_flowHashIndexStep2_get(uint32 step1Idx)
{
	...
	mask = (ASIC_CHIP_ID==RTL9603CVD_CHIP_ID) ? 0x7F83U : 0x7E03U;
	...
	case FB_MODE_4K:	//FB_MODE_1K
		if(ASIC_CHIP_ID==RTL9603CVD_CHIP_ID)
		{
			...
			mask = mask & 0xffU;				// Hid_s2[7:0] = { ~hid[7], hid[6:2], ~hid[1:0]} => 8 bits mask
		...
		else	// not 9603CVD
		{
			...
			mask = mask & 0x3ffU;				// Hid_s2[9:0] = { ~hid[9], hid[8:2], ~hid[1:0]} => 10 bits mask
			hashidx = hashidx ^ mask;
			hashidx = hashidx << 2;			// shift to 12 bits 4 way entry index
		}
		break;
...
uint32 _rtk_rg_flowHashIPv6DstAddr_get(uint8 ipDes[16])
{
	...
	hashIdx = (hashIdx >> 31) ^ (hashIdx & 0x7fffffffU);
...
uint32 _rtk_rg_hashTrapBandwidthSharing_get(uint32 SPA, uint64 SMAC, uint64 DMAC, uint32 innerSIP, uint32 innerDIP, uint32 innerSrcPort, uint32 innerDstPort)
{
	...
	if(enabled == ENABLED)
	{
		hashValue ^= (SPA&0x7U);											// R2~R0
		hashValue ^= (((SPA&0x1U) ^ ((SPA>>1)&0x1U) ^ ((SPA>>2)&0x1U)) << 3);	// R3 = SPA0 ^ SPA1 ^ SPA2
	}
	//TRAP_HASH_SMAC
	...
	{			
		for(i=0U; i< 32U; i+=4U)
			hashValue ^= ((innerDIP>>i)&0xfU);
	}
	//TRAP_HASH_SPORT_INNER
	rtk_trap_cpuTrapHashMask_get(TRAP_HASH_SPORT_INNER, &enabled);
	if(enabled == ENABLED)	// tcp or udp
	{
		for(i=0U; i< 16U; i+=4U)
			hashValue ^= ((innerSrcPort>>i)&0xfU);
	}
	...
	{
		for(i=0U; i< 16U; i+=4U)
			hashValue ^= ((innerDstPort>>i)&0xfU);
	}

	return (hashValue&0xfU);	// possible value: 0~15
}

uint32 _rtk_rg_fbMode_set(rtk_rg_asic_fbMode_t fbMode)
{
	uint32 numFlowEntry = 0U;
	uint32 memSize = 0U;
	void *ddrMemBase = NULL;
	uint32 supportedModeMask = 0xffU;	// allow setting fbMode if no any related compiler flag, e.g. model code.
	...
	supportedModeMask = (1U<<FB_MODE_4K)|(1U<<FB_MODE_8K)|(1U<<FB_MODE_16K)|(1U<<FB_MODE_32K);
#elif defined(CONFIG_FC_FLOWBASED_DYN_MAX_16K)
	supportedModeMask = (1U<<FB_MODE_4K)|(1U<<FB_MODE_8K)|(1U<<FB_MODE_16K);
#elif defined(CONFIG_FC_FLOWBASED_DYN_MAX_8K)
	supportedModeMask = (1U<<FB_MODE_4K)|(1U<<FB_MODE_8K);
#elif defined(CONFIG_FC_FLOWBASED_DYN_MAX_4K)
	supportedModeMask = (1U<<FB_MODE_4K);
#endif

#elif defined(CONFIG_RG_FLOW_4K_MODE)
	supportedModeMask = (1U<<FB_MODE_4K);
#elif defined(CONFIG_RG_FLOW_8K_MODE)
	supportedModeMask = (1U<<FB_MODE_8K);
#elif defined(CONFIG_RG_FLOW_16K_MODE)
	supportedModeMask = (1U<<FB_MODE_16K);
#elif defined(CONFIG_RG_FLOW_32K_MODE)
	supportedModeMask = (1U<<FB_MODE_32K);
#endif

	if((supportedModeMask & (1<<fbMode))==0x0U)
	{
	...
	if(fbMode != FB_MODE_4K){
		...
		ddrMemBase = _rtk_rg_memoryAlignAlloc(1024U, memSize);
...
#if defined(CONFIG_RTL9607C_SERIES)
	if(ASIC_CHIP_ID==RTL9607C_CHIP_ID && ASICDRIVERVER==CHIP_REV_ID_A)
		rtk_rg_asic_l2tpUdpSport_set(0U);
#endif	
