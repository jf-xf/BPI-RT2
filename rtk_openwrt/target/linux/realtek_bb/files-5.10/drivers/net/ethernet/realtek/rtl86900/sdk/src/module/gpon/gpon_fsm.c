...
static int32 silence_enable = 0;

static int32 gPsTypeb = 0;
...
static void gmac_fsm_o4_eqd(gpon_dev_obj_t *obj)
{
    ...
    {
        gPsTypeb = 0;
        if(obj->fault_callback[RTK_GPON_ALARM_TYPEB])
        (obj->fault_callback[RTK_GPON_ALARM_TYPEB])(RTK_GPON_ALARM_TYPEB, 1);
        obj->alarm_tbl[RTK_GPON_ALARM_TYPEB] = 1;
    }
}
...
void gpon_fsm_to1_expire(struct timer_list *data)
{
...
void gpon_fsm_to2_expire(struct timer_list *data)
{
    /*for compiler warning*/
    if(data){}
...
int32 gpon_fsm_silence_set(uint32 enable)
{
    silence_enable = enable;
    return RT_ERR_OK;
}
