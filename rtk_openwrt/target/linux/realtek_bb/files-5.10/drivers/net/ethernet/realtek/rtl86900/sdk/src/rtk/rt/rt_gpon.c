...
static uint32 gOmciMirroringPort = 0xFFFFFFFF;
...
rt_rtk_gpon_omci_tx(uint32 msgLen, uint8 *pMsg)
{
    ...
    osal_memcpy(omci.msg, pMsg, RTK_GPON_OMCI_MSG_LEN_TX);
    omci.len = msgLen;
...
int32 rt_rtk_gpon_attribute_get(rt_gpon_attribute_type_t attributeType,rt_gpon_attribute_t *pAttributeValue)
{
    ...
    switch(attributeType)
    {
        ...
        case RT_GPON_ATTRIBUTE_TYPE_GTC_INFO:
            ...
            pAttributeValue->gtc_info.responTime = 35531;/*ns*/
#endif
...
rt_rtk_gpon_omci_mirror_set(rt_enable_t enable, rt_port_t mirroringPort)
{
    ...
    if(ENABLED == enable)
        gOmciMirroringPort = mirroringPort;
    else
        gOmciMirroringPort = 0xFFFFFFFF;//disable mirror function

#if defined(CONFIG_SDK_KERNEL_LINUX)
    rtk_gponapp_omci_mirror_set(&gOmciMirroringPort);
#endif
...
static int32
rt_rtk_gpon_omci_mirror_get(rt_enable_t *pEnable, rt_port_t *pMirroringPort)
{
    ...
    if(0xFFFFFFFF == gOmciMirroringPort)
    {
        *pEnable = 0;
        *pMirroringPort = 0;      
    }
    ...
    {
       *pEnable = 1;
       *pMirroringPort = gOmciMirroringPort;      
    }
...
rt_g3_sid_set(rt_enable_t enable, uint32 sid)
{
   ...
   /* Prevent warning */
    ...
    if(ret!= RT_ERR_OK)
    {
        osal_printf("[%s] %d set sid:%d fail\n", __FUNCTION__, __LINE__, sid);
    }
...
rt_gpon_omci_mirror_get(rt_enable_t *pEnable, rt_port_t *pMirroringPort)
{
    ...
    /* function body */
#ifdef CONFIG_LUNA_G3_SERIES
    if (NULL == RT_MAPPER->rt_gpon_omci_mirror_get)
        return RT_ERR_DRIVER_NOT_FOUND;
    RTK_API_LOCK();
    ret = RT_MAPPER->rt_gpon_omci_mirror_get(pEnable, pMirroringPort);
    RTK_API_UNLOCK();

#else
    ret = rt_rtk_gpon_omci_mirror_get(pEnable, pMirroringPort);
#endif
    return ret;
}   /* end of rt_gpon_omci_mirror_get */
