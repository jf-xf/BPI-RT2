...
#include <dal/rtl8277c/dal_rt_rtl8277c_gpon.h>
#include <dal/rtl8277c/dal_rtl8277c_gpon.h>
#include <dal/rtl8277c/dal_rtl8277c_l2.h>
#include <dal/rtl8277c/dal_rtl8277c_stat.h>
...
/*CA 8277C GEM flow 0~7 for OMCC use */
#define LOGIC_FLOW_TO_RTL8277C_FLOW(_gemFlow) ((_gemFlow)+8)
...
#define RTL8277C_LOG_DBG(fmt, arg...)  printk(KERN_INFO "[rtl8277c]:\t" fmt "\n",## arg) 
#endif
...
#define RTL8277C_GPON_MAX_TCONT            (31)
#define RTL8277C_GPON_MAX_TCONT_QUEUE      (8)
#define RTL8277C_GPON_MAX_PON_QUEUE        (248)
#define RTL8277C_GPON_MAX_GEM_FLOW         (248)


#define RTL8277C_XGPON_MAX_TCONT          (31)
#define RTL8277C_XGPON_MAX_TCONT_QUEUE    (8)
#define RTL8277C_XGPON_MAX_PON_QUEUE      (248)
#define RTL8277C_XGPON_MAX_GEM_FLOW       (248)


#define RTL8277C_MAX_TCONT ((gPonMode == ONU_PON_MAC_GPON) ? RTL8277C_GPON_MAX_TCONT:RTL8277C_XGPON_MAX_TCONT)
#define RTL8277C_MAX_TCONT_QUEUE ((gPonMode == ONU_PON_MAC_GPON) ? RTL8277C_GPON_MAX_TCONT_QUEUE:RTL8277C_XGPON_MAX_TCONT_QUEUE)
#define RTL8277C_MAX_PON_QUEUE ((gPonMode == ONU_PON_MAC_GPON) ? RTL8277C_GPON_MAX_PON_QUEUE:RTL8277C_XGPON_MAX_PON_QUEUE)
#define RTL8277C_MAX_GEM_FLOW ((gPonMode == ONU_PON_MAC_GPON) ? RTL8277C_GPON_MAX_GEM_FLOW:RTL8277C_XGPON_MAX_GEM_FLOW)


//Tcont ID 0 for OMCC channel use
#define LOGIC_TCONID_TO_RTL8277C_TCONID(_logicTcontId) ((_logicTcontId == RT_GPON_OMCC_TCONT_ID)?OMCC_TCONT_IDX:g8277cTcontTbl[_logicTcontId].phyTcontId)



#define RTL8277C_TCONID_TO_LDPID(_tcontId) ((_tcontId)+0x20)
...
/* Data Declaration                                             */
/****************************************************************/
static dal_rtl8277c_usFlowMap_t *g8277cGemUsFlowMap=NULL;
static dal_rtl8277c_dsFlowMap_t *g8277cGemDsFlowMap=NULL;
static dal_rtl8277c_tcont_info_t *g8277cTcontTbl=NULL;
...
static dal_gpon_us_flow_counter_t g8277cUsGemCount[AAL_XGPON_GEM_PORT_NUM];
static dal_gpon_ds_flow_counter_t g8277cDsGemCount[AAL_XGPON_GEM_PORT_NUM];
static rt_gpon_pm_counter_omci_t g8277cOmciCount;


/*Defien local function*/
static int32 
_dal_rtl8277c_gpon_db_init(void)
{
    unsigned short gemFlow,tcontId;


    if(NULL==g8277cGemUsFlowMap)
    {
        g8277cGemUsFlowMap = kmalloc(sizeof(dal_rtl8277c_usFlowMap_t) * RTL8277C_MAX_GEM_FLOW, GFP_KERNEL);
        if (!g8277cGemUsFlowMap)
        {
            RTL8277C_LOG_ERR("%s %d malloc usflowMap db error",
    ...
    for(gemFlow = 0;gemFlow<RTL8277C_MAX_GEM_FLOW;gemFlow++)
    {
        g8277cGemUsFlowMap[gemFlow].isValid     = FALSE;
        g8277cGemUsFlowMap[gemFlow].tcontId     = 0x0;
        g8277cGemUsFlowMap[gemFlow].tcQueueId   = 0x0;
        g8277cGemUsFlowMap[gemFlow].gemIdx      = 0x0;
        g8277cGemUsFlowMap[gemFlow].aesEn       = 0x0;
    }


    if(NULL==g8277cGemDsFlowMap)
    {
        g8277cGemDsFlowMap = kmalloc(sizeof(dal_rtl8277c_dsFlowMap_t) * RTL8277C_MAX_GEM_FLOW, GFP_KERNEL);
        if (!g8277cGemDsFlowMap)
        {
            RTL8277C_LOG_ERR("%s %d malloc dsflowMap db error",
                __FUNCTION__, __LINE__);
            RT_ERR(ret, (MOD_DAL|MOD_GPON), "");
            return RT_ERR_FAILED;
        }
    ...
    for(gemFlow = 0;gemFlow<RTL8277C_MAX_GEM_FLOW;gemFlow++)
    {
        g8277cGemDsFlowMap[gemFlow].isValid     = FALSE;
        g8277cGemDsFlowMap[gemFlow].gemIdx      = 0x0;
        g8277cGemDsFlowMap[gemFlow].gemPortId   = 0x0;
        g8277cGemDsFlowMap[gemFlow].loopEn      = 0x0;
        g8277cGemDsFlowMap[gemFlow].loopLspid   = 0x0;
    }


    if(NULL==g8277cTcontTbl)
    {
        g8277cTcontTbl = kmalloc(sizeof(dal_rtl8277c_tcont_info_t) * RTL8277C_MAX_TCONT, GFP_KERNEL);
        if (!g8277cTcontTbl)
        {        
            RTL8277C_LOG_ERR("%s %d malloc tcont db error",
                __FUNCTION__, __LINE__);
            RT_ERR(ret, (MOD_DAL|MOD_GPON), "");
            return RT_ERR_FAILED;
    ...
    for(tcontId=0;tcontId<RTL8277C_MAX_TCONT;tcontId++)
    {
        g8277cTcontTbl[tcontId].allocId = TCONT_ALLOC_ID_984_987_RESERVED;
        dal_rt_rtl8277c_ponmisc_tcont_llid_logic2phy_get(tcontId, &g8277cTcontTbl[tcontId].phyTcontId);
        RTL8277C_LOG_DBG("%s %d tcontId[%d].phyTcontId=%d",
            __FUNCTION__, __LINE__,tcontId, g8277cTcontTbl[tcontId].phyTcontId);
...
{
    unsigned int i;
    for(i=0;i<RTL8277C_MAX_TCONT;i++)
    {
        if(g8277cTcontTbl[i].allocId == allocId)
        {
            *pTcontId = i;
            return RT_ERR_OK;
        }
    ...
    for(i=0;i<RTL8277C_MAX_TCONT;i++)
    {
        if(g8277cTcontTbl[i].allocId == TCONT_ALLOC_ID_984_987_RESERVED)
        {
            g8277cTcontTbl[i].allocId = allocId;
            *pTcontId = i;
            return RT_ERR_OK;
        }
    }
...
{
    if(TCONT_ALLOC_ID_984_987_RESERVED != g8277cTcontTbl[tcontId].allocId)
    {
        *pAllocId = g8277cTcontTbl[tcontId].allocId;
        return RT_ERR_OK;
    }    
...
_GetNonUsedTcontId(unsigned int *pTcontId)
{
    ...
    for(i=0;i<RTL8277C_MAX_TCONT;i++)
    {
        if(TCONT_ALLOC_ID_984_987_RESERVED == g8277cTcontTbl[i].allocId)
        {
            *pTcontId = i;
            return RT_ERR_OK;
        }
...
_DelTcontId(unsigned int tcontId)
{
    ...
    RT_PARAM_CHK((tcontId >= RTL8277C_MAX_TCONT), RT_ERR_INPUT);
...
__tcont_tbl_is_empty (unsigned short ignoreGemFlowId)
{
    ...
    unsigned int tcontId = g8277cGemUsFlowMap[ignoreGemFlowId].tcontId;
    ...
    for (gemFlowId = 0; gemFlowId < RTL8277C_MAX_GEM_FLOW; gemFlowId++) 
    {
        if(ignoreGemFlowId == gemFlowId)
            continue;
        
        if (g8277cGemUsFlowMap[gemFlowId].isValid) 
        {
            if(g8277cGemUsFlowMap[gemFlowId].tcontId == tcontId)
                return FALSE;
        }
    }
...
{
    unsigned short gemFlowId;
    unsigned int gemPortId = g8277cGemUsFlowMap[ignoreGemFlowId].gemPortId;
    ...
    for (gemFlowId = 0; gemFlowId < RTL8277C_MAX_GEM_FLOW; gemFlowId++) 
    {
        ...
        if (g8277cGemUsFlowMap[gemFlowId].isValid) 
        {
            if(g8277cGemUsFlowMap[gemFlowId].gemPortId == gemPortId)
                return FALSE;
        }
    }
...
static int _rtl8277c_GetPhy2LogicTcontMap(unsigned int phyTcontId,unsigned int *pLogicTcontId)
{
    unsigned char  tcontId;

    if(phyTcontId>=(RTL8277C_MAX_TCONT+1))
        return RT_ERR_FAILED;

    for(tcontId = 0;tcontId<RTL8277C_MAX_TCONT;tcontId++)
    {
        if(g8277cTcontTbl[tcontId].phyTcontId == phyTcontId)
        {
...
static unsigned int _rtl8277c_SetGemFlowMap(
    GEM_FLOW_DIR dir,
    unsigned short gemFlowId, 
...
{    
    /* parameter check */
    RT_PARAM_CHK((gemFlowId >= RTL8277C_MAX_GEM_FLOW), RT_ERR_INPUT);

    if(GEM_FLOW_DIR_DS == dir)
    {
        g8277cGemDsFlowMap[gemFlowId].isValid       = TRUE; 
        g8277cGemDsFlowMap[gemFlowId].gemIdx        = gemIdx;
        g8277cGemDsFlowMap[gemFlowId].gemPortId     = gemPortId;
    }

    else
    {      
        g8277cGemUsFlowMap[gemFlowId].isValid       = TRUE;
        g8277cGemUsFlowMap[gemFlowId].tcontId       = pUsFlow->tcontId;
        g8277cGemUsFlowMap[gemFlowId].tcQueueId     = pUsFlow->tcQueueId;
        g8277cGemUsFlowMap[gemFlowId].aesEn         = pUsFlow->aesEn;
        g8277cGemUsFlowMap[gemFlowId].gemIdx        = gemIdx;
        g8277cGemUsFlowMap[gemFlowId].gemPortId     = gemPortId;
...
static unsigned int 
_rtl8277c_DeleteGemFlowMap(unsigned short gemFlowId,GEM_FLOW_DIR dir)
{   
    /* parameter check */
    RT_PARAM_CHK((gemFlowId >= RTL8277C_MAX_GEM_FLOW), RT_ERR_INPUT);
    ...
    if(GEM_FLOW_DIR_DS == dir)
    {
        g8277cGemDsFlowMap[gemFlowId].isValid   = FALSE;
        g8277cGemDsFlowMap[gemFlowId].gemIdx    = 0x0;
        g8277cGemDsFlowMap[gemFlowId].gemPortId = 0x0;
        g8277cGemDsFlowMap[gemFlowId].loopEn    = 0x0;
        g8277cGemDsFlowMap[gemFlowId].loopLspid = 0x0;

    } 
    else
    {
        g8277cGemUsFlowMap[gemFlowId].isValid       = FALSE;
        g8277cGemUsFlowMap[gemFlowId].tcontId       = 0x0;
        g8277cGemUsFlowMap[gemFlowId].tcQueueId     = 0x0;
        g8277cGemUsFlowMap[gemFlowId].aesEn         = 0x0;
        g8277cGemUsFlowMap[gemFlowId].gemIdx        = 0x0;
        g8277cGemUsFlowMap[gemFlowId].gemPortId     = 0x0;
...
/*Direct call AAL API*/
static int 
_rtl8277c_gpon_aal_tcont_set(unsigned int tcontId, unsigned int allocId)
{
    ...
    if(IS_OMCC_TCONT(tcontId)){
        RTL8277C_LOG_ERR("%s:%d::ERROR: TCONT %d only used for OMCC!",__FUNCTION__,__LINE__,tcontId);
        return RT_ERR_FAILED;
    }

    if (allocId > 0xFFFF ){
        RTL8277C_LOG_ERR("%s:%d:: ERROR alloc_id is :%d",__FUNCTION__,__LINE__,allocId);
        return RT_ERR_FAILED;
    }
    ...
    if(CA_PON_MODE == ONU_PON_MAC_GPON)
    {
        ...
        {
            RTL8277C_LOG_ERR("%s:%d::create tcont_id:%d fail",__FUNCTION__,__LINE__,tcontId);
            return RT_ERR_FAILED;
        }
    }
    else
    ...
    RTL8277C_LOG_DBG("Add TCONT %u Alloc-ID %u",tcontId,  allocId);
...
static int 
_rtl8277c_gpon_aal_tcont_del(unsigned int tcontId)
{
    aal_xgpon_bwmp_alloc_id_cfg_msk_t mask;
    aal_xgpon_bwmp_alloc_id_cfg_t alloc_id_cfg;
    aal_gpon_tcont_cfg_msk_t gpon_mask;
    ...
    if(CA_PON_MODE == ONU_PON_MAC_GPON)
    {
        ...
        if(RT_ERR_OK != _rtl8277c_GetPhy2LogicTcontMap(tcontId, &logic_tcontId))
        {
            RTL8277C_LOG_ERR("%s %d get phy:%u to logic T-cont fail\n",  
        ...
        if (CA_E_OK != aal_gpon_tcont_set(SATURN_DEV_ID, FALSE, allocId, gpon_mask, &gpon_alloc_id_cfg))
        {
            RTL8277C_LOG_ERR("%s:%d::delete tcont_id:%d fail",__FUNCTION__,__LINE__,tcontId);
            return RT_ERR_FAILED;
        }
    }
    else
    {
        ...
        {
            RTL8277C_LOG_ERR("%s:%d::ERROR delete tcont_id:%d fail",__FUNCTION__,__LINE__,tcontId);
            return RT_ERR_FAILED;
        }
#endif
...
static int 
_rtl8277c_gpon_aal_usGem_set(unsigned int usFlowId, const rt_gpon_usFlow_t *pUsFlow, unsigned short *pGemIdx)
{
    ...
    RT_PARAM_CHK((usFlowId >= RTL8277C_MAX_GEM_FLOW), RT_ERR_INPUT);
#if 0
    if(IS_OMCC_TCONT(pUsFlow->tcontId)){
        RTL8277C_LOG_ERR("%s:%d::ERROR: TCONT %d only used for OMCC!",__FUNCTION__,__LINE__,pUsFlow->tcontId);
        return RT_ERR_FAILED;
    }
#endif
    internal_gem_idx = LOGIC_FLOW_TO_RTL8277C_FLOW(usFlowId);
    ...
    if(CA_PON_MODE == ONU_PON_MAC_GPON)
    {
        ...
        {
            RTL8277C_LOG_ERR("%s:%d::ERROR set us gem fail",__FUNCTION__,__LINE__);
            return RT_ERR_FAILED;
        } 
    }
    else
    {
        ...
        {
            RTL8277C_LOG_ERR("%s:%d::ERROR set usbg xgem fail",__FUNCTION__,__LINE__);
            return RT_ERR_FAILED;
        }
    }
    ...
    {
        RTL8277C_LOG_ERR("%s:%d::ERROR set pvtbl fail",__FUNCTION__,__LINE__);
        return RT_ERR_FAILED;
    }
...
static int 
_rtl8277c_gpon_aal_usGem_get(unsigned int usFlowId, rt_gpon_usFlow_t *pUsFlow)
{
    aal_xgpon_usbg_gem_config_t us_xgem_cfg;
    unsigned int internal_gem_idx;
    ...
    internal_gem_idx = g8277cGemUsFlowMap[usFlowId].gemIdx;
    ...
    if(CA_PON_MODE == ONU_PON_MAC_GPON)
    {
        ...
        {
            RTL8277C_LOG_ERR("%s:%d::ERROR set usbg gem fail",__FUNCTION__,__LINE__);
            return RT_ERR_FAILED;
        }
    ...
    else
    {
        ...
        {
            RTL8277C_LOG_ERR("%s:%d::ERROR get usbg xgem fail",__FUNCTION__,__LINE__);
            return RT_ERR_FAILED;
        }
...
static int 
_rtl8277c_gpon_aal_dsGem_set(unsigned int dsFlowId, const rt_gpon_dsFlow_t *pDsFlow, unsigned short *pGemIdx)
{
    ...
    RT_PARAM_CHK((dsFlowId >= RTL8277C_MAX_GEM_FLOW), RT_ERR_INPUT);


    internal_gem_idx = LOGIC_FLOW_TO_RTL8277C_FLOW(dsFlowId);
    ...
    if(CA_PON_MODE == ONU_PON_MAC_GPON)
    {
        ...
        {
            RTL8277C_LOG_ERR("%s:%d::set ds gem fail",__FUNCTION__,__LINE__);
            return RT_ERR_FAILED;
        }
    }
    else
    ...
    if (internal_gem_idx > 0 && pDsFlow->gemPortId != ds_xgem_cfg.gem_id) {
        ...
        {
            RTL8277C_LOG_ERR("%s:%d::set pdc map fail",__FUNCTION__,__LINE__);
            return RT_ERR_FAILED;
        }
...
static int 
_rtl8277c_gpon_aal_dsGem_get(unsigned int dsFlowId, rt_gpon_dsFlow_t *pDsFlow)
{
    aal_xgpon_dsfp_xgem_cfg_t   ds_xgem_cfg;
    unsigned int internal_gem_idx;
    ...
    if(CA_PON_MODE == ONU_PON_MAC_GPON)
    {
        ...
        gem_id = g8277cGemDsFlowMap[dsFlowId].gemPortId;
    ...
    else
    {
        ...
        {
            RTL8277C_LOG_ERR("%s:%d::get ds gem fail",__FUNCTION__,__LINE__);
            return RT_ERR_FAILED;
        }
...
static int 
_rtl8277c_gpon_aal_usGem_del(unsigned int usFlowId)
{
    aal_xgpon_usbg_gem_config_msk_t us_mask;
    aal_xgpon_usbg_gem_config_t us_xgem_cfg;
    unsigned int internal_gem_idx;
    ...
    if(CA_PON_MODE == ONU_PON_MAC_GPON)
    {
        ...
        {
            RTL8277C_LOG_ERR("%s:%d::Error Disable usbg gem fail",__FUNCTION__,__LINE__);
            return RT_ERR_FAILED;
        }
        ...
        if (__tcont_tbl_is_empty(usFlowId)) 
        {
            ...
            unsigned int tcontId = g8277cGemUsFlowMap[usFlowId].tcontId;
            ...
            {
                RTL8277C_LOG_ERR("%s:%d::Error tcont:%d flush fail",__FUNCTION__,__LINE__, tcontId);
                return RT_ERR_FAILED;
            }
            ...
            if (tcontId != gem_id) 
            {
                ...
                {
                    RTL8277C_LOG_ERR("%s:%d::Error pus pvtbl set fail",__FUNCTION__,__LINE__);
                    return RT_ERR_FAILED;
                }
            }
        }
    ...
    else
    {
        ...
        if (__tcont_tbl_is_empty(usFlowId)) 
        {
            ...
            {
                RTL8277C_LOG_ERR("%s:%d::Error bwmp get fail",__FUNCTION__,__LINE__);
                return RT_ERR_FAILED;
            }
            ...
            {
                RTL8277C_LOG_ERR("%s:%d::Error dsfp get fail",__FUNCTION__,__LINE__);
                return RT_ERR_FAILED;
            }
#if 0
...
static int 
_rtl8277c_gpon_aal_dsGem_del(unsigned int dsFlowId)
{
    aal_xgpon_dsfp_xgem_cfg_msk_t ds_mask;
    aal_xgpon_dsfp_xgem_cfg_t   ds_xgem_cfg;
    ...
    if(CA_PON_MODE == ONU_PON_MAC_GPON)
    {
        ...
        ds_gem_id = g8277cGemDsFlowMap[dsFlowId].gemPortId;
        ds_gem_cfg_mask.u32 = 0xffffffff;
        memset(&ds_gem_cfg, 0, sizeof(ds_gem_cfg));
        ds_gem_cfg.gem_index = internal_gem_idx;
        ds_gem_cfg.vld = 0;
    ...
    else
    {
        ...
        if (CA_E_OK != aal_xgpon_dsfp_xgem_set(SATURN_DEV_ID, internal_gem_idx, ds_mask, &ds_xgem_cfg))
        {
            RTL8277C_LOG_ERR("%s:%d::Disable ds gem fail",__FUNCTION__,__LINE__);
            return RT_ERR_FAILED;
        }
    }
...
static int 
_rtl8277c_gpon_aal_loopGem_set(uint8_t enable, uint32 pdcGemIdx, uint32 usGemIdx, uint32 lspid)
{
    ...
    {
        RTL8277C_LOG_ERR("%s:%d::Set PDC map pdcGemIdx:%d fail", __FUNCTION__, __LINE__, pdcGemIdx);
        return RT_ERR_FAILED;
    }
    ...
    {
        RTL8277C_LOG_ERR("%s:%d::Set ILPB lspid:%d fail", __FUNCTION__, __LINE__, lspid);
        return RT_ERR_FAILED;
    }
...
int32
_rtl8277c_gpon_omci_mirror(uint32 msgLen, uint8 *pMsg)
{
    int encap_length;
    ...
    if (p_skb == NULL) 
    {
        RTL8277C_LOG_ERR("%s %d : netdev_alloc_skb fail!!",__FUNCTION__,__LINE__);
        return RT_ERR_FAILED;
    }
...
static uint32_t _rtl8277c_gpon_omci_gen_crc(uint8_t *p, uint32_t len)
{
...
static int _rtl8277c_gpon_omci_build_crc(uint8_t * omci_ptr)
{
    ...
    {
        genCrc = _rtl8277c_gpon_omci_gen_crc(omci_ptr,sizeof(rt_gpon_omci_base_pkt_t)-sizeof(genCrc));
        if(!genCrc)
        {
            RTL8277C_LOG_ERR("%s %d : Generate Crc Error!!",__FUNCTION__,__LINE__);
            return RT_ERR_FAILED;
        }
    ...
    else if (0x0b == pHdr->devId) //extend omci format
    {
        ...
        genCrc = _rtl8277c_gpon_omci_gen_crc(omci_ptr,sizeof(rt_gpon_omci_ext_pkt_t)+contLen);
        if(!genCrc)
        {
            RTL8277C_LOG_ERR("%s %d : Generate Crc Error!!",__FUNCTION__,__LINE__);
            return RT_ERR_FAILED;
    ...
    {
        RTL8277C_LOG_ERR("%s %d : OMCI device id(%d))is not suppert",__FUNCTION__,__LINE__,pHdr->devId);
        return RT_ERR_FAILED;
    }
...
static int _rtl8277c_gpon_omci_check_crc(uint8_t * omci_ptr)
{
    rt_gpon_omci_pkt_hdr_t *pHdr = (rt_gpon_omci_pkt_hdr_t *)omci_ptr;
    ...
    if (0x0a == pHdr->devId) // base omci format
    {
        ...
        if (oriCrc != genCrc)
        {
            RTL8277C_LOG_ERR("%s %d : GenCrc [0x%x] OriCrc [0x%x]",__FUNCTION__,__LINE__,genCrc, oriCrc);
            g8277cOmciCount.omci_rx_mic_error_count++;
            return RT_ERR_FAILED;
        }
        g8277cOmciCount.omci_rx_baseline_count++;
    }
    else if (0x0b == pHdr->devId) //extend omci format
    {
        rt_gpon_omci_ext_pkt_t *pExtHdr = (rt_gpon_omci_ext_pkt_t *)omci_ptr;
        ...
        g8277cOmciCount.omci_rx_extended_count++;
...
static int _rtl8277c_gpon_omci_build_mic(uint8_t * omci_ptr)
{
    rt_gpon_omci_pkt_hdr_t *pHdr = (rt_gpon_omci_pkt_hdr_t *)omci_ptr;
    uint32_t genMic = 0;
    ...
    if (0x0a == pHdr->devId) // base omci format
    {
        ...
        {
            RTL8277C_LOG_ERR("%s %d generate OMCI MIC fail\n",__FUNCTION__,__LINE__);
            return RT_ERR_FAILED;
        }
    ...
    else if (0x0b == pHdr->devId) //extend omci format
    {
        ...
        {
            RTL8277C_LOG_ERR("%s %d : Generate MIC Error!!",__FUNCTION__,__LINE__);
            return RT_ERR_FAILED;
        }
...
static int _rtl8277c_gpon_omci_check_mic(uint8_t * omci_ptr)
{
    rt_gpon_omci_pkt_hdr_t *pHdr = (rt_gpon_omci_pkt_hdr_t *)omci_ptr;
    ...
    if (0x0a == pHdr->devId) // base omci format
    {
        ...
        if (oriMic != genMic)
        {
            RTL8277C_LOG_ERR("%s %d : GenMic [0x%x] oriMic [0x%x]",__FUNCTION__,__LINE__,genMic, oriMic);
            g8277cOmciCount.omci_rx_mic_error_count++;
            return RT_ERR_FAILED;
        }
        g8277cOmciCount.omci_rx_baseline_count++;
...
int _rtl8277c_receive_from_nic(struct napi_struct *napi,struct net_device *dev, struct sk_buff *skb, nic_hook_private_t *nh_priv)
{
    ...
    if (isOmciFlag == TRUE)
    {
        if(gOmciRxCallback)
        {
            ...
            {
                if(_rtl8277c_gpon_omci_check_mic(omci_ptr)!= RT_ERR_OK)
                {
                    RTL8277C_LOG_ERR("%s %d : OMCI check mic Error!!",__FUNCTION__,__LINE__);
                    return RE8670_RX_STOP;
                }
            }
            else if(CA_PON_MODE == ONU_PON_MAC_GPON)
            {
                if(_rtl8277c_gpon_omci_check_crc(omci_ptr)!= RT_ERR_OK)
                {
                    RTL8277C_LOG_ERR("%s %d : OMCI check crc Error!!",__FUNCTION__,__LINE__);
                    return RE8670_RX_STOP;
                }
            }
            else
            {
                RTL8277C_LOG_ERR("%s %d : Not support PON MODE!!",__FUNCTION__,__LINE__);
                return RE8670_RX_STOP;
            }
            ...
            {
                _rtl8277c_gpon_omci_mirror(omci_len, omci_ptr);     
            }
        }
...
static int _rtl8277c_gpon_pm_phyLos_get(rt_gpon_pm_counter_t *pPmCnt)
{
    ...
    if(CA_PON_MODE == ONU_PON_MAC_GPON)
    {
        ...
        {
            RTL8277C_LOG_ERR("%s:%d::get ONU PHY/LOS count fail",__FUNCTION__,__LINE__);
            return RT_ERR_FAILED;
        }
    ...
    else
    {
		...
		{
			RTL8277C_LOG_ERR("%s:%d::get dspf bip dw0 fail",__FUNCTION__,__LINE__);
            return RT_ERR_FAILED;
		}
		if (CA_E_OK != aal_xgpon_dsfp_bip_dw0_get(SATURN_DEV_ID,&bip32_data0))
		{
			RTL8277C_LOG_ERR("%s:%d::get dspf bip dw1 fail",__FUNCTION__,__LINE__);
            return RT_ERR_FAILED;
		}
		if (CA_E_OK != aal_xgpon_dsfp_bip_err_get(SATURN_DEV_ID,&bip32_err))
		{
			RTL8277C_LOG_ERR("%s:%d::get dspf bip err fail",__FUNCTION__,__LINE__);
            return RT_ERR_FAILED;
		}
		if (CA_E_OK != aal_xgpon_dsfp_psbd_hec_err_get(SATURN_DEV_ID,&psbd_hec_err))
		{
			RTL8277C_LOG_ERR("%s:%d::get dspf psbd hec err fail",__FUNCTION__,__LINE__);
            return RT_ERR_FAILED;
		}
		if (CA_E_OK != aal_xgpon_dsfp_psbd_hec_uncorrect_cnt_get(SATURN_DEV_ID,&psbd_hec_uncorrect_cnt))
		{
			RTL8277C_LOG_ERR("%s:%d::get psbd hec uncor fail",__FUNCTION__,__LINE__);
            return RT_ERR_FAILED;
		}
		if (CA_E_OK != aal_xgpon_dsfp_xgtc_hec_correct_cnt_get(SATURN_DEV_ID, &xgtc_hec_correct_cnt))
		{
			RTL8277C_LOG_ERR("%s:%d::get dspf xgtc hec cor fail",__FUNCTION__,__LINE__);
            return RT_ERR_FAILED;
		}
		if (CA_E_OK != aal_xgpon_dsfp_xgtc_hec_uncorrect_cnt_get(SATURN_DEV_ID,&xgtc_hec_uncorrect_cnt))
        {
			RTL8277C_LOG_ERR("%s:%d::get dspf xgtc hec uncor fail",__FUNCTION__,__LINE__);
            return RT_ERR_FAILED;
        }
...
static int _rtl8277c_gpon_pm_fec_get(rt_gpon_pm_counter_t *pPmCnt)
{
    ...
    if(CA_PON_MODE == ONU_PON_MAC_GPON)
    {
        ...
        {
            RTL8277C_LOG_ERR("%s:%d::get ONU fec count fail",__FUNCTION__,__LINE__);
            return RT_ERR_FAILED;
        }
...
static int _rtl8277c_gpon_pm_xgem_get(rt_gpon_pm_counter_t *pPmCnt)
{
    ...
    if(CA_PON_MODE == ONU_PON_MAC_GPON)
    {
        ...
        for (gem = 0; gem < AAL_GPON_SYSTEM_MAX_GEM_NUM; gem++)
        {
            ...
            {
                RTL8277C_LOG_ERR("%s:%d::get ONU DS GEM count %d fail",__FUNCTION__,__LINE__,gem);
                return RT_ERR_FAILED;
            }
            ...
            {
                RTL8277C_LOG_ERR("%s:%d::get ONU US GEM count %d fail",__FUNCTION__,__LINE__,gem);
                return RT_ERR_FAILED;
            }
        ...
        {
            RTL8277C_LOG_ERR("%s:%d::get gpon stats fail",__FUNCTION__,__LINE__);
            return RT_ERR_FAILED;
        }
    ...
    else
    {
		...
		for(xgem_idx = 0; xgem_idx <AAL_GPON_SYSTEM_MAX_GEM_NUM; xgem_idx++){
			...
			{
				RTL8277C_LOG_ERR("%s:%d::get usbg xgem mib fail",__FUNCTION__,__LINE__);
				return RT_ERR_FAILED;
			}
			if (CA_E_OK != aal_xgpon_dsfp_xgem_mib_get( SATURN_DEV_ID, xgem_idx,&dsfp_xgem_mib))
			{
				RTL8277C_LOG_ERR("%s:%d::get dsfp xgem mib fail",__FUNCTION__,__LINE__);
				return RT_ERR_FAILED;
			}
		...
		{
            RTL8277C_LOG_ERR("%s:%d::get usbg stat fail",__FUNCTION__,__LINE__);
            return RT_ERR_FAILED;
        }
		...
		{
            RTL8277C_LOG_ERR("%s:%d::get dsfp xgem err1 fail",__FUNCTION__,__LINE__);
            return RT_ERR_FAILED;
        }
		if (CA_E_OK != aal_xgpon_dsfp_xgem_err0_get(SATURN_DEV_ID,&dsfp_xgem_hec_err0))
		{
            RTL8277C_LOG_ERR("%s:%d::get dsfp xgem err0 fail",__FUNCTION__,__LINE__);
            return RT_ERR_FAILED;
        }
		...
		{
            RTL8277C_LOG_ERR("%s:%d::get dsfp xgem los word fail",__FUNCTION__,__LINE__);
            return RT_ERR_FAILED;
        }
        ...
        {
            RTL8277C_LOG_ERR("%s:%d::get dsfp xgem err key count fail",__FUNCTION__,__LINE__);
            return RT_ERR_FAILED;
        }
...
static int _rtl8277c_gpon_pm_ploam1_get(rt_gpon_pm_counter_t *pPmCnt)
{
    ...
    if(CA_PON_MODE == ONU_PON_MAC_GPON)
    {
        ...
        for(i = AAL_GPON_PLOAM_RX_MIB_UPSTREAM_OVERHEAD; i <= AAL_GPON_PLOAM_RX_MIB_EXTENDED_BURST_LENGTH; i++)
        {
            ...
            {
                RTL8277C_LOG_ERR("%s:%d::get ONU PLOAM1 count fail",__FUNCTION__,__LINE__);
                return RT_ERR_FAILED;
            }
    ...
    else
    {
        ...
        {
            RTL8277C_LOG_ERR("%s:%d::get ONU PLOAM1 RX_MIC_ERR fail",__FUNCTION__,__LINE__);
            return RT_ERR_FAILED;
        }
        ...
        {
            RTL8277C_LOG_ERR("%s:%d::get ONU PLOAM1 RX_DS fail",__FUNCTION__,__LINE__);
            return RT_ERR_FAILED;
        }
        ...
        {
            RTL8277C_LOG_ERR("%s:%d::get ONU PLOAM1 RX_RANGE_TIME fail",__FUNCTION__,__LINE__);
            return RT_ERR_FAILED;
        }
        ...
        {
            RTL8277C_LOG_ERR("%s:%d::get ONU PLOAM1 RX_PROTECT_CTRL fail",__FUNCTION__,__LINE__);
            return RT_ERR_FAILED;
        }
        ...
        {
            RTL8277C_LOG_ERR("%s:%d::get ONU PLOAM1 RX_ADJ_TX_WAVELEN fail",__FUNCTION__,__LINE__);
            return RT_ERR_FAILED;
        }
...
static int _rtl8277c_gpon_pm_ploam2_get(rt_gpon_pm_counter_t *pPmCnt)
{
    ...
    if(CA_PON_MODE == ONU_PON_MAC_GPON)
    {
        ...
        {
            RTL8277C_LOG_ERR("%s:%d::get ONU PLOAM2 count fail",__FUNCTION__,__LINE__);
            return RT_ERR_FAILED;
        }
    ...
    else
    {
        ...
        {
            RTL8277C_LOG_ERR("%s:%d::get ONU PLOAM2 RX_SYS_PROFILE fail",__FUNCTION__,__LINE__);
            return RT_ERR_FAILED;
        }
        ...
        {
            RTL8277C_LOG_ERR("%s:%d::get ONU PLOAM2 RX_CHANNEL_PROFILE fail",__FUNCTION__,__LINE__);
            return RT_ERR_FAILED;
        }
        ...
        {
            RTL8277C_LOG_ERR("%s:%d::get ONU PLOAM2 RX_BURST_PROFILE fail",__FUNCTION__,__LINE__);
            return RT_ERR_FAILED;
        }
        ...
        {
            RTL8277C_LOG_ERR("%s:%d::get ONU PLOAM2 RX_ASSIGN_ONU_ID fail",__FUNCTION__,__LINE__);
            return RT_ERR_FAILED;
        }
        ...
        {
            RTL8277C_LOG_ERR("%s:%d::get ONU PLOAM2 RX_DEACTIVATE fail",__FUNCTION__,__LINE__);
            return RT_ERR_FAILED;
        }
        ...
        {
            RTL8277C_LOG_ERR("%s:%d::get ONU PLOAM2 RX_DISABLE fail",__FUNCTION__,__LINE__);
            return RT_ERR_FAILED;
        }
        ...
        {
            RTL8277C_LOG_ERR("%s:%d::get ONU PLOAM2 RX_REQ_REGISTRATION fail",__FUNCTION__,__LINE__);
            return RT_ERR_FAILED;
        }
        ...
        {
            RTL8277C_LOG_ERR("%s:%d::get ONU PLOAM2 RX_ASSIGN_ALLOC_ID fail",__FUNCTION__,__LINE__);
            return RT_ERR_FAILED;
        }
        ...
        {
            RTL8277C_LOG_ERR("%s:%d::get ONU PLOAM2 RX_KEY_CTRL fail",__FUNCTION__,__LINE__);
            return RT_ERR_FAILED;
        }
        ...
        {
            RTL8277C_LOG_ERR("%s:%d::get ONU PLOAM2 RX_SLEEP_ALLOW fail",__FUNCTION__,__LINE__);
            return RT_ERR_FAILED;
        }
...
static int _rtl8277c_gpon_pm_ploam3_get(rt_gpon_pm_counter_t *pPmCnt)
{
    ...
    if(CA_PON_MODE == ONU_PON_MAC_GPON)
    {
        ...
        {
            RTL8277C_LOG_ERR("%s:%d::get ONU PLOAM3 count fail",__FUNCTION__,__LINE__);
            return RT_ERR_FAILED;
        }
...
static int _rtl8277c_gpon_pm_chan_tuning1_get(rt_gpon_pm_counter_t *pPmCnt)
{
    if(CA_PON_MODE == ONU_PON_MAC_GPON)
    {
        //FIXME
...
static int _rtl8277c_gpon_pm_chan_tuning2_get(rt_gpon_pm_counter_t *pPmCnt)
{
...
static int _rtl8277c_gpon_pm_chan_tuning3_get(rt_gpon_pm_counter_t *pPmCnt)
{
    if(CA_PON_MODE == ONU_PON_MAC_GPON)
    {
        //FIXME
...
static int _rtl8277c_gpon_pm_omci_get(rt_gpon_pm_counter_t *pPmCnt)
{
#if 0
    ...
    if (CA_E_OK != ca_ngp2_port_pm_counters_get(SATURN_DEV_ID,
                                               ...
                                               (ca_uint8_t *)&stats))
    {
        RTL8277C_LOG_ERR("%s:%d::get ONU OMCI count fail",__FUNCTION__,__LINE__);
        return RT_ERR_FAILED;
    }
...
#endif
    pPmCnt->omci.omci_rx_baseline_count     = g8277cOmciCount.omci_rx_baseline_count;
    pPmCnt->omci.omci_rx_extended_count     = g8277cOmciCount.omci_rx_extended_count;
    pPmCnt->omci.omci_rx_mic_error_count    = g8277cOmciCount.omci_rx_mic_error_count;
    /*Read clear*/
    g8277cOmciCount.omci_rx_baseline_count = 0;
    g8277cOmciCount.omci_rx_extended_count = 0;
    g8277cOmciCount.omci_rx_mic_error_count = 0;
    ...
    if(CA_PON_MODE == ONU_PON_MAC_GPON)
    {
        ...
        AAL_FOR_ALL_OMCI_XGEM_DO(gem) {
            ...
            aal_gpon_us_gem_port_mib_get(SATURN_DEV_ID, 2, gem, &gem_up);
            count_up[0] += gem_up.pcnt;
        }
...
static int _rtl8277c_gpon_omcc_get(rt_gpon_omcc_t *pOmcc)
{
    int32 ret = RT_ERR_OK;
    ...
    if(CA_PON_MODE == ONU_PON_MAC_GPON)
    {
        ...
        {
            RTL8277C_LOG_ERR("%s:%d::Error gpon onu get fail",__FUNCTION__,__LINE__);
            return RT_ERR_FAILED;
        }
 ...
 * Note:
 */
int32 dal_rt_rtl8277c_gpon_deinit(uint8 pon_mode)
{
    int32 ret = RT_ERR_OK;
    ca_uint32_t ppid = 0;
 ...
 * Note:
 */
int32 dal_rt_rtl8277c_gpon_init(void)
{
    int ret;
...
//    mdelay(1000);
    ...
    dal_rt_rtl8277c_ponmisc_mode_set(RT_GPON_MODE,RT_BOTH10G_SPEED);
#endif
    ...
    printk("%s %d FPGA init at XGSPON\n",__FUNCTION__,__LINE__);
#endif

    _dal_rtl8277c_gpon_db_init();
        

    /*reset GEM counters*/
    memset(g8277cDsGemCount, 0x0, sizeof(dal_gpon_ds_flow_counter_t)*AAL_XGPON_GEM_PORT_NUM);
    memset(g8277cUsGemCount, 0x0, sizeof(dal_gpon_us_flow_counter_t)*AAL_XGPON_GEM_PORT_NUM);
    ...
    dal_rtl8277c_gpon_init();
    gpon_init = INIT_COMPLETED;
    return RT_ERR_OK;    
}   /* end of dal_rtl8277c_gpon_init */
 ...
 */
int32
dal_rt_rtl8277c_gpon_serialNumber_set(rt_gpon_serialNumber_t *pSN)
{
    ...
    if(CA_PON_MODE == ONU_PON_MAC_GPON)
    {
        ...
        if(CA_E_OK != ret)
        {
            RTL8277C_LOG_ERR("%s %d set gpon sn  fail\n",__FUNCTION__,__LINE__);
            return RT_ERR_FAILED;
        }
    ...
    else
    {
        ...
        if(CA_E_OK != ret)
        {
            RTL8277C_LOG_ERR("%s %d set rand_cfg.seed  fail\n",__FUNCTION__,__LINE__);
            return RT_ERR_FAILED;
        }
    ...
    return RT_ERR_OK;
}   /* end of dal_rtl8277c_gpon_serialNumber_set */
 ...
 */
int32
dal_rt_rtl8277c_gpon_registrationId_set(rt_gpon_registrationId_t *pRegId)
{
    ca_status_t   ret;
    ...
    if(g_admin_state)
    {
        RTL8277C_LOG_ERR("%s %d set gpon rgsd id fail\n",__FUNCTION__,__LINE__);
        return RT_ERR_FAILED;
    }
    ...
    {
        memcpy(g_xgpn_basic_info.registration_id, pRegId->regId, GPON_RGSD_ID_LEN);
    ...
    return RT_ERR_OK;
}   /* end of dal_rt_rtl8277c_registrationId_set */
 ...
 */
int32
dal_rt_rtl8277c_gpon_activate(rt_gpon_initialState_t initState)
{
    ...
    if (ret != CA_E_OK)
    {
        RTL8277C_LOG_ERR("set pon port mac enable failed. ret = %d\r\n", ret);
        return ret;
    }
    g_admin_state = 1;
...
}   /* end of dal_rt_rtl8277c_activate */
...
*/
int32
dal_rt_rtl8277c_gpon_deactivate(void)
{
    int32   ret;
    ca_uint32_t ppid = 0;
    ...
    if (ret != CA_E_OK)
    {
        RTL8277C_LOG_ERR("set pon port mac disable failed. ret = %d\r\n",  ret);
        return ret;
    }
    g_admin_state = 0;
...
}   /* end of dal_rt_rtl8277c_gpon_deactivate */




/* Function Name:
 *      dal_rt_rtl8277c_gpon_onuState_get
 * Description:
 *      Get ONU State
 * Input:
 *
 ...
 * Note:
 */
int32 dal_rt_rtl8277c_gpon_onuState_get(rt_gpon_onuState_t  *pOnuState)
{
    int32 ret;
    rtk_port_t pon_port;
    ...
    if(ret != CA_E_OK)
    {
        RTL8277C_LOG_ERR( "Get ONU state fail, return failed ret value = %x\n",ret);
        return RT_ERR_FAILED;
    }
    ret = RT_ERR_OK;
    ...
    if(ret!=RT_ERR_OK){
        RTL8277C_LOG_ERR( "The onu state is out of range\n");
        return RT_ERR_FAILED;
    }
    
    return RT_ERR_OK;
}   /* end of   dal_rtl8277c_gpon_onuState_get */
 ...
 */
int32
dal_rt_rtl8277c_gpon_omci_tx(uint32 msgLen, uint8 *pMsg)
{
    ...
    if(CA_PON_MODE == ONU_PON_MAC_XGPON1 || CA_PON_MODE == ONU_PON_MAC_XGSPON || CA_PON_MODE == ONU_PON_MAC_NGPON2 || CA_PON_MODE == ONU_PON_MAC_NGPON2_D10G)
    {
        if(_rtl8277c_gpon_omci_build_mic(pMsg)!= RT_ERR_OK)
        {
            RTL8277C_LOG_ERR("%s %d : OMCI build mic Error!!",__FUNCTION__,__LINE__);
            return RT_ERR_FAILED;
        }
    }
    else if(CA_PON_MODE == ONU_PON_MAC_GPON)
    {
        if(_rtl8277c_gpon_omci_build_crc(pMsg)!= RT_ERR_OK)
        {
            RTL8277C_LOG_ERR("%s %d : OMCI build crc Error!!",__FUNCTION__,__LINE__);
            return RT_ERR_FAILED;
        }
    }
    else
...
//#endif
    ...
    if(ENABLED == gOmciMirrorEnable)
    {
        _rtl8277c_gpon_omci_mirror(msgLen, pMsg);     
    }
 ...
 */
int32
dal_rt_rtl8277c_gpon_omci_rx_callback_register(rt_gpon_omci_rx_callback omciRx)
{
    int32   ret;
    rtk_port_t pon;
    ...
    {
        if((ret = nic_register_rxhook(portmask,RENIC_RXPRI_PATCH,_rtl8277c_receive_from_nic))!=RT_ERR_OK)
        {
            RTL8277C_LOG_ERR("nic_register_rxhook Error!!\n");
            return ret;
        }
    ...
    return RT_ERR_OK;
}   /* end of dal_rt_rtl8277c_gpon_omci_rx_callback_register */
 ...
 */
int32 
dal_rt_rtl8277c_gpon_tcont_set(uint32 *pTcontId, uint32 allocId)
{
    ...
    else
    {
        ...
        {
            RTL8277C_LOG_ERR("%s %d Get non used TcontId fail\n",
                __FUNCTION__,__LINE__);
            return RT_ERR_FAILED;
        }

        
        if (RT_ERR_OK == _rtl8277c_gpon_aal_tcont_set(LOGIC_TCONID_TO_RTL8277C_TCONID(tcontId), allocId))
        {
        ...
        {
            RTL8277C_LOG_ERR("%s %d set ca gpon tcont fail\n",__FUNCTION__,__LINE__);
            return RT_ERR_FAILED;
        }
    }
    RTL8277C_LOG_DBG("%s %d allocId:%u tcontId:%d phytcont=%d\n",__FUNCTION__,__LINE__,allocId,*pTcontId,LOGIC_TCONID_TO_RTL8277C_TCONID(tcontId));  
 ...
 */
int32 
dal_rt_rtl8277c_gpon_tcont_get(uint32 tcontId, uint32 *pAllocId)
{
    ...
    {
        //RTL8277C_LOG_DBG("%s %d : no matched TCONT id [%d]\n", __FUNCTION__, __LINE__, tcontId);  
        return RT_ERR_FAILED;
    }
 ...
 */
int32 
dal_rt_rtl8277c_gpon_tcont_del(uint32 tcontId)
{
    ...
    if(RT_ERR_OK != _rtl8277c_gpon_aal_tcont_del(LOGIC_TCONID_TO_RTL8277C_TCONID(tcontId)))
    {
        RTL8277C_LOG_ERR("%s %d set del tcont fail\n",__FUNCTION__,__LINE__);
        return RT_ERR_FAILED;
    }
    ...
    RTL8277C_LOG_DBG("%s %d tcontId:%d phytcont=%d\n",__FUNCTION__,__LINE__,tcontId,LOGIC_TCONID_TO_RTL8277C_TCONID(tcontId));
 ...
 * Note:
 */
int32 dal_rt_rtl8277c_gpon_usFlow_set(uint32 usFlowId, const rt_gpon_usFlow_t *pUsFlow)
{
    ...
    RT_PARAM_CHK((NULL == pUsFlow), RT_ERR_NULL_POINTER);
    RT_PARAM_CHK((usFlowId >= RTL8277C_MAX_GEM_FLOW), RT_ERR_INPUT);
    RT_PARAM_CHK((pUsFlow->tcontId >= RTL8277C_MAX_TCONT && pUsFlow->tcontId!= RT_GPON_OMCC_TCONT_ID), RT_ERR_INPUT);
    RT_PARAM_CHK((pUsFlow->tcQueueId >= RTL8277C_MAX_TCONT_QUEUE), RT_ERR_INPUT);
    
    RTL8277C_LOG_DBG(
    "%s %d flowId:%d gemportId:%d tcont:%d tcontQ:%d\n",
    __FUNCTION__,__LINE__,
    usFlowId,
    pUsFlow->gemPortId,
    pUsFlow->tcontId,
    ...
    if((TRUE == g8277cGemUsFlowMap[usFlowId].isValid) && 
        (g8277cGemUsFlowMap[usFlowId].tcQueueId == pUsFlow->tcQueueId) &&
        (g8277cGemUsFlowMap[usFlowId].tcontId) == LOGIC_TCONID_TO_RTL8277C_TCONID(pUsFlow->tcontId) &&
        g8277cGemUsFlowMap[usFlowId].aesEn == pUsFlow->aesEn)

    {        
        RTL8277C_LOG_ERR("%s %d Update gemFlowId:%d \n",
    ...
    usFlowCfg.tcontId = LOGIC_TCONID_TO_RTL8277C_TCONID(pUsFlow->tcontId);
    if (RT_ERR_OK != _rtl8277c_gpon_aal_usGem_set(
                                usFlowId, 
                                &usFlowCfg, 
                                &gemIdx))
    {
        RTL8277C_LOG_ERR("%s %d set us flow fail\n",
            __FUNCTION__,__LINE__);
        return RT_ERR_FAILED;
    }
    ...
    if(RT_ERR_OK != _rtl8277c_SetGemFlowMap(GEM_FLOW_DIR_US, usFlowId, &usFlowCfg, gemIdx, pUsFlow->gemPortId))
    {
        RTL8277C_LOG_ERR("%s %d set gemflow map fail\n",__FUNCTION__,__LINE__);
        return RT_ERR_FAILED;            
    }
 ...
 * Note:
 */
int32 dal_rt_rtl8277c_gpon_usFlow_get(uint32 usFlowId, rt_gpon_usFlow_t *pUsFlow)
{
    ...
    if(TRUE != g8277cGemUsFlowMap[usFlowId].isValid) 
    {        
        //RTL8277C_LOG_ERR("%s %d invalid us flow [%d]\n", __FUNCTION__, __LINE__, usFlowId);
        return RT_ERR_ENTRY_NOTFOUND;
    }

    if (RT_ERR_OK != _rtl8277c_gpon_aal_usGem_get(
                                usFlowId, 
                                pUsFlow))
    {
        RTL8277C_LOG_ERR("%s %d get us gem fail\n",
            __FUNCTION__,__LINE__);
        return RT_ERR_FAILED;
    }
    

    if(IS_OMCC_TCONT(g8277cGemUsFlowMap[usFlowId].tcontId))
    {
    ...
    else
    {
        ...
        if(RT_ERR_OK != _rtl8277c_GetPhy2LogicTcontMap(g8277cGemUsFlowMap[usFlowId].tcontId, &logicTcontId))
        {
            RTL8277C_LOG_ERR("%s %d get phy:%u to logic T-cont fail\n",  
                __FUNCTION__, __LINE__, g8277cGemUsFlowMap[usFlowId].tcontId);
    ...
    pUsFlow->tcQueueId      = g8277cGemUsFlowMap[usFlowId].tcQueueId;
 ...
 * Note:
 */
int32 dal_rt_rtl8277c_gpon_usFlow_del(uint32 usFlowId)
{
    /* check Init status */
    RT_INIT_CHK(gpon_init);
    ...
    RTL8277C_LOG_DBG("%s %d flowId:%d gemIdx:0x%x\n",
        __FUNCTION__,
        __LINE__,
        usFlowId,
        g8277cGemUsFlowMap[usFlowId].gemIdx);

    /* function body */

    /*Check the US flow is exist*/
    if(FALSE == g8277cGemUsFlowMap[usFlowId].isValid)
        return RT_ERR_OK;
    

    if (RT_ERR_OK != _rtl8277c_gpon_aal_usGem_del(usFlowId))
    {
        RTL8277C_LOG_ERR("%s %d del us flow fail\n",
            __FUNCTION__,__LINE__);
        return RT_ERR_FAILED;
    }


    if(RT_ERR_OK != _rtl8277c_DeleteGemFlowMap(usFlowId, GEM_FLOW_DIR_US))
    {
        RTL8277C_LOG_ERR("%s %d del gemflow map fail\n",__FUNCTION__,__LINE__);
        return RT_ERR_FAILED;            
    }
 ...
 * Note:
 */
int32 dal_rt_rtl8277c_gpon_usFlow_delAll(void)
{
    int32 ret = RT_ERR_OK;
    uint32 i;
    ...
    for(i=0;i<RTL8277C_MAX_GEM_FLOW;i++)
    {
        /*Delete DS gem flow*/
        if(TRUE == g8277cGemUsFlowMap[i].isValid)
        {
            if(RT_ERR_OK != dal_rt_rtl8277c_gpon_usFlow_del(i))
            {
                ret = RT_ERR_FAILED;
            }
 ...
 * Note:
 */
int32 dal_rt_rtl8277c_gpon_dsFlow_set(uint32 dsFlowId, const rt_gpon_dsFlow_t *pDsFlow)
{
    unsigned short gemIdx = 0;
    ...
    /* function body */
    if (RT_ERR_OK != _rtl8277c_gpon_aal_dsGem_set(
                                dsFlowId, 
                                pDsFlow, 
                                &gemIdx))
    {
        RTL8277C_LOG_ERR("%s %d set ds flow fail\n",__FUNCTION__,__LINE__);
        return RT_ERR_FAILED;
    }

        //save the gemport index
    if(RT_ERR_OK != _rtl8277c_SetGemFlowMap(GEM_FLOW_DIR_DS, dsFlowId, NULL, gemIdx, pDsFlow->gemPortId))
    {
        RTL8277C_LOG_ERR("%s %d set gemflow map fail\n",__FUNCTION__,__LINE__);
        return RT_ERR_FAILED;            
    }
 ...
 * Note:
 */
int32 dal_rt_rtl8277c_gpon_dsFlow_get(uint32 dsFlowId, rt_gpon_dsFlow_t *pDsFlow)
{
    ...
    /* function body */
    if(TRUE != g8277cGemDsFlowMap[dsFlowId].isValid)
    {
        //RTL8277C_LOG_ERR("%s %d invalid ds flow [%d]\n", __FUNCTION__, __LINE__, dsFlowId);
        return RT_ERR_ENTRY_NOTFOUND;
    }
    if (RT_ERR_OK != _rtl8277c_gpon_aal_dsGem_get(
                                dsFlowId, 
                                pDsFlow))
    {
        RTL8277C_LOG_ERR("%s %d get ds flow fail\n",__FUNCTION__,__LINE__);
        return RT_ERR_FAILED;
    }
 ...
 * Note:
 */
int32 dal_rt_rtl8277c_gpon_dsFlow_del(uint32 dsFlowId)
{
    ...
    RTL8277C_LOG_DBG(
        ...
        __FUNCTION__,__LINE__,
        dsFlowId,
        g8277cGemDsFlowMap[dsFlowId].gemIdx);
    ...
    if(FALSE == g8277cGemDsFlowMap[dsFlowId].isValid)
        return RT_ERR_ENTRY_NOTFOUND;


    /*Recovery loop GEM setting before delete ds gem*/
    if(TRUE == g8277cGemDsFlowMap[dsFlowId].loopEn)
    {

        if (RT_ERR_OK != _rtl8277c_gpon_aal_loopGem_set(FALSE, 
                LOGIC_FLOW_TO_RTL8277C_FLOW(dsFlowId),
                0,
                g8277cGemDsFlowMap[dsFlowId].loopLspid))
        {
            RTL8277C_LOG_ERR("%s %d disable loop gem fail,dsFlowId=%d \n",__FUNCTION__,__LINE__, dsFlowId);
            return RT_ERR_FAILED;
        }
    }


    if (RT_ERR_OK != _rtl8277c_gpon_aal_dsGem_del(dsFlowId))
    {
        RTL8277C_LOG_ERR("%s %d del ds flow fail\n",__FUNCTION__,__LINE__);
        return RT_ERR_FAILED;
    }

    if(RT_ERR_OK != _rtl8277c_DeleteGemFlowMap(dsFlowId, GEM_FLOW_DIR_DS))
    {
        RTL8277C_LOG_ERR("%s %d set us gemflow map fail\n",__FUNCTION__,__LINE__);
        return RT_ERR_FAILED;            
    }
 ...
 * Note:
 */
int32 dal_rt_rtl8277c_gpon_dsFlow_delAll(void)
{
    int ret = RT_ERR_OK;
    unsigned int i;
    ...
    {
        /*Delete DS gem flow*/
        if(TRUE == g8277cGemDsFlowMap[i].isValid)
        {
            if(RT_ERR_OK != dal_rt_rtl8277c_gpon_dsFlow_del(i))
            {
                ret = RT_ERR_FAILED;
            }
        }
 ...
 * Note:
 */
int32 dal_rt_rtl8277c_loop_gemPort_set(uint32 gemPortId, rt_enable_t enable)
{
    ...
    {
        if(TRUE == g8277cGemDsFlowMap[i].isValid && 
            gemPortId == g8277cGemDsFlowMap[i].gemPortId)
        {
            isFound = TRUE;
            dsFlowId = i;
            pdcGemIdx = g8277cGemDsFlowMap[i].gemIdx; 
            
            RTL8277C_LOG_DBG("%s %d dsFlowId=%d, pdcGemIdx=%d",
    ...
    for(i=0;i<RTL8277C_MAX_GEM_FLOW;i++)
    {
        /*Delete DS gem flow*/
        if(TRUE == g8277cGemUsFlowMap[i].isValid &&
            gemPortId == g8277cGemUsFlowMap[i].gemPortId)
        {
            isFound = TRUE;
            usGemIdx = g8277cGemUsFlowMap[i].gemIdx;
            lspid = RTL8277C_TCONID_TO_LDPID(g8277cGemUsFlowMap[i].tcontId);

            RTL8277C_LOG_DBG("%s %d usGemIdx=%d, lspid=0x%X",
    ...
    if (RT_ERR_OK != _rtl8277c_gpon_aal_loopGem_set(enable, 
            pdcGemIdx,
            usGemIdx,
            lspid))
    {
        RTL8277C_LOG_ERR("%s %d set loop gem fail,dsFlowId=%d \n",__FUNCTION__,__LINE__, gemPortId);
        return RT_ERR_FAILED;
    }

    g8277cGemDsFlowMap[dsFlowId].loopEn = enable;
    g8277cGemDsFlowMap[dsFlowId].loopLspid = enable? lspid: 7;
 ...
 * Note:
 */
int32 dal_rt_rtl8277c_loop_gemPort_get(uint32 gemPortId, rt_enable_t *pEnable)
{
    unsigned int i;
    ...
    *pEnable = 0;
    for(i=0;i<RTL8277C_MAX_GEM_FLOW;i++)
    {
        if(TRUE == g8277cGemDsFlowMap[i].isValid && 
            gemPortId == g8277cGemDsFlowMap[i].gemPortId)
        {
            *pEnable = g8277cGemDsFlowMap[i].loopEn; 
            return RT_ERR_OK;
 ...
 * Note:
 */
int32 dal_rt_rtl8277c_gpon_ponQueue_set(
    uint32 tcontId, 
    uint32 tcQueueId,  
    const rt_gpon_queueCfg_t *pQueuecfg)
{
    ...
    RT_PARAM_CHK((tcontId >= RTL8277C_MAX_TCONT), RT_ERR_INPUT);
    RT_PARAM_CHK((tcQueueId >= RTL8277C_MAX_TCONT_QUEUE), RT_ERR_INPUT);
    ...
    ret = dal_rtl8277c_ponmac_queue_add(&queue, &queueCfg);

    if(RT_ERR_OK != ret)
    {
        RTL8277C_LOG_ERR("%s %d dal_rtl8277c_ponmac_queue_add fail \n",__FUNCTION__,__LINE__);
        return RT_ERR_FAILED;
    }
    return RT_ERR_OK;
    ...
    rt_gpon_queueCfg_t *pQueuecfg)
{
    ...
    ret = dal_rtl8277c_ponmac_queue_get(&queue, &queueCfg);

    if(RT_ERR_OK != ret)
    {
        //RTL8277C_LOG_ERR("%s %d dal_rtl8277c_ponmac_queue_get fail \n",__FUNCTION__,__LINE__);
        return RT_ERR_FAILED;
    }
    
    pQueuecfg->cir = (RTL8277C_METER_RATE_MAX == queueCfg.cir)?RT_GPON_PON_QUEUE_PIR_UNLIMITED:RATE_KBPS_TO_8KBPS(queueCfg.cir);
    pQueuecfg->pir = (RTL8277C_METER_RATE_MAX == queueCfg.pir)?RT_GPON_PON_QUEUE_PIR_UNLIMITED:RATE_KBPS_TO_8KBPS(queueCfg.pir);
 ...
 * Note:
 */
int32 dal_rt_rtl8277c_gpon_ponQueue_del(    
    uint32 tcontId, 
    uint32 tcQueueId)
{
    rtk_ponmac_queue_t queue;
    ...
    return dal_rtl8277c_ponmac_queue_del(&queue);
 ...
 */
int32 
dal_rt_rtl8277c_gpon_scheInfo_get(rt_gpon_schedule_info_t *pScheInfo)
{
    /* check Init status */
    RT_INIT_CHK(gpon_init);
    ...
    pScheInfo->max_flow         = RTL8277C_MAX_GEM_FLOW;
    pScheInfo->max_pon_queue    = RTL8277C_MAX_PON_QUEUE;
    pScheInfo->max_tcont        = RTL8277C_MAX_TCONT;
    pScheInfo->max_tcon_queue   = RTL8277C_MAX_TCONT_QUEUE;
 ...
 */
int32 
dal_rt_rtl8277c_gpon_flowCounter_get(uint32 flowId, rt_gpon_flow_counter_type_t flowCntType, rt_gpon_flow_counter_t *pFlowCnt)
{
    ...
    /* parameter check */
    RT_PARAM_CHK((flowId >= RTL8277C_MAX_GEM_FLOW), RT_ERR_INPUT);
    ...
    if(CA_PON_MODE == ONU_PON_MAC_GPON)
    {
        ...
        switch(flowCntType) 
        {
            ...
            {
                if(g8277cGemDsFlowMap[flowId].isValid == TRUE)
                {
                internal_gem_idx = g8277cGemDsFlowMap[flowId].gemIdx;
                    ...
                    {
                        RTL8277C_LOG_ERR("%s:%d::get ONU DS GEM count %d fail",__FUNCTION__,__LINE__,internal_gem_idx);
                        return RT_ERR_FAILED;
                    }
                    pFlowCnt->dsgem.gem_block = ds_gem.fcnt + g8277cDsGemCount[internal_gem_idx].dsgem.gem_block;
                    pFlowCnt->dsgem.gem_byte = ds_gem.bcnt + g8277cDsGemCount[internal_gem_idx].dsgem.gem_byte;

                    g8277cDsGemCount[internal_gem_idx].dseth.eth_pkt_fwd = ds_gem.pcnt;
                    g8277cDsGemCount[internal_gem_idx].dsgem.gem_block = 0;
                    g8277cDsGemCount[internal_gem_idx].dsgem.gem_byte = 0;
            ...
            {
                if(g8277cGemUsFlowMap[flowId].isValid == TRUE)
                {
                    internal_gem_idx = g8277cGemUsFlowMap[flowId].gemIdx;
                    ...
                    if(CA_E_OK == ret)
                    {
                        pFlowCnt->usgem.gem_block = us_gem.fcnt + g8277cUsGemCount[internal_gem_idx].usgem.gem_block;
                        pFlowCnt->usgem.gem_byte = us_gem.bcnt + g8277cUsGemCount[internal_gem_idx].usgem.gem_byte;
        
                        g8277cUsGemCount[internal_gem_idx].useth.eth_cnt = us_gem.pcnt;
                        g8277cUsGemCount[internal_gem_idx].usgem.gem_block = 0;
                        g8277cUsGemCount[internal_gem_idx].usgem.gem_byte = 0;
                    }
                }
            ...
            case RT_GPON_CNT_TYPE_FLOW_DS_ETH:
            {
                if(g8277cGemDsFlowMap[flowId].isValid == TRUE)
                {
                    ...
                    pFlowCnt->dseth.eth_pkt_fwd = ds_gem.pcnt + g8277cDsGemCount[internal_gem_idx].dseth.eth_pkt_fwd;

                    g8277cDsGemCount[internal_gem_idx].dsgem.gem_block = ds_gem.fcnt;
                    g8277cDsGemCount[internal_gem_idx].dsgem.gem_byte = ds_gem.bcnt;
                    g8277cDsGemCount[internal_gem_idx].dseth.eth_pkt_fwd = 0;
                }
                break;
            }
            case RT_GPON_CNT_TYPE_FLOW_US_ETH:
            {
                if(g8277cGemUsFlowMap[flowId].isValid == TRUE)
                {
                    ...
                    if(CA_E_OK == ret)
                    {
                        pFlowCnt->useth.eth_cnt = us_gem.pcnt + g8277cUsGemCount[internal_gem_idx].useth.eth_cnt;

                        g8277cUsGemCount[internal_gem_idx].usgem.gem_block = us_gem.fcnt;
                        g8277cUsGemCount[internal_gem_idx].usgem.gem_byte = us_gem.bcnt;
                        g8277cUsGemCount[internal_gem_idx].useth.eth_cnt = 0;
                    }
                }
                break;
            }
            default:
                ret = CA_E_ERROR;
                RTL8277C_LOG_ERR("%s %d un-support flowCntType:%d\n",__FUNCTION__,__LINE__,flowCntType);
                break;
        }
    }
    else
        ...
        switch(flowCntType) 
        {
            ...
            memset(&dsfp_xgem_mib, 0x0, sizeof(aal_xgpon_dsfp_xgem_mib_config_t));
                ret = aal_xgpon_dsfp_xgem_mib_get(SATURN_DEV_ID,internal_gem_idx&0xFF,&dsfp_xgem_mib);
                    if(CA_E_OK == ret)
                    {
                        pFlowCnt->dsgem.gem_block = dsfp_xgem_mib.frame_cnt + g8277cDsGemCount[internal_gem_idx].dsgem.gem_block;
                        pFlowCnt->dsgem.gem_byte = dsfp_xgem_mib.byte_cnt + g8277cDsGemCount[internal_gem_idx].dsgem.gem_byte;
    
                        g8277cDsGemCount[internal_gem_idx].dseth.eth_pkt_fwd = dsfp_xgem_mib.pkt_cnt;
                        g8277cDsGemCount[internal_gem_idx].dsgem.gem_block = 0;
                        g8277cDsGemCount[internal_gem_idx].dsgem.gem_byte = 0;
                    }
                }
            ...
            memset(&usbg_xgem_mib, 0x0, sizeof(aal_xgpon_usbg_xgem_mib_config_t));
                    ...
                    if(CA_E_OK == ret)
                    {
                        pFlowCnt->usgem.gem_block = usbg_xgem_mib.frame_cnt + g8277cUsGemCount[internal_gem_idx].usgem.gem_block;
                        pFlowCnt->usgem.gem_byte = usbg_xgem_mib.byte_cnt + g8277cUsGemCount[internal_gem_idx].usgem.gem_byte;
    
                        g8277cUsGemCount[internal_gem_idx].useth.eth_cnt = usbg_xgem_mib.pkt_cnt;
                        g8277cUsGemCount[internal_gem_idx].usgem.gem_block = 0;
                        g8277cUsGemCount[internal_gem_idx].usgem.gem_byte = 0;
                    }
                }
            ...
            case RT_GPON_CNT_TYPE_FLOW_DS_ETH:
            {
                if(g8277cGemDsFlowMap[flowId].isValid == TRUE)
                {
                    ...
                    if(CA_E_OK == ret)
                    {
                        pFlowCnt->dseth.eth_pkt_fwd = dsfp_xgem_mib.pkt_cnt + g8277cDsGemCount[internal_gem_idx].dseth.eth_pkt_fwd;
    
                        g8277cDsGemCount[internal_gem_idx].dsgem.gem_block = dsfp_xgem_mib.frame_cnt;
                        g8277cDsGemCount[internal_gem_idx].dsgem.gem_byte = dsfp_xgem_mib.byte_cnt;
                        g8277cDsGemCount[internal_gem_idx].dseth.eth_pkt_fwd = 0;
                    }
                }
                break;
            ...
            case RT_GPON_CNT_TYPE_FLOW_US_ETH:
            {
                if(g8277cGemUsFlowMap[flowId].isValid == TRUE)
                {
                    ...
                    if(CA_E_OK == ret)
                    {
                        pFlowCnt->useth.eth_cnt = usbg_xgem_mib.pkt_cnt + g8277cUsGemCount[internal_gem_idx].useth.eth_cnt;

                        g8277cUsGemCount[internal_gem_idx].usgem.gem_block = usbg_xgem_mib.frame_cnt;
                        g8277cUsGemCount[internal_gem_idx].usgem.gem_byte = usbg_xgem_mib.byte_cnt;
                        g8277cUsGemCount[internal_gem_idx].useth.eth_cnt = 0;
                    }
                }
                break;
...
}   /* end of dal_rt_rtl8277c_gpon_flowCounter_get */
 ...
 */
int32 
dal_rt_rtl8277c_gpon_pmCounter_get(rt_gpon_pm_type_t pmCntType, rt_gpon_pm_counter_t *pPmCnt)
{
    ca_status_t ret = CA_E_OK;
    ...
    switch (pmCntType)
    {
        case RT_GPON_PM_TYPE_PHY_LOS:
            ret = _rtl8277c_gpon_pm_phyLos_get(pPmCnt);
        break;
        case RT_GPON_PM_TYPE_FEC:
            ret = _rtl8277c_gpon_pm_fec_get(pPmCnt);
        break;
        case RT_GPON_PM_TYPE_XGEM:
            ret = _rtl8277c_gpon_pm_xgem_get(pPmCnt);
        break;
        case RT_GPON_PM_TYPE_PLOAM_PM1:
            ret = _rtl8277c_gpon_pm_ploam1_get(pPmCnt);
        break;
        case RT_GPON_PM_TYPE_PLOAM_PM2:
            ret = _rtl8277c_gpon_pm_ploam2_get(pPmCnt);
        break;
        case RT_GPON_PM_TYPE_PLOAM_PM3:
            ret = _rtl8277c_gpon_pm_ploam3_get(pPmCnt);
        break;
        case RT_GPON_PM_TYPE_CHAN_TUNING_PM1:
            ret = _rtl8277c_gpon_pm_chan_tuning1_get(pPmCnt);
        break;
        case RT_GPON_PM_TYPE_CHAN_TUNING_PM2:
            ret = _rtl8277c_gpon_pm_chan_tuning2_get(pPmCnt);
        break;
        case RT_GPON_PM_TYPE_CHAN_TUNING_PM3:
            ret = _rtl8277c_gpon_pm_chan_tuning3_get(pPmCnt);
        break;
        case RT_GPON_PM_TYPE_OMCI:
            ret = _rtl8277c_gpon_pm_omci_get(pPmCnt);
        break;
        default:
        break;
    }
 ...
 */
int32 
dal_rt_rtl8277c_gpon_ponTag_get(rt_gpon_ponTag_t *pPonTag)
{
    /* check Init status */
    RT_INIT_CHK(gpon_init);
 ...
 */
int32 
dal_rt_rtl8277c_gpon_msk_set(rt_gpon_msk_t *pMsk)
{
    ...
    {
        RTL8277C_LOG_ERR("%s %d get aes plmp glb ctrl fail\n",__FUNCTION__,__LINE__);
        return RT_ERR_FAILED;
    }
	if(aes_plmp_gbl_ctrl.sma_mode == 0)
	{
		...
		{
			RTL8277C_LOG_ERR("%s %d set aes plmp glb ctrl fail\n",__FUNCTION__,__LINE__);
			return RT_ERR_FAILED;
		}
	}
	...
	{
        RTL8277C_LOG_ERR("%s %d set aes plmp msk fail\n",__FUNCTION__,__LINE__);
        return RT_ERR_FAILED;
    }

	if (CA_E_OK != xgpn_aes_key_generate_by_msk(SATURN_DEV_ID, &pMsk->mskData[0]))
	{
        RTL8277C_LOG_ERR("%s %d set aes key ge by msk fail\n",__FUNCTION__,__LINE__);
        return RT_ERR_FAILED;
    }
 ...
 */
int32 
dal_rt_rtl8277c_gpon_mcKey_set(rt_gpon_mc_key_t *pMcKey)
{
    uint32 keyIdx = 0;
    ...
    if(aal_pon_mac_mode_get(SATURN_DEV_ID) != ONU_PON_MAC_GPON){
            ...
            case CA_MC_FLAG_MC_ENCRYPTION_KEY_SET:
                if(keyIdx == CA_NGP2_KEY_INDEX_0){
                    if(!key_ctrl_cfg.key0_idx_vld){
                        ...
                        {
							RTL8277C_LOG_ERR("%s %d set Multicast Key fail\n",__FUNCTION__,__LINE__);
							ret = RT_ERR_FAILED;
						}
                    }
                }
    ...
    switch(attributeType)
    {
        case RT_GPON_ATTRIBUTE_TYPE_DBRU_STATUS:
            ...
            else
            {
                ...
                {
                    RTL8277C_LOG_ERR("%s %d : aal_xgpon_usbg_xgtc_ctrl_set Error!! ret=%x",__FUNCTION__,__LINE__,ret);
                    ret = RT_ERR_FAILED;
                }               
            }
            break;
        ...
        case RT_GPON_ATTRIBUTE_TYPE_SD_THRESHOLD:
            if(CA_PON_MODE == ONU_PON_MAC_GPON)
            {
                ...
                {
                    RTL8277C_LOG_ERR("%s %d : aal_gpon_sf_degrd_alrm_thrsd_set Error!! ret=%x",__FUNCTION__,__LINE__,ret);;
                    ret = RT_ERR_FAILED;
                }
    ...
    switch(attributeType)
    {
        case RT_GPON_ATTRIBUTE_TYPE_DBRU_STATUS:
            ...
            else
            {
                ...
                {
                    RTL8277C_LOG_ERR("%s %d : aal_xgpon_usbg_xgtc_ctrl_get Error!! ret=%x",__FUNCTION__,__LINE__,ret);
                    return RT_ERR_FAILED;
                }
        ...
        case RT_GPON_ATTRIBUTE_TYPE_GEM_BLOCK_LENGTH:
            ...
            {/*support 48, 64, 128, 256, 512, 1024, 1536, 2048 bytes*/
                ...
                {
                    RTL8277C_LOG_ERR("%s %d : aal_gpon_mac_cfg_get Error!! ret=%x",__FUNCTION__,__LINE__,ret);
                    return RT_ERR_FAILED;
                }
        ...
        case RT_GPON_ATTRIBUTE_TYPE_SD_THRESHOLD:
            if(CA_PON_MODE == ONU_PON_MAC_GPON)
            {
                ...
                {
                    RTL8277C_LOG_ERR("%s %d : aal_gpon_sf_degrd_alrm_thrsd_get Error!!, ret=%x",__FUNCTION__,__LINE__,ret);
                    ret = RT_ERR_FAILED;
                }
                pAttributeValue->sdThreshold = th_cfg.degrade_threshold;
            }
        ...
        case RT_GPON_ATTRIBUTE_TYPE_GTC_INFO:
            if(CA_PON_MODE == ONU_PON_MAC_GPON)
            {
                aal_gpon_us_frm_ctrl_cfg_t frm_cfg;

                if (CA_E_OK != (ret = aal_gpon_eqd_get(SATURN_DEV_ID, 0, &eqd)))
                {
                    RTL8277C_LOG_ERR("%s %d : aal_gpon_eqd_get Error!! ret=%x",__FUNCTION__,__LINE__,ret);
                    ret = RT_ERR_FAILED;
                }
                pAttributeValue->gtc_info.eqd = eqd;
                
                if (CA_E_OK != (ret = aal_gpon_us_frm_ctrl_get(SATURN_DEV_ID, &frm_cfg)))
                {
                    RTL8277C_LOG_ERR("%s %d : aal_gpon_us_frm_ctrl_get Error!! ret=%x",__FUNCTION__,__LINE__,ret);
                    ret = RT_ERR_FAILED;
                }
                /* response_time is base_time add offset*/
                if((frm_cfg.frame_var>>8)&0x1)
                {
                    pAttributeValue->gtc_info.responTime = 35048 - (0x200-(frm_cfg.frame_var&0x1ff))*643/100;
                }
                else
                {
                    pAttributeValue->gtc_info.responTime = 35048 + frm_cfg.frame_var*643/100;
                }
            }
            else
            {
                ...
                {
                    RTL8277C_LOG_ERR("%s %d : aal_xgpon_bwmp_eqd_sts_get Error!! ret=%x",__FUNCTION__,__LINE__,ret);
                    ret = RT_ERR_FAILED;
                }
                pAttributeValue->gtc_info.eqd = eqd;
                pAttributeValue->gtc_info.responTime = 35000;
            }
            break;
        default:
            ret = RT_ERR_INPUT;
 ...
 * Note:
 */
int32 dal_rt_rtl8277c_gpon_usFlow_phyData_get
(uint32 usFlowId, uint32 *pTcontId, uint32 *pTcQueueId, uint32 *pGemIdx)
{
    ...
    *pTcontId        = RTL8277C_TCONID_TO_LDPID(g8277cGemUsFlowMap[usFlowId].tcontId);
    *pTcQueueId      = g8277cGemUsFlowMap[usFlowId].tcQueueId;
    *pGemIdx         = g8277cGemUsFlowMap[usFlowId].gemIdx;   
...
dal_rt_rtl8277c_gpon_omcc_get(rt_gpon_omcc_t *pOmcc)
{
    ...
    ret = _rtl8277c_gpon_omcc_get(pOmcc);
    if (RT_ERR_OK != ret)
    {
        RTL8277C_LOG_ERR("%s:%d::Error OMCC get fail",__FUNCTION__,__LINE__);
    }
...
/* Function Name:
 *      dal_rt_rtl8277c_gpon_omci_mirror_get
 * Description:
 ...
 */
int32
dal_rt_rtl8277c_gpon_omci_mirror_get(rt_enable_t *pEnable, rt_port_t *pMirroringPort)
{
    /* parameter check */
    RT_PARAM_CHK((NULL == pEnable), RT_ERR_NULL_POINTER);
    ...
    *pMirroringPort = gOmciMirrorEnable?gMirroringPort:0;
    ...
    return RT_ERR_OK;
}   /* end of dal_rt_rtl8277c_gpon_omci_mirror_get */
...
dal_rt_rtl8277c_gpon_fec_get(rt_gpon_fec_status_t *pFecStatus)
{
    ...
    if(CA_PON_MODE == ONU_PON_MAC_GPON)
    {
        ...
        {
            RTL8277C_LOG_ERR("%s %d : aal_gpon_fec_get Error!!, ret=%x",__FUNCTION__,__LINE__,ret);
            return RT_ERR_FAILED;
        }
    ...
    else
    {
        ...
        if(CA_PON_MODE == ONU_PON_MAC_NGPON2)
        {
            ...
            {
                RTL8277C_LOG_ERR("%s %d : aal_xgpon_dsfp_oc_body_get Error!!, ret=%x",__FUNCTION__,__LINE__,ret);
                return RT_ERR_FAILED;
            }
        ...
        for(fifo_id = 0; fifo_id < __XGPON_MAX_FIFOS, chkCnt<3 ; fifo_id++) {
            for(grant_id = 0; grant_id < __XGPON_MAX_GRANTS, chkCnt<3; grant_id++) {
                ...
                {
                    RTL8277C_LOG_ERR("%s %d : aal_xgpon_bwmp_grt_ff_ctrl_cfg_get Error!!, ret=%x",__FUNCTION__,__LINE__,ret);
                    return RT_ERR_FAILED;
                }
...
dal_rt_rtl8277c_gpon_ploam_rx_register(rt_gpon_ploam_rx_callback func)
{
    ...
    if(CA_PON_MODE == ONU_PON_MAC_GPON)
    {
        ...
        {
            RTL8277C_LOG_ERR("%s %d : aal_gpon_mac_cfg_set for plmd_ff_wr_ext Error!!, ret=%x",__FUNCTION__,__LINE__,ret);
            return RT_ERR_FAILED;
        }
    }
    else 
