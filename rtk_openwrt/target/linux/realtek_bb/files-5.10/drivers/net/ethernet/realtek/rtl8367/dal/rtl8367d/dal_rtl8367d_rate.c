 ...
 */
rtksw_api_ret_t dal_rtl8367d_rate_shareMeter_set(rtksw_meter_id_t index, rtksw_meter_type_t type, rtksw_rate_t rate, rtksw_enable_t ifg_include)
{
    ...
    switch (type)
    {
        case METER_TYPE_KBPS:
            ...
            if((retVal = rtl8367d_setAsicRegBit(RTL8367D_REG_METER_IFG_CTRL0 + (index / 16), (index % 16), (ifg_include == RTKSW_ENABLED) ? 1 : 0)) != RT_ERR_OK)
                return retVal;
    ...
    /* Set Type */
    if ((retVal = rtl8367d_setAsicRegBit(RTL8367D_REG_METER_MODE_SETTING0 + (index / 16), (index % 16), (rtksw_uint32)type)) != RT_ERR_OK)
        return retVal;
 ...
 *
 */
rtksw_api_ret_t dal_rtl8367d_rate_shareMeter_get(rtksw_meter_id_t index, rtksw_meter_type_t *pType, rtksw_rate_t *pRate, rtksw_enable_t *pIfg_include)
{
    rtksw_api_ret_t retVal;
 ...
 */
rtksw_api_ret_t dal_rtl8367d_rate_shareMeterBucket_set(rtksw_meter_id_t index, rtksw_uint32 bucket_size)
{
    rtksw_api_ret_t retVal;
 ...
 */
rtksw_api_ret_t dal_rtl8367d_rate_shareMeterBucket_get(rtksw_meter_id_t index, rtksw_uint32 *pBucket_size)
{
    rtksw_api_ret_t retVal;
 ...
 */
rtksw_api_ret_t dal_rtl8367d_rate_igrBandwidthCtrlRate_set(rtksw_port_t port, rtksw_rate_t rate, rtksw_enable_t ifg_include, rtksw_enable_t fc_enable)
{
    rtksw_api_ret_t retVal;
    ...
    regAddr = RTL8367D_REG_INGRESSBW_PORT0_RATE_CTRL0 + (rtksw_switch_port_L2P_get(port) * 0x20);
    ...
    regAddr = RTL8367D_REG_PORT0_MISC_CFG + (rtksw_switch_port_L2P_get(port) * 0x20);
    if((retVal = rtl8367d_setAsicRegBit(regAddr, RTL8367D_PORT0_MISC_CFG_INGRESSBW_IFG_OFFSET, (ifg_include == RTKSW_ENABLED) ? 1 : 0)) != RT_ERR_OK)
        return retVal;

    regAddr = RTL8367D_REG_PORT0_MISC_CFG + (rtksw_switch_port_L2P_get(port) * 0x20);
    if((retVal = rtl8367d_setAsicRegBit(regAddr, RTL8367D_PORT0_MISC_CFG_INGRESSBW_FLOWCTRL_OFFSET, (fc_enable == RTKSW_ENABLED) ? 1 : 0)) != RT_ERR_OK)
        return retVal;
 ...
 *     The ifg_include parameter is used for rate calculation with/without inter-frame-gap and preamble.
 */
rtksw_api_ret_t dal_rtl8367d_rate_igrBandwidthCtrlRate_get(rtksw_port_t port, rtksw_rate_t *pRate, rtksw_enable_t *pIfg_include, rtksw_enable_t *pFc_enable)
{
    rtksw_api_ret_t retVal;
    rtksw_uint32 regAddr, regData;
    ...
    *pFc_enable = (regData == 1) ? RTKSW_ENABLED : RTKSW_DISABLED;
 ...
 *     The ifg_include parameter is used for rate calculation with/without inter-frame-gap and preamble.
 */
rtksw_api_ret_t dal_rtl8367d_rate_egrBandwidthCtrlRate_set( rtksw_port_t port, rtksw_rate_t rate,  rtksw_enable_t ifg_include)
{
    rtksw_api_ret_t retVal;
    rtksw_uint32 regAddr, regData;
    ...
    regAddr = RTL8367D_REG_PORT0_EGRESSBW_CTRL0 + (rtksw_switch_port_L2P_get(port) * 2);
    regData = (rate >> 3) & 0xFFFF;
    ...
    if(rtksw_switch_isHsgPort(port) == RT_ERR_OK)
    {
        regAddr = RTL8367D_REG_PORT0_EGRESSBW_CTRL1 + (rtksw_switch_port_L2P_get(port) * 2);
        regData = ((rate >> 3) & 0x70000) >> 16;
    ...
    if((retVal = rtl8367d_setAsicRegBit(RTL8367D_REG_SCHEDULE_WFQ_CTRL, RTL8367D_SCHEDULE_WFQ_CTRL_OFFSET, (ifg_include == RTKSW_ENABLED) ? 1 : 0)) != RT_ERR_OK)
        return retVal;
 ...
 *     The ifg_include parameter is used for rate calculation with/without inter-frame-gap and preamble.
 */
rtksw_api_ret_t dal_rtl8367d_rate_egrBandwidthCtrlRate_get(rtksw_port_t port, rtksw_rate_t *pRate, rtksw_enable_t *pIfg_include)
{
    rtksw_api_ret_t retVal;
    rtksw_uint32 regAddr, regData, regData2;
 ...
 *      None
 */
rtksw_api_ret_t dal_rtl8367d_rate_egrQueueBwCtrlEnable_get(rtksw_port_t port, rtksw_qid_t queue, rtksw_enable_t *pEnable)
{
    rtksw_api_ret_t retVal;
    rtksw_uint32 aprEnable;
    ...
    if((retVal = rtl8367d_getAsicRegBit(RTL8367D_REG_SCHEDULE_APR_CTRL0, rtksw_switch_port_L2P_get(port), &aprEnable)) != RT_ERR_OK)
        return retVal;

    *pEnable = (aprEnable == 1) ? RTKSW_ENABLED : RTKSW_DISABLED;
    return RT_ERR_OK;
}
 ...
 *      None
 */
rtksw_api_ret_t dal_rtl8367d_rate_egrQueueBwCtrlEnable_set(rtksw_port_t port, rtksw_qid_t queue, rtksw_enable_t enable)
{
    rtksw_api_ret_t retVal;
    ...
    if((retVal = rtl8367d_setAsicRegBit(RTL8367D_REG_SCHEDULE_APR_CTRL0, rtksw_switch_port_L2P_get(port), (enable == RTKSW_ENABLED) ? 1 : 0)) != RT_ERR_OK)
        return retVal;
 ...
 */
rtksw_api_ret_t dal_rtl8367d_rate_egrQueueBwCtrlRate_get(rtksw_port_t port, rtksw_qid_t queue, rtksw_meter_id_t *pIndex)
{
    rtksw_api_ret_t retVal;
    rtksw_uint32 apridx;
    ...
    if((retVal = rtl8367d_getAsicRegBits(RTL8367D_SCHEDULE_PORT_APR_METER_REG(phy_port, queue), RTL8367D_SCHEDULE_PORT_APR_METER_MASK(phy_port), &apridx)) != RT_ERR_OK)
        return retVal;

    *pIndex = apridx + ((rtksw_switch_port_L2P_get(port) % 4) * 8);
     return RT_ERR_OK;
}
 ...
 *    The unit of granularity is 8Kbps.
 */
rtksw_api_ret_t dal_rtl8367d_rate_egrQueueBwCtrlRate_set(rtksw_port_t port, rtksw_qid_t queue, rtksw_meter_id_t index)
{
    rtksw_api_ret_t retVal;
