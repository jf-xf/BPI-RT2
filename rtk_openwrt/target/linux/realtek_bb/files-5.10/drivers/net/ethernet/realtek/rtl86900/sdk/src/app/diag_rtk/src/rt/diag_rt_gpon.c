...
void _diag_rt_gpon_show_us_flow(void)
{
    ...
    for(flow_id=0;flow_id<schldInfo.max_flow;flow_id++)
    {
        ...
        if(ret == RT_ERR_OK)
        {
            ...
            diag_util_mprintf("   %4d |     %5d |  %4d |   %4d |   %1s\r\n",
...
void _diag_rt_gpon_globalCounter_show(rt_gpon_pm_type_t type)
{
    switch(type)
    {
        ...
        case RT_GPON_DIAG_PM_TYPE_OMCI:
        {
            ...
            diag_util_mprintf("Total RX OMCI baseline   	: %lu\r\n",(long unsigned int)counter.omci.omci_rx_baseline_count);
            diag_util_mprintf("Total RX OMCI extended		: %lu\r\n",(long unsigned int)counter.omci.omci_rx_extended_count);
            diag_util_mprintf("Total RX OMCI MIC/CRC error	: %lu\r\n",(long unsigned int)counter.omci.omci_rx_mic_error_count);
            diag_util_mprintf("============================================================\r\n");
            break;
        }
    ...
    cparser_context_t *context)
{
    ...
    diag_util_mprintf("egress rate(Kbps): %d\n",rate);
...
/*
 *rt_gpon set omci-mirror ( disable | enable ) mirroing-port <UINT:port>*/
cparser_result_t 
cparser_cmd_rt_gpon_set_omci_mirror_disable_enable_mirroing_port_port(cparser_context_t *context, 
    uint32_t *port_ptr)
{
    int32 ret = RT_ERR_FAILED;
    rt_enable_t enable;
    ...
    DIAG_UTIL_ERR_CHK(rt_gpon_omci_mirror_set(enable, *port_ptr), ret);

    return CPARSER_OK;

}    /* end of cparser_cmd_rt_gpon_set_omci_mirror_disable_enable_mirroing_port_port */

/*
 * rt_gpon get omci-mirror */
cparser_result_t 
cparser_cmd_rt_gpon_get_omci_mirror(
    cparser_context_t *context)
{
    int32 ret = RT_ERR_FAILED;
    rt_enable_t enable = 0;   
    rt_port_t port = 0;

    DIAG_UTIL_PARAM_CHK();
    DIAG_UTIL_ERR_CHK(rt_gpon_omci_mirror_get(&enable, &port), ret);

    diag_util_mprintf("OMCI mirroring status: %s\n", enable == 1 ? "Enable":"Disable");
    diag_util_mprintf("OMCI mirroring port: %d\n", port);

    return CPARSER_OK;
}    /* end of cparser_cmd_rt_gpon_get_omci_mirror */
