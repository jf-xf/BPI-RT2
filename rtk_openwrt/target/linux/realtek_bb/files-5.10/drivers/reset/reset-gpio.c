...
static ssize_t duration_ms_store(struct gpio_reset_line *line,
				 ...
				 const char *buf, size_t count)
{
	if (kstrtou32(buf, 0, &line->duration_ms) < 0)
		return -EINVAL;
...
/*
 * Add GPIO_INIT_MODE to make the gpio init high/low state as the same
 * as the assert value.
 */
#define GPIO_INIT_MODE (1 << 1)
#define GPIO_INIT_VALUE_MASK (1 << 0)
	...
	struct gpio_reset_line *line)
{
	...
	if (line->asserted_value & GPIO_INIT_MODE) {
		line->asserted_value &= GPIO_INIT_VALUE_MASK;
		flags = line->asserted_value ? GPIOF_OUT_INIT_HIGH : GPIOF_OUT_INIT_LOW;
	}
	else
		flags = line->asserted_value ? GPIOF_OUT_INIT_LOW : GPIOF_OUT_INIT_HIGH;

	ret = devm_gpio_request_one(dev, line->gpio, flags, line->name);
	if (ret)
		return ret;
