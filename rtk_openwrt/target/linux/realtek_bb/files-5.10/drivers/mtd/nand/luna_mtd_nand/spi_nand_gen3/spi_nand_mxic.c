...
__SECTION_INIT_PHASE_DATA
spi_nand_cmd_info_t mxic_qio_cmd_info = {
    .w_cmd = PROGRAM_LOAD_X4_OP,
    .w_addr_io = SIO_WIDTH,
    .w_data_io = QIO_WIDTH,
...
probe_mxic_spi_nand_chip(void)
{
    ...
    for(i=0 ; i<ELEMENT_OF_SNAF_INFO(mxic_chip_info) ; i++){
        if(mxic_chip_info[i].dev_id == did8){
            if((did8 == DID_MX35LF2G14AC) || (did8 == DID_MX35LF2GE4AD) || (did8 == DID_MX35LF4GE4AD)){
                nsu_enable_on_die_ecc(0);
            }else if((did8 == DID_MX35LF1GE4AB) || (did8 == DID_MX35LF2GE4AB)){
...
#elif __DEVICE_USING_QIO
            if(did8 == DID_MX35LF1GE4AB){
                mxic_chip_info[i]._cmd_info = &mxic_x4_cmd_info;
            }else{
                mxic_chip_info[i]._cmd_info = &mxic_qio_cmd_info;
            }
#endif
            ...
            if(mxic_chip_info[i]._page_size  == SNAF_MODEL_PAGE_SIZE_4096B){
                mxic_chip_info[i]._ecc_encode= _nsu_ecc_encode_4Kpage_ptr;
                mxic_chip_info[i]._ecc_decode= _nsu_ecc_decode_4Kpage_ptr;
            }
