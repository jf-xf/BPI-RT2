...
/***********************************************
  *  FMSH's ID Definition
  ***********************************************/
#define MID_FMSH     (0xA1)
#define DID_FM25S01  (0xA1)
#define DID_FM25S01A (0xE4)
#define DID_FM25S02A (0xE5)
#define DID_FM25G02C (0x92)

// policy decision
    //input: #define NSU_PROHIBIT_QIO, or NSU_PROHIBIT_DIO  (in project/info.in)
    //       #define NSU_FMSH_USING_QIO, NSU_FMSH_USING_DIO, NSU_FMSH_USING_SIO  (in project/info.in)
    ...
    #if defined(NSU_FMSH_USING_QIO) || defined(NSU_FMSH_USING_DIO)
    ...
    #define __DEVICE_USING_QIO 0
        ...
        #endif
    #elif defined(NSU_FMSH_USING_DIO)
...
spi_nand_cmd_info_t fmsh_x4_cmd_info = {
...
__SECTION_INIT_PHASE_DATA
spi_nand_flash_info_t fmsh_chip_info[] = {
    {
        .man_id              = MID_FMSH,
        .dev_id              = DID_FM25S01,
...
probe_fmsh_spi_nand_chip(void)
{
    ...
    u32_t rdid = fmsh_read_id(0);

    if(MID_FMSH != (rdid>>8)) return VZERO;

    u16_t did8  = rdid&0xFF;
    u32_t i;
    for(i=0 ; i<ELEMENT_OF_SNAF_INFO(fmsh_chip_info) ; i++){
        if(fmsh_chip_info[i].dev_id == did8){
            if((did8 == DID_FM25S01) || (did8== DID_FM25S01A) || (did8 == DID_FM25S02A)){
                nsu_disable_on_die_ecc(0);
            }else if(did8 == DID_FM25G02C){
                FM25G02C_disable_on_die_ecc(0);
            }

#if __DEVICE_REASSIGN
#if __DEVICE_USING_SIO
            fmsh_chip_info[i]._cmd_info = _nsu_cmd_info_ptr;
#elif __DEVICE_USING_DIO
            fmsh_chip_info[i]._cmd_info = _nsu_x2_cmd_info_ptr;
#elif __DEVICE_USING_QIO
            fmsh_chip_info[i]._cmd_info = &fmsh_x4_cmd_info;
#endif
            fmsh_chip_info[i]._model_info= &nsu_model_info;
            fmsh_chip_info[i]._reset     = _nsu_reset_ptr;
            fmsh_chip_info[i]._ecc_encode= _nsu_ecc_encode_ptr;
            fmsh_chip_info[i]._ecc_decode= _nsu_ecc_decode_ptr;

#if __DEVICE_USING_QIO
            if((did8 == DID_FM25S01) || (did8 == DID_FM25S01A)){
                fmsh_chip_info[i]._cmd_info = _nsu_x2_cmd_info_ptr;
            }else if((did8 == DID_FM25S02A) || (did8 == DID_FM25G02C)){
                fmsh_quad_enable(0);
            }
...
fmsh_init_rest(void)
{
    ...
    u32_t rdid = fmsh_read_id(cs);
    //u32_t cs0_id = (_spi_nand_info->man_id<<8) | _spi_nand_info->dev_id;
    if(((rdid>>8)!=MID_FMSH) || ((rdid&0xFF)!=_spi_nand_info->dev_id))
      { return 0; }
...
REG_SPI_NAND_INIT_REST_FUNC(fmsh_init_rest);
