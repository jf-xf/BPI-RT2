...
#define DID_W25M02GV        (0xAB21)
#define DID_W25N01KVZEIR    (0xAE21)
#define DID_W25N02KV        (0xAA22)
#define DID_W25N04KV        (0xAA23)
...
//The Winbond specific function
void winbond_ecc_encode(u32_t ecc_ability, void *dma_addr, void *fake_ptr_cs);
s32_t winbond_ecc_decode(u32_t ecc_ability, void *dma_addr, void *fake_ptr_cs);
...
__SECTION_RUNTIME void
winbond_ecc_encode(u32_t ecc_ability, void *dma_addr, void *fake_ptr_cs)
{
    return;
}
...
probe_winbond_spi_nand_chip(void)
{
    ...
    for(i=0 ; i<ELEMENT_OF_SNAF_INFO(winbond_chip_info) ; i++){
        if(winbond_chip_info[i].dev_id == did){
            if((did == DID_W25N02KV) || (did == DID_W25N04KV)){
            ...
            winbond_chip_info[i]._ecc_ability= ECC_MODEL_6T;
            ...
            if((did == DID_W25N02KV) || (did == DID_W25N04KV)){
                ...
                winbond_chip_info[i]._model_info->_page_read_ecc = _nsu_page_read_with_ode_ptr;
                winbond_chip_info[i]._model_info->_page_write_ecc = _nsu_page_write_with_ode_ptr;
            }else{
                winbond_chip_info[i]._ecc_ability= ECC_MODEL_6T;
                winbond_chip_info[i]._ecc_encode= _nsu_ecc_encode_ptr;
                winbond_chip_info[i]._ecc_decode= _nsu_ecc_decode_ptr;
...
winbond_init_rest(void)
{
    ...
    // misc
    nsu_block_unprotect(cs);
    if(DID_W25N02KV != _spi_nand_info->dev_id){
        nsu_disable_on_die_ecc(cs);
    }
    if(DID_W25M02GV == _spi_nand_info->dev_id){
