...
u32_t snaf_drv_ver  = SYM_SPI_NAND_VER;    // magic_1 + magic_2 + cntrl_ver + drv_ver
...
SECTION_SPI_NAND u32_t
nsc_stack2die_addr_transform(spi_nand_flash_info_t *info, u32_t blk_pge_addr, spi_nand_die_sel_t *sel_die_func)
{
    u32_t total_blks_per_cs = SNAF_NUM_OF_BLOCK(info);
    u32_t blk_idx = (blk_pge_addr>>PAGE_SHF);
    u32_t die_idx = ((blk_idx%total_blks_per_cs) >= (total_blks_per_cs>>1))?1:0;
    u32_t cs=_EXTRACT_CS_INFO(blk_pge_addr, PGE_BLK_SHF(info->_num_block));

    sel_die_func(cs, die_idx);
    return _REMVOE_STACK2DIE_INFO(blk_pge_addr, STACKDIE_PGE_BLK_SHF(info->_num_block));
}
...
symb_fdefine(SNAF_STACK2DIE_ADDR_TRANS_FUNC, nsc_stack2die_addr_transform);
