...
// policy decision
    ...
    #if defined(NSU_ZENTEL_USING_QIO) || defined(NSU_ZENTEL_USING_DIO)
...
spi_nand_cmd_info_t zentel_512MB_x4_cmd_info = {
...
__SECTION_INIT_PHASE_DATA
spi_nand_cmd_info_t zentel_1Gb_x4_cmd_info = {
...
spi_nand_flash_info_t zentel_chip_info[] = {
    {
        ...
        ._ecc_ability        = ECC_MODEL_6T,
            ...
            ._model_info     = &snaf_rom_general_model,
                ...
                ._cmd_info   = &zentel_512MB_x4_cmd_info,
            #endif
        #endif
    },
    {
        ...
        ._ecc_ability        = ECC_MODEL_6T,
            ...
            ._model_info     = &snaf_rom_general_model,
                ...
                ._cmd_info   = &nsc_x2_cmd_info,
            #elif __DEVICE_USING_QIO
                ._cmd_info   = &zentel_1Gb_x4_cmd_info,
            #endif
        #endif
    },
};
...
probe_zentel_spi_nand_chip(void)
{
    ...
    u32_t rdid = zentel_read_id(0);
    if(MID_ZENTEL != (rdid >>16)) return VZERO;

    u32_t i;
    for(i=0 ; i<ELEMENT_OF_SNAF_INFO(zentel_chip_info) ; i++){
        if (zentel_chip_info[i].dev_id == (rdid&0xFFFF)) {
            #if __DEVICE_REASSIGN
                #if __DEVICE_USING_SIO
                    zentel_chip_info[i]._cmd_info = _nsu_cmd_info_ptr;
                #elif __DEVICE_USING_DIO
                    zentel_chip_info[i]._cmd_info = _nsu_x2_cmd_info_ptr;
                #elif __DEVICE_USING_QIO
                    if(DID_A5U12A21ASC_AWS == zentel_chip_info[i].dev_id){
                        zentel_chip_info[i]._cmd_info = &zentel_512MB_x4_cmd_info;
                    }else if(DID_A5U1GA21BSC_BWS == zentel_chip_info[i].dev_id){
                        zentel_chip_info[i]._cmd_info = &zentel_1Gb_x4_cmd_info;
                    }
                #endif
...
int
zentel_init_rest(void)
{
    u32_t cs=1;

    // check ID, cs0 and cs1 should be identical
    u32_t rdid = zentel_read_id(cs);
    //u32_t cs0_id = (_spi_nand_info->man_id<<8) | _spi_nand_info->dev_id;
    if(((rdid>>16)!=MID_ZENTEL) || ((rdid&0xFFFF)!=_spi_nand_info->dev_id))
      { return 0; }
...
REG_SPI_NAND_INIT_REST_FUNC(zentel_init_rest);
