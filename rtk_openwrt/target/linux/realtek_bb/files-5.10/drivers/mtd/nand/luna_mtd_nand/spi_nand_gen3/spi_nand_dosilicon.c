...
#define DID_DS35Q1GB_IB (0xF1)
#define DID_DS35Q2GB_IB (0xF2)
...
//The Dosilicon specific function
void dosilicon_ecc_encode(u32_t ecc_ability, void *dma_addr, void *fake_ptr_cs);
s32_t dosilicon_ecc_decode(u32_t ecc_ability, void *dma_addr, void *fake_ptr_cs);
...
spi_nand_flash_info_t dosilicon_chip_info[] = {
    {
        ...
        ._ecc_ability        = ECC_USE_ODE,
            ...
            ._model_info     = VZERO,
        #else
            ._ecc_encode     = dosilicon_ecc_encode,
            ._ecc_decode     = dosilicon_ecc_decode,
...
dosilicon_ecc_encode(u32_t ecc_ability, void *dma_addr, void *fake_ptr_cs)
{
    return;
}

__SECTION_RUNTIME s32_t
dosilicon_ecc_decode(u32_t ecc_ability, void *dma_addr, void *fake_ptr_cs)
{
	u32_t cs = 0;
	u8_t eccs = (nsu_get_feature_reg(cs, 0xC0) >> 4) & 0x7;

	if(0 == eccs) return ECC_DECODE_SUCCESS;
	else if(1 == eccs) return 3;
	else if(3 == eccs) return 6;
	else if(5 == eccs) return 8;
...
probe_dosilicon_spi_nand_chip(void)
{
    ...
    for(i=0 ; i<ELEMENT_OF_SNAF_INFO(dosilicon_chip_info) ; i++){
        if(dosilicon_chip_info[i].dev_id == did8){
            if((did8 == DID_DS35Q1GB_IB) || (did8 == DID_DS35Q2GB_IB)){
...
#ifdef APRO_NSU_DRIVER_PATCH
            dosilicon_chip_info[i]._ecc_ability= ECC_MODEL_6T;
            dosilicon_chip_info[i]._ecc_encode= _nsu_ecc_encode_ptr;
            dosilicon_chip_info[i]._ecc_decode= _nsu_ecc_decode_ptr;
#else
            if((did8 == DID_DS35Q1GB_IB) || (did8 == DID_DS35Q2GB_IB)){
                dosilicon_chip_info[i]._ecc_encode= dosilicon_ecc_encode;
                dosilicon_chip_info[i]._ecc_decode= dosilicon_ecc_decode;
                dosilicon_chip_info[i]._model_info->_page_read_ecc = _nsu_page_read_with_ode_ptr;
                dosilicon_chip_info[i]._model_info->_page_write_ecc = _nsu_page_write_with_ode_ptr;
            }else{
                dosilicon_chip_info[i]._ecc_ability= ECC_MODEL_6T;
                dosilicon_chip_info[i]._ecc_encode= _nsu_ecc_encode_ptr;
                dosilicon_chip_info[i]._ecc_decode= _nsu_ecc_decode_ptr;
            }
#endif /* #ifdef APRO_NSU_DRIVER_PATCH */
