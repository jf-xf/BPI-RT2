...
static string_map* compiled_map = NULL;
...
static int do_match_test(unsigned char match_type,  const char* reference, char* query)
{
	int matches = 0;
	struct regexp* r;
	switch(match_type)
...
static int http_match(const struct ipt_weburl_info* info, const unsigned char* packet_data, int packet_length)
{
	int test = 0; 
...
static int https_match(const struct ipt_weburl_info* info, const unsigned char* packet_data, int packet_length)
{
	int test = 0;
...
static struct tcphdr *__find_v4_payload(const struct sk_buff *skb, unsigned char **payload, unsigned short *payload_length) {
	struct iphdr* iph;
	struct tcphdr* tcp_hdr;
	unsigned short offset;
	
	iph     = (struct iphdr*)(skb_network_header(skb));
	tcp_hdr = (struct tcphdr*)( ((unsigned char*)iph) + (iph->ihl*4) );
	offset  = (tcp_hdr->doff*4) + (iph->ihl*4);
	*payload_length = ntohs(iph->tot_len) - offset;
	*payload = ((unsigned char*)iph) + offset;
	return tcp_hdr;
}

static struct tcphdr *__find_v6_payload(const struct sk_buff *skb, unsigned char **payload, unsigned short *payload_length) {
	struct ipv6hdr *ip6h;
	struct tcphdr *tcp_hdr;
	unsigned short offset;
		
	ip6h    = ipv6_hdr(skb);	
	tcp_hdr = (struct tcphdr*)( ((unsigned char*)ip6h) + sizeof(*ip6h) );	
	offset  = (tcp_hdr->doff*4) + sizeof(struct ipv6hdr);
	*payload_length = ntohs(ip6h->payload_len) - offset;
	*payload = ((unsigned char*)ip6h) + offset;
	
	return tcp_hdr;
}

static bool __match(const struct sk_buff *skb, struct xt_action_param *par, struct tcphdr * (*fn)(const struct sk_buff *, unsigned char **, unsigned short *))
{
	...
	tcp_hdr = fn(linear_skb, &payload, &payload_length);
	
	/* if payload length <= 10 bytes don't bother doing a check, otherwise check for match */
	if(payload_length > 10)
	{
		if(strncasecmp((char*)payload, "GET ", 4) == 0 || strncasecmp(  (char*)payload, "POST ", 5) == 0 || strncasecmp((char*)payload, "HEAD ", 5) == 0)
		{
...
static bool match4(const struct sk_buff *skb, struct xt_action_param *par) {
	return __match(skb, par, __find_v4_payload);
}

static bool match6(const struct sk_buff *skb, struct xt_action_param *par) {
	return __match(skb, par, __find_v6_payload);
}
...
{
	compiled_map = NULL;
	return xt_register_matches(weburl_match, ARRAY_SIZE(weburl_match));

}

static void __exit fini(void)
{
	xt_unregister_matches(weburl_match, ARRAY_SIZE(weburl_match));
	if(compiled_map != NULL)
	{
