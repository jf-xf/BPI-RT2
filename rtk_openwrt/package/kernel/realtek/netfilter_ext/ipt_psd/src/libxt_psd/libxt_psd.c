...
#define SCAN_TIMEOUT_THRESHOLD		360000
...
static void psd_mt_help(void) {
	printf(
		...
		" --psd-specified-port  port 		 specify port\n"
		" --psd-timeout-threshold  delay	 Portscan detection timeout threshold, allow host after certain amount of time\n\n");
}
...
static const struct option psd_mt_opts[] = {
	...
	{.name = "psd-specified-port", .has_arg = true, .val = '5'},
	{.name = "psd-timeout-threshold", .has_arg = true, .val = '6'},
	{NULL}
};
...
static void psd_mt_init(struct xt_entry_match *match) {
	...
	psdinfo->specified_port = 0;
	psdinfo->timeout_threshold = SCAN_TIMEOUT_THRESHOLD;
}
...
#define XT_PSD_OPT_SPORT 0x10
#define XT_PSD_OPT_TOTRESH 0x20
                     ...
                     const void *entry, struct xt_entry_match **match)
{
	...
	switch (c) {
		...
		/* PSD-specified-port */
		case '5':
			if (*flags & XT_PSD_OPT_SPORT)
				xtables_error(PARAMETER_PROBLEM, "Can't specify --psd-specified-port twice");
			if (!xtables_strtoui(optarg, NULL, &num, 0, 65535))
				xtables_error(PARAMETER_PROBLEM, "bad --psd-specified-port '%s'", optarg);
			psdinfo->specified_port= num;
			*flags |= XT_PSD_OPT_SPORT;
			return true;
		/* PSD-timeout-threshold */
		case '6':
			if (*flags & XT_PSD_OPT_TOTRESH)
				xtables_error(PARAMETER_PROBLEM, "Can't specify --psd-timeout-threshold twice");
			if (!xtables_strtoui(optarg, NULL, &num, 0, PSD_MAX_TIMEOUT))
				xtables_error(PARAMETER_PROBLEM, "bad --psd-delay-threshold '%s'", optarg);
			psdinfo->timeout_threshold = num;
			*flags |= XT_PSD_OPT_TOTRESH;
			return true;
	}
	return false;
}
...
static void psd_mt_save(const void *ip, const struct xt_entry_match *match)
{
	...
	printf("--psd-timeout-threshold %u ", psdinfo->timeout_threshold);
	printf("--psd-lo-ports-weight %u ", psdinfo->lo_ports_weight);
	printf("--psd-hi-ports-weight %u ", psdinfo->hi_ports_weight);
	printf("--psd-specified-port %u ", psdinfo->specified_port);
}
