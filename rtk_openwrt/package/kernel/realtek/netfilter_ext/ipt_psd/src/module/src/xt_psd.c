...
{
	if(psd->specified_port == port)
		return psd->lo_ports_weight;
...
{
	//if spcified port for this rule, add weight when duplicate syn on this port
	/*
	...
	if (host->weight >= psdinfo->weight_threshold) /* already matched */
		return true;
	if ((psdinfo->specified_port==0) && port_in_list(host, proto, tcph->dest))
		return false;
...
static bool
is_previous_hit_timeout(const struct host *h, unsigned long timeout_threshold,
                unsigned long now)
{
	return h->timeout == 0 ||
		   (now - h->timeout >= (timeout_threshold * HZ) / 100 &&
	       time_after_eq(now, h->timeout));
}
               ...
               const struct xt_psd_info *psdinfo, unsigned int hash)
{
	...
	if (curr != NULL) {
		/* check if this address has been hit within timeout_threshold. If so handle it  without check with rule*/
		if (!is_previous_hit_timeout(curr, psdinfo->timeout_threshold, now)){
			return true;
		}
...
static int psd_mt_check(const struct xt_mtchk_param *par)
{
	...
	if (info->timeout_threshold > PSD_MAX_TIMEOUT ||
		info->delay_threshold > PSD_MAX_RATE ||
	    info->weight_threshold > PSD_MAX_RATE ||
	    info->lo_ports_weight > PSD_MAX_RATE ||
	    info->hi_ports_weight > PSD_MAX_RATE)
		return -EINVAL;
	if(info->specified_port > 65535)
		return -EINVAL;
	return 0;
}
