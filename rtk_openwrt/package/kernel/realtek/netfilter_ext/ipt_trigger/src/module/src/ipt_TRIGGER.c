...
static void trigger_timeout(struct timer_list *val)
{	
    struct ipt_trigger *trig= container_of(val, struct ipt_trigger, timeout);
                ...
                const void *targinfo)
{
	...
	/* Alter the destination of incoming packet. */
 	newrange= ((struct nf_nat_range2)
		{ .flags = mr->range[0].flags | NF_NAT_RANGE_MAP_IPS,
		  .min_addr = found->srcip,
		  .max_addr = found->srcip,
