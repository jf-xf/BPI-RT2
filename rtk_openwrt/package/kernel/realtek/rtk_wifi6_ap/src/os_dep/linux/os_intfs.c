...
#ifdef CONFIG_PCI_HCI
	PPCI_DATA pci_data = dvobj_to_pci(adapter_to_dvobj(adapter));
...
int rtw_os_ndev_register(_adapter *adapter, const char *name)
{
	...
	if(rtw_init_netdev_name(ndev, name)) {
          	ret = _FAIL;
          	goto exit;
        }
...
u8 rtw_init_default_value(_adapter *padapter)
{
	...
	pregistrypriv->band = pregistrypriv->wireless_mode;
	if (pregistrypriv->band_type == BAND_CAP_2G)		//2G
		pregistrypriv->band &= ~(WLAN_MD_11A);
	else if (pregistrypriv->band_type == BAND_CAP_5G)	//5G
		pregistrypriv->band &= ~(WLAN_MD_11G | WLAN_MD_11B);
	...
	padapter->bc_rate = NO_FIX_RATE;
	padapter->mc_rate = NO_FIX_RATE;
...
u8 rtw_init_drv_sw(_adapter *padapter)
{
	...
	rtw_init_timer(&(padapter->ofdma_timer), ofdma_timer_handler, padapter);
#endif
...
#ifdef CONFIG_CSA_IE
	if (is_primary_adapter(padapter))
		_cancel_timer_ex(&adapter_to_rfctl(padapter)->set_csa_timer);
#endif
...
#ifdef CONFIG_WFA_OFDMA_Logo_Test
	_cancel_timer_ex(&padapter->ofdma_timer);
#endif
...
static _adapter *rtw_drv_add_vir_if(struct dvobj_priv *dvobj)
{
	...
	struct wifi_mib_priv *priv_mib = NULL;
#endif
...
#ifdef CONFIG_RTW_AP_EXT_SUPPORT
	priv_mib = &padapter->registrypriv.wifi_mib;
...
int rtw_os_ndevs_register(struct dvobj_priv *dvobj)
{
	...
	for (i = 0; i < dvobj->iface_nums; i++) {
		...
		if (adapter) {
			...
			else if (dvobj->dev_id == IFACE_ID2)
				name = regsty->if3name;
			#endif
			else
				name = "wlan%d";
...
static int _netdev_open(struct net_device *pnetdev)
{
	...
	if (!rtw_hw_is_init_completed(dvobj)) {
		...
		if(padapter->registrypriv.wifi_mib.force_hw_tx)
		{
...
int netdev_open(struct net_device *pnetdev)
{
	...
	skb_queue_head_init(&padapter->stp_tx_queue);
#endif
...
#ifdef CONFIG_RTW_A4_STA
	padapter->a4_db_entry_cache = NULL;
#endif

	padapter->cache_vlan_id = 0;
	padapter->cache_vlan_dev = NULL;
...
void rtw_clear_trx_stats(_adapter *padapter, struct dvobj_priv *dvobj)
{
	struct xmit_priv *pxmitpriv = &(padapter->xmitpriv);
	struct recv_priv *precvpriv = &(padapter->recvpriv);
	int i = 0;
	...
	memset(&padapter->tx_logs, 0, sizeof(padapter->tx_logs));
}
...
int netdev_close(struct net_device *pnetdev)
{
	...
	padapter->cache_vlan_id = 0;
	padapter->cache_vlan_dev = NULL;
	rtw_clear_trx_stats(padapter, dvobj);
...
void rtw_recover_device(struct dvobj_priv *dvobj)
{
	...
	if (pri_adapter->amsdu_hw_timer_enable)
		rtw_core_set_gt3(pri_adapter, 1, 1000);
#endif
}
