	...
	u8 ht, bool started)
{
	...
	mutex_lock(&adapter->pnetdev->ieee80211_ptr->mtx);
	cfg80211_ch_switch_notify(adapter->pnetdev, &chdef);
	mutex_unlock(&adapter->pnetdev->ieee80211_ptr->mtx);
...
int rtw_cfg80211_sync_iftype(_adapter *adapter)
{
	...
	WLAN_BSSID_EX *network = &pmlmepriv->cur_network.network;

	if (!(nl80211_iftype_to_rtw_mlme_state(rtw_wdev->iftype) & MLME_STATE(adapter)) ||
		network->InfrastructureMode != nl80211_iftype_to_rtw_network_type(rtw_wdev->iftype)) {
...
#if defined(PLATFORM_ECOS) && defined(WFO_ASYNC_INFORM_BSS)
	if ((u32)bss == 0xFFFFFFFF) {
		goto exit;
	}
	printk("==> [%s:%d], WRONG bss!! eCos put bss"
		"@wfo_radio_cmd_receiver_cfg80211_inform_bss_frame\n", __func__, __LINE__);
#endif /* PLATFORM_ECOS & WFO_ASYNC_INFORM_BSS */
...
static int rtw_cfg80211_ap_set_encryption(struct net_device *dev, struct ieee_param *param)
{
	...
	if (is_broadcast_mac_addr(param->sta_addr)) {
		...
		if (!psta) {
			...
			#if defined(RTW_WKARD_AP_CMD_DISPATCH) || defined(RTW_STA_ALLOC_FREE_SAFE)
	...
	if ((psecuritypriv->dot11AuthAlgrthm == dot11AuthAlgrthm_8021X || psecuritypriv->dot11AuthAlgrthm == dot11AuthAlgrthm_Auto) && psta) { /* psk/802_1x */
		if (param->u.crypt.set_tx == 1) {
			...
			if (psta->dot118021XPrivacy != _NO_PRIVACY_) {
				...
				if (psecuritypriv->ndisauthtype == Ndis802_11AuthModeWPA2PSK || psecuritypriv->ndisauthtype == Ndis802_11AuthModeWPAPSK)
...
static void rtw_cfg80211_fill_sta_rate_info(struct sta_info *psta, struct station_info *sinfo)
{
	enum rate_flags {
		RATE_FLAG_LEGACY,
		RATE_FLAG_HT,
	...
	struct rate_info *rinfo[] = {&sinfo->txrate, &sinfo->rxrate};
	struct rate_info *info = NULL;
	int i, is_short_gi = 0;
	u8   current_giltf = 0, giltf_level = 0;
	u16 current_rate, legacy_rate;
	u8 current_bw;
	u8 nss, idx, flags = 0;

	for (i = 0; i < sizeof(rinfo)/sizeof(rinfo[0]); i++) {
		info = rinfo[i];
		...
		if(current_rate >= RTW_DATA_RATE_HE_NSS1_MCS0 && current_rate <= RTW_DATA_RATE_HE_NSS4_MCS11) {
			flags = RATE_FLAG_HE;
			idx = (current_rate - RTW_DATA_RATE_HE_NSS1_MCS0) % 16;
			nss = 1 + (current_rate - RTW_DATA_RATE_HE_NSS1_MCS0) / 16;
		}
		else if(current_rate >= RTW_DATA_RATE_VHT_NSS1_MCS0 && current_rate <= RTW_DATA_RATE_VHT_NSS4_MCS9) {
			flags = RATE_FLAG_VHT;
			idx = (current_rate - RTW_DATA_RATE_VHT_NSS1_MCS0) % 16;
			nss = 1 + (current_rate - RTW_DATA_RATE_VHT_NSS1_MCS0) / 16;
		}
		else if (current_rate >= RTW_DATA_RATE_MCS0 && current_rate <= RTW_DATA_RATE_MCS31) {
			flags = RATE_FLAG_HT;
			idx = current_rate - RTW_DATA_RATE_MCS0;
			nss = 1 + (current_rate - RTW_DATA_RATE_MCS0) / 8;
		}
		else if (current_rate <= RTW_DATA_RATE_OFDM54) {
			flags = RATE_FLAG_LEGACY;
			legacy_rate = bitrate[current_rate];
		}
		else {
			RTW_WARN("%s: Rate info not correct\n", __func__);
			return;
		}

		/* fill rate info */
		if (current_bw == CHANNEL_WIDTH_160)
			info->bw = RATE_INFO_BW_160;
		...
		if (flags == RATE_FLAG_HE) {
			info->flags = RATE_INFO_FLAGS_HE_MCS;
			info->mcs = idx;
			info->nss = nss;
			if (giltf_level == 2)
				info->he_gi = NL80211_RATE_INFO_HE_GI_0_8;
			else if (giltf_level == 1)
				info->he_gi = NL80211_RATE_INFO_HE_GI_1_6;
			else
				info->he_gi = NL80211_RATE_INFO_HE_GI_3_2;
		}
		else if (flags == RATE_FLAG_VHT) {
			...
			info->flags = 0;
			info->legacy = legacy_rate;
		}
	}

	sinfo->filled |= (STATION_INFO_TX_BITRATE | STATION_INFO_RX_BITRATE);
	...
	struct station_info *sinfo)
{
	...
	if (psta) {
		if (!MLME_IS_STA(padapter)
			...
			sinfo->filled |= STATION_INFO_SIGNAL;
			sinfo->signal = translate_percentage_to_dbm(rtw_phl_get_sta_rssi(psta->phl_sta));
		}
		sinfo->filled |= STATION_INFO_INACTIVE_TIME;
		...
		rtw_cfg80211_fill_sta_rate_info(psta, sinfo);
	}
...
static int cfg80211_rtw_set_wiphy_params(struct wiphy *wiphy, u32 changed)
{
	...
	padapter->registrypriv.wifi_mib.rtsthres = wiphy->rts_threshold;
...
static int rtw_cfg80211_set_wpa_ie(_adapter *padapter, u8 *pie, size_t ielen)
{
	...
	padapter->securitypriv.wpa_psk = 0;
	padapter->securitypriv.wpa_group_cipher = _NO_PRIVACY_;
	padapter->securitypriv.wpa_pairwise_cipher = _NO_PRIVACY_;
	padapter->securitypriv.wpa2_group_cipher = _NO_PRIVACY_;
	padapter->securitypriv.wpa2_pairwise_cipher = _NO_PRIVACY_;
#if defined(CONFIG_RTL_CFG80211_WAPI_SUPPORT)
	padapter->securitypriv.wapi_group_cipher = _NO_PRIVACY_;
	padapter->securitypriv.wapi_pairwise_cipher = _NO_PRIVACY_;
#endif
	padapter->securitypriv.akmp = 0;
	...
	if (pwpa && wpa_ielen > 0) {
		if (rtw_parse_wpa_ie(pwpa, wpa_ielen + 2, &group_cipher, &pairwise_cipher, NULL) == _SUCCESS) {
			...
			padapter->securitypriv.wpa_group_cipher = group_cipher;
			padapter->securitypriv.wpa_pairwise_cipher = pairwise_cipher;
			padapter->securitypriv.wpa_psk |= BIT(0);
	...
	if (pwpa2 && wpa2_ielen > 0) {
		if (rtw_parse_wpa2_ie(pwpa2, wpa2_ielen + 2, &group_cipher, &pairwise_cipher, NULL, &akm, &mfp_opt) == _SUCCESS) {
			padapter->securitypriv.dot11AuthAlgrthm = dot11AuthAlgrthm_8021X;
			padapter->securitypriv.ndisauthtype = Ndis802_11AuthModeWPA2PSK;
			padapter->securitypriv.wpa2_group_cipher = group_cipher;
			padapter->securitypriv.wpa2_pairwise_cipher = pairwise_cipher;
			padapter->securitypriv.wpa_psk |= BIT(1);
			padapter->securitypriv.akmp = akm;
	...
	if (pwapi && wapi_ielen > 0) {
		if (rtw_parse_wapi_ie(pwapi, wapi_ielen + 2, &group_cipher, &pairwise_cipher, NULL, &akm, &mfp_opt) == _SUCCESS) {
			padapter->securitypriv.dot11AuthAlgrthm = dot11AuthAlgrthm_WAPI;
			padapter->securitypriv.ndisauthtype = Ndis802_11AuthModeWAPI;
			padapter->securitypriv.wapi_group_cipher = group_cipher;
			padapter->securitypriv.wapi_pairwise_cipher = pairwise_cipher;
			padapter->securitypriv.wpa_psk |= BIT(2);
			padapter->securitypriv.akmp = akm;
	...
	{/* check p2p_ie for assoc req; */
		...
		if (p2p_ie) {
			...
			_rtw_spinlock_bh(&pmlmepriv->ie_update_lock);
			...
			if (pmlmepriv->p2p_assoc_req_ie == NULL) {
				RTW_INFO("%s()-%d: rtw_malloc() ERROR!\n", __FUNCTION__, __LINE__);
				_rtw_spinunlock_bh(&pmlmepriv->ie_update_lock);
				goto exit;
			}
		...
		struct cfg80211_ap_settings *settings)
{
	...
	if (psecuritypriv->dot11PrivacyAlgrthm == _WEP40_ ||
		...
		if (settings->auth_type == NL80211_AUTHTYPE_OPEN_SYSTEM)
			psecuritypriv->dot11AuthAlgrthm = dot11AuthAlgrthm_Open;
		else if (settings->auth_type == NL80211_AUTHTYPE_SHARED_KEY)
			psecuritypriv->dot11AuthAlgrthm = dot11AuthAlgrthm_Shared;
		else
			psecuritypriv->dot11AuthAlgrthm = dot11AuthAlgrthm_Auto;

		for (i = 0; i < WEP_KEYS; i++) {
			if (psecuritypriv->dot11DefKeylen[i])
				rtw_ap_set_wep_key(adapter, psecuritypriv->dot11DefKey[i].skey,
					psecuritypriv->dot11DefKeylen[i], i, 1);
		}
	}
	...
	if (settings->ssid && settings->ssid_len) {
		...
		_rtw_memset(&pbss_network_ext->Ssid, 0, sizeof(NDIS_802_11_SSID));
...
static int cfg80211_rtw_channel_switch(struct wiphy *wiphy,
			struct net_device *ndev, struct cfg80211_csa_settings *params)
{
	int ret = 0;
#ifdef CONFIG_CSA_IE
	...
	RTW_PRINT(FUNC_NDEV_FMT"\n", FUNC_NDEV_ARG(ndev));

	if(params->chandef.width == NL80211_CHAN_WIDTH_20_NOHT || params->chandef.width == NL80211_CHAN_WIDTH_20)
		bw = CHANNEL_WIDTH_20;
	else if(params->chandef.width == NL80211_CHAN_WIDTH_40)
		bw = CHANNEL_WIDTH_40;
	else if(params->chandef.width == NL80211_CHAN_WIDTH_80)
		bw = CHANNEL_WIDTH_80;
	else if(params->chandef.width == NL80211_CHAN_WIDTH_80P80)
		bw = CHANNEL_WIDTH_80_80;
	else if(params->chandef.width == NL80211_CHAN_WIDTH_160)
		bw = CHANNEL_WIDTH_160;
	else if(params->chandef.width == NL80211_CHAN_WIDTH_5)
		bw = CHANNEL_WIDTH_5;
	else if(params->chandef.width == NL80211_CHAN_WIDTH_10)
		bw = CHANNEL_WIDTH_10;
	else
	{
		RTW_ERR("invalid bandwith\n");
		ret = -EINVAL;
		goto exit;
	}

	offset = (params->chandef.chan->center_freq > params->chandef.center_freq1)
					? CHAN_OFFSET_LOWER
					: CHAN_OFFSET_UPPER;

	rfctl->csa_ch = rtw_freq2ch(params->chandef.chan->center_freq);

	rfctl->csa_op_class = rtw_get_op_class_by_chbw(rfctl->csa_ch, bw, offset);
	if(rfctl->csa_op_class == 0)
	{
		RTW_ERR("invalid op_class\n");
		ret = -EINVAL;
		goto exit;
	}

	rfctl->csa_switch_mode = params->block_tx;
	rfctl->csa_cntdown = params->count;

	ret = rtw_add_beacon(adapter, params->beacon_csa.head, params->beacon_csa.head_len,
	                     params->beacon_csa.beacon_ies, params->beacon_csa.beacon_ies_len,
...
dump:
	...
	if (tx_ret != _SUCCESS || dump_cnt > 1) {
		RTW_WARN(FUNC_ADPT_FMT" %s (%d/%d) in %d ms\n",
			 FUNC_ADPT_ARG(padapter),
...
static int rtw_cfg80211_set_beacon_wpsp2pie(struct net_device *ndev, char *buf, int len)
{
	...
	if (len > 0) {
		...
		if (p2p_ie) {
			...
			_rtw_spinlock_bh(&pmlmepriv->p2p_beacon_ie);
			...
			if (pmlmepriv->p2p_beacon_ie == NULL) {
				RTW_INFO("%s()-%d: rtw_malloc() ERROR!\n", __FUNCTION__, __LINE__);
				_rtw_spinunlock_bh(&pmlmepriv->p2p_beacon_ie);
				return -EINVAL;
	...
	struct cfg80211_external_auth_params *params)
{
	...
	if (MLME_IS_AP(padapter)) {
		...
		for (i = 0; i < pdvobj->iface_nums; i++) {
			...
			if (params->pmkid)
				_rtw_set_pmksa(dev, params->bssid, (u8 *)params->pmkid);
