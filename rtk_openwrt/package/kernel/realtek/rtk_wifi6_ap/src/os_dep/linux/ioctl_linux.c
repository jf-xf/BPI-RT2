	...
	*/
	...
	while (1) {
		...
		if ((_rtw_memcmp(dst_bssid, src_bssid, ETH_ALEN)) == _TRUE) {
			inframode = pnetwork->network.InfrastructureMode;
			is_infra_mode = 1;
			break;
		}
	}
	...
	if (is_infra_mode) {
		if (!rtw_set_802_11_infrastructure_mode(padapter, inframode, 0)) {
			ret = -1;
			goto cancel_ps_deny;
		}
	}
			    ...
			    union iwreq_data *wrqu, char *extra)
{
	...
	u8 *dst_ssid, *src_ssid, set_infra_mode = 0;
	NDIS_802_11_NETWORK_INFRASTRUCTURE infra_struc_mode;
	uint ret = 0, len;
	...
	if (wrqu->essid.flags && wrqu->essid.length) {
		...
		while (1) {
			...
			if ((_rtw_memcmp(dst_ssid, src_ssid, ndis_ssid.SsidLength) == _TRUE) &&
				...
				set_infra_mode = 1;
				infra_struc_mode = pnetwork->network.InfrastructureMode;
				break;
			}
		}
		_rtw_spinunlock_bh(&queue->lock);
		...
		if (set_infra_mode) {
			if (rtw_set_802_11_infrastructure_mode(padapter, infra_struc_mode, 0) == _FALSE) {
				ret = -1;
				goto cancel_ps_deny;
			}
		}
			   ...
			   union iwreq_data *wrqu, char *extra)
{
	...
	//printk("[%s %d]   max_rate=[%d]\n",__func__,__LINE__,max_rate );
			   ...
			   char *extra)
{
	...
	case ANDROID_WIFI_CMD_LINKSPEED: {
		u32 mbps = rtw_get_cur_max_rate(padapter) / 10;
...
#ifdef GBWC
	case SIOCGBWC:
		RTW_INFO("set case SIOCGBWC\n");
		rtw_gbwc_setting(dev , info , wdata , extra);
		break;
#endif
#ifdef CONFIG_RTW_AP_EXT_SUPPORT
	case ACL_ADD_STA:
...
#endif
#ifdef GBWC
	{ SIOCGBWC, IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK, 0 , "gbwc" },
#endif
#ifdef CONFIG_RTW_AP_EXT_SUPPORT
