...
void setup_wifi_module_param(void)
{
	...
	phl_log_level = (u8)RTK_SHM_DRAM->wifi_param.phl_log_level;
}
#endif
					...
					const struct pci_device_id *pdid)
{
	...
	/* Get PCIe sub-did */
	pci_read_config_byte(pdev, 0x2e, &sdid_l);
	pci_read_config_byte(pdev, 0x2f, &sdid_h);

	dvobj->pci_subdid = ((sdid_h << 8) | sdid_l);
	RTW_PRINT("pci subdid: %04X \n", dvobj->pci_subdid);
...
_adapter *rtw_pci_primary_adapter_init(struct dvobj_priv *dvobj, struct pci_dev *pdev)
{
	...
	/* Overwrite default values in rtw_mib in terms of configuration */
	overwrite_rtw_mib_default_tbl(priv_mib);
