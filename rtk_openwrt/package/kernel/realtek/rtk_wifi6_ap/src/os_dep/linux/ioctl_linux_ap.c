...
/*init*/
	...
	if (padapter->opclass_requested_channel_nr > 0xFF) { //for coverity check
		ret = -EINVAL;
		goto exit;
	}

	if (padapter->opclass_requested_channel_nr > 0) {
...
#else
	queue = &(padapter->mlmepriv.scanned_queue);
#endif
		...
		for (j = 0; j < padapter->opclass_requested_channel_nr; j++)
		{
			...
			if (padapter->opclass_scan_result->channels[j].channel == recv_bcn.ch)
			{
				...
				_rtw_memcpy(padapter->opclass_scan_result->channels[j].neighbors[padapter->opclass_scan_result->channels[j].neighbor_nr].timestamp
						, rtw_get_timestampe_from_ie(pnetwork->network.IEs), 8);
...
func_return:
	if (padapter) padapter->opclass_channel_scan = 0;
	return ret;
}
...
int rtw_sta_extra_info_get(struct net_device *dev, struct iw_request_info *info, union iwreq_data *wrqu, char *extra)
{
	...
	while ((plist != phead))
	{
		...
		extra_sta_info->aid = psta->phl_sta->aid;
		extra_sta_info->flags |= STA_INFO_FLAG_ASOC;
		if(psta->state & WIFI_SLEEP_STATE)
			extra_sta_info->flags |= STA_INFO_FLAG_ASLEEP;
...
#endif
#ifdef GBWC
int rtw_gbwc_setting(struct net_device *dev, struct iw_request_info *info, union iwreq_data *wrqu, char *extra)
{
	_adapter *padapter = (_adapter *)rtw_netdev_priv(dev);
	u8 sta_addr[ETH_ALEN];
	char cmd[4] = {0}, tmp[64] = {0},chr=':';
	unsigned int sep = 0;
	struct sta_info *pstat = NULL;
	...
	for ( i = 0 ; i < strlen(extra); ++i)
	{
		...
		strncat(tmp, extra+i, 1);
		if(sep && (j > 0) && (j < 2*ETH_ALEN-1) && (j % 2))
			strncat(tmp, &chr, 1);
		j++;
	}
	if (sep != 1)
	{
		RTW_WARN("sep[%d] Usage: iwpriv wlanX gbwc add,[mac]\n", sep);
		return -1;
	}

	ret = sscanf(tmp, "%3s,"MAC_SFMT,cmd,
		&sta_addr[0], &sta_addr[1], &sta_addr[2],
		&sta_addr[3], &sta_addr[4], &sta_addr[5]);
	if (ret != 7)
	{
		RTW_WARN("ret[%d] Usage: iwpriv wlanX gbwc add,[mac]\n", ret);
		return -1;
	}
	pstat = rtw_get_stainfo(&padapter->stapriv, sta_addr);
	if (!pstat)
	...
	if(!strncmp(cmd,"add",3))
	{
		if (count >= _NUM_GBWC)
		{
			RTW_WARN("Exseed NUM_GBWC(%d)\n", _NUM_GBWC);
			return -1;
		}
		for (i = 0 ; i < count ; ++i)
		{
			if (!memcmp(padapter->registrypriv.wifi_mib.gbwcEntry.entry[i].mac, sta_addr, MAC_ADDR_LEN))
			{
				return 0;
			}
		}
		memcpy(padapter->registrypriv.wifi_mib.gbwcEntry.entry[i].mac, sta_addr, MAC_ADDR_LEN);
		padapter->registrypriv.wifi_mib.gbwcEntry.count++;
		pstat->GBWC_in_group = 1;

	}
	else if(!strncmp(cmd,"del",3))
	{
		pstat->GBWC_in_group = 0;
		for (i = 0 ; i < count ; ++i)
		{
			if (!memcmp(padapter->registrypriv.wifi_mib.gbwcEntry.entry[i].mac, sta_addr, MAC_ADDR_LEN))
			{
				padapter->registrypriv.wifi_mib.gbwcEntry.count--;
				memcpy(padapter->registrypriv.wifi_mib.gbwcEntry.entry[i].mac, padapter->registrypriv.wifi_mib.gbwcEntry.entry[count-1].mac, MAC_ADDR_LEN);
				break;
			}
		}
	...
	{
		RTW_WARN("Usage: iwpriv wlanX gbwc add,[mac]\n");
		return -1;
	}

	RTW_INFO("======>%s[%d:%s]%s %02X:%02X:%02X:%02X:%02X:%02X\n",
...
int rtw_ioctl_private_set(struct net_device *dev, struct iwreq *wrq, u16 subcmd){
	...
	switch (subcmd){
		...
		case SIOCMAP_UPDATEPOLICY:
			ret = rtw_map_update_metric_policy(dev, NULL, &wrq->u.data, extra);
			break;
		...
		case SIOCMAP_SET_TXMAXPOWER:
			ret = rtw_ioctl_set_tx_max_power(dev , NULL , &wrq->u , extra);
			break;
#ifdef CONFIG_RTW_MULTI_AP_R2
		case SIOCMAP_CAC:
