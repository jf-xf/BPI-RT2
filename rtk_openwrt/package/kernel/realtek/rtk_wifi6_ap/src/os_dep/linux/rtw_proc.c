...
static ssize_t proc_setup_shm_wifi_param(struct file *file,
		const char __user *buffer, size_t count, loff_t *pos, void *data)
{
	...
	/* Update Linux side wifi driver param to SHM */
	RTK_SHM_DRAM->wifi_param.rtw_ht_enable = rtw_ht_enable;
	RTK_SHM_DRAM->wifi_param.rtw_vht_enable = rtw_vht_enable;
	RTK_SHM_DRAM->wifi_param.rtw_he_enable = rtw_he_enable;
	RTK_SHM_DRAM->wifi_param.rtw_drv_log_level = rtw_drv_log_level;
	RTK_SHM_DRAM->wifi_param.phl_log_level = phl_log_level;
...
const struct rtw_proc_hdl drv_proc_hdls[] = {
	...
	RTW_PROC_HDL_SSEQ("set_shm_wifi_param", NULL, proc_setup_shm_wifi_param),
#endif /* WFO_SKIP_PCIE_SLOT */
#endif /* !CPTCFG_WFO_VIRT_SAME_CPU */
#endif /* !__ECOS */
};
...
ssize_t proc_set_csa_trigger(struct file *file, const char __user *buffer, size_t count, loff_t *pos, void *data)
{
	...
	char tmp[32];
	u8 ch = 0, cntdown = 0;
	u8 set_ie_mode = 0, switch_mode = 0, bw = 0, offset = 0, op_class = 0;
	...
	if (buffer && !copy_from_user(tmp, buffer, count)) {
		int num = sscanf(tmp, "%hhu %hhu %hhu %hhu %hhu %hhu", &ch, &cntdown, &set_ie_mode, &switch_mode, &bw, &offset);

		if (num != 2 && num != 6)
			goto exit;

		if (num == 6) {
			if ((op_class = rtw_get_op_class_by_chbw(ch, bw, offset)) == 0) {
				RTW_ERR("invalid op_class\n");
				goto exit;
			}

			if ((set_ie_mode & CSA_SET_IE_CSA) == 0
#ifdef CONFIG_ECSA_IE
				&& (set_ie_mode & CSA_SET_IE_ECSA) == 0
#endif
				) {
				RTW_ERR("invalid set_ie_mode\n");
				goto exit;
			}
		}
		...
		if (num == 2) {
			...
			if (set_ie_mode & CSA_SET_IE_CSA)
				rfctl->csa_set_ie = 1;

#ifdef CONFIG_ECSA_IE
			if (set_ie_mode & CSA_SET_IE_ECSA)
				rfctl->ecsa_set_ie = 1;
#endif
		}
...
static int proc_get_mib_dfs(struct seq_file *m, void *v)
{
	...
	RTW_PRINT_SEL(m, "csa_switch_mode: %u\n", rfctl->csa_switch_mode);
	RTW_PRINT_SEL(m, "csa_countdown: %u\n", rfctl->csa_cntdown);
	RTW_PRINT_SEL(m, "csa_op_class: %u\n", rfctl->csa_op_class);
	RTW_PRINT_SEL(m, "csa_set_ie: %u\n", rfctl->csa_set_ie);
#endif
#ifdef CONFIG_ECSA_IE
	RTW_PRINT_SEL(m, "ecsa_set_ie: %u\n", rfctl->ecsa_set_ie);
#endif
	...
	if (buffer && !copy_from_user(tmp, buffer, count)) {
		...
		if (num == 2) {
			...
			if (strcmp("switch", cmd) == 0)
				acs_change_bss_chbw(padapter, _FALSE, 0);
		}
	}
...
static int proc_get_adptvty_info(struct seq_file *m, void *v)
{
	...
	RTW_PRINT_SEL(m, "manual_edca    : %u\n", padapter->registrypriv.manual_edca);
	RTW_PRINT_SEL(m, "edcca_mode     : %u\n", mib->edcca_mode);
	RTW_PRINT_SEL(m, "\n");

	RTW_PRINT_SEL(m, "asoc_sta_num    : %u\n", pstapriv->asoc_list_cnt);
	RTW_PRINT_SEL(m, "scan_ap_num     : %u\n", scan_ap_num);
	RTW_PRINT_SEL(m, "txop(c300)      : 0x%x\n", rtw_phl_read32(padapter->dvobj->phl, 0xC300));
	RTW_PRINT_SEL(m, "active Tx(c390) : 0x%x\n", rtw_phl_read32(padapter->dvobj->phl, 0xC390));
	RTW_PRINT_SEL(m, "resp Tx(cc04)   : 0x%x\n", rtw_phl_read32(padapter->dvobj->phl, 0xCC04));
...
static inline int proc_get_drv_ver(struct seq_file *m, void *v)
{
	RTW_PRINT_SEL(m, "Realtek Wi-Fi driver verion: %s\n", REL_VERSION);
	return 0;
}
...
#endif /* CONFIG_ADPTVTY_CONTROL */
	RTW_PROC_HDL_SSEQ("drv_ver", proc_get_drv_ver, NULL),
};
...
static int proc_get_phydm_cmd(struct seq_file *m, void *v)
{
	...
	if (NULL == phydm_msg) {
		...
		return 0;
#if 0
		phydm_msg = rtw_vmalloc(PHYDM_MSG_LEN);
		if (NULL == phydm_msg)
			return -ENOMEM;
	...
	rtw_vmfree(phydm_msg, PHYDM_MSG_LEN);
	phydm_msg = NULL;
