...
module_param(rtw_acs, int, 0644);
	...
	char *ifname	= "wlan1";	/* 1st 5G */
	char *if2name	= "wlan0";	/* 2.4G */
	char *if3name	= "wlan2";	/* 2nd 5G */
	/* ifname_id should be matched with ifname wlanX order */
	/* ifname_id is used to differentiate the second same frequency band */
	int ifname_id	= 1;	/* wlan1 */
	int if2name_id	= 0;	/* wlan0 */
	int if3name_id	= 2;	/* wlan2 */
#else
	char *ifname	= "wlan0";	/* 1st 5G */
	char *if2name	= "wlan2";	/* 2.4G */
	char *if3name	= "wlan1";	/* 2nd 5G */
	/* ifname_id should be matched with ifname wlanX order */
	/* ifname_id is used to differentiate the second same frequency band */
	int ifname_id	= 0;	/* wlan0 */
	int if2name_id	= 2;	/* wlan2 */
	int if3name_id	= 1;	/* wlan1 */
#endif
module_param(ifname, charp, 0644);
MODULE_PARM_DESC(ifname, "The name of first registry interface");
module_param(if2name, charp, 0644);
MODULE_PARM_DESC(if2name,"The name of second registry interface");
module_param(if3name, charp, 0644);
MODULE_PARM_DESC(if3name, "The name of second registry interface");
#else /* !CONFIG_RTW_TRIBAND_SUPPORT */
#if defined(CONFIG_2G_ON_WLAN0)
char *ifname = "wlan1";
int ifname_id = 1;	/* wlan1 */
...
void rtw_core_update_default_setting (struct dvobj_priv *dvobj)
{
	...
	do {
		...
		phl_com->proto_sw_cap[0].max_amsdu_len = (rx_buf_size > 7991) ? 1 : 0;
		// use below only if define rx_buf_size (> 11454), or coverity will warn.
		/*phl_com->proto_sw_cap[0].max_amsdu_len = (rx_buf_size > 11454) ? 2 :
												 (rx_buf_size > 7991) ? 1 : 0;*/
	} while (0);
	#else
	...
	phl_com->proto_sw_cap[0].stbc_tx_greater_80mhz = 1;
	phl_com->proto_sw_cap[0].stbc_rx_greater_80mhz = 1;
	...
	phl_com->proto_sw_cap[1].stbc_tx_greater_80mhz = 1;
	phl_com->proto_sw_cap[1].stbc_rx_greater_80mhz = 1;
	...
	switch (dvobj->dev_id) {
		case IFACE_ID0:
			dvobj->wlan_id = ifname_id;
			break;

		case IFACE_ID1:
			dvobj->wlan_id = if2name_id;
			break;

		#ifdef CONFIG_RTW_TRIBAND_SUPPORT
		case IFACE_ID2:
			dvobj->wlan_id = if3name_id;
			break;
		#endif

		default:
			dvobj->wlan_id = 0xff;
			RTW_ERR("Device ID is not recognized. \n");
			break;
	}
	rtw_set_rfe_type(dvobj);
#endif /* CONFIG_RTW_DEV_IS_SINGLE_BAND */
...
#ifdef CONFIG_RTW_ANTENNA_DIVERSITY
	phl_com->dev_sw_cap.antdiv_sup = true;
#endif
...
int rtw_stbc_cap = 0x13;
	...
	registry_par->ifname_id  = (u8)ifname_id;
	registry_par->if2name_id = (u8)if2name_id;
#ifdef CONFIG_RTW_TRIBAND_SUPPORT
	registry_par->if3name_id = (u8)if3name_id;
#endif
