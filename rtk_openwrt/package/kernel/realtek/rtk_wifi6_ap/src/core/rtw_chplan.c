...
#ifdef CONFIG_RTW_SELF_CHANLIST
struct ch_list_t rtw_channel_def_5g[] = {
	...
	/* RTW_CHD_5G_53 */	CH_LIST_ENT(28, 36, 40, 44, 48, 52, 56, 60, 64, 100, 104, 108, 112, 116, 120, 124, 128, 132, 136, 140, 144, 149, 153, 157, 161, 165, 169, 173, 177, 0),
};
...
static char rtw_dfs_regd_mapping_by_regd[] = { /* enum rtw_regd & enum rtw_dfs_regd */
	/* RTW_REGD_NA = 0 */ RTW_DFS_REGD_ETSI,
	/* RTW_REGD_FCC = 1 */ RTW_DFS_REGD_FCC,
	/* RTW_REGD_MKK = 2 */ RTW_DFS_REGD_JAP,
	/* RTW_REGD_ETSI = 3 */ RTW_DFS_REGD_ETSI,
	/* RTW_REGD_IC = 4 */ RTW_DFS_REGD_ETSI,
	/* RTW_REGD_KCC = 5 */ RTW_DFS_REGD_ETSI,
	/* RTW_REGD_NCC = 6 */ RTW_DFS_REGD_ETSI,
	/* RTW_REGD_ACMA = 7 */ RTW_DFS_REGD_ETSI,
	/* RTW_REGD_CHILE = 8 */ RTW_DFS_REGD_ETSI,
	/* RTW_REGD_MEX = 9 */ RTW_DFS_REGD_ETSI,
	/* RTW_REGD_WW */ RTW_DFS_REGD_ETSI,
};

void rtw_5g_dfs_regd_mapping_by_regd(_adapter *adapter, u8 id)
{
	struct dvobj_priv *dvobj = adapter_to_dvobj(adapter);
	struct registry_priv *registry = adapter_to_regsty(adapter);
	enum phl_cmd_type cmd_type = PHL_CMD_DIRECTLY;
	u32 cmd_timeout = 0;
	u8 reg_num;

	if (is_primary_adapter(adapter) && is_supported_5g(registry->band_type)) {
		reg_num = rtw_dfs_regd_mapping_by_regd[RTW_ChannelPlanMap[id].regd_5g];
		registry->wifi_mib.dfs_regions = reg_num;
...
static u8 init_channel_set_from_rtk_priv_2g_5g(_adapter *adapter, RT_CHANNEL_INFO *channel_set, u8 *ch_num)
{
	...
	if (b2_4GBand && (chplan < sizeof(RTW_ChannelPlanMap) / sizeof(struct chplan_ent_t))) {
		u8 chd_2g = RTW_ChannelPlanMap[chplan].chd_2g;

		if (chd_2g < sizeof(rtw_channel_def_2g) / sizeof(struct ch_list_t)) {
...
#if CONFIG_IEEE80211_BAND_5GHZ
	if (b5GBand && (chplan < sizeof(RTW_ChannelPlanMap) / sizeof(struct chplan_ent_t))) {
		bool dfs;
		u8 chd_5g = RTW_ChannelPlanMap[chplan].chd_5g;

#ifdef CONFIG_RTW_SELF_CHANLIST
		if (adapter->registrypriv.wifi_mib.self_channel_enable == 1) {
			chd_5g = RTW_CHD_5G_53;
		}
#endif

		if (chd_5g < sizeof(rtw_channel_def_5g) / sizeof(struct ch_list_t)) {
...
#ifdef CONFIG_RTW_SELF_CHANLIST
void set_self_chan(struct _ADAPTER *padapter, unsigned int *chan_list)
{
	int i=0;
	int size = chan_list[0]+2;
	int channel_def_size = CH_LIST_LEN(rtw_channel_def_5g[RTW_CHD_5G_53])+2;

	struct rf_ctl_t *rfctl = adapter_to_rfctl(padapter);

	_rtw_memset(rtw_channel_def_5g[RTW_CHD_5G_53].len_ch_attr, 0, 30);

	i=0;
	while (i<size) {
		rtw_channel_def_5g[RTW_CHD_5G_53].len_ch_attr[i] = chan_list[i];
		i++;
	}

	init_channel_set_from_rtk_priv(padapter, rfctl->channel_set);
