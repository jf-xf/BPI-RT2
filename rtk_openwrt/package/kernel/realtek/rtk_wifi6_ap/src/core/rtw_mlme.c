...
bool rtw_update_scanned_network(_adapter *adapter, WLAN_BSSID_EX *target)
{
	...
	if (!target_find) {
		...
		if(target->Ssid.SsidLength == 0 && pnetwork->network.Ssid.SsidLength != 0) {
			RTW_INFO("(%s %d)Skip info with SSID:%s being overwritten by NULL\n",
				__func__, __LINE__, pnetwork->network.Ssid.Ssid);
			update_ie = _FALSE;
		}
...
u8 _rtw_sitesurvey_condition_check(const char *caller, _adapter *adapter, bool check_sc_interval)
{
	...
	if (check_fwstate(pmlmepriv, WIFI_AP_STATE)){
		if(check_fwstate(pmlmepriv, WIFI_UNDER_WPS)) {
			...
			goto _exit;
		} else if (rtw_ap_sta_states_check(adapter) == _TRUE) {
			RTW_INFO("%s ("ADPT_FMT") : scan abort!!AP mode under sta linking or key handshaking\n",
				caller, ADPT_ARG(adapter));
			ss_condition = SS_DENY_SELF_AP_UNDER_STA_LINKING;
			goto _exit;
		}
	} else {
...
void rtw_free_assoc_resources(_adapter *adapter, u8 lock_scanned_queue)
{
	...
	/* lock_scanned_queue is always TRUE, move lock to _rtw_find_same_network
...
void rtw_indicate_disconnect(_adapter *padapter, u16 reason, u8 locally_generated)
{
	...
	if(MLME_IS_STA(padapter))
	{
		for (i = 0; i < dvobj->iface_nums; i++) {
			...
			{
				if((check_fwstate(mlme, WIFI_OP_CH_SWITCHING)) == _TRUE)
				{
					clr_fwstate(mlme, WIFI_OP_CH_SWITCHING);
					update_beacon(iface, 0xFFF, NULL, _TRUE, 0);
					RTW_INFO("\n[%s %d]=%s mlme->fw_state=%d\n",__FUNCTION__,__LINE__,iface->pnetdev->name,mlme->fw_state);
				}
			}
		}
	}
...
#ifdef CONFIG_LIFETIME_FEATURE
	if (rtw_is_adapter_up(GET_PRIMARY_ADAPTER(padapter)) && GET_PRIMARY_ADAPTER(padapter)->registrypriv.wifi_mib.lifetime) {
		us256 = RTL_MS_TO_256US(GET_PRIMARY_ADAPTER(padapter)->registrypriv.wifi_mib.lifetime);
		rtw_phl_set_lifetime(GET_HAL_INFO(GET_PRIMARY_ADAPTER(padapter)->dvobj), 1, us256);
	}
#endif
		...
		if (check_fwstate(pmlmepriv, WIFI_UNDER_LINKING)) {
			...
			if (check_fwstate(pmlmepriv, WIFI_ASOC_STATE)) {
				if (the_same_macaddr == _TRUE)
					...
					if (MLME_IS_STA(adapter)) {
						/* Remove fix current BSS in scanned_queue, or hidden AP
						   will show in survey_info when connecting to hidden AP */
						#if 0
						if (ptarget_wlan)
...
void rtw_stassoc_event_callback(_adapter *adapter, u8 *pbuf)
{
	...
	if (MLME_IS_AP(adapter) || MLME_IS_MESH(adapter)) {
		...
		if (psta) {
			...
			/* temporarily clear done state to avoid sta being freed during rtw_hw_connected_apmode */
			if (!rtw_test_and_clear_bit(RTW_BIT_STA_ALLOC_DONE, &psta->check_state)) {
				RTW_WARN("[%s %d][%s] %pM already be freed, do nothing\n", __func__, __LINE__, adapter->pnetdev->name,
					pstassoc->macaddr);
				goto exit;
			}
			...
			rtw_set_bit(RTW_BIT_STA_ALLOC_DONE, &psta->check_state);

#if 0	/* set this h2c when adding ul grp */
...
static void dynamic_vow_judgement(struct dvobj_priv *obj, _adapter *adapter)
{
	u32 pktNumIncrease = obj->tx_wmm_pkts[XMIT_VO_QUEUE] - obj->tx_wmm_pkts_prev[XMIT_VO_QUEUE];
	u8 lowestPktNum = (adapter->registrypriv.wifi_mib.vow_para) >> 8;
	u8 f_aggnum = (adapter->registrypriv.wifi_mib.vow_para) & 0xff;
	u32 i = 0, viThreshold = 500;

	if (pktNumIncrease >= viThreshold &&
			adapter->registrypriv.wifi_mib.vow_mode == 0) {
		if (is_primary_adapter(adapter)) {
			rtw_phl_write32(obj->phl, 0xc300, 0xa42b);
			rtw_phl_write32(obj->phl, 0xc308, 0x4319);
			rtw_phl_write32(obj->phl, 0xc30c, 0x3219);
		}
		adapter->registrypriv.wifi_mib.vow_mode = 3;
		adapter->txForce_enable = 1;
		adapter->txForce_aggnum = f_aggnum;
		rtw_phl_set_vow_vi_lowest_thr(obj->phl, lowestPktNum);
		txsc_clear(adapter, 1);
		RTW_PRINT("[%s] Set vow VI lowest pkt number:%d, vow_mode:%d\n",
						__func__, lowestPktNum, adapter->registrypriv.wifi_mib.vow_mode);
	} else if (pktNumIncrease < viThreshold &&
			adapter->registrypriv.wifi_mib.vow_mode != 0) {
		if (--adapter->registrypriv.wifi_mib.vow_mode == 0) {
			if (is_primary_adapter(adapter)) {
				rtw_phl_write32(obj->phl, 0xc300, 0x642b);
				rtw_phl_write32(obj->phl, 0xc308, 0x5e4319);
				rtw_phl_write32(obj->phl, 0xc30c, 0x2f3219);
			}
			adapter->registrypriv.wifi_mib.vow_mode = 0;
			adapter->txForce_enable = 0;
			adapter->txForce_aggnum = INV_TXFORCE_VAL;
			rtw_phl_set_vow_vi_lowest_thr(obj->phl, 0);
			txsc_clear(adapter, 1);
			RTW_PRINT("[%s] Disable vow VI lowest pkt number:%d, vow_mode:%d\n",
						__func__, lowestPktNum, adapter->registrypriv.wifi_mib.vow_mode);
		}
	}
...
#endif
#ifdef CONFIG_RTW_VOW_ENHANCEMENT
	dynamic_vow_judgement(pdvobjpriv, padapter);
#endif
...
void rtw_core_watchdog_sw_hdlr(void *drv_priv)
{
	...
	if (adapter->octoscope_enable)
		rtw_octoscope_handler(adapter);
#endif
...
bool rtw_adjust_chbw(_adapter *adapter, u8 req_ch, u8 *req_bw, u8 *req_offset)
{
	...
	if (req_ch < 14)
		allowed_bw = get_highest_bw_cap(REGSTY_BW_2G(regsty));
	else if (req_ch == 14)
		allowed_bw = CHANNEL_WIDTH_20;
	else
		allowed_bw = get_highest_bw_cap(REGSTY_BW_5G(regsty));

	allowed_bw = rtw_hw_largest_bw(adapter_to_dvobj(adapter), allowed_bw);
	if(rfctl->csa_ch)
	{
		if(*req_bw > allowed_bw)
			allowed_bw = *req_bw;
	}
#if defined(CONFIG_RTW_MULTI_AP)
	if((GET_MAP_BSS_TYPE(adapter) & MULTI_AP_BACKHAUL_STA) || (GET_MAP_BSS_TYPE(adapter) & MULTI_AP_BACKHAUL_BSS) || (GET_MAP_BSS_TYPE(adapter) & MULTI_AP_FRONTHAUL_BSS))
	{
		*req_bw = rtw_hw_largest_bw(adapter_to_dvobj(adapter), *req_bw);
		RTW_INFO("%s, sync bandwith with remote ap under MULTI_AP\n", __FUNCTION__);
