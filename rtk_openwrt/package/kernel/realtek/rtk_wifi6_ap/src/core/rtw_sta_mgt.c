...
void _rtw_init_stainfo(struct sta_info *psta, unsigned char is_first)
{
	...
	if (is_first) {
		...
		for (i = 0; i < WLAN_MAX_VENDOR_IE_NUM; i++) {
			if (psta->vendor_ie[i]) {
				rtw_mfree(psta->vendor_ie[i], psta->vendor_ielen[i]);
				psta->vendor_ie[i] = NULL;
				psta->vendor_ielen[i] = 0;
			}
		}
#endif
...
u32	_rtw_init_sta_priv(struct	sta_priv *pstapriv)
{
	...
	struct latency_sta_info *l_psta;
	#endif
	s32 i;
	u32 ret = _FAIL;
...
#ifdef CONFIG_GREEN_CHANNEL_FOR_LATENCY_STA
		pstapriv->pallocated_latency_sta_buf = rtw_zvmalloc(
			sizeof(struct latency_sta_info) * num_latency_sta + MEM_ALIGNMENT_OFFSET);
		if (!pstapriv->pallocated_latency_sta_buf)
			goto exit;

		pstapriv->platency_sta_buf = pstapriv->pallocated_latency_sta_buf;
		if ((SIZE_PTR)pstapriv->platency_sta_buf & MEM_ALIGNMENT_PADDING)
			pstapriv->platency_sta_buf += MEM_ALIGNMENT_OFFSET -
				((SIZE_PTR)pstapriv->platency_sta_buf & MEM_ALIGNMENT_PADDING);

		pstapriv->free_latency_queue = rtw_zvmalloc(sizeof(_queue));
		_rtw_init_queue(pstapriv->free_latency_queue);

		l_psta = (struct latency_sta_info *)(pstapriv->platency_sta_buf);
		for (i = 0; i < num_latency_sta; i++) {
			l_psta->set_enable = 0;

			rtw_list_insert_tail(&l_psta->free_list, get_list_head(pstapriv->free_latency_queue));
			l_psta++;
		}
#endif
...
#ifdef CONFIG_GREEN_CHANNEL_FOR_LATENCY_STA
		pstapriv->free_latency_queue = pri_adapter->stapriv.free_latency_queue;
#endif
	}
...
#ifdef CONFIG_GREEN_CHANNEL_FOR_LATENCY_STA
	_rtw_init_listhead(&pstapriv->latency_sta_list);
#endif
...
#ifdef CONFIG_GREEN_CHANNEL_FOR_LATENCY_STA
	_rtw_spinlock_init(&pstapriv->latency_sta_list_lock);
	_rtw_spinlock_bh(&pstapriv->latency_sta_list_lock);
	pstapriv->latency_sta_list_cnt = 0;
...
u32	_rtw_free_sta_priv(struct	sta_priv *pstapriv)
{
	...
	if (pstapriv) {
		...
		if (is_primary_adapter(pstapriv->padapter)) {
			...
			#ifdef CONFIG_GREEN_CHANNEL_FOR_LATENCY_STA
			if (pstapriv->pallocated_latency_sta_buf)
				rtw_vmfree(pstapriv->pallocated_latency_sta_buf,
					sizeof(struct latency_sta_info) * num_latency_sta + MEM_ALIGNMENT_OFFSET);
			if (pstapriv->free_latency_queue)
				rtw_vmfree(pstapriv->free_latency_queue, sizeof(_queue));
			#endif
		}
...
struct sta_info *rtw_get_stainfo(struct sta_priv *pstapriv, const u8 *hwaddr)
{
	...
	if(psta && psta->phl_sta && psta->phl_sta->active && EQ_MAC_ADDR(psta->phl_sta->mac_addr, hwaddr)) {
		...
		if(pstapriv->padapter->a4_db_entry_cache != NULL) {
			if (EQ_MAC_ADDR(pstapriv->padapter->a4_db_entry_cache->mac, hwaddr)) {
				psta = pstapriv->padapter->a4_db_entry_cache->psta;
				if (!psta || !psta->phl_sta || pstapriv->padapter->a4_db_entry_cache->m2u_ignore_cnt >= 5) {
					pstapriv->padapter->a4_db_entry_cache = NULL;
					psta = NULL;
				}
				if(psta)
					return psta;
...
struct latency_sta_info *rtw_check_latency_sta_exist(_adapter *adapter, u8 *mac_addr) {
	...
	_list	*phead, *plist;
	struct latency_sta_info *l_sta, *t_l_sta = NULL;
	//u8 find = 0;
	...
	while ((rtw_end_of_queue_search(phead, plist)) == _FALSE) {
		...
		if (_rtw_memcmp(l_sta->mac_addr, mac_addr, MAC_ALEN) == true) {
			t_l_sta = l_sta;
			break;
		}
		plist = get_next(plist);
