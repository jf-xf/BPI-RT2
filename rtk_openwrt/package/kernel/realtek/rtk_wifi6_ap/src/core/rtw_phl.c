...
void rtw_dump_phl_sta_info(void *sel, struct sta_info *sta)
{
	...
	RTW_PRINT_SEL(sel, "[STA] r-idx:%d mac:%02x-%02x-%02x-%02x-%02x-%02x aid:%d mid:%d band:%d ch-bw-ofst:%d-%d-%d\n",
			phl_sta->wrole->id,
...
static void core_hdl_phl_evt(struct dvobj_priv *dvobj, struct phl_msg *msg)
{
	...
	if (evt_id == MSG_EVT_BCN_RESEND) {
		...
		iface = dvobj_get_primary_adapter(dvobj);
		rfctl = adapter_to_rfctl(iface);
		is_dfs_ch = rtw_chset_is_dfs_chbw(rfctl->channel_set,
						iface->mlmeextpriv.cur_channel,
						iface->mlmeextpriv.cur_bwmode,
						iface->mlmeextpriv.cur_ch_offset);

		/* if nor under cac time and cur channel is dfs ch, do tx pause */
		if (!IS_UNDER_CAC(rfctl) && is_dfs_ch)
			rtw_dfs_csa_hw_tx_pause(iface, _TRUE);

		/* workaround for non-dfs channel but DFS detecting is on */
		if (!is_dfs_ch) {
			phl_com->dfs_info.is_radar_detected = false;
...
#if defined(CONFIG_PLATFORM_RTL8198D) || defined(CONFIG_PLATFORM_RTL8197F)
		      ...
		      u8 offset, u8 do_rfk)
{
	...
	if (do_rfk)
		status = rtw_phl_cmd_set_ch_bw(a->phl_role,
						&chdef, RFK_TYPE_FORCE_DO,
						cmd_type, cmd_timeout);
	else
		status = rtw_phl_cmd_set_ch_bw(a->phl_role,
						&chdef, RFK_TYPE_FORCE_NOT_DO,
						cmd_type, cmd_timeout);
...
#ifdef TX_BEAMFORMING
	if (rtw_phl_snd_chk_in_progress(phl_info) == 0) {
...
#ifdef CONFIG_GREEN_CHANNEL_FOR_LATENCY_STA
	l_sta = rtw_check_latency_sta_exist(a, sta->phl_sta->mac_addr);

	if (l_sta) {
		sta->phl_sta->is_latency_sta = 0;

		l_sta->set_enable = 0;

		/* Call disable latency H2C API*/
		rtw_phl_set_fw_latency_flow_ctrl(a->dvobj->phl, sta->phl_sta->macid,
						 0, a->registrypriv.wifi_mib.fw_fctrl_intv);
	}
#endif
...
int rtw_hw_connected_apmode(struct _ADAPTER *a, struct sta_info *sta)
{
	...
	if ((a->registrypriv.wifi_mib.ax_sgi & 0xf) != 0xf)
		rtw_phl_set_manual_giltf(phl, sta->phl_sta, a->registrypriv.wifi_mib.ax_sgi);

#ifdef CONFIG_GREEN_CHANNEL_FOR_LATENCY_STA
	l_sta = rtw_check_latency_sta_exist(a, sta->phl_sta->mac_addr);
	if (l_sta && l_sta->set_enable==0) {
		/* Call enable latency H2C API*/
		l_sta->set_enable = 1;
		sta->phl_sta->is_latency_sta = 1;
		rtw_phl_set_fw_latency_flow_ctrl(a->dvobj->phl, sta->phl_sta->macid,
							 1, a->registrypriv.wifi_mib.fw_fctrl_intv);
	}
#endif
...
BOOLEAN rtw_dfs_hal_radar_detect(_adapter *padapter)
{
	...
	if (phl_com->dfs_info.is_radar_detected) {
		is_rd_detect = true;
	} else {
		is_rd_detect = false;
	}
...
u8 rtw_dfs_get_region_domain(struct dvobj_priv *dvobj)
{
	...
	u8 dfs_region = RTW_DFS_REGD_ETSI;

	/* if MIB (dfs_regions) has valid region, use MIB setting first */
	if (registry->wifi_mib.dfs_regions != RTW_DFS_REGD_NONE &&
		registry->wifi_mib.dfs_regions < RTW_DFS_REGD_NUM)
		dfs_region = registry->wifi_mib.dfs_regions;

	RTW_INFO("[%s] dfs_region:%u\n", __func__, dfs_region);
...
u8 rtw_hw_set_beacon_ignore_edcca(_adapter *padapter, u8 enable)
{
	enum rtw_phl_status status;

	status = rtw_phl_cmd_wrole_change(padapter->dvobj->phl, padapter->phl_role,
				WR_CHG_BCN_IGNORE_EDCCA, &enable, sizeof(u8), PHL_CMD_DIRECTLY, 0);
	if (status != RTW_PHL_STATUS_SUCCESS) {
		RTW_ERR("%s: fail to set beacon ignore edcca\n", __func__);
		return _FAIL;
	}
