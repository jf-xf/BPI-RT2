		...
		struct nb_rpt_hdr *nb_report)
{
	...
	pbuf = beacon_rep->integrated_bcn_rpt;
	len = beacon_rep->integrated_bcn_rpt_len;
...
struct dot11k_beacon_measurement_report * get_sta_rm_bcn_rpt(struct sta_info *psta, u8 *pframe, int len)
{
	unsigned char element_len = pframe[len + 1];
	struct dot11k_beacon_measurement_report *bcn_rpt = NULL;
	int i;
	u8 bssid_exist = 0;

	#ifdef CONFIG_RTW_AP_EXT_SUPPORT
	for (i = 0 ; i < psta->rm_beacon_rpt_num; i++) {
		if (_TRUE == _rtw_memcmp(psta->rm_beacon_rpt_list[i].info.bssid,
		             pframe + len + 20, ETH_ALEN)) {
			bcn_rpt = &psta->rm_beacon_rpt_list[i];
			bssid_exist = 1;
			#ifdef CONFIG_RTW_MULTI_AP
			psta->rm_beacon_rpt_len[i]      = element_len;
			psta->rm_becaon_meas_token[i]   = pframe[len+2];
			psta->rm_beacon_rpt_mode[i]     = pframe[len+3];
			#endif
	...
	if (psta->rm_beacon_rpt_num < RTW_MAX_RADIO_RPT_NUM
	    && bssid_exist == 0) {
		#ifdef CONFIG_RTW_AP_EXT_SUPPORT
		bcn_rpt = &psta->rm_beacon_rpt_list[psta->rm_beacon_rpt_num];
		memset(bcn_rpt, 0x00, sizeof(struct dot11k_beacon_measurement_report));
		#endif /* CONFIG_RTW_AP_EXT_SUPPORT */
		#ifdef CONFIG_RTW_MULTI_AP
...
int _rm_parse_single_beacon_mens_report(_adapter *padapter,
                        ...
                        u8 *pframe, int len, int frame_len, struct dot11k_beacon_measurement_report *beacon_rep)
{
	//struct dot11k_beacon_measurement_report *beacon_rep = NULL;
	unsigned char element_len = pframe[len + 1];
	unsigned char report_len = element_len;
	unsigned char report_copy_len = 0;
	...
	if (pframe[len + 3] == 0) {
		...
		while (report_copy_len < report_len + 2)
		{
			...
			if (subelement_id == _FRAME_BODY_SUBIE_) {
				...
				if (pframe[len + 2 + subelement_len]  == _FRAME_BODY_FRAGMENT_ && 0 != (pframe[len + 2 + subelement_len + 3] & 0x7f)) {
					subelements_copy_len = 2;
					len += 2;
					frame_fragment = 1;
					// only if fragment present and fragment id is not 0, add 2
				} else {
					subelements_copy_len = 14;
					len += 14; // id + len + fixed-length field
				}

				while (subelements_copy_len < subelement_len + 2) {
			...
			else if (subelement_id == _FRAME_BODY_FRAGMENT_){
				int j;
				len += 2 + subelement_len;

				/* remove length of frame body fragment for multi-ap */
				#if defined(CONFIG_RTW_AP_EXT_SUPPORT) && defined(CONFIG_RTW_MULTI_AP)
				for (j= 0 ; j < psta->rm_beacon_rpt_num; j++) {
					if (beacon_rep && _TRUE == _rtw_memcmp(psta->rm_beacon_rpt_list[j].info.bssid, beacon_rep->info.bssid, ETH_ALEN)) {
						psta->rm_beacon_rpt_len[j] -= (2+subelement_len);
						break;
					}
				}
				#endif
		...
		if (beacon_rep) {
			...
			/* integrate the beacon reports for parsing all capability and fill mlme->nb_info */
			if (frame_fragment) {
				// if it's the 2nd or later fragment, skip the subelement id and len to aggregate beacon IEs
				memcpy(beacon_rep->integrated_bcn_rpt + beacon_rep->integrated_bcn_rpt_len, pframe + subelements_begin + 2, subelements_copy_len - 2);
				beacon_rep->integrated_bcn_rpt_len += subelements_copy_len - 2;
			} else {
				memcpy(beacon_rep->integrated_bcn_rpt + beacon_rep->integrated_bcn_rpt_len, pframe + subelements_begin, subelements_copy_len);
				beacon_rep->integrated_bcn_rpt_len += subelements_copy_len;
			}
	...
	while (len < frame_len) {
		...
		if (element_id == _MEAS_RSP_IE_) {
			if(pdiag_body[len + 4] == bcn_rep) {
				struct dot11k_beacon_measurement_report *bcn_rpt = get_sta_rm_bcn_rpt(psta, pdiag_body, len);
				len = _rm_parse_single_beacon_mens_report(padapter, psta, pdiag_body, len, frame_len, bcn_rpt);
				continue;
			}
		}
		len += 2 + element_len;
