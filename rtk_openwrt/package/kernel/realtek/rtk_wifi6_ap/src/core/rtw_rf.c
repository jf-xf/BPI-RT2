...
u8 rtw_get_op_class_by_ch_cch(u8 ch, u8 cch)
{
	u8 bw = 0, offset = 0, gid = 0;
	...
	gid = rtw_get_op_class_by_chbw(ch, bw, offset);
...
{
	enum opc_bw opc_bw;
	u8 valid = 0, tmp_bw = 0, tmp_offset = 0;
	int i;
	...
	tmp_bw = opc_bw_to_ch_width(opc_bw);

	if (opc_bw == OPC_BW40PLUS)
		tmp_offset = CHAN_OFFSET_UPPER;
	else if (opc_bw == OPC_BW40MINUS)
		tmp_offset = CHAN_OFFSET_LOWER;

	if (rtw_get_offset_by_chbw(ch, tmp_bw, &tmp_offset)) {
		valid = 1;
		*bw = tmp_bw;
		*offset = tmp_offset;
	}
