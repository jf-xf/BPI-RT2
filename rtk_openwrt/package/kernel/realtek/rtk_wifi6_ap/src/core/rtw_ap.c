...
void init_mlme_ap_info(_adapter *padapter)
{
	...
	_rtw_spinlock_init(&pmlmepriv->ie_update_lock);
}
...
void free_mlme_ap_info(_adapter *padapter)
{
	...
	_rtw_spinlock_free(&pmlmepriv->ie_update_lock);
...
#define timer_deviation 200

u8 chk_sta_is_alive(struct sta_info *psta);
u8 chk_sta_is_alive(struct sta_info *psta)
{
	u8 ret = _FALSE;
	unsigned long passtime = rtw_get_passing_time_ms(psta->sta_stats.last_rx_time);
	...
	if (rtw_no_sta_alive_check || passtime < 2000 + timer_deviation) {
		ret = _TRUE;
	} else {
		#if 0
...
static u8 sta_amsdu_num_by_rate(_adapter *adapter, struct sta_info *sta)
{
	...
	if (sta->traffic_mode == TRAFFIC_MODE_RX)
		...
		{
			if(!rtw_phl_check_bf_mu_entry(adapter->dvobj->phl, sta->phl_sta))
			{
				switch (tx_rate) {
					case RTW_DATA_RATE_VHT_NSS1_MCS7:
					case RTW_DATA_RATE_VHT_NSS1_MCS8:
...
void sta_dynamic_control(_adapter *adapter, struct sta_info *sta)
{
	...
	throughput_th_high = (WIFI_ROLE_IS_ON_5G(adapter)) ? (adapter->registrypriv.wifi_mib.throughput_5g_th_high) : (adapter->registrypriv.wifi_mib.throughput_2g_th_high);
	throughput_th_medium = (WIFI_ROLE_IS_ON_5G(adapter)) ? (adapter->registrypriv.wifi_mib.throughput_5g_th_mid) : (adapter->registrypriv.wifi_mib.throughput_2g_th_mid);
	...
	else if (sta->phl_sta->is_latency_sta)
		sta_amsdu_num = AMSDU_0_MAX_NUM;
#endif
...
void throughput_dynamic_control(_adapter *adapter)
{
	...
	if (!primary_adapter
			...
			|| primary_adapter->registrypriv.wifi_mib.vow_mode
#endif
			|| primary_adapter->registrypriv.manual_edca)
		return;
	...
	#endif
	{
		...
		/* Client mode */
		if (adapter->phl_role->type == PHL_RTYPE_STATION) {

			rtw_hw_set_edca(adapter, WMM_BEST_EFFORT,(adapter->be_txop_sta_mode&0x7ff) <<16| (adapter->be_cwmax_sta_mode&0xf) <<12|(adapter->be_cwmin_sta_mode&0xf) <<8|(adapter->be_aifsn_sta_mode*9+16));
			rtw_hw_set_edca(adapter, WMM_BACKGROUND,(adapter->bk_txop_sta_mode&0x7ff) <<16| (adapter->bk_cwmax_sta_mode&0xf) <<12|(adapter->bk_cwmin_sta_mode&0xf) <<8|(adapter->bk_aifsn_sta_mode*9+16));
			rtw_hw_set_edca(adapter, WMM_VIDEO,(adapter->vi_txop_sta_mode&0x7ff) <<16| (adapter->vi_cwmax_sta_mode&0xf) <<12|(adapter->vi_cwmin_sta_mode&0xf) <<8|(adapter->vi_aifsn_sta_mode*9+16));
			rtw_hw_set_edca(adapter, WMM_VOICE,(adapter->vo_txop_sta_mode&0x7ff) <<16| (adapter->vo_cwmax_sta_mode&0xf) <<12|(adapter->vo_cwmin_sta_mode&0xf) <<8|(adapter->vo_aifsn_sta_mode*9+16));
			...
			if (adapter->dvobj->th_mode == 1) {
			...
			#endif
				{
					if (WIFI_ROLE_IS_ON_24G(adapter))
						rtw_hw_set_edca(adapter, 0, 0x138652b);
					else {
						if (sta->phl_sta->wmode == WLAN_MD_11A_AC) {
							...
							#endif
								rtw_hw_set_edca(adapter, 0, 0xbc652f);
						}
						#endif
						else {
						#ifdef RTW_WKARD_APSEC_MAXTXTP
				...
				if (sta->vendor == HT_IOT_PEER_APPLE &&
					...
					rtw_hw_set_edca(adapter, 0, 0x652b);
			}

			#ifdef CONFIG_RTW_MANUAL_EDCA
			if( adapter->registrypriv.manual_ap_sta_edca ) {
					dynamic_EDCA_para(adapter);
				}
			#endif
		}
	}
...
{
	u32 fail_cnt_thr = adapter->registrypriv.wifi_mib.fast_leave_thr & 0xFF;
	u32 chk_trx_intvl = ((adapter->registrypriv.wifi_mib.fast_leave_thr & 0xFF00) >> 8) * 1000;
	u32 cur_tp = sta->sta_stats.tx_tp_kbits >> 10;
	unsigned long passtime = rtw_get_passing_time_ms(sta->sta_stats.last_rx_time);

	if (!fail_cnt_thr || passtime < chk_trx_intvl
...
#ifdef CONFIG_RTW_A4_STA
		RTW_PRINT("[%s] check STA(macid/txnok/tp/passtime/a4:%d/%d/%d/%ld/%s) status is disconnected\n",
					__func__, sta->phl_sta->macid, sta->fast_txnok_chk, cur_tp, passtime, (sta->flags & WLAN_STA_A4)?"Y":"N");
#else
		RTW_PRINT("[%s] check STA(macid/txnok/tp/passtime:%d/%d/%d/%ld) status is disconnected\n",
					__func__, sta->phl_sta->macid, sta->fast_txnok_chk, cur_tp, passtime);
#endif
		...
		if (--psta->expire_to > 0) {
			if ((pstapriv->expire_to - psta->expire_to) % snd_null_intvl == 0) {
				if (MLME_IS_AP(padapter)) {
					RTW_INFO(FUNC_ADPT_FMT" Issue null data to sta "MAC_FMT", snd_null_intvl%d\n",
...
#if 0 // for SRRC
	if ((pstapriv->asoc_list_cnt <= 2) && (scan_ap_num == 0)) {
		u_match_cnt = U_MATCH_CNT_TH;
		goto force_stop;
	} else {
		u_match_cnt = 0;
		goto force_stop;
...
force_stop:
	...
	if(in_adptvty_test != adpt->adptvty_test) {
		...
		if(in_adptvty_test == _TRUE) {
			RTW_PRINT("%s : Enter adaptivity mode !!!", __func__);
			/* MAC - manual txop and set value*/
			padapter->registrypriv.manual_edca = 1;
			rtw_hw_set_edca(padapter, 0, 0xa42b);
			/* MAC - disable beacon_ignore_edcca */
			rtw_hw_set_beacon_ignore_edcca(padapter, 0);
			/* MAC - active Tx chk edcca (c390)*/
			rtw_phl_mac_sifs_chk_cca_en(padapter->dvobj->phl, padapter->phl_role, 1);
			/* MAC - resp Tx chk edcca (cc04)*/
			rtw_phl_mac_resp_chk_cca(padapter->dvobj->phl, padapter->phl_role, 1);
			/* BB - set edcca_mode as adaptivity */
			padapter->dvobj->phl_com->edcca_mode = 1;
			mib->edcca_mode = 1;
		} else {
			RTW_PRINT("%s : Restore to normal mode !!!", __func__);
			/* MAC - dynamic txop */
			padapter->registrypriv.manual_edca = 0;
			/* MAC - enable beacon_ignore_edcca if needed*/
			//rtw_hw_set_beacon_ignore_edcca(padapter, ENABLE);
			/* MAC - active Tx chk edcca (c390)*/
			rtw_phl_mac_sifs_chk_cca_en(padapter->dvobj->phl, padapter->phl_role, 0);
			/* MAC - resp Tx chk edcca (cc04)*/
			rtw_phl_mac_resp_chk_cca(padapter->dvobj->phl, padapter->phl_role, 0);
			/* BB - restore edcca_mode to normal */
			padapter->dvobj->phl_com->edcca_mode = 0;
			mib->edcca_mode = 0;
		}
	}
...
void expire_timeout_chk(_adapter *padapter)
{
	...
	u8 vht_2g_sta_num = 0;
	int i;
	int vht_2g_sta_offset;
	int vht_2g_sta_list[NUM_STA] = {0};
	...
	if (is_primary_adapter(padapter)) {
		if(padapter->amsdu_hw_timer_enable) {
			if(padapter->amsdu_hw_timer_alive == 0)
				rtw_core_set_gt3(padapter, 1, 1000);
			padapter->amsdu_hw_timer_alive = 0;
		}
	}
#endif
	...
	while ((rtw_end_of_queue_search(phead, plist)) == _FALSE) {
		...
		if (padapter->registrypriv.wifi_mib.vht_proprietary & VHT_2G_SURVEY &&
		    ...
		    !psta->phl_sta->vht_2g_supported) {
			vht_2g_sta_offset = rtw_stainfo_offset(pstapriv, psta);
			if (stainfo_offset_valid(vht_2g_sta_offset))
				vht_2g_sta_list[vht_2g_sta_num++] = vht_2g_sta_offset;
		}
#endif /* CONFIG_24G_256QAM */
...
int rtw_check_beacon_data(_adapter *padapter, u8 *pbuf,  int len)
{
...
#ifdef CONFIG_RTW_MANUAL_EDCA
				if(pregistrypriv->manual_ap_sta_edca) {
	...
	if(padapter->registrypriv.ht_enable &&
		...
		if (p && ie_len > 0) {
			...
			/* For now, hostapd configures only BW related bits, so set other bits here */
			if (channel <= 14)
				pht_cap->cap_info |= cpu_to_le16(IEEE80211_HT_CAP_DSSSCCK40);
			...
			if (!TEST_FLAG(pmlmepriv->htpriv.stbc_cap, STBC_HT_ENABLE_RX))
				...
				/* Only one spatial-stream STBC RX is supported */
				pht_cap->cap_info |= cpu_to_le16(IEEE80211_HT_CAP_RX_STBC_1R);
	...
	if(padapter->registrypriv.ht_enable &&
		...
		if (pmlmepriv->htpriv.bss_coexist || padapter->registrypriv.coex_2g_40m) {
			...
			if (pmlmepriv->ext_capab_ie_len < 1)
	...
	pbss_network->IELength += rtw_fix_ie(ie + _BEACON_IE_OFFSET_, pbss_network->IELength - _BEACON_IE_OFFSET_, WIFI_BEACON);
...
void _update_beacon(_adapter *padapter, u16 ie_id, u8 *oui, u8 tx, u8 flags, const char *tag)
{
	...
	pnetwork = &pmlmeext->mlmext_info.network;
	...
	/* some IEs are not updated in place */
	if (reorder)
		pnetwork->IELength += rtw_fix_ie(pnetwork->IEs + _BEACON_IE_OFFSET_,
...
int rtw_ht_operation_update(_adapter *padapter)
{
	...
	if (!(pmlmepriv->ht_op_mode & HT_INFO_OPERATION_MODE_NON_HT_STA_PRESENT) &&
	    (pmlmepriv->num_sta_no_ht || ht_protection) && (!padapter->registrypriv.protectionDisabled)) {
	 ...
	 */
	new_op_mode = 0;
	if (pmlmepriv->num_sta_no_ht && (!padapter->registrypriv.protectionDisabled)/*||
	    (pmlmepriv->ht_op_mode & HT_INFO_OPERATION_MODE_NON_GF_DEVS_PRESENT)*/)
		new_op_mode = OP_MODE_MIXED;
	else if ((phtpriv_ap->ht_cap.cap_info & cpu_to_le16(IEEE80211_HT_CAP_SUP_WIDTH))
		 && pmlmepriv->num_sta_ht_20mhz)
		new_op_mode = OP_MODE_20MHZ_HT_STA_ASSOCED;
	else if (ht_protection && (!padapter->registrypriv.protectionDisabled))
...
void bss_cap_update_on_sta_join(_adapter *padapter, struct sta_info *psta)
{
	...
	if ((psta->flags & WLAN_STA_NONERP) && (!padapter->registrypriv.protectionDisabled)){
		if (!psta->nonerp_set) {
			psta->nonerp_set = 1;
...
u16 rtw_ap_parse_sta_security_ie(_adapter *adapter, struct sta_info *sta, struct rtw_ieee802_11_elems *elems)
{
	...
	if (wpa_ie == NULL) {
		...
		if ((sec->wpa_psk > 0)
			...
			_rtw_spinlock_bh(&mlme->bcn_update_lock);
			if (mlme->wps_beacon_ie) {
				...
				if (!selected_registrar) {
					...
					_rtw_spinunlock_bh(&mlme->bcn_update_lock);
					goto exit;
				}
			}
			_rtw_spinunlock_bh(&mlme->bcn_update_lock);
...
void rtw_add_ap_neighbor_info(_adapter *adapter, union recv_frame *precv_frame)
{
	...
	if (rtw_get_bcn_keys(adapter, pframe_data, len, &recv_bcn) == _FAIL){
		RTW_WARN("[%s]rtw_get_bcn_keys = fail \n",__FUNCTION__);
		return;
	}
