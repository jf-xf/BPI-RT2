...
void dump_vendor_to_str(struct seq_file *m, char *str, u8 vendor)
{
	...
	case HT_IOT_PEER_OCTOSCOPE:
		...
		break;
	case HT_IOT_PEER_CHANGESELF:
		_os_snprintf(vendor_s, sizeof(vendor_s), "CHANGESELF");
		break;
	default:
	...
	for (i = 0; i < NUM_STA; i++) {
		...
		while ((rtw_end_of_queue_search(phead, plist)) == _FALSE) {
			...
			/* if(extra_arg == psta->phl_sta->aid) */
			{
				...
				wmode = pregpriv->band & psta->phl_sta->wmode;
				...
				RTW_PRINT_SEL(m, "sleep_q_total_len=%d\n", ATOMIC_READ(&dvobj->sleep_q_total_len));
...
#endif
				RTW_PRINT_SEL(m, "wd_stbc_cnt: %u \n", ATOMIC_READ(&psta->phl_sta->wd_stbc_cnt));
				RTW_PRINT_SEL(m, "stbc_disable: %u \n", psta->phl_sta->hal_sta->stbc_info.stbc_disable);

				RTW_PRINT_SEL(m, "STBC Rx(HT,VHT,HE(160M,80M))=(0x%02X,0x%02X,(0x%02X,0x%02X))\n"
											,psta->phl_sta->asoc_cap.stbc_ht_rx
											, psta->phl_sta->asoc_cap.stbc_vht_rx
											, psta->phl_sta->asoc_cap.stbc_rx_greater_80mhz
				...
				for (j = 0; j < psta->ipmc_num; j++)
					RTW_PRINT_SEL(m, "mcmac=" MAC_FMT ", client_num: %u\n", MAC_ARG(psta->ipmc[j].mcmac), psta->ipmc[j].client_num);
#endif
...
void _write_sta_info(struct seq_file *m, struct sta_info *psta, int staNum, int cli_conn_success)
{
	...
	RTW_PRINT_SEL(m, "    rts_sel: %d \n", psta->phl_sta->rts_sel);
...
#endif
	if(padapter->phl_role->type == PHL_RTYPE_STATION){

		RTW_PRINT_SEL(m, "    hw_wmm: %x \n", padapter->phl_role->hw_wmm);

		RTW_PRINT_SEL(m, "    be_aifsn_sta_mode: %x \n", padapter->be_aifsn_sta_mode);
		RTW_PRINT_SEL(m, "    be_cwmin_sta_mode: %x \n", padapter->be_cwmin_sta_mode);
		RTW_PRINT_SEL(m, "    be_cwmax_sta_mode: %x \n", padapter->be_cwmax_sta_mode);
		RTW_PRINT_SEL(m, "    be_txop_sta_mode: %x \n", padapter->be_txop_sta_mode);

		RTW_PRINT_SEL(m, "    bk_aifsn_sta_mode: %x \n", padapter->bk_aifsn_sta_mode);
		RTW_PRINT_SEL(m, "    bk_cwmin_sta_mode: %x \n", padapter->bk_cwmin_sta_mode);
		RTW_PRINT_SEL(m, "    bk_cwmax_sta_mode: %x \n", padapter->bk_cwmax_sta_mode);
		RTW_PRINT_SEL(m, "    bk_txop_sta_mode: %x \n", padapter->bk_txop_sta_mode)	;

		RTW_PRINT_SEL(m, "    vi_aifsn_sta_mode: %x \n", padapter->vi_aifsn_sta_mode);
		RTW_PRINT_SEL(m, "    vi_cwmin_sta_mode: %x \n", padapter->vi_cwmin_sta_mode);
		RTW_PRINT_SEL(m, "    vi_cwmax_sta_mode: %x \n", padapter->vi_cwmax_sta_mode);
		RTW_PRINT_SEL(m, "    vi_txop_sta_mode: %x \n", padapter->vi_txop_sta_mode)	;

		RTW_PRINT_SEL(m, "    vo_aifsn_sta_mode: %x \n", padapter->vo_aifsn_sta_mode);
		RTW_PRINT_SEL(m, "    vo_cwmin_sta_mode: %x \n", padapter->vo_cwmin_sta_mode);
		RTW_PRINT_SEL(m, "    vo_cwmax_sta_mode: %x \n", padapter->vo_cwmax_sta_mode);
		RTW_PRINT_SEL(m, "    vo_txop_sta_mode: %x \n", padapter->vo_txop_sta_mode);
	}
	RTW_PRINT_SEL(m, "==============================\n");
}
