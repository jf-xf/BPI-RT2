...
extern const u16 HE_MCS_DATA_RATEStr[4][3][48];
u32 rtw_he_get_highest_data_rate(_adapter *padapter)
{
	/*const u16 HE_MCS_DATA_RATEStr[4][3][48];
		idx0:20M/40M/80M/160M
		idx1:GI   0.8 us ; 1.6us ; 3.2us
		idx2:mcs 1ss 2ss 3ss 4ss
	1. BW(20/40/80/160)
	2. GI(assume we can use 0.8us (0.8!B1.6!B3.2))
	3. 2T2R/3T3R?
	*/
	...
	//printk("[%s %d]xtxr=%d\n",__func__,__LINE__,xtxr);

	xtxr=rtw_phl_get_xtxr(dvobj->phl);
	if(xtxr>=1 && xtxr<=4){
		//printk("[%s %d]rate=[%d]Mbps\n",__func__,__LINE__,HE_MCS_DATA_RATEStr[pmlmeext->cur_bwmode][0][xtxr*12 -1]);
		return HE_MCS_DATA_RATEStr[pmlmeext->cur_bwmode][0][xtxr*12 -1];
	}else{
		RTW_ERR("[%s %d]xtxr=%d\n",__func__,__LINE__,xtxr);
		return 0;
	}
