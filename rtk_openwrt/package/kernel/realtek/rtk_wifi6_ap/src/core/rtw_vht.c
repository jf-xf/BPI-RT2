			...
			struct protocol_cap_t *dft_proto_cap, struct role_cap_t *dft_cap)
{
	...
	pvhtpriv->sgi_160m = TEST_FLAG(pregistrypriv->short_gi, BIT3) ? _TRUE : _FALSE;
...
void update_sta_vht_info_apmode_bf_cap(_adapter *padapter, struct sta_info *psta)
{
	...
	/* B13 B14 B15 Compressed Steering Number of Beamformer  */
	if (TEST_FLAG(pvhtpriv_ap->beamform_cap, BEAMFORMING_VHT_BEAMFORMEE_ENABLE) &&
		GET_VHT_CAPABILITY_ELE_SU_BFER(pvhtpriv_sta->vht_cap)) {
		psta->phl_sta->asoc_cap.num_snd_dim = GET_VHT_CAPABILITY_ELE_SU_BFER_SOUND_DIM_NUM(pvhtpriv_sta->vht_cap);

	}


	/* B16 B17 B18 Number of Sounding Dimensions  */
	if (TEST_FLAG(pvhtpriv_ap->beamform_cap, BEAMFORMING_VHT_BEAMFORMER_ENABLE) &&
	   GET_VHT_CAPABILITY_ELE_SU_BFEE(pvhtpriv_sta->vht_cap)) {
		psta->phl_sta->asoc_cap.bfme_sts =  GET_VHT_CAPABILITY_ELE_SU_BFEE_STS_CAP(pvhtpriv_sta->vht_cap);
...
void	update_sta_vht_info_apmode(_adapter *padapter, void *sta)
{
	...
	if (pvhtpriv_sta->notify_present)
		...
		if (bw_mode < CHANNEL_WIDTH_80 && psta->phl_sta->chandef.band == BAND_ON_5G)
...
void rtw_process_vht_op_mode_notify(_adapter *padapter, u8 *pframe, void *sta)
{
	...
	if (target_bw != psta->phl_sta->chandef.bw) {
		if (rtw_hw_is_band_support(adapter_to_dvobj(padapter), target_bw)
		    && REGSTY_IS_BW_5G_SUPPORT(regsty, ch_width_to_bw_cap(target_bw))
		   ) {
			update_ra = _TRUE;
...
u32	rtw_build_vht_cap_ie(_adapter *padapter, u8 *pbuf)
{
	...
	// current registry_priv->bw_mode didn't support 80M+80M, comment it to bypass coverity check
	...
	if (rtw_hw_chk_bw_cap(adapter_to_dvobj(padapter), BW_CAP_160M) && REGSTY_IS_BW_5G_SUPPORT(pregistrypriv, BW_CAP_160M))
	...
	bw = rtw_hw_largest_bw(adapter_to_dvobj(padapter), get_highest_bw_cap(REGSTY_BW_5G(pregistrypriv)));
...
u32 rtw_restructure_vht_ie(_adapter *padapter, u8 *in_ie, u8 *out_ie, uint in_len, uint *pout_len)
{
	...
	*pout_len += rtw_build_vht_cap_ie(padapter, out_ie + *pout_len);
	...
	/* find the largest bw supported by both registry and hal */
	max_bw = rtw_hw_largest_bw(adapter_to_dvobj(padapter), get_highest_bw_cap(REGSTY_BW_5G(pregistrypriv)));
