...
void rtw_tdls_process_vht_operation(_adapter *padapter, struct sta_info *ptdls_sta, u8 *data, u8 Length)
{
	...
	if (GET_VHT_OPERATION_ELE_CHL_WIDTH(data) >= 1) {
		...
		if (rtw_hw_is_band_support(adapter_to_dvobj(padapter), operation_bw) && REGSTY_IS_BW_5G_SUPPORT(regsty, ch_width_to_bw_cap(operation_bw))
...
void rtw_tdls_process_vht_op_mode_notify(_adapter *padapter, struct sta_info *ptdls_sta, u8 *data, u8 Length)
{
	...
	if (rtw_hw_is_band_support(adapter_to_dvobj(padapter), target_bw) && REGSTY_IS_BW_5G_SUPPORT(regsty, ch_width_to_bw_cap(target_bw))
