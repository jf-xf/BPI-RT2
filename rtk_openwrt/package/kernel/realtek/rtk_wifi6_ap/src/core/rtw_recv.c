		...
		if (is_bmc) { /* broadcast cases */
			...
			if (subtype == WIFI_DEAUTH || subtype == WIFI_DISASSOC) {
				if (!MLME_IS_MESH(adapter)) {
					...
					cfg80211_rx_unprot_mlme_mgmt(adapter->pnetdev, get_recvframe_data(precv_frame), get_recvframe_len(precv_frame));
#else
...
sint validate_recv_data_frame(_adapter *adapter, union recv_frame *precv_frame)
{
	...
	if((pattrib->src[0] ==0xaa) && (pattrib->src[1] ==0xbb) && (pattrib->src[2] ==0xcc)
		...
		rtk_wlan_genl_indicate(adapter->pnetdev->name, WIFI_EZMESH_ARP_LOOP, adapter->mac_addr, 0);
#endif
...
static int rtw_recv_indicatepkt_check(union recv_frame *rframe, u8 *ehdr_pos, u32 pkt_len)
{
	...
	if (ntohs(ehdr->h_proto) == 0x888e) {
		if (!_rtw_memcmp(ehdr, adapter->mac_addr, ETH_ALEN)) {
			RTW_PRINT(ADPT_FMT" receive da:%02x%02x%02x%02x%02x%02x EAPOL pkt, drop it.\n",
				ADPT_ARG(adapter),
		...
		if (eapol_type == BAD_EAPOL) {
			goto exit;
		} else if ((eapol_type == EAPOL_1_4 || eapol_type == EAPOL_3_4) && pattrib->encrypt == 0) {
			if (rframe->u.hdr.psta)
...
u8 amsdu_check_subpkt_a1_valid(_adapter *padapter, u8 *a1_addr, struct sta_info *psta)
{
	struct sta_priv *pstapriv = &padapter->stapriv;
	struct sta_info *relay_sta = NULL;
	u8 i = 0, hit = 0;

	/* check for two sta as a source to ETH */
	...
	/* check for sta relay stuation */
	if (hit) {
		relay_sta = rtw_get_stainfo(pstapriv, a1_addr);
		if (relay_sta && (relay_sta != padapter->self_sta)) {
			hit = 0;
			padapter->core_logs.rxCnt_deamsdu_bypass[4]++;
		}
	}
...
/* CONFIG_RTW_A4_STA, todo: amsdu */
	...
	if(rxsc_entry && DBG_PRINT_RCV_ONCE) {
		RTW_PRINT("[%s:%d]\n[%d] forward_to:%d, to_fr_ds:%d, status:%d, is_amsdu:%d, bypass_deamsdu:%d\n",
			__func__, __LINE__, DBG_PRINT_RCV_ONCE,
			rxsc_entry->forward_to, rattrib->to_fr_ds, rxsc_entry->status, rxsc_entry->is_amsdu,
	...
	if (!padapter->registrypriv.wifi_mib.bypass_deamsdu ||
		rattrib->to_fr_ds == 1 ||
		(rxsc_entry && rxsc_entry->forward_to == RXSC_FWD_STA))
		goto skip_bypass_deamsdu;

	is_subpkt_a1_match = amsdu_check_subpkt_a1_valid(padapter, prframe->u.hdr.rx_data, psta);
#ifdef CORE_RXSC_AMSDU
...
skip_bypass_deamsdu:
	...
	while (a_len > ETH_HLEN) {
		...
		if (3 == rattrib->to_fr_ds) {
			CP_MAC_ADDR(rattrib->src, sa);
			CP_MAC_ADDR(rattrib->dst, da);
...
move_to_next:
	...
	for (i = 0; i < nr_subframes; i++) {
		...
		if (sub_pkt) {
#if defined(CONFIG_RTW_BYPASS_DEAMSDU) && defined(PLATFORM_LINUX)
		if (padapter->registrypriv.wifi_mib.bypass_deamsdu && rattrib->to_fr_ds != 1) {
#ifdef CORE_RXSC_AMSDU
			if (rxsc_entry && rxsc_entry->forward_to == RXSC_FWD_KERNEL &&
				/*EQ_MAC_ADDR(sub_pkt->data, rxsc_entry->rxsc_ethhdr.h_dest)*/
				is_subpkt_a1_match && !is_a1_mixed) {
				pkt = sub_pkt;
				pkt->protocol = eth_type_trans(pkt, padapter->pnetdev);
				pkt->dev = padapter->pnetdev;
				pkt->ip_summed = CHECKSUM_NONE; /* CONFIG_TCP_CSUM_OFFLOAD_RX */
...
skip_deamsdu:
	...
	if (rxsc_entry && (RXSC_ENTRY_VALID == rxsc_entry->status))
		core_rxsc_cache_rframe(padapter, prframe);
	else
#endif
...
void core_update_recvframe_mdata(struct dvobj_priv *dvobj, union recv_frame *prframe, struct rtw_recv_pkt *rx_req)
{
	...
	prxattrib->first_ppdu = mdata->ppdu_cnt_chg;
