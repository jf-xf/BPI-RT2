...
static __inline__ int __nat25_add_pppoe_tag(struct sk_buff *skb, struct pppoe_tag *tag)
{
	...
	if (skb_tailroom(skb) < data_len) {
		RTW_ERR("skb_tailroom() failed in add SID tag!\n");
		return -1;
	}
...
static void __nat25_db_print(_adapter *priv)
{
	...
	printk("scdb_ipv6: %x:%x:%x:%x\n", priv->scdb_ipv6[0], priv->scdb_ipv6[1], priv->scdb_ipv6[2], priv->scdb_ipv6[3]);
#endif
...
void nat25_db_cleanup(_adapter *priv)
{
	...
	_rtw_memset(priv->scdb_ipv6, 0, sizeof(unsigned int) * 4);
	...
	/*---------------------------------------------------*/
		...
		case NAT25_INSERT: {
			...
			if (memcmp(&iph->saddr, "\x0\x0\x0\x0\x0\x0\x0\x0\x0\x0\x0\x0\x0\x0\x0\x0", 16)) {
				...
				__nat25_db_network_insert(priv, skb->data + ETH_ALEN, networkAddr);
				priv->scdb_ipv6[0] = iph->saddr.s6_addr32[0];
				priv->scdb_ipv6[1] = iph->saddr.s6_addr32[1];
				priv->scdb_ipv6[2] = iph->saddr.s6_addr32[2];
				priv->scdb_ipv6[3] = iph->saddr.s6_addr32[3];
				__nat25_db_print(priv);
