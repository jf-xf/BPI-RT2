...
void report_survey_event(_adapter *padapter, union recv_frame *precv_frame)
{
	...
	if ((ch_set_idx >= 0) && (chset[ch_set_idx].hidden_bss_cnt == 1)
		...
		&& pmlmeext->sitesurvey_res.scan_param->ssid_num != 0) {
				   ...
				   struct _WLAN_BSSID_EX *pbuf)
{
	struct mlme_ext_priv *pmlmeext = &a->mlmeextpriv;
	struct mlme_ext_info *pmlmeinfo = &pmlmeext->mlmext_info;
	struct registry_priv *pregpriv = &a->registrypriv;
	struct _WLAN_BSSID_EX *pnetwork = &pmlmeinfo->network;
	...
	if (pmlmeinfo->HT_enable && (pregpriv->band & WLAN_MD_11N))
		ht = 1;
	if (pmlmeinfo->VHT_enable && (pregpriv->band & WLAN_MD_11AC))
		vht = 1;
