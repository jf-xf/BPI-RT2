...
u32 check_crc_payload(int invaild)
{
	if (invaild) {
		return _FAIL;
	}
...
u32 rtw_tkip_decrypt(_adapter *padapter, u8 *precvframe)
{
	...
	if (prxattrib->encrypt == _TKIP_) {
		...
		if (stainfo != NULL) {
			...
			length_tmp = length; // To avoid coverity issue

			*((u32 *)crc) = le32_to_cpu(getcrc32(payload, length - 4));

			if (length_tmp >= 4) {
				res = check_crc_payload(crc[3] != payload[length_tmp - 1] ||
										crc[2] != payload[length_tmp - 2] ||
										crc[1] != payload[length_tmp - 3] ||
										crc[0] != payload[length_tmp - 4]);
			}
