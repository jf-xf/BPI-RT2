...
void rtw_amsdu_hw_timeout(unsigned long data)
{
	...
	padapter->amsdu_hw_timer_alive = 1;
...
static inline u8 start_amsdu_merge(_adapter *padapter, u8 cur, struct sk_buff *pskb_first)
{
	...
	while (merge_cnt > 0) {
		...
		if (/*(pskb->len > 128) || */(skb_headroom(pskb_first) < 8) ||
			...
			((pskb_first->len + pskb->len + 12) > MAX_SUB_PKT_SIZE)) {
#ifdef CONFIG_SWQ_SKB_ARRAY
...
u8 txsc_amsdu_dequeue_swq(_adapter *padapter, struct txsc_pkt_entry *txsc_pkt)
{
	...
	do {
		if(small_pkt_merge) {
			if(((256 * padapter->amsdu_merge_cnt) + amsdu_deq_size) > psta->txsc_amsdu_max_size) {
				break;
			}
		}
...
#ifdef WKARD_CHANGESELF_STA_AMSDU_LEN
		if (MLME_IS_AP(padapter) && (psta->vendor == HT_IOT_PEER_CHANGESELF)
		&& (psta->phl_sta->asoc_cap.max_amsdu_len == 0))
			psta->phl_sta->asoc_cap.max_amsdu_len = 1;
#endif
...
static u8 txsc_get_sc_entry(_adapter *padapter, struct sk_buff *pskb, struct txsc_pkt_entry *txsc_pkt)
{
	...
	if (padapter->vw_enable || is_client_mode) {
		...
		if (pskb->cb[_SKB_CB_FLAGS] & _PKT_TYPE_DHCP) {
			txsc_pkt->step = TXSC_SKIP;
			goto exit;
		}
#endif
...
u8 txsc_apply_sc_cached_entry(_adapter *padapter, struct txsc_pkt_entry *txsc_pkt, u8 vw_pkt)
{
	...
	if (psta->phl_sta->is_latency_sta && padapter->registrypriv.wifi_mib.latency_hiq) {
		txreq->mdata.dma_ch = RTW_PHL_RING_CAT_HIQ;
		txreq->mdata.cat = RTW_PHL_RING_CAT_HIQ;
	}
	#endif
...
s32 txsc_amsdu_timeout_tx(struct sta_info *psta, u8 ac)
{
	...
	u32 throughput_th_high = (WIFI_ROLE_IS_ON_5G(padapter)) ?
							((psta->phl_sta->chandef.bw == CHANNEL_WIDTH_160) ? 1200 :
							(psta->phl_sta->chandef.bw == CHANNEL_WIDTH_80) ? 700 : 400 ) : 400;
