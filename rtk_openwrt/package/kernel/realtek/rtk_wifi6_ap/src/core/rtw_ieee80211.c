...
inline u8 *rtw_set_ie_ch_switch_wrapper(u8 *buf, u32 *buf_len, u8 new_bw,
				u8 new_center_ch)
{
	u8 ie_data[5], ie_len = 0;

	/* only set Wide Bandwidth Channel Switch subelement now */
	if (new_bw != CHANNEL_WIDTH_20) {
		ie_data[ie_len++] = WLAN_EID_WIDE_BANDWIDTH_CHANNEL_SWITCH;
		ie_data[ie_len++] = 3;
		ie_data[ie_len++] = (new_bw == CHANNEL_WIDTH_40) ? 0 : 1;	// 0 signifies only a 40M bandwidth
		ie_data[ie_len++] = new_center_ch;
		ie_data[ie_len++] = 0;
	}

	return (ie_len != 0) ? rtw_set_ie(buf, WLAN_EID_CHANNEL_SWITCH_WRAPPER, ie_len, ie_data, buf_len) : buf;
}
...
inline u8 *rtw_set_ie_extended_ch_switch(u8 *buf, u32 *buf_len, u8 ch_switch_mode,
				u8 op_class, u8 new_ch, u8 ch_switch_cnt)
{
	u8 ie_data[4];
	...
	ie_data[1] = op_class;
	ie_data[2] = new_ch;
	ie_data[3] = ch_switch_cnt;

	return rtw_set_ie(buf, WLAN_EID_EXTENDED_CHANNEL_SWITCH, 4, ie_data, buf_len);
}
#endif
 ...
 */
u8 *rtw_get_ie_ex(const u8 *in_ie, int in_len, u8 eid, const u8 *oui, u8 oui_len, u8 *ie, uint *ielen)
{
	uint cnt;
...
#ifdef CONFIG_HAPD_OWE
		case OWE_OUI_TYPE:
			elems->owe_transition_ie = pos;
			elems->owe_transition_ie_len = elen;
			break;
#endif
		default:
		RTW_DBG("Unknown Wfa "
...
static const u8 beacon_body[] = {
	...
	WLAN_EID_BSS_AVERAGE_ACCESS_DELAY,
	WLAN_EID_ANTENNA,
	WLAN_EID_BSS_AVAILABLE_ADM_CAPA,
	WLAN_EID_BSS_AC_ACCESS_DELAY,
	WLAN_EID_MEASUREMENT_PILOT_TRANSMISSION,
	...
	WLAN_EID_EMERGENCY_ALERT_ID,
	WLAN_EID_MESH_ID,
	WLAN_EID_MESH_CONFIG,
	WLAN_EID_MESH_AWAKE_WINDOW,
	WLAN_EID_BEACON_TIMING,
	WLAN_EID_MCCAOP_ADVERTISEMENT_OVERVIEW,
	WLAN_EID_MCCAOP_ADVERTISEMENT,
	WLAN_EID_MESH_CHANNEL_SWITCH_PARAMETERS,
	WLAN_EID_QMF_POLICY,
	...
	WLAN_EID_OPERATING_MODE_NOTIFICATION,
	WLAN_EID_REDUCED_NEIGHBOR_REPORT,
	WLAN_EID_TVHT_OPERATION,
	WLAN_EID_EXTENSION, WLAN_EID_EXT_ESTIMATED_SERVICE_PARAMS,
	WLAN_EID_EXTENSION, WLAN_EID_EXT_FUTURE_CHANNEL_GUIDANCE,
	WLAN_EID_CAG_NUMBER,
	WLAN_EID_FILS_INDICATION,
	WLAN_EID_AP_CSN,
	WLAN_EID_DILS,
	/* TODO: Max Channel Switch Time */
	/* TODO: Estimated Service Parameters Outbound */
	/* TODO: Service Hint */
	/* TODO: Service Hash */
	WLAN_EID_RSNX,
	WLAN_EID_EXTENSION, WLAN_EID_EXT_MULTIPLE_BSSID_CONFIGURATION,
	WLAN_EID_EXTENSION, WLAN_EID_EXT_HE_CAPABILITIES,
	WLAN_EID_EXTENSION, WLAN_EID_EXT_HE_OPERATION,
	WLAN_EID_TWT,
	/* TODO: UORA Parameter Set */
	WLAN_EID_EXTENSION, WLAN_EID_EXT_COLOR_CHANGE_ANNOUNCEMENT,
	WLAN_EID_EXTENSION, WLAN_EID_EXT_SPATIAL_REUSE,
	WLAN_EID_EXTENSION, WLAN_EID_EXT_HE_MU_EDCA_PARAMS,
	/* TODO: ESS Report */
	/* TODO: NDP Feedback Report Parameter Set */
	/* TODO: HE BSS Load */
	WLAN_EID_EXTENSION, WLAN_EID_EXT_HE_6GHZ_BAND_CAP,
	/* TODO: TWT Constraint Parameters */
	WLAN_EID_EXTENSION, WLAN_EID_EXT_MULTI_LINK,
	WLAN_EID_EXTENSION, WLAN_EID_EXT_EHT_CAPABILITIES,
	WLAN_EID_EXTENSION, WLAN_EID_EXT_EHT_OPERATION,
	WLAN_EID_EXTENSION, WLAN_EID_EXT_MULTI_LINK_TRAFFIC_INDICATION,
	WLAN_EID_EXTENSION, WLAN_EID_EXT_TID_TO_LINK_MAPPING,
	WLAN_EID_VENDOR_SPECIFIC,
	WLAN_EID_MMIE,
};

static const u8 probe_resp_body[] = {
	/* Timestamp */
	/* Beacon Interval */
	/* Capability Information */
	WLAN_EID_SSID,
	...
	WLAN_EID_QLOAD_REPORT,
	WLAN_EID_MULTI_BAND,
	WLAN_EID_DMG_CAPABILITIES,
	WLAN_EID_DMG_OPERATION,
	WLAN_EID_MULTIPLE_MAC_SUBLAYERS,
	...
	/* TODO: TSF Timer Accuracy */
	/* TODO: S1G Relay Discovery */
	WLAN_EID_S1G_CAPABILITIES,
	WLAN_EID_S1G_OPERATION,
	/* TODO: MAD */
	/* TODO: Short Beacon Interval */
	/* TODO: S1G Open-Loop Link Margin Index */
	/* TODO: S1G Relay */
	/* TODO: Max Channel Switch Time */
	/* TODO: CDMG Capabilities */
	/* TODO: Extended Cluster Report */
	/* TODO: CMMG Capabilities */
	/* TODO: CMMG Operation */
	/* TODO: Estimated Service Parameters Outbound */
	/* TODO: Service Hint */
	/* TODO: Service Hash */
...
static int rtw_skip_ie(u8 *start, int len)
{
	/* assume a valid IE buffer */
	u8 *p = start;
	u8 ielen = p[1];
...
int rtw_fix_ie(u8 *start, u32 len, int type)
{
	const u8 *ids = NULL;
	int n = 0, reorder = 0, i = 0, j = 0;
	u8 *p = NULL, *tmp = NULL;
	/* assume no more than 64 IEs */
	struct {
		u8 *start;
		int len;
		int order;
	...
	case WIFI_BEACON:
		ids = beacon_body;
		n = sizeof(beacon_body);
		break;

	case WIFI_PROBERSP:
		ids = probe_resp_body;
		n = sizeof(probe_resp_body);
		break;
	...
	while ((p + 2 <= start + len) &&
	       (p + 2 + p[1] <= start + len)) {
		/* RDE needs special handling for Resource Descriptors */
		if (p[0] == WLAN_EID_RIC_DATA) {
			RTW_ERR("%s: RDE not yet\n", __func__);
			return 0;
		}

		j = 0;
		while (j < n) {
			if (ids[j] == WLAN_EID_EXTENSION) {
				if (p[0] == ids[j] && p[1] &&
				    p[2] == ids[j + 1])
					break;
				else
		...
		if (j < n) {
			if (i >= 64) {
				RTW_ERR("%s: too many IEs!\n", __func__);
				return 0;
			}

			ie[i].start = p;
			ie[i].len = rtw_skip_ie(p, start + len - p);
			ie[i].order = j;
			p += ie[i].len;

			if (i && !reorder && ie[i].order < ie[i - 1].order)
				reorder = 1;

			i++;
		} else {
			/* not found, remove invalid IEs */
			reorder = 1;
			p += rtw_skip_ie(p, start + len - p);
		}
	}
	...
	p = tmp;
	for (i = 0; i < n; i++) {
		_rtw_memcpy(p, ie[i].start, ie[i].len);
		p += ie[i].len;
	}

	_rtw_memcpy(start, tmp, p - tmp);
	_rtw_mfree(tmp, len);
