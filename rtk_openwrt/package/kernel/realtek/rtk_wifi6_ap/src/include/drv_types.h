...
/* make txreq more in 52c 160M platform, because it has higher phy rate */
#if defined(CONFIG_RTL8852CE) || defined(CONFIG_RTL8852DE)
#define MAX_TX_RING_NUM_5G	4096
#else
...
enum wifi_pkt_category {
	HNDSK_AUTH_PKT = 0,
	HNDSK_ASSOC_RSP_PKT = 1,
	HNDSK_EAPOL_4_1_PKT = 2,
	HNDSK_EAPOL_4_3_PKT = 3,
	HNDSK_DHCP_DISCOVER = 4,
	HNDSK_DHCP_OFFER = 5,
	HNDSK_DHCP_REQUEST = 6,
	HNDSK_DHCP_DECLINE = 7,
	HNDSK_DHCP_ACK = 8,
	HNDSK_DHCP_NAK = 9,
	HNDSK_DHCP_RELEASE = 10,
	HNDSK_DHCP_INFORM = 11,
	HNDSK_MGMT_MAX = 12,
	/* ================== */
	CATEGORY_END = 0xFF,
};
...
struct tx_logs {
	...
	u32 core_tx_recycle_data_frame_ok;
	u32 core_tx_recycle_data_frame_fail;
...
#ifdef CONFIG_CSA_IE
enum csa_set_ie_mode {
	CSA_SET_IE_CSA = BIT0,
	CSA_SET_IE_ECSA = BIT1,
};
#endif
...
struct rf_ctl_t {
	...
	u8 csa_ch;
	u8 csa_disconnect_sta;		/* not expected to be used in normal CSA flow */
#ifdef CONFIG_CSA_IE
	u8 csa_switch_mode;
	u8 csa_cntdown;
	u8 csa_op_class;
	u8 csa_set_ie;
	_timer set_csa_timer;
#endif
...
#define MAX_RX_RECYCLE_RING_ENTRY 4096	/* 2's power */
...
#ifdef MACID_NUM_SW_LIMIT
#define MAX_UNASSOC_STA_NUM (MACID_NUM_SW_LIMIT)
#else
...
struct _ADAPTER {
	...
	**	refer to iface_id if iface_nums>2 and isprimary is false and the adapter_type value is 0xff.*/
	...
	unsigned int		stp_tx_cnt;
	unsigned int		stp_consuming_Q;
	struct sk_buff_head	stp_tx_queue;
#endif
...
#ifdef CONFIG_WFA_OFDMA_Logo_Test
	u8 dynamic_dl_grp_inited;
	u8 dynamic_ul_grp_inited;
	_timer ofdma_timer;
#endif
	u8 run_cmd_en;
	...
	*/
	...
	u8 vw_check_len;
#endif // #ifdef CONFIG_VW_REFINE

	#ifdef CONFIG_OCTOSCOPE_REFINE
	u8 octoscope_enable;
	u8 octoscope_sta_cnt;
	u8 octoscope_cur_testcase;
	u32 octoscope_big_pkt;
	u32 octoscope_medium_pkt;
	u32 octoscope_small_pkt;
	#endif
...
#define NO_FIX_BW		0xFF
	...
	struct rtw_a4_db_entry			*a4_db_entry_cache;
	u32	cnt_a4_tx;
	u32	cnt_a4_txsc;
...
#endif /*CONFIG_ADPTVTY_CONTROL*/
	u8 be_aifsn_sta_mode;
	u8 be_cwmin_sta_mode;
	u8 be_cwmax_sta_mode;
	u16 be_txop_sta_mode;

	u8 bk_aifsn_sta_mode;
	u8 bk_cwmin_sta_mode;
	u8 bk_cwmax_sta_mode;
	u16 bk_txop_sta_mode;

	u8 vi_aifsn_sta_mode;
	u8 vi_cwmin_sta_mode;
	u8 vi_cwmax_sta_mode;
	u16 vi_txop_sta_mode;

	u8 vo_aifsn_sta_mode;
	u8 vo_cwmin_sta_mode;
	u8 vo_cwmax_sta_mode;
	u16 vo_txop_sta_mode;
	u16 bc_rate;
	u16 mc_rate;
#ifdef CONFIG_AMSDU_HW_TIMER
	...
	u8 amsdu_hw_timer_alive;
#endif
	u16 cache_vlan_id;
	struct net_device *cache_vlan_dev;
};
