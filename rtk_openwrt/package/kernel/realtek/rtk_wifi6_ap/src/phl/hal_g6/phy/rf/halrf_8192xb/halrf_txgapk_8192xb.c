	...
	(struct rf_info *rf, enum phl_phy_idx phy, enum rf_path path)
{
	...
	#else
	txgapk_info->is_txgapk_ok = halrf_do_one_shot_8192xb(rf, path, 0x8000, MASKDWORD, process_id1[path]);
	...
	halrf_wreg(rf, 0x8010, 0x000000ff, 0x00);
	#else
	txgapk_info->is_txgapk_ok = halrf_do_one_shot_8192xb(rf, path, 0x8000, MASKDWORD, process_id2[path]);
	#endif
	...
	(struct rf_info *rf, enum phl_phy_idx phy, enum rf_path path, bool is_dbcc)
{
	...
	if (!is_dbcc) {
		...
		halrf_wreg(rf, 0x8010, 0x000000ff, 0x00);
		#else
		txgapk_info->is_txgapk_ok = halrf_do_one_shot_8192xb(rf, path, 0x8000, MASKDWORD, process_id[path]);
...
void halrf_txgapk_ctrl_efem_8192xb(struct rf_info *rf, bool efem_on)
{
	u8 rfe_type = rf->phl_com->dev_cap.rfe_type;

	if ((rfe_type == 53) || (rfe_type == 54)){
		if (efem_on) {
			RF_DBG(rf, DBG_RF_TXGAPK, "[TXGAPK]======> Efem ON, back to HW control by restore Reg"); 
			//after
#if 0
			...
			//before
			RF_DBG(rf, DBG_RF_TXGAPK, "[TXGAPK]======> Efem OFF");
			
			//halrf_wreg(rf, 0x5890, MASKDWORD, 0x00000000);
			halrf_wreg(rf, 0x5890, BIT(6), 0x0); // GPIO[6]=0
			halrf_wreg(rf, 0x5890, BIT(7),  0x0); // GPIO[7]=0
			halrf_wreg(rf, 0x5890, BIT(11),  0x0); // GPIO[11]=0
			halrf_wreg(rf, 0x5880, MASKDWORD, 0x77777777); //SW control 
			halrf_wreg(rf, 0x5884, MASKDWORD, 0x77777777); //SW control 
			
		//	halrf_wreg(rf, 0x7890, MASKDWORD, 0x00000000);
			halrf_wreg(rf, 0x7890, BIT(1), 0x0);    // GPIO[1]=0
			halrf_wreg(rf, 0x7890, BIT(2), 0x0);    // GPIO[2]=0
			halrf_wreg(rf, 0x7890, BIT(3), 0x0);    // GPIO[3]=0	
					...
					enum phl_phy_idx phy)
{
	...
	_txgapk_backup_bb_registers_8192xb(rf, bb_reg, bb_bkup, TXGAPK_BB_REG_NUM_8192XB);
	halrf_txgapk_ctrl_efem_8192xb(rf, false);
	...
	halrf_txgapk_ctrl_efem_8192xb(rf, true);
	if ((rfe_type == 53) || (rfe_type == 54))
		_txgapk_reload_bb_registers_8192xb(rf, bb_reg, bb_bkup, TXGAPK_BB_REG_NUM_8192XB);
