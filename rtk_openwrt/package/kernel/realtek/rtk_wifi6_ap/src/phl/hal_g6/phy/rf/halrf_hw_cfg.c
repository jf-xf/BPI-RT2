...
void halrf_config_edpk_para(void *rf_void)
{
	struct rf_info *rf = (struct rf_info *)rf_void;
	struct rtw_hal_com_t *hal_com = rf->hal_com;
...
#ifdef RF_8192XB_SUPPORT
		if (rf->ic_type == RF_RTL8192XB) {
			if (hal_com->dev_hw_cap.rfe_type > 50)
				halrf_edpk_config_para_8192xb(rf);
		}
#endif
		...
		if (rf->ic_type == RF_RTL8852C) {
			if (hal_com->dev_hw_cap.rfe_type > 50)
				halrf_edpk_config_para_8852c(rf);
		}
#endif
	return;
}
