	...
	struct rf_info *rf)
{
	...
	if ((rf->dbg_component & DBG_RF_DPK_TRACK) == 0x0 && rf->hal_com->cv != CAV &&
	    rf->phl_com->dev_cap.rfe_type < 50)
		return;
	...
	for (path = 0; path < DPK_RF_PATH_MAX_8852C; path++) {
		...
		if (rf->rfk_is_processing != true && halrf_rreg(rf, 0x80f4, BIT(29)) == 0x0 && rf->phl_com->dev_cap.rfe_type >= 50) {
			if (cur_ther - dpk->bp[path][kidx].ther_dpk < 0)
				halrf_wreg(rf, 0x81bc + (path << 8) + (kidx << 2), 0x00003F00, 0x04); /*[13:8] thermal slope*/
			else
				halrf_wreg(rf, 0x81bc + (path << 8) + (kidx << 2), 0x00003F00, 0x0a); /*[13:8] thermal slope*/
		}
	}
#endif
}
