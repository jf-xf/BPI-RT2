...
enum rtw_hal_status rtw_hal_bb_acs_mntr_result(struct hal_info_t *hal_info, void *rpt)
{
	...
	if ((result & NHM_SUCCESS) != NHM_SUCCESS) {
		return RTW_HAL_STATUS_FAILURE;
	} else {
		if ((result & CLM_SUCCESS) == CLM_SUCCESS){
			acs_rpt->clm_ratio = mntr_rpt.clm_ratio;
		}
		else{
			acs_rpt->clm_ratio = mntr_rpt.nhm_cca_ratio;
		}
...
void _dump_sw_grp_set(struct rtw_sw_grp_set *sw_grp)
{
	...
	for (i = 0; i < 8; i++) {
		...
		if (sw_grp->swgrp_bitmap[i].cmdend)
			break;
	}
}
...
enum rtw_hal_status
rtw_hal_bb_ra_stbc_status(struct hal_info_t *hal_info,
				struct rtw_phl_stainfo_t *sta)
{
	if (!rtw_halbb_ra_stbc_status(hal_info->bb, sta))
		return RTW_HAL_STATUS_FAILURE;
