...
void halrf_addck_ori_8192xb(struct rf_info *rf)
{
	struct halrf_dack_info *dack = &rf->dack;
	u32 c;
	/*S0*/
...
void halrf_dac_cal_8192xb(struct rf_info *rf, bool force)
{
	...
	if (hal_i->cv == CAV) {
		halrf_rxck_force_8192xb(rf, RF_PATH_A, true, ADC_480M);
		halrf_rxck_force_8192xb(rf, RF_PATH_B, true, ADC_480M);
	} else {
		halrf_rxck_force_8192xb(rf, RF_PATH_A, true, ADC_960M);
		halrf_rxck_force_8192xb(rf, RF_PATH_B, true, ADC_960M);
	}
//	halrf_btc_rfk_ntfy(rf, phy_map, RF_BTC_DACK, RFK_ONESHOT_START);
	halrf_addck_ori_8192xb(rf);
	halrf_rxck_force_8192xb(rf, RF_PATH_A, false, ADC_960M);
	halrf_rxck_force_8192xb(rf, RF_PATH_B, false, ADC_960M);
