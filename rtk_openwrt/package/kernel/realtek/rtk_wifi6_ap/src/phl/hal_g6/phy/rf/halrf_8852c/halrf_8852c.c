...
void halrf_by_rate_dpd_8852c(struct rf_info *rf, enum phl_phy_idx phy)
{
	halrf_mac_set_pwr_reg_8852c(rf, phy, 0xd22c + (phy << 13), 0x1F800000, 0x27); /*[28:23]*/
	halrf_mac_set_pwr_reg_8852c(rf, phy, 0xd230 + (phy << 13), 0x0003FFFF, 0x24924); /*[17:0]*/
}

void halrf_dpk_init_8852c(struct rf_info *rf)
{
	/*by rate dpd*/
	if (rf->phl_com->dev_cap.rfe_type == 54) {
		halrf_by_rate_dpd_8852c(rf, HW_PHY_0);
