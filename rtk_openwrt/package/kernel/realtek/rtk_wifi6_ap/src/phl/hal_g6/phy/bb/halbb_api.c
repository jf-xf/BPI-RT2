...
void halbb_agc_fix_gain(struct bb_info *bb, bool enable, enum rf_path path)
{
	...
	#ifdef BB_8192XB_SUPPORT
	case BB_RTL8192XB:
		halbb_agc_fix_gain_8192xb(bb, enable, path);
		break;
	#endif
...
void halbb_agc_elna_idx(struct bb_info *bb, bool elna_idx, enum rf_path path)
{
	...
	#ifdef BB_8192XB_SUPPORT
	case BB_RTL8192XB:
		halbb_agc_elna_idx_8192xb(bb, elna_idx, path);
		break;
	#endif
			  ...
			  enum rf_path path)
{
	...
	#ifdef BB_8192XB_SUPPORT
	case BB_RTL8192XB:
		halbb_agc_tia_shrink_8192xb(bb, shrink_en, shrink_init, path);
		break;
	#endif
		       ...
		       bool nlgc_agc_en, enum rf_path path)
{
	...
	#ifdef BB_8192XB_SUPPORT
	case BB_RTL8192XB:
		halbb_agc_step_en_8192xb(bb, pre_pd_agc_en, linear_agc_en,
					post_pd_agc_en, nlgc_agc_en, path);
		break;
	#endif
	...
	if (_os_strcmp(input[1], "-h") == 0) {
		...
		if (_os_strcmp(input[2], "default") == 0) {
			...
			BB_DBG_CNSL(out_len, used, output + used, out_len - used,
				    "default\n");
			BB_DBG_CNSL(out_len, used, output + used, out_len - used,
				    "fix_gain {en} {path: 0/1}\n");
			BB_DBG_CNSL(out_len, used, output + used, out_len - used,
				    "set_igi {lna} {tia} {rxbb} {path: 0/1}\n");
			BB_DBG_CNSL(out_len, used, output + used, out_len - used,
				    "set_elna {elna_idx} {path: 0/1}\n");
			BB_DBG_CNSL(out_len, used, output + used, out_len - used,
