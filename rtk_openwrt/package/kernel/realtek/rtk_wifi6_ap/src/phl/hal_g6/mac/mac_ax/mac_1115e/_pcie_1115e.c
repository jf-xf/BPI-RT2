...
u32 ltr_dyn_ctrl_1115e(struct mac_ax_adapter *adapter, enum mac_ax_ltr_dyn_ctrl_tp type,
		       void *param)
{
	struct mac_ax_intf_ops *ops = adapter_to_intf_ops(adapter);
	...
	case LTR_DYN_CTRL_FAST_INIT:
		...
		if (ret != MACSUCCESS) {
			PLTFM_MSG_ERR("[ERR]pcie ltr set fail at leave ips %d\n", ret);
			return ret;
		}
		break;
	case LTR_DYN_CTRL_INIT:
		val32 = MAC_REG_R32(R_BE_LTR_DECISION_CTRL);
		if (val32 & LTR_EN_BITS)
			return MACSUCCESS;
...
u32 set_pcie_speed_1115e(struct mac_ax_adapter *adapter,
			 enum mac_ax_pcie_phy set_speed)
{
	...
	support_gen = get_pcie_sup_speed_1115e(adapter);
	...
	if (support_gen == MAC_AX_PCIE_PHY_GEN1) {
		...
		ret = get_pcie_speed_1115e(adapter, &link_speed);
...
u32 get_pcie_speed_1115e(struct mac_ax_adapter *adapter,
			 u8 *speed)
{
	u32 ret;
	u32 val32;
...
u32 get_pcie_sup_speed_1115e(struct mac_ax_adapter *adapter)
{
	u32 ret;
	u32 support_gen;
	struct mac_ax_priv_ops *p_ops = adapter_to_priv_ops(adapter);
...
u32 get_pcie_support_width_1115e(struct mac_ax_adapter *adapter, u16 *width)
{
	u32 ret;
	u32 val32;
	struct mac_ax_priv_ops *p_ops = adapter_to_priv_ops(adapter);

	ret = p_ops->mio_r32_pcie(adapter, PCIE_LINK_CAP, &val32);
	if (ret != MACSUCCESS)
		return ret;
...
u32 get_pcie_link_width_1115e(struct mac_ax_adapter *adapter, u16 *width)
{
	u32 ret;
	u32 val32;
	struct mac_ax_priv_ops *p_ops = adapter_to_priv_ops(adapter);

	ret = p_ops->mio_r32_pcie(adapter, PCIE_LINK_CTRL_STS_OFFSET, &val32);
	if (ret != MACSUCCESS)
		return ret;
...
u32 set_pcie_link_width_1115e(struct mac_ax_adapter *adapter,
			      enum mac_ax_pcie_link_width set_width)
{
	u32 ret, cnt;
	u32 val32;
	u16 support_width, link_width;
	struct mac_ax_priv_ops *p_ops = adapter_to_priv_ops(adapter);
	...
	if (set_width == link_width) {
		...
		ret = p_ops->mio_w32_pcie(adapter, PCIE_LANE_RESIZING_OFFSET, val32);
		if (ret != MACSUCCESS)
			return ret;
