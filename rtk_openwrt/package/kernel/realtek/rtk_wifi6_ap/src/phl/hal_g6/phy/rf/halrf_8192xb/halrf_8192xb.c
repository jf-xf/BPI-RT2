...
{
	u8 phy_map;
	u32 reg1, reg2, reg3, reg4, reg5, reg6, reg7, i = 0;
	...
	if (is_afe) {
		...
		reg4 = halrf_rreg(rf, 0xc0c4 + (path << 8), MASKDWORD);
		reg5 = halrf_rreg(rf, 0xc0e8 + (path << 8), MASKDWORD);
		reg6 = halrf_rreg(rf, 0x8034, MASKDWORD);
		reg7 = halrf_rreg(rf, 0x80d4, MASKDWORD);
...
//		halrf_wreg(rf, 0xc0d8 + (path << 8), 0x000001E0, 0x7); /*[8:5] for ADC 320M*/
		...
		while ((halrf_rreg(rf, 0x80fc, BIT(16)) == 0x0) && (i < 1500)) {
			halrf_delay_us(rf, 2);
			i++;
		}
		...
		halrf_wreg(rf, 0xc0c4 + (path << 8), MASKDWORD, reg4);
		halrf_wreg(rf, 0xc0e8 + (path << 8), MASKDWORD, reg5);
		halrf_wreg(rf, 0x8034, MASKDWORD, reg6);
		halrf_wreg(rf, 0x80d4, MASKDWORD, reg7);
...
void halrf_rx_dck_8192xb(struct rf_info *rf, enum phl_phy_idx phy, bool is_afe) 
{
	...
	halrf_wreg(rf, 0x8080, MASKDWORD, 0x00000004); /*nctl_rst*/
	...
	halrf_wreg(rf, 0x8080, MASKDWORD, 0x00000000); /*nctl_rst*/
}
...
void halrf_by_rate_dpd_8192xb(struct rf_info *rf, enum phl_phy_idx phy)
{
	halrf_mac_set_pwr_reg_8192xb(rf, phy, 0xd22c + (phy << 13), 0x1F800000, 0x25); /*[28:23]*/
	halrf_mac_set_pwr_reg_8192xb(rf, phy, 0xd230 + (phy << 13), 0x0003FFFF, 0x24924); /*[17:0]*/
}
...
{
	/*by rate dpd*/
	halrf_by_rate_dpd_8192xb(rf, HW_PHY_0);
...
bool halrf_set_ch_lck_8192xb(struct rf_info *rf, u32 path, u32 val)
{
	u32 c = 0;
	bool timeout = false;
...
void halrf_lck_8192xb(struct rf_info *rf)
{
	...
	halrf_set_ch_lck_8192xb(rf, 0, temp18);
