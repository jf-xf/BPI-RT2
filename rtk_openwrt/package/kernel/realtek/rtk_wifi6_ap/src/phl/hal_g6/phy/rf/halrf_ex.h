...
enum rtw_hal_status halrf_chl_rfk_trigger_traditional(void *rf_void,
			   enum phl_phy_idx phy_idx,
			   enum rfk_tri_type rfk_tri_typ);   //no outer, test cmd
enum rtw_hal_status halrf_dack_trigger(void *rf_void, bool force); //no outer
enum rtw_hal_status halrf_txa_lctk_trigger(void *rf_void,
						enum phl_phy_idx phy_idx); //no outer
enum rtw_hal_status halrf_rx_dck_trigger(void *rf_void,
				enum phl_phy_idx phy_idx, bool is_afe);  //no outer, test cmd
...
enum rtw_hal_status halrf_scan_rx_dck_trigger(void *rf_void, enum phl_phy_idx phy_idx,
				enum rfk_tri_type tri_type);  //no outer, test cmd
enum rtw_hal_status halrf_iqk_trigger(void *rf_void,
		       enum phl_phy_idx phy_idx,
		       bool force);
void halrf_lck_trigger(void *rf_void); //no outer, test cmd
...
bool halrf_reg_tbl_6g_exist(struct rf_info *rf, u8 band, u8 reg, u8 type);
...
void halrf_edpk_pre_tx(void *rf_void, enum phl_phy_idx phy,u16 cnt, u16 period, s16 pwr, u32 delay);
...
bool halrf_get_dpk_by_rate(void *rf_void,
	enum phl_phy_idx phy, u32 vector_index, u32 rate_index);
