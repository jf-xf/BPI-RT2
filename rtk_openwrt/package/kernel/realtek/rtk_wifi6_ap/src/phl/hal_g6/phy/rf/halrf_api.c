			...
			enum phl_msg_evt_id event, enum phl_phy_idx phy_idx)
{
	...
	switch (event) {
		...
		case MSG_EVT_HAL_START_OK:
			...
			halrf_inn_tssi_trigger(rf, HW_PHY_0, false);
		break;
	...
	switch (idx) {
		case RF_EVENT_PWR_TRK:
			if (func == RF_EVENT_OFF)
				...
				halrf_inn_tssi_trigger(rf, phy, true);
		break;
		...
		case RF_EVENT_IQK:
			if (func == RF_EVENT_OFF)
				...
				halrf_inn_iqk_trigger(rf, phy, false);
			}
		break;
		...
		case RF_EVENT_DPK:
			if (func == RF_EVENT_OFF)
				...
				halrf_inn_dpk_trigger(rf, phy, false);
		break;
	...
	enum phl_phy_idx phy, u32 vector_index, u32 rate_index)
{
	...
	switch (vector_index) {
		...
		case HALRF_VHT_FMT:
			...
			break;
		case HALRF_HE_SU_FMT:
		case HALRF_HE_ER_SU_FMT:
		case HALRF_HE_MU_FMT:
		case HALRF_HE_TB_FMT:
			rate_addr = 0xd230;
