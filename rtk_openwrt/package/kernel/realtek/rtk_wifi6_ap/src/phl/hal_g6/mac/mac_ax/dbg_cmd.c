...
static const struct mac_hal_cmd_info mac_hal_cmd_i[] = {
	...
	{"fw_cap", MAC_MAC_FW_CAP, cmd_mac_fw_cap, ""},
	...
	{"txrpt_dbg", MAC_TXRPT_DBG, cmd_dump_txrpt_dbg_info, "Not ready"},
	{"wow_evt", MAC_MAC_WOW_TRI_EVT, cmd_wow_tri_evt, "Configure Wowlan Triggered Event"},
	{"swtx", MAC_MAC_SWTXMODE, cmd_mac_fw_swtx, \
	 "SW mode enable and pipeline chain config: <swmode_en> <chain_count>"},
	/*@do not move this element to other position*/
};
		    ...
		    char *output, u32 out_len, u32 *used)
{
	...
	/* dump fw pc */
	fw_pc_dbg_dump_ax(adapter, 15, 1000);
	return MACSUCCESS;
}
			   ...
			   u32 input_num, char *output, u32 out_len, u32 *used)
{
	...
	if (input_num == 2) {
		...
		PLTFM_MSG_ERR("level 4 : SER L2 diag;\n");
	}

	if (diag_level == MAC_DIAG_LEVEL_4) {
...
{
	struct mac_ax_priv_ops *p_ops = adapter_to_priv_ops(adapter);
	struct mac_fw_cap_info info;
	enum mac_fw_feat_sel sel = ALL_FEAT_SEL;
	u32 ret;

	p_ops->get_fw_cap(adapter, &info);

	if (input_num == 2) {
		if (PLTFM_STRCMP(input[1], "ALL") == 0)
			sel = ALL_FEAT_SEL;
		else if (PLTFM_STRCMP(input[1], "BEACON") == 0)
			sel = BEACON_SEL;
		else if (PLTFM_STRCMP(input[1], "MBSSID") == 0)
			sel = MBSSID_SEL;
		else if (PLTFM_STRCMP(input[1], "MLO") == 0)
			sel = MLO_SEL;
		else if (PLTFM_STRCMP(input[1], "DYNAMIC_MLO") == 0)
			sel = MLO_DM_SEL;
		else if (PLTFM_STRCMP(input[1], "MULTI_CHIP_MLO") == 0)
			sel = MC_MLO_SEL;
		else if (PLTFM_STRCMP(input[1], "EMLSR") == 0)
			sel = EMLSR_SEL;
		else if (PLTFM_STRCMP(input[1], "TWTAP") == 0)
			sel = TWT_AP_SEL;
		else if (PLTFM_STRCMP(input[1], "SMPS") == 0)
			sel = SMPS_SEL;
		else if (PLTFM_STRCMP(input[1], "F2PCMD") == 0)
			sel = F2PCMD_SEL;
		else if (PLTFM_STRCMP(input[1], "MU_MIMO") == 0)
			sel = MU_MIMO_SEL;
		else if (PLTFM_STRCMP(input[1], "SOUNDING") == 0)
			sel = SOUND_MER_SEL;
		else if (PLTFM_STRCMP(input[1], "UL_OFDMA") == 0)
			sel = UL_OFDMA_SEL;
		else if (PLTFM_STRCMP(input[1], "DL_OFDMA") == 0)
			sel = DL_OFDMA_SEL;
		else
			PLTFM_MSG_ERR("Sel Error!!\n");

		ret = p_ops->dump_fw_cap(adapter, &info, sel);
		if (ret != MACSUCCESS)
			PLTFM_MSG_ERR("Dump_fw_cap Error!!\n");
	} else {
		PLTFM_MSG_ALWAYS("Please type cmd mac fw_cap <feat_sel >"\
				 "1.  BEACON\n" \
				 "2.  MBSSID\n" \
				 "3.  MLO\n"\
				 "4.  DYNAMIC_MLO\n" \
		    ...
		    char *output, u32 out_len, u32 *used)
{
	...
	chain_count = (u8)PLTFM_STRTOUL(input[2], 10);

	mac_txmode_switch(adapter, swtx, chain_count);
