	...
	struct mp_context *mp, struct mp_config_arg *arg)
{
	...
	if (bb_tx_path > RF_PATH_ABCD) {
		PHL_INFO("%s: Invaled TX path (%d)\n",
			__FUNCTION__, bb_tx_path);
		goto exit;
	}
	...
	if (bb_rx_path > RF_PATH_ABCD) {
		PHL_INFO("%s: Invaled RX path (%d)\n",
			__FUNCTION__, bb_rx_path);
		goto exit;
	}
	...
	struct mp_context *mp, struct mp_config_arg *arg)
{
	...
	hal_status = rtw_hal_set_ch_bw(mp->hal,mp->cur_phy, &chdef, RFK_TYPE_FORCE_NOT_DO);
	if(hal_status != RTW_HAL_STATUS_SUCCESS) {
	...
	hal_status = rtw_hal_rf_chl_rfk_trigger(hal_com, mp->cur_phy, RFK_TYPE_MP_SET_CHBW);
	if(hal_status != RTW_HAL_STATUS_SUCCESS) {
