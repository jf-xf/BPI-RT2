		     ...
		     struct mac_ax_trx_info *info)
{
	...
	if (is_chip_id(adapter, MAC_AX_CHIP_ID_8852A) ||
		...
		reg = band == MAC_AX_BAND_1 ? R_AX_RCR_C1 : R_AX_RCR;
		val8 = MAC_REG_R8(reg);
		val8 = SET_CLR_WORD(val8, RX_CH_4_USER, B_AX_CH_EN);
		MAC_REG_W8(reg, val8);
#endif
	} else if (is_chip_id(adapter, MAC_AX_CHIP_ID_8852C) ||
		   is_chip_id(adapter, MAC_AX_CHIP_ID_8192XB) ||
		...
		reg = band == MAC_AX_BAND_1 ? R_AX_RCR_C1 : R_AX_RCR;
		val8 = MAC_REG_R8(reg);
		val8 = (u8)SET_CLR_WORD(val8, RX_CH_8_USER, B_AX_CH_EN_V1);
		MAC_REG_W8(reg, val8);
		...
		PLTFM_MSG_ERR("[ERR]rmac init invalid chip id\n");
		return MACCHIPID;
	}
...
u32 mac_trx_mode_switch(struct mac_ax_adapter *adapter,
			enum mac_ax_trx_mode trx_mode)
{
	struct mac_ax_intf_ops *ops = adapter_to_intf_ops(adapter);

	u8 val8 = MAC_REG_R8(R_AX_PTCL_COMMON_SETTING_0);

	if (trx_mode == MAC_AX_TRX_SW_MODE) {
		val8 &= ~(B_AX_CMAC_TX_MODE_0 | B_AX_CMAC_TX_MODE_1);
	} else {
		val8 |= B_AX_CMAC_TX_MODE_0 | B_AX_CMAC_TX_MODE_1;
		val8 &= ~(B_AX_PTCL_TRIGGER_SS_EN_0 |
