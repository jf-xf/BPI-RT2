...
u32 ltr_dyn_ctrl_8852d(struct mac_ax_adapter *adapter, enum mac_ax_ltr_dyn_ctrl_tp type,
		       void *param)
{
	struct mac_ax_intf_ops *ops = adapter_to_intf_ops(adapter);
...
u32 set_pcie_speed_8852d(struct mac_ax_adapter *adapter,
			 enum mac_ax_pcie_phy set_speed)
{
	...
	support_gen = get_pcie_sup_speed_8852d(adapter);
	...
	if (support_gen == MAC_AX_PCIE_PHY_GEN1) {
		...
		ret = get_pcie_speed_8852d(adapter, &link_speed);
...
u32 get_pcie_speed_8852d(struct mac_ax_adapter *adapter,
			 u8 *speed)
{
	u32 ret;
	u32 val32;
...
u32 get_pcie_sup_speed_8852d(struct mac_ax_adapter *adapter)
{
	u32 ret;
	u32 support_gen;
	struct mac_ax_priv_ops *p_ops = adapter_to_priv_ops(adapter);
...
u32 get_pcie_support_width_8852d(struct mac_ax_adapter *adapter, u16 *width)
{
	u32 ret;
	u32 val32;
	struct mac_ax_priv_ops *p_ops = adapter_to_priv_ops(adapter);
...
u32 get_pcie_link_width_8852d(struct mac_ax_adapter *adapter, u16 *width)
{
	u32 ret;
	u32 val32;
	struct mac_ax_priv_ops *p_ops = adapter_to_priv_ops(adapter);
...
u32 set_pcie_link_width_8852d(struct mac_ax_adapter *adapter,
