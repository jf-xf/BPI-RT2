...
rtw_hal_snd_release_proc_sta_res(void *hal, struct rtw_phl_stainfo_t *sta)
{
	...
	do {
		if (hal_sta->bf_entry != NULL) {
			bf_entry = (struct hal_bf_entry *)hal_sta->bf_entry;
			if (bf_entry->bfee->type == HAL_BFEE_MU){

				if (hal_bf_del_mu_sta_fw(hal, sta)){
					PHL_TRACE(COMP_PHL_SOUND, _PHL_ERR_, "%s : hal_bf_del_mu_sta_fw fail\n", __func__);
				} else
					PHL_TRACE(COMP_PHL_SOUND, _PHL_INFO_, "%s : hal_bf_del_mu_sta_fw ok\n", __func__);

			}
			if ( sta  && (ATOMIC_READ(&sta->already_inform_txbf_to_fw))) {
				if (hal_mac_ax_send_fw_to_del_snd(hal_info, hal_sta)) {
					PHL_TRACE(COMP_PHL_SOUND, _PHL_INFO_, "%s : hal_mac_ax_send_fw_to_del_snd fail\n", __func__);
				} else
					PHL_TRACE(COMP_PHL_SOUND, _PHL_INFO_, "%s : hal_mac_ax_send_fw_to_del_snd ok\n", __func__);
			}
		...
		ATOMIC_SET(&sta->already_inform_txbf_to_fw, false);
...
void
rtw_hal_snd_ndpa_sta_info_he(void *hal, struct rtw_phl_stainfo_t *psta_info,
			     u32 *ndpa, enum channel_width bw, u8 fb_type)
{
	...
	if (0 == fb_type) {
		...
		if (phl_com->txbf_force_ng_cb){
			ndpa_sta->fb_ng = HAL_NDPA_AX_FB_SU_NG_4;
			ndpa_sta->cb = HAL_NPDA_AX_CB_SU64_MU97;
		}			
	} else if (1 == fb_type) {
		...
		if (phl_com->txbf_force_ng_cb){
			ndpa_sta->fb_ng = HAL_NDPA_AX_FB_MU_NG_4;
			ndpa_sta->cb = HAL_NPDA_AX_CB_SU64_MU97;
		}
...
bool
rtw_hal_check_snd_en(struct rtw_phl_stainfo_t *phl_sta)
{
	u8 ret = false;
	u16 rate;
	struct hal_bf_entry *bf_entry = (struct hal_bf_entry *) phl_sta->hal_sta->bf_entry;

	if (!bf_entry || !bf_entry->bfee)
		return ret;
	...
	switch (phl_sta->hal_sta->ra_info.rpt_rt_i.mode) {
		case HAL_LEGACY_MODE:
			ret = false;
			DBGP("warining! LEGACY_MODE shound not sounding");
			break;
		case HAL_HT_MODE:
			ret = false;
			DBGP("warining! HAL_HT_MODE shound not sounding");
			break;
		case HAL_VHT_MODE:
			...
			break;
		case HAL_HE_MODE:
			if(((rate + 0x180)  >= RTW_DATA_RATE_HE_NSS1_MCS3) && ((rate + 0x180) <= RTW_DATA_RATE_HE_NSS1_MCS11))
				ret = true;
			break;
		default:
		    ...
		    struct rtw_phl_stainfo_t *psta, u32 *npda_sta)
{
	...
	cmd->f2p_index = psta->hal_sta->f2p_index;
	ATOMIC_SET(&psta->already_inform_txbf_to_fw, true);

	if (rtw_hal_check_snd_en(psta) == true)
		cmd->sounding_en = true;
	else
		cmd->sounding_en = false;
	...
	cmd->wd[1].data_bw = psta->hal_sta->ra_info.rpt_rt_i.bw;
		  		      ...
		  		      u8 ndpa_idx, u8 last)
{
	...
	ATOMIC_SET(&sta->already_inform_txbf_to_fw, true);
		  		     ...
		  		     u8 ndpa_idx, u8 bfrp_idx, u8 bfrp_u_idx)
{
	...
	if (cmd->mode == 1)
		cmd->bfrp.he_para[bfrp_idx].user[bfrp_u_idx].ul_mcs = 0;/* HE-MCS0, TXBF trig mode: for long range*/
	else
		cmd->bfrp.he_para[bfrp_idx].user[bfrp_u_idx].ul_mcs = 3;/* HE-MCS3, MU: Default value of golden */
