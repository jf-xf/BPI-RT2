...
//				halrf_wreg(rf, 0x5890, MASKDWORD, 0x00000840);
				halrf_wreg(rf, 0x5890, BIT(11), 0x1);
				halrf_wreg(rf, 0x5890, BIT(7), 0x0);
				halrf_wreg(rf, 0x5890, BIT(6), 0x1);
				halrf_wreg(rf, 0x5880, MASKDWORD, 0x77777777);
				halrf_wreg(rf, 0x5884, MASKDWORD, 0x77777777);
				halrf_set_ant_main_or_aux(rf, path, true);
			} else {
//				halrf_wreg(rf, 0x7890, MASKDWORD, 0x0000000a);
				halrf_wreg(rf, 0x7890, BIT(1), 0x1);
				halrf_wreg(rf, 0x7890, BIT(2), 0x0);
				halrf_wreg(rf, 0x7890, BIT(3), 0x1);
				halrf_wreg(rf, 0x7880, MASKDWORD, 0x77777777);
...
//				halrf_wreg(rf, 0x5884, MASKDWORD, 0x77770777);
				halrf_set_ant_main_or_aux(rf, path, false);
			} else {
...
//				halrf_wreg(rf, 0x7884, MASKDWORD, 0x77777777);
	...
	if (before_k)
		RF_DBG(rf, DBG_RF_DPK, "[DPK] Set S%d eFEM control for RFE type %d\n", path, rfe_type);
	else
		RF_DBG(rf, DBG_RF_DPK, "[DPK] Restore S%d eFEM control for RFE type %d\n", path, rfe_type);
}
	...
	u8 kpath)
{
	...
	_dpk_bkup_bb_8192xb(rf, bb_reg, bb_bkup);
	...
	for (path = 0; path < DPK_RF_PATH_MAX_8192XB; path++) {
		if (kpath & BIT(path)) {
			...
			_dpk_efem_control_8192xb(rf, path, true);
	...
	for (path = 0; path < DPK_RF_PATH_MAX_8192XB; path++) {
		if (kpath & BIT(path)) {
			...
			_dpk_efem_control_8192xb(rf, path, false);
	...
	_dpk_reload_bb_8192xb(rf, bb_reg, bb_bkup);
	...
	enum phl_phy_idx phy)
{
	...
	if (rf->hal_com->band[phy].cur_chandef.band == BAND_ON_5G) {
		...
		result = 1;
	} else if (rf->phl_com->dev_cap.rfe_type > 50 && rf->phl_com->dev_cap.rfe_type < 90) {
