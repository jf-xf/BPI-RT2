				 ...
				 struct hal_init_info_t *init_92xbe)
{
	...
	if (((struct dvobj_priv*)phl_com->drv_priv)->pci_subdid == 0xb83b)
		init_92xbe->ic_name = "rtl8832brvte";
}
...
#if defined(RTW_WKARD_TXBD_UPD_LMT)
...
void hal_init_int_default_value_8192xbe(struct hal_info_t *hal, enum rtw_hal_int_set_opt opt)
{
	...
	hal_com->_intr[1].mask_dflt = (u32)(
		0);
	hal_com->_intr[1].en = false;//(opt == INT_SET_OPT_SER_START ? false : false);
