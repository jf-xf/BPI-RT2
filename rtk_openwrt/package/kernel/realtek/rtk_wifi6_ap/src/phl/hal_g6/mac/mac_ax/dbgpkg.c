...
#endif
	{FWCMD_C2H_FUNC_C2H_L1_2_TEST, c2h_pcie_l12_test},
...
void fw_pc_dbg_dump_ax(struct mac_ax_adapter *adapter, u32 count, u32 delay_us)
{
	u32 index;
	struct mac_ax_intf_ops *ops = adapter_to_intf_ops(adapter);
	...
	if (FWDL_IS_AX_MIPS) {
		...
		PLTFM_MSG_ALWAYS("R_AX_WLCPU_PORT_PC = 0x%04X\n", R_AX_DBG_PORT_SEL);
	...
	if (FWDL_IS_AX_RISCV) {
		PLTFM_MSG_ALWAYS("R_AX_WLCPU_PORT_PC = 0x%04X\n", R_AX_WLCPU_PORT_PC);
		for (index = 0; index < count; index++) {
			PLTFM_MSG_ALWAYS("fw PC = 0x%08X\n", MAC_REG_R32(R_AX_WLCPU_PORT_PC));
			PLTFM_DELAY_US(delay_us);
		}
	}
#endif
...
u32 fw_st_dbg_dump(struct mac_ax_adapter *adapter)
{
	...
	fw_pc_dbg_dump_ax(adapter, 15, 10);
	...
	for (idx = 0; idx < TX_FLOW_DMP_NUM; idx++) {
		...
		if (is_chip_id(adapter, MAC_AX_CHIP_ID_8852C) ||
			...
			for (sub_idx = 0; sub_idx <= 3; sub_idx++) {
				...
				val32 = SET_CLR_WORD(val32, sub_idx, B_AX_DBG_USER_SEL);
...
u32 mac_fw_dbg_dle_cfg(struct mac_ax_adapter *adapter, bool lock)
{
	...
	if (ret)
		PLTFM_MSG_ERR("[WARN] %s h2c fail ret %d", __func__, ret);
...
u32 mac_test_l12(struct mac_ax_adapter *adapter)
{
	...
	if (ret != MACSUCCESS)
		PLTFM_MSG_ERR("[ERR]%s h2c tx fail\n", __func__);
...
u32 diagnosis_memoryleak(struct mac_ax_adapter *adapter, u32 *check_number)
{
	...
	*check_number += 1;
	if (adapter->drv_info->init_mac_mem_baseline == current_mem) {
		PLTFM_MSG_ALWAYS("No Memory Leak\n");
	} else {
		PLTFM_MSG_ERR("Memory usage baseline after mac_hal_init: %d\n",
			      adapter->drv_info->init_mac_mem_baseline);
		PLTFM_MSG_ERR("Current memory usage: %d\n", current_mem);
		cnt += 1;
	}
#endif
...
u32 mac_sys_st_check(struct mac_ax_adapter *adapter, u32 diag_level, u32 *total_check_number)
{
	...
	*total_check_number = 0;
	...
	check_number = 0;
#ifdef DBG_HAL_MAC_MEM_MOINTOR
	cnt += diagnosis_memoryleak(adapter, &check_number);
...
static u32 diagnosis_FW_ap_feat(struct mac_ax_adapter *adapter, u32 *check_number, u32 *fail_number)
{
#define RETRY_WAIT_PKT_US 10
	u32 cnt = 0;
	u32 ret = MACSUCCESS;

	struct h2c_info h2c_info = {0};
	struct fwcmd_ap_self_diag fwcmd_tbl;
	u8 i;
	...
	h2c_info.agg_en = 1;
	h2c_info.content_len = sizeof(struct fwcmd_ap_self_diag);
	h2c_info.h2c_cat = FWCMD_H2C_CAT_MAC;
	h2c_info.h2c_class = FWCMD_H2C_CL_AP_SELF_DIAG;
	h2c_info.h2c_func = FWCMD_H2C_FUNC_AP_SELF_DIAG;
	h2c_info.rec_ack = 0;
	h2c_info.done_ack = 1;

	fwcmd_tbl.dword0 =
	cpu_to_le32(SET_WORD(0, FWCMD_H2C_AP_SELF_DIAG_MODE) |
		    SET_WORD(0, FWCMD_H2C_AP_SELF_DIAG_FEATURE));

	ret = mac_h2c_common(adapter, &h2c_info, (u32 *)&fwcmd_tbl);
	if (ret)
		PLTFM_MSG_ERR("[ERR]%s: Send H2C fail\n", __func__);

	adapter->sm.ap_self_diag_state = MAC_AX_AP_SELF_DIAG_H2C_DONE;

	/* Wait for C2H */
	cnt = TX_PAUSE_WAIT_PKT_CNT;

	while (--cnt) {
		if (adapter->sm.ap_self_diag_state == MAC_AX_AP_SELF_DIAG_C2H_DONE)
			break;
		PLTFM_DELAY_US(RETRY_WAIT_PKT_US);
	}
	if (!cnt) {
		PLTFM_MSG_ERR("[ERR]AP_SELF_DIAG DONE ACK timeout\n");
		return MACPROCERR;
	}

	for (i = 0; i < 8; i++) {
		if (adapter->sm.ap_self_diag_feature[i] != 0xFF) {
			switch (adapter->sm.ap_self_diag_feature[i]) {
			case 0:
				PLTFM_MSG_ALWAYS("SU diagnosis:\n");
				*check_number += 4;

				// error
				if (adapter->sm.ap_self_diag_err_bitmap[i] &
				    COMMON_ERR_SW_TX_MODE_SETTING) {
					PLTFM_MSG_ALWAYS("\t[ERR]SW TX mode setting error\n");
					(*fail_number)++;
				}
				if (adapter->sm.ap_self_diag_err_bitmap[i] &
				    COMMON_ERR_SS2FRPT_QUERY_STUCK) {
					PLTFM_MSG_ALWAYS("\t[ERR]SS2FRPT Query STUCK happened\n");
					(*fail_number)++;
				}

				if (adapter->sm.ap_self_diag_err_bitmap[i] &
				    COMMON_ERR_NOT_ISSUE_ANY_TXCMD) {
					PLTFM_MSG_ALWAYS("\t[WRN]ISSUE TXCMD_NUM"\
							 "equal's to 0 in TBTT\n");
					(*fail_number)++;
				}

				// warning
				if (adapter->sm.ap_self_diag_warn_bitmap[i] &
				    COMMON_WARN_PKT_ERR_RATE) {
					PLTFM_MSG_ALWAYS("\t[WRN]PKT_ERR_RATE less"\
							 "than 80%% in TBTT\n");
					(*fail_number)++;
				}
				break;
			case 1:
				PLTFM_MSG_ALWAYS("OFDMA diagnosis:\n");
				break;
			case 2:
				PLTFM_MSG_ALWAYS("MU diagnosis:\n");
				break;
			case 3:
				PLTFM_MSG_ALWAYS("TWT diagnosis:\n");
				break;
			case 4:
				PLTFM_MSG_ALWAYS("SND diagnosis:\n");
				break;
			default:
				PLTFM_MSG_ALWAYS("unknow type %d diagnosis:\n",
						 adapter->sm.ap_self_diag_feature[i]);
				break;
			}
			PLTFM_MSG_ALWAYS("\tWARN_BITMAP: %x\n",
					 adapter->sm.ap_self_diag_warn_bitmap[i]);
			PLTFM_MSG_ALWAYS("\tERR_BITMAP: %x\n",
					 adapter->sm.ap_self_diag_err_bitmap[i]);
		}
	}
...
u32 mac_fw_st_check(struct mac_ax_adapter *adapter, u32 diag_level, u32 *total_check_number)
{
	...
	*total_check_number = 0;
	...
	/*
	 * u32 check_number = 0;
	 * cnt += diagnosis_FW_system(adapter, &check_number);
	 * *total_check_number += check_number;
	 * cnt += diagnosis_FW_nic_feat(adapter, &check_number);
	 * *total_check_number += check_number;
	 */
...
u32 mac_pcie_status_dump(struct mac_ax_adapter *adapter)
{
	...
	PLTFM_MSG_ALWAYS("[PCIE] Bus Master: %d\n",
			 GET_FIEL2(cfg_reg, PCIE_BUS_MASTER_SH, PCIE_BUS_MASTER_BITS_MSK));
	...
	PLTFM_MSG_ALWAYS("[PCIE] ASPM Control: %d\n",
			 cfg_reg & (PCIE_BIT_STS_L0S | PCIE_BIT_STS_L1));
