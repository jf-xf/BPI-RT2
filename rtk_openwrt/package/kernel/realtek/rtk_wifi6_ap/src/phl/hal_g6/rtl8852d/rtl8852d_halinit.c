 ...
 *
 *****************************************************************************/
#define _RTL8852D_HALINIT_C_
#include "../hal_headers.h"
#include "../hal_api.h"
#include "rtl8852d_hal.h"


void init_hal_spec_8852d(struct rtw_phl_com_t *phl_com,
					struct hal_info_t *hal)
{
	...
	hw_proto_cap[0].stbc_tx_greater_80mhz = 1;
	hw_proto_cap[1].stbc_tx_greater_80mhz = 1;
	...
	hw_proto_cap[0].stbc_rx_greater_80mhz = 1;
	hw_proto_cap[1].stbc_rx_greater_80mhz = 1;
#endif
	...
	hal_com->dev_hw_cap.hw_stype_cap = EFUSE_HW_STYPE_NONE_8852D;
	hal_com->dev_hw_cap.wl_func_cap = EFUSE_WL_FUNC_NONE;

	hal_com->dev_hw_cap.drv_info_sup = RTW_DEV_CAP_ENABLE;

	hal_com->dev_hw_cap.antdiv_sup = true;
}


void init_default_value_8852d(struct hal_info_t *hal)
{
...
u32 _hal_cfg_rom_fw_8852d(enum rtw_fw_type fw_type, struct rtw_fw_info_t *fw_info,
			  char *ic_name)
{
	...
	_os_snprintf(fw_info->rom_path, MAX_PATH_LEN, "%s%s%s%s", hal_phy_folder,
		     ic_name, _os_path_sep, "rtl8852dfw_rom.bin");
...
u32 _hal_cfg_intnal_fw_8852d(struct rtw_phl_com_t *phl_com, enum rtw_fw_type fw_type, struct rtw_fw_info_t *fw_info)
{
	/* any related to fw from header can be defined here */
	return RTW_HAL_STATUS_SUCCESS;
}

static u32 _hal_read_fw_8852d(enum rtw_fw_type fw_type,
	u8 *tgt_buf, u32 *tgt_buf_size, char *ic_name)
{
	...
	_os_snprintf(path, MAX_PATH_LEN, "%s%s%s%s%s%s", hal_phy_folder,
		     ic_name, _os_path_sep, "RTL8852Dfw", filename_postfix, ".bin");
...
static u32 _hal_cfg_extnal_fw_8852d(struct rtw_phl_com_t *phl_com,
	enum rtw_fw_type fw_type, struct rtw_fw_info_t *fw_info, char *ic_name)
{
	...
	switch (fw_type) {
	case RTW_FW_NIC:
		_hal_read_fw_8852d(RTW_FW_NIC, fw_info->buf,
			&fw_info->buf_size, ic_name);
		/* preload wowlan fw */
		_hal_read_fw_8852d(RTW_FW_WOWLAN, fw_info->wow_buf,
	...
	case RTW_FW_WOWLAN:
		...
		break;
	case RTW_FW_SPIC:
		_hal_read_fw_8852d(RTW_FW_SPIC, fw_info->buf,
			&fw_info->buf_size, ic_name);
		tgt_buf = fw_info->buf;
		tgt_buf_size = fw_info->buf_size;
		break;
	case RTW_FW_AP:
		_hal_read_fw_8852d(RTW_FW_AP, fw_info->buf,
			&fw_info->buf_size, ic_name);
		tgt_buf = fw_info->buf;
		tgt_buf_size = fw_info->buf_size;
		break;
	...
	if (tgt_buf && (0 != tgt_buf_size)) {
		...
		fw_info->ram_size = 0;
		return _hal_read_fw_8852d(fw_type, fw_info->ram_buff,
			&fw_info->ram_size, ic_name);
	}

}

enum rtw_hal_status hal_cfg_fw_8852d(struct rtw_phl_com_t *phl_com,
				     struct hal_info_t *hal,
				     char *ic_name,
				     enum rtw_fw_type fw_type)
{
	...
	if (fw_cap->dlrom_en) {
		if (RTW_HAL_STATUS_SUCCESS !=_hal_cfg_rom_fw_8852d(fw_type, fw_info,
			ic_name))
			goto init_fw_fail;
	} else {
		if(fw_info->rom_buff) {
	...
	if (fw_cap->fw_src == RTW_FW_SRC_EXTNAL) {
		...
		if (RTW_HAL_STATUS_SUCCESS != _hal_cfg_extnal_fw_8852d(phl_com, fw_type,
			fw_info, ic_name))
			goto init_fw_fail;
		...
		if (RTW_HAL_STATUS_SUCCESS != _hal_cfg_intnal_fw_8852d(phl_com, fw_type,
			fw_info))
			goto init_fw_fail;
	} else {
		goto init_fw_fail;
...
enum rtw_hal_status hal_get_efuse_8852d(struct rtw_phl_com_t *phl_com,
					struct hal_info_t *hal,
...
#endif
enum rtw_hal_status hal_start_8852d(struct rtw_phl_com_t *phl_com,
				   struct hal_info_t *hal,
				   struct hal_init_info_t *init_info)
{
	enum rtw_hal_status hal_status = RTW_HAL_STATUS_FAILURE;
...
enum rtw_hal_status hal_stop_8852d(struct rtw_phl_com_t *phl_com, struct hal_info_t *hal)
{
	enum rtw_hal_status hal_status = RTW_HAL_STATUS_FAILURE;
...
#ifdef CONFIG_WOWLAN
enum rtw_hal_status
hal_wow_init_8852d(struct rtw_phl_com_t *phl_com,
...
enum rtw_hal_status
hal_wow_deinit_8852d(struct rtw_phl_com_t *phl_com,
				struct hal_info_t *hal_info, struct rtw_phl_stainfo_t *sta,
					struct hal_init_info_t *init_info)
{
	struct hal_ops_t *hal_ops = hal_get_ops(hal_info);
...
#ifdef RTW_PHL_BCN //fill 8852d bcn ops

enum rtw_hal_status hal_config_beacon_8852d(struct rtw_phl_com_t *phl_com,
                                            struct hal_info_t *hal,
...
enum rtw_hal_status hal_update_beacon_8852d(struct rtw_phl_com_t *phl_com,
                                            struct hal_info_t *hal,
                                            struct rtw_bcn_entry *bcn_entry)
{
...
enum rtw_hal_status
hal_mp_init_8852d(struct rtw_phl_com_t *phl_com,
...
#if 0/* do not reload fw when mp init */
	hal_status = rtw_hal_redownload_fw(phl_com, hal_info);
	if (hal_status != RTW_HAL_STATUS_SUCCESS) {
		PHL_ERR("%s: redownload fw fail(%d)!!\n", __func__, hal_status);
		goto exit;
...
enum rtw_hal_status
hal_mp_deinit_8852d(struct rtw_phl_com_t *phl_com,
				struct hal_info_t *hal_info,
					struct hal_init_info_t *init_info)
{
	struct hal_ops_t *hal_ops = hal_get_ops(hal_info);
...
#if 0/* do not reload fw when mp deinit */
	hal_status = rtw_hal_redownload_fw(phl_com, hal_info);
	if (hal_status != RTW_HAL_STATUS_SUCCESS) {
		PHL_ERR("%s: redownload fw fail(%d)!!\n", __func__, hal_status);
		goto exit;
