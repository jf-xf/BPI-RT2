...
bool halbb_stbc_mapping(struct bb_info *bb,  struct rtw_phl_stainfo_t *phl_sta_i, u8 hw_mode)
{
	...
	if (hw_mode & HE_SUPPORT) {
		...
		if ((((bb->phl_com->id.id & 0xF8) >> 3) == 1) &&
			(((bb->phl_com->id.id & 0x1F0000)>> 16) == 2)) {
			stbc_en = false;
...
bool rtw_halbb_raregistered(struct bb_info *bb, struct rtw_phl_stainfo_t *phl_sta_i)
{
	...
	asoc_cap_i = &phl_sta_i->asoc_cap;
	ra_cfg->he_bf_cap = asoc_cap_i->he_su_bfme;
	ra_cfg->vht_bf_cap = asoc_cap_i->vht_su_bfme;
	ra_cfg->stbc_cap_160M = asoc_cap_i->stbc_rx_greater_80mhz;
	...
	if (asoc_cap_i->dcm_max_const_rx)
		...
		ra_cfg->dcm_cap = 0;
	mode = halbb_hw_mode_mapping(bb, mode, phl_sta_i);
	...
	BB_DBG(bb, DBG_RA, "he_bf_cap = %d, vht_bf_cap=%d, 160M_STBC=%d, 80M_STBC=%d\n",
		ra_cfg->he_bf_cap,ra_cfg->vht_bf_cap, ra_cfg->stbc_cap_160M, tx_stbc);
	ra_cfg->upd_all= true;
...
bool rtw_halbb_ra_stbc_status(struct bb_info *bb,
	struct rtw_phl_stainfo_t *phl_sta_i)
{
	bool ret_val = false;
	u8 cmdlen = sizeof(struct bb_h2c_ra_stbc);
	u32 *bb_h2c;
	struct bb_h2c_ra_stbc ra_cfg_stbc;
	struct rtw_hal_stainfo_t *hal_sta_i;
	...
	hal_sta_i = phl_sta_i->hal_sta;
	bb_h2c = (u32*) &ra_cfg_stbc;
	ra_cfg_stbc.macid_l = (u8) (phl_sta_i->macid);
	ra_cfg_stbc.macid_m = (u8) (phl_sta_i->macid >> 8);
	BB_DBG(bb, DBG_RA, "H2C stbc rpt:macid=%d\n", phl_sta_i->macid);
	ret_val = halbb_fill_h2c_cmd(bb, cmdlen, RA_H2C_STBC_STATUS, HALBB_H2C_RA, bb_h2c);
	return ret_val;
}

u32 halbb_get_fw_stbc_status(struct bb_info *bb, u16 len, u8 *c2h)
{
	u16 macid_rpt;
	struct rtw_hal_stainfo_t *hal_sta_i;
	struct rtw_phl_stainfo_t *phl_sta_i;
	struct halbb_stbc_sts *ra_rpt_i;
	struct rtw_stbc_info *stbc_info_i;

	if (!c2h)
		return 0;
	ra_rpt_i = (struct halbb_stbc_sts *)c2h;
	macid_rpt = ra_rpt_i->rpt_macid_l + (ra_rpt_i->rpt_macid_m << 8);
	/* export macid to let driver parse*/
	bb->phl_com->macid_rpt = macid_rpt;
	...
	stbc_info_i = &hal_sta_i->stbc_info;
	/* export variable into driver for disable stbc*/
	stbc_info_i->stbc_disable = ra_rpt_i->stbc_disable;

	BB_DBG(bb, DBG_RA,
		   "C2H STBC RPT:macid=%d, stbc_disable=%d\n",
		   macid_rpt, ra_rpt_i->stbc_disable);
...
u32 halbb_get_fw_ra_rpt(struct bb_info *bb, u16 len, u8 *c2h)
{
	...
	if (ra_rpt_i->is_mu) {
		...
		BB_DBG(bb, DBG_RA,
		       "SU RA RPT: macid = %d, mode = %d, giltf = %x, mcs_nss = %x, retry=%x, stbc=%d\n",
...
{
	struct halbb_ra_tx_hist_c2h_rpt *c2h_rpt;
	u8 i = 0;

	if (!c2h)
		return;

	c2h_rpt = (struct halbb_ra_tx_hist_c2h_rpt *)c2h;
