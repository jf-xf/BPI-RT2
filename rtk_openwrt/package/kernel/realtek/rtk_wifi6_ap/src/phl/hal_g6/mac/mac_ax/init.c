...
u32 hci_func_en(struct mac_ax_adapter *adapter)
{
	...
	if (reg == 0) {
		PLTFM_MSG_ERR("%s: assign reg fail", __func__);
		return MACNOITEM;
	}
	...
	if (fwdl_info->fw_en) {
		...
		if (fwdl_info->dlram_en) {
			if (fwdl_info->fw_from_hdr) {
				...
				ret = get_dle_rsvd_info(adapter, &dle_info);
				if (ret != MACSUCCESS) {
					PLTFM_MSG_ERR("[ERR]get_dle_rsvd_info %d", ret);
					goto end;
				}
				...
				if (chk_patch_otp_power_issue(adapter)) {
					...
					if (ret != MACSUCCESS) {
						PLTFM_MSG_ERR("[ERR]patch_otp_power_issue %d\n",
							      ret);
						goto end;
					}
				}
