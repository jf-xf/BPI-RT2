	...
	struct mp_context *mp, struct mp_tx_arg *arg, struct mp_plcp_param_t *plcp_tx_struct)
{
	...
	hal_status = rtw_hal_bb_set_plcp_tx(hal_info->hal_com,
										...
										mp->cur_phy,
										&arg->plcp_sts);
	PHL_INFO("%s: ppdu type: %d\n", __FUNCTION__, plcp_tx_struct->ppdu_type);
	PHL_INFO("%s: PHY rate: usr[0]=MCS%d , usr[1]=MCS%d , usr[2]=MCS%d , usr[3]=MCS%d\n", __FUNCTION__,
			mp->usr[0].mcs, mp->usr[1].mcs, mp->usr[2].mcs, mp->usr[3].mcs);
	PHL_INFO("Set RF DPK by rate, use phy rate MCS%d of usr[0] (SU only).\n", mp->usr[0].mcs);
	rtw_hal_rf_set_dpk_by_rate(mp->hal, mp->cur_phy,
				plcp_tx_struct->ppdu_type, mp->usr[0].mcs);
