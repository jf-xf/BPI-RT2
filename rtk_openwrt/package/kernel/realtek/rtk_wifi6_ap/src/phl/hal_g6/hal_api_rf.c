...
void rtw_hal_rf_set_dpk_by_rate(void *hal, enum phl_phy_idx phy,
				u32 ppdu_type, u32 mcs)
{
	struct hal_info_t *hal_info = (struct hal_info_t *) hal;

	PHL_INFO ("[HAL] %s: ppdu_type=%d, mcs=%d\n", __func__, ppdu_type, mcs);

	halrf_set_dpk_by_rate(hal_info->rf, phy, ppdu_type, mcs);
}
