			...
			bool is_connect)
{
	...
	if (rinfo->net_type == MAC_AX_NET_TYPE_AP) {
		...
		if(is_self == true) {
			...
			if (hal_info->hal_com->dev_hw_cap.bw_sup & BW_CAP_160M)
				rinfo->dl_bw = MAC_AX_BW_160M;
			else if (hal_info->hal_com->dev_hw_cap.bw_sup & BW_CAP_80M)
				rinfo->dl_bw = MAC_AX_BW_80M;
			else if (hal_info->hal_com->dev_hw_cap.bw_sup & BW_CAP_40M)
				rinfo->dl_bw = MAC_AX_BW_40M;
			else
				rinfo->dl_bw = MAC_AX_BW_20M;
		} else {
...
rtw_hal_mac_ser_set_error_status(struct hal_info_t *hal_info, enum RTW_PHL_SER_RCVY_STEP err)
{
	...
	if (err == RTW_PHL_SER_L1_RESET_START_DMAC) {
		err_info = MAC_AX_ERR_L1_RESET_START_DMAC;
	} else if (err == RTW_PHL_SER_L1_DISABLE_EN) {
...
rtw_hal_mac_dl_grp_info_dump(struct mac_ax_ss_dl_grp_upd *info)
{
	...
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "trigger_ul_threshold 0x%x\n", info->trigger_ul_threshold);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "trigger_len 0x%x\n", info->trigger_len);
}

void
rtw_hal_mac_upd_ss_ul_sta_dump(struct mac_ax_ss_ul_sta_upd *info)
{
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "\n%s\n", __func__);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "mode 0x%x\n", info->mode);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "macid 0x%x 0x%x 0x%x 0x%x\n", info->macid[0], info->macid[1], info->macid[2], info->macid[3]);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "bsr_thres_sel 0x%x\n", info->bsr_thres_sel);
}

void
rtw_hal_mac_set_ul_grp_info_dump(struct mac_ax_ss_ul_grp_upd *info)
{
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "\n%s\n", __func__);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "macid 0x%x 0x%x \n", info->macid_u0, info->macid_u1);
	PHL_TRACE(COMP_PHL_OFDMA, _PHL_INFO_, "grp_bitmap 0x%x\n", info->grp_bitmap);
}
					...
					struct rtw_phl_mac_ax_fixmode_para *tbl)
{
	...
	fixmode_para.set_ulmode = tbl->set_ulmode;
	fixmode_para.ulmode = tbl->ulmode;
					...
					struct rtw_phl_mac_ss_dl_grp_upd *tbl)
{
	...
	info.trigger_ul_threshold = tbl->trigger_ul_threshold;
	info.trigger_len = tbl->trigger_len;
...
rtw_hal_mac_set_ul_grp_info(void *hal,
					struct rtw_phl_mac_ss_ul_grp_upd *tbl)
{
	struct hal_info_t *hal_info = (struct hal_info_t *)hal;
	struct mac_ax_adapter *mac = hal_to_mac(hal_info);
	...
	info.macid_u0 = tbl->macid_u0;
	info.macid_u1 = tbl->macid_u1;
	info.grp_bitmap = tbl->grp_bitmap;

	rtw_hal_mac_set_ul_grp_info_dump(&info);
...
hal_mac_ax_send_beacon(struct hal_info_t *hal, struct rtw_bcn_entry *bcn_entry)
{
	...
	if (bcn_cmn->offset_ecsa_cntdown)
		info.ecsa_ofst = (u16)((bcn_cmn->offset_ecsa_cntdown << 1) | BIT(0));
#endif
			       ...
			       u8 band, enum rtw_rx_fltr_mode mode)
{
	...
	do {
		...
		if (   (err == 0)
			...
			PHL_INFO("Set RX packet size filter to %uB\n",
...
rtl_hal_dump_sec_cam_tbl(struct rtw_hal_com_t *hal_com)
{
	...
	PHL_PRINT("entry  valid  mac_id  key_id  key_type sec_cam_idx key\n");
	...
	for (i = 0; i < sec_cam_tbl_sz; i++) {
		...
		PHL_PRINT("  %3d    %3d     %3d     %3d       %3d         %3d   %x %x %x %x \n",
			i, entry->valid, entry->mac_id, entry->key_id, entry->key_type,
			entry->sec_cam_info->sec_cam_idx,
			entry->sec_cam_info->key[0],
			entry->sec_cam_info->key[1],
			entry->sec_cam_info->key[2],
			entry->sec_cam_info->key[3]);
	}
}
...
{
	struct mac_ax_adapter *mac = hal_to_mac(hal_info);
	u32 hal_cmd_ary_size = sizeof(hal_cmd_i) / sizeof(struct hal_cmd_info);
	u8 id = 0, i = 0;

	if (hal_cmd_ary_size == 0) { return; }

	/* Parsing Cmd ID */
	if (input_num) {
		for (i = 0; i < hal_cmd_ary_size; i++) {
			if (_os_strcmp(hal_cmd_i[i].name, input[0]) == 0) {
				id = hal_cmd_i[i].id;
				PHL_INFO("enter hal dbg %s\n", hal_cmd_i[i].name);
				break;
			}
		}
	}
		       ...
		       struct hal_ax_fwcmd_snd *hal_cmd)
{
	...
	snd_cmd.sfp.f2p_period = hal_info->phl_com->txbf_period_fw;
	snd_cmd.sfp.f2p_index = hal_cmd->f2p_index;
	snd_cmd.sfp.f2p_type = HAL_SNDF2P_ADD;
	snd_cmd.sfp.sounding_en= hal_cmd->sounding_en;

	for (i = 0; i < 16; i++) {    //HAL_MAX_HE_SND_STA_NUM: wait for halmac temporarily
...
enum rtw_hal_status
hal_mac_ax_send_fw_to_del_snd(struct hal_info_t *hal_info,
		      struct rtw_hal_stainfo_t *hal_sta)
{
	...
	struct mac_ax_fwcmd_snd snd_cmd = {0};
	u8 i = 0, he = 0, sta_nr = 0;
	snd_cmd.frexgtype = HAL_FEXG_TYPE_AC_SU;
	snd_cmd.sfp.f2p_index = hal_sta->f2p_index;
	snd_cmd.sfp.f2p_type = HAL_SNDF2P_DEL;

	PHL_TRACE(COMP_PHL_SOUND, _PHL_INFO_, "[fw_snd] snd_cmd.sfp.f2p_index = 0x%x \n", snd_cmd.sfp.f2p_index);
	PHL_TRACE(COMP_PHL_SOUND, _PHL_INFO_, "[fw_snd] snd_cmd.sfp.f2p_type = 0x%x \n", snd_cmd.sfp.f2p_type);
	hstatus = mac->ops->set_snd_para(mac, &snd_cmd);
	PHL_TRACE(COMP_PHL_SOUND, _PHL_INFO_, "[fw_snd] hal_mac_ax_send_fw_to_del_snd hstatus = 0x%x\n", hstatus);
...
rtw_hal_mac_upd_ss_ul_sta(struct hal_info_t *hal_info, struct rtw_phl_stainfo_t *sta, u8 enable)
{
	...
	info.bsr_thres_sel = sta->is_latency_sta;

	rtw_hal_mac_upd_ss_ul_sta_dump(&info);
...
enum rtw_hal_status
rtw_hal_mac_set_beacon_ignore_edcca(struct rtw_hal_com_t *hal_com, u8 port, u8 mbssid, u8 band, u8 enable)
{
	struct hal_info_t *hal_info = (struct hal_info_t *)hal_com->hal_priv;
	struct mac_ax_adapter *mac = hal_to_mac(hal_info);
	struct mac_ax_bcn_ignore_edcca bcn_ignore_edcca = {0};
	u32 err = 0;

	bcn_ignore_edcca.port = port;
	bcn_ignore_edcca.mbssid = mbssid;
	bcn_ignore_edcca.band = band;
	bcn_ignore_edcca.ignore_edcca_en = enable;

	err = mac->ops->set_bcn_ignore_edcca(mac, &bcn_ignore_edcca);
	if (err)
		return RTW_HAL_STATUS_FAILURE;
...
enum rtw_hal_status
rtw_hal_mac_sifs_chk_cca_en(struct rtw_hal_com_t *hal_com, u8 band, u8 en)

{
	...
	if (mac->ops->sifs_chk_cca_en) {
		hal_status = mac->ops->sifs_chk_cca_en(mac, band, en);
	}
...
enum rtw_hal_status
rtw_hal_mac_resp_chk_cca(struct rtw_hal_com_t *hal_com, u8 band, u8 en)
{
	enum rtw_hal_status hal_status = RTW_HAL_STATUS_SUCCESS;
	struct hal_info_t *hal_info = (struct hal_info_t *)hal_com->hal_priv;
	struct mac_ax_adapter *mac = hal_to_mac(hal_info);
	struct mac_ax_resp_chk_cca param;

	param.band = band;
	param.resp_ack_chk_cca_en = en;

	if (mac->ops->set_hw_value) {
		hal_status = mac->ops->set_hw_value(mac, MAC_AX_HW_SET_RESP_ACK_CHK_CCA, (void *)&param);
	}
