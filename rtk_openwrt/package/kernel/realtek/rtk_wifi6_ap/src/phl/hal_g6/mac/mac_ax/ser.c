...
static void dump_err_status_dmac(struct mac_ax_adapter *adapter)
{
	...
	if (dmac_err & B_AX_WSEC_ERR_FLAG) {
		...
		if (is_chip_id(adapter, MAC_AX_CHIP_ID_8852A) ||
			...
			PLTFM_MSG_ERR("SEC_Register 0x9D00 =0x%08x\n",
...
u32 mac_chk_l1_rvy_status(struct mac_ax_adapter *adapter, u32 *rvy_status)
{
	switch (adapter->sm.fw_rst) {
	case MAC_AX_FW_RESET_PRE_RECV:
		*rvy_status = MAC_AX_ERR_L1_PREERR_DMAC;
		break;
	case MAC_AX_FW_RESET_RECV:
		*rvy_status = MAC_AX_ERR_L1_ERR_DMAC;
		break;
	case MAC_AX_FW_RESET_PROCESS:
		*rvy_status = MAC_AX_ERR_L1_RESET_DISABLE_DMAC_DONE;
		break;
	case MAC_AX_FW_RESET_RECV_DONE:
		*rvy_status = MAC_AX_ERR_L1_RESET_RECOVERY_DONE;
		break;
	default:
		*rvy_status = MAC_AX_ERR_L1_RESET_RECOVERY_DONE;
		break;
