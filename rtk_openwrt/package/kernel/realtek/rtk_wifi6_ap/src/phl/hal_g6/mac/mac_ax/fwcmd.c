...
u32 c2h_ap_self_diag_rst(struct mac_ax_adapter *adapter, u8 *buf,
			 u32 len, struct rtw_c2h_info *info)
{
	u8 i;
	u8 *content = buf + FWCMD_HDR_LEN;

	if (info->c2h_func == FWCMD_C2H_FUNC_AP_SELF_DIAG_RST) {
		if (adapter->sm.ap_self_diag_state != MAC_AX_AP_SELF_DIAG_H2C_DONE) {
			PLTFM_MSG_ERR("[ERR]ap self diag h2c not sending\n");
			return MACPROCERR;
		}

		for (i = 0; i < 8; i++) {
			adapter->sm.ap_self_diag_feature[i] =
				GET_FIELD(le32_to_cpu(*(u32 *)(content + (i *
					  AP_SELF_DIAG_RST_ELEMENT_SIZE))),
					  FWCMD_C2H_AP_SELF_DIAG_RST_FEATURE_0);
			adapter->sm.ap_self_diag_warn_bitmap[i] =
				le32_to_cpu(*(u32 *)(content +
					    ((i * AP_SELF_DIAG_RST_ELEMENT_SIZE) +
					     AP_SELF_DIAG_RST_WARN_BITMAP_OFFSET)));
			adapter->sm.ap_self_diag_err_bitmap[i] =
				le32_to_cpu(*(u32 *)(content +
					    ((i * AP_SELF_DIAG_RST_ELEMENT_SIZE) +
					     AP_SELF_DIAG_RST_ERR_BITMAP_OFFSET)));
		}
	}
	adapter->sm.ap_self_diag_state = MAC_AX_AP_SELF_DIAG_C2H_DONE;
...
static struct c2h_proc_class c2h_proc_mac[] = {
	...
	{FWCMD_C2H_CL_WOW_DIAG, c2h_wow_diag},
	{FWCMD_C2H_CL_WOW_TRI_EVT, c2h_wow_tri_evt},
	{FWCMD_C2H_CL_STA_DIAG, c2h_sta_diag},
	{FWCMD_C2H_CL_AP_SELF_DIAG_RST, c2h_ap_self_diag_rst},
	{FWCMD_C2H_CL_NULL, NULL},
};
