			    ...
			    struct mac_ax_mudecision_para *info)
{
	...
	h2c_info.agg_en = 0;
	h2c_info.content_len = sizeof(struct fwcmd_mude_para_tblud);
	...
	tbl = (struct fwcmd_mude_para_tblud *)PLTFM_MALLOC(h2c_info.content_len);
	if (!tbl)
		return MACBUFALLOC;
		       ...
		       struct rtw_phl_ax_ul_fixinfo *info)
{
	...
	cpu_to_le32(SET_WORD(info->multiport_id,
		    ...
		    SET_WORD(info->store_idx,
			     ...
			     FWCMD_H2C_UL_FIXINFO_ULINFO_NO_NEXT_SS);
	...
	ret = mac_h2c_common(adapter, &h2c_info, dword_start);

	PLTFM_FREE(dword_start, h2c_info.content_len);
		     ...
		     u8 *psigb_addr)
{
	...
	h2c_info.agg_en = 0;
	h2c_info.content_len = sizeof(struct fwcmd_test_para);
	h2c_info.h2c_cat = FWCMD_H2C_CAT_MAC;
	h2c_info.h2c_class = FWCMD_H2C_CL_FR_EXCHG;
	h2c_info.h2c_func = FWCMD_H2C_FUNC_F2P_TEST;
	h2c_info.rec_ack = 0;
	h2c_info.done_ack = 0;
		      ...
		      struct mac_ax_dctl_info *mask, u8 macid, u8 operation)
{
	...
	h2c_info.agg_en = 1;
	h2c_info.content_len = sizeof(struct fwcmd_dctlinfo_ud);
	h2c_info.h2c_cat = FWCMD_H2C_CAT_MAC;
	h2c_info.h2c_class = FWCMD_H2C_CL_FR_EXCHG;
	h2c_info.h2c_func = FWCMD_H2C_FUNC_DCTLINFO_UD;
	h2c_info.rec_ack = 0;
	h2c_info.done_ack = 0;

	tbl = (struct fwcmd_dctlinfo_ud *)PLTFM_MALLOC(h2c_info.content_len);
	if (!tbl)
		return MACBUFALLOC;
			 ...
			 u8 macid, u8 operation)
{
	...
	h2c_info.agg_en = 1;
	h2c_info.content_len = sizeof(struct fwcmd_dctlinfo_ud_v1);
	h2c_info.h2c_cat = FWCMD_H2C_CAT_MAC;
	h2c_info.h2c_class = FWCMD_H2C_CL_FR_EXCHG;
	h2c_info.h2c_func = FWCMD_H2C_FUNC_DCTLINFO_UD_V1;
	h2c_info.rec_ack = 0;
	h2c_info.done_ack = 0;

	tbl = (struct fwcmd_dctlinfo_ud_v1 *)PLTFM_MALLOC(h2c_info.content_len);
	if (!tbl)
		return MACBUFALLOC;
	...
	cpu_to_le32(SET_WORD(info->qos_field_h, FWCMD_H2C_DCTRL_QOS_FIELD_H) |
		    SET_WORD(info->hw_exseq_macid, FWCMD_H2C_DCTRL_HW_EXSEQ_MACID) |
		    (info->qos_field_h_en ? FWCMD_H2C_DCTRL_V1_QOS_DATA : 0) |
		    SET_WORD(info->aes_iv_l, FWCMD_H2C_DCTRL_AES_IV_L));
	...
	cpu_to_le32(SET_WORD(mask->qos_field_h, FWCMD_H2C_DCTRL_QOS_FIELD_H) |
		    SET_WORD(mask->hw_exseq_macid, FWCMD_H2C_DCTRL_HW_EXSEQ_MACID) |
		    (mask->qos_field_h_en ? FWCMD_H2C_DCTRL_V1_QOS_DATA : 0) |
		    SET_WORD(mask->aes_iv_l, FWCMD_H2C_DCTRL_AES_IV_L));
		       ...
		       struct mac_ax_shcut_mhdr *info, u8 shcut_camid)
{
	...
	u32 *src, *dest;
	struct h2c_info h2c_info = {0};
	struct fwcmd_shcut_update *content;
			...
			struct rtw_hal_mac_ax_cctl_info *mask, u8 macid)
{
	...
	role = role_srch_no_lock(adapter, macid);
	if (!role) {
		      ...
		      struct rtw_hal_mac_ax_cctl_info *mask, u8 macid, u8 operation)
{
	...
	h2c_info.agg_en = 0;
	h2c_info.content_len = sizeof(struct fwcmd_cctlinfo_ud);
	h2c_info.h2c_cat = FWCMD_H2C_CAT_MAC;
	h2c_info.h2c_class = FWCMD_H2C_CL_FR_EXCHG;
	...
	if (adapter->sm.fwdl == MAC_AX_FWDL_INIT_RDY) {
		...
		if (operation)
			cctl_info_debug_write(adapter, macid, tbl);
#else
		ret = MACFWNONRDY;
#endif
	}
...
u32 mac_bacam_init_upd(struct mac_ax_adapter *adapter, u8 users,
		       u8 offset, u8 band)
{
	u32 ret = MACSUCCESS;
	u32 i;
	struct h2c_info h2c_info = {0};
	struct fwcmd_ba_cam_init *tbl;
	...
	h2c_info.agg_en = 0;
	h2c_info.content_len = sizeof(struct fwcmd_ba_cam_init);
	h2c_info.h2c_cat = FWCMD_H2C_CAT_MAC;
	...
	if (adapter->sm.fwdl != MAC_AX_FWDL_INIT_RDY) {
		...
		tbl->dword0 =
			cpu_to_le32((band ? FWCMD_H2C_BA_CAM_INIT_BAND_SEL : 0) |
				    SET_WORD(users, FWCMD_H2C_BA_CAM_INIT_USERS) |
				    SET_WORD(offset, FWCMD_H2C_BA_CAM_INIT_OFFSET));
		ret = mac_h2c_common(adapter, &h2c_info, (u32 *)tbl);
		PLTFM_FREE(tbl, h2c_info.content_len);
	}
...
u32 mac_bacam_init(struct mac_ax_adapter *adapter)
{
	...
	ret = mac_bacam_init_upd(adapter, entry_num_b0, offset, MAC_AX_BAND_0);
	if (ret) {
	...
	offset = entry_num_b0 + offset;
	ret = mac_bacam_init_upd(adapter, entry_num_b1, offset, MAC_AX_BAND_1);
	if (ret) {
		PLTFM_MSG_ERR("[ERR]bacam init fail %d\n", ret);
		return ret;
	}
		   ...
		   struct mac_ax_bacam_info *info)
{
	...
	h2c_info.agg_en = 0;
	h2c_info.content_len = sizeof(struct fwcmd_ba_cam);
	h2c_info.h2c_cat = FWCMD_H2C_CAT_MAC;
	h2c_info.h2c_class = FWCMD_H2C_CL_BA_CAM;
	...
	default:
		ret = MACNOITEM;
		PLTFM_MSG_ERR("[ERR]Unsupported chip %d\n", ret);
		PLTFM_FREE(tbl, h2c_info.content_len);
		return ret;
	}
		      ...
		      struct mac_ax_ss_ul_grp_upd *info)
{
	...
	h2c_info.agg_en = 0;
	h2c_info.content_len = sizeof(struct fwcmd_ul_grp_upd);
	h2c_info.h2c_cat = FWCMD_H2C_CAT_MAC;
	h2c_info.h2c_class = FWCMD_H2C_CL_MEDIA_RPT;
		      ...
		      struct mac_ax_ss_ul_sta_upd *info)
{
	...
	h2c_info.agg_en = 0;
	h2c_info.content_len = sizeof(struct fwcmd_ss_ulsta_upd);
	h2c_info.h2c_cat = FWCMD_H2C_CAT_MAC;
	h2c_info.h2c_class = FWCMD_H2C_CL_FR_EXCHG;
	...
	cpu_to_le32(SET_WORD(info->mode, FWCMD_H2C_SS_ULSTA_UPD_MODE) |
		    (info->bsr_thres_sel ? FWCMD_H2C_SS_ULSTA_UPD_BSR_TH_SEL : 0));
		   ...
		   struct mac_ax_mu_sta_upd *info)
{
	...
	h2c_info.agg_en = 0;
	h2c_info.content_len = sizeof(struct fwcmd_mu_sta_upd);
	h2c_info.h2c_cat = FWCMD_H2C_CAT_MAC;
	h2c_info.h2c_class = FWCMD_H2C_CL_MEDIA_RPT;
		      ...
		      struct mac_ax_fwstatus_payload *info)
{
	...
	u32 *src, *dest;
	struct h2c_info h2c_info = {0};
	struct mac_ax_fwstatus_payload content;

	h2c_info.agg_en = 0;
	h2c_info.content_len = sizeof(struct mac_ax_fwstatus_payload);
	h2c_info.h2c_cat = FWCMD_H2C_CAT_TEST;
	h2c_info.h2c_class = FWCMD_H2C_CL_FW_STATUS_TEST;
	h2c_info.h2c_func = 0;
	h2c_info.rec_ack = 0;
	h2c_info.done_ack = 0;
...
u32 mac_fw_flow_ctrl_upd(struct mac_ax_adapter *adapter,
			 struct mac_ax_fw_flow_ctrl_tbl *info)
{
	u32 ret = 0;
	struct h2c_info h2c_info = {0};
	struct fwcmd_fw_flow_ctrl *tbl;

	h2c_info.agg_en = 0;
	h2c_info.content_len = sizeof(struct fwcmd_fw_flow_ctrl);
	h2c_info.h2c_cat = FWCMD_H2C_CAT_MAC;
	h2c_info.h2c_class = FWCMD_H2C_CL_MEDIA_RPT;
	h2c_info.h2c_func = FWCMD_H2C_FUNC_FW_FLOW_CTRL;
	h2c_info.rec_ack = 0;
	h2c_info.done_ack = 1;

	tbl = (struct fwcmd_fw_flow_ctrl *)PLTFM_MALLOC(h2c_info.content_len);
	if (!tbl)
		return MACNPTR;

	PLTFM_MEMSET(tbl, 0, sizeof(struct fwcmd_fw_flow_ctrl));

	if (info->flow_ctrl_en)
		tbl->dword0 |= cpu_to_le32(FWCMD_H2C_FW_FLOW_CTRL_FW_FLOW_CONTROL_EN);

	tbl->dword0 |=
	cpu_to_le32(SET_WORD(info->macid_nums, FWCMD_H2C_FW_FLOW_CTRL_MACID_NUMS));

	tbl->dword1 =
	cpu_to_le32(SET_WORD(info->macid[0], FWCMD_H2C_FW_FLOW_CTRL_MACID_U0) |
			SET_WORD(info->flow_type[0], FWCMD_H2C_FW_FLOW_CTRL_FLOW_TYPE_U0) |
			SET_WORD(info->max_interval[0], FWCMD_H2C_FW_FLOW_CTRL_MAX_TX_INTERVAL_U0) |
			SET_WORD(info->min_interval[0], FWCMD_H2C_FW_FLOW_CTRL_MIN_TX_INTERVAL_U0) |
			SET_WORD(info->target_throughput[0],
				 FWCMD_H2C_FW_FLOW_CTRL_TARGET_THROUGHPUT_U0));

	tbl->dword2 =
	cpu_to_le32(SET_WORD(info->macid[1], FWCMD_H2C_FW_FLOW_CTRL_MACID_U1) |
			SET_WORD(info->flow_type[1], FWCMD_H2C_FW_FLOW_CTRL_FLOW_TYPE_U1) |
			SET_WORD(info->max_interval[1], FWCMD_H2C_FW_FLOW_CTRL_MAX_TX_INTERVAL_U1) |
			SET_WORD(info->min_interval[1], FWCMD_H2C_FW_FLOW_CTRL_MIN_TX_INTERVAL_U1) |
			SET_WORD(info->target_throughput[1],
				 FWCMD_H2C_FW_FLOW_CTRL_TARGET_THROUGHPUT_U1));

	tbl->dword3 =
	cpu_to_le32(SET_WORD(info->macid[2], FWCMD_H2C_FW_FLOW_CTRL_MACID_U2) |
			SET_WORD(info->flow_type[2], FWCMD_H2C_FW_FLOW_CTRL_FLOW_TYPE_U2) |
			SET_WORD(info->max_interval[2], FWCMD_H2C_FW_FLOW_CTRL_MAX_TX_INTERVAL_U2) |
			SET_WORD(info->min_interval[2], FWCMD_H2C_FW_FLOW_CTRL_MIN_TX_INTERVAL_U2) |
			SET_WORD(info->target_throughput[2],
				 FWCMD_H2C_FW_FLOW_CTRL_TARGET_THROUGHPUT_U2));

	tbl->dword4 =
	cpu_to_le32(SET_WORD(info->macid[3], FWCMD_H2C_FW_FLOW_CTRL_MACID_U3) |
			SET_WORD(info->flow_type[3], FWCMD_H2C_FW_FLOW_CTRL_FLOW_TYPE_U3) |
			SET_WORD(info->max_interval[3], FWCMD_H2C_FW_FLOW_CTRL_MAX_TX_INTERVAL_U3) |
			SET_WORD(info->min_interval[3], FWCMD_H2C_FW_FLOW_CTRL_MIN_TX_INTERVAL_U3) |
			SET_WORD(info->target_throughput[3],
				 FWCMD_H2C_FW_FLOW_CTRL_TARGET_THROUGHPUT_U3));

	tbl->dword5 =
	cpu_to_le32(SET_WORD(info->macid[4], FWCMD_H2C_FW_FLOW_CTRL_MACID_U4) |
			SET_WORD(info->flow_type[4], FWCMD_H2C_FW_FLOW_CTRL_FLOW_TYPE_U4) |
			SET_WORD(info->max_interval[4], FWCMD_H2C_FW_FLOW_CTRL_MAX_TX_INTERVAL_U4) |
			SET_WORD(info->min_interval[4], FWCMD_H2C_FW_FLOW_CTRL_MIN_TX_INTERVAL_U4) |
			SET_WORD(info->target_throughput[4],
				 FWCMD_H2C_FW_FLOW_CTRL_TARGET_THROUGHPUT_U4));

	tbl->dword6 =
	cpu_to_le32(SET_WORD(info->macid[5], FWCMD_H2C_FW_FLOW_CTRL_MACID_U5) |
			SET_WORD(info->flow_type[5], FWCMD_H2C_FW_FLOW_CTRL_FLOW_TYPE_U5) |
			SET_WORD(info->max_interval[5], FWCMD_H2C_FW_FLOW_CTRL_MAX_TX_INTERVAL_U5) |
			SET_WORD(info->min_interval[5], FWCMD_H2C_FW_FLOW_CTRL_MIN_TX_INTERVAL_U5) |
			SET_WORD(info->target_throughput[5],
				 FWCMD_H2C_FW_FLOW_CTRL_TARGET_THROUGHPUT_U5));

	tbl->dword7 =
	cpu_to_le32(SET_WORD(info->macid[6], FWCMD_H2C_FW_FLOW_CTRL_MACID_U6) |
			SET_WORD(info->flow_type[6], FWCMD_H2C_FW_FLOW_CTRL_FLOW_TYPE_U6) |
			SET_WORD(info->max_interval[6], FWCMD_H2C_FW_FLOW_CTRL_MAX_TX_INTERVAL_U6) |
			SET_WORD(info->min_interval[6], FWCMD_H2C_FW_FLOW_CTRL_MIN_TX_INTERVAL_U6) |
			SET_WORD(info->target_throughput[6],
				 FWCMD_H2C_FW_FLOW_CTRL_TARGET_THROUGHPUT_U6));

	tbl->dword8 =
	cpu_to_le32(SET_WORD(info->macid[7], FWCMD_H2C_FW_FLOW_CTRL_MACID_U7) |
			SET_WORD(info->flow_type[7], FWCMD_H2C_FW_FLOW_CTRL_FLOW_TYPE_U7) |
			SET_WORD(info->max_interval[7], FWCMD_H2C_FW_FLOW_CTRL_MAX_TX_INTERVAL_U7) |
			SET_WORD(info->min_interval[7], FWCMD_H2C_FW_FLOW_CTRL_MIN_TX_INTERVAL_U7) |
			SET_WORD(info->target_throughput[7],
				 FWCMD_H2C_FW_FLOW_CTRL_TARGET_THROUGHPUT_U7));
...
u32 mac_txmode_switch(struct mac_ax_adapter *adapter, u8 swtx, u8 chain_count)
{
	u32 ret = 0;
	struct h2c_info h2c_info = {0};
	u32 write_ptr;

	write_ptr = cpu_to_le32((swtx ? FWCMD_H2C_TXMODE_SWITCH_MODE : 0));
	write_ptr |=
	cpu_to_le32(SET_WORD(chain_count,
			     FWCMD_H2C_TXMODE_SWITCH_SW_MODE_MAX_CHAIN_NUM));

	h2c_info.agg_en = 0;
	h2c_info.content_len = sizeof(u32);
	h2c_info.h2c_cat = FWCMD_H2C_CAT_MAC;
	h2c_info.h2c_class = FWCMD_H2C_CL_FR_EXCHG;
	h2c_info.h2c_func = FWCMD_H2C_FUNC_TXMODE_SWITCH;
	h2c_info.rec_ack = 0;
	h2c_info.done_ack = 0;

	ret = mac_h2c_common(adapter, &h2c_info, (u32 *)&write_ptr);
	if (swtx)
		PLTFM_MSG_TRACE("Tx mode switched to [SW] by h2c\n");
	else
		PLTFM_MSG_TRACE("Tx mode switched to [HW] by h2c\n");

	return ret;
}

u32 mac_mru_grp_upd(struct mac_ax_adapter *adapter,
		    struct mac_ax_mru_grp_info *grp_info)
{
	u32 ret = 0;
	u8 index = 0;
	u8 ac = 0;
	u16 ac_bsr_sel[MAC_AX_CMAC_AC_SEL_MAX] = {0};

	struct h2c_info h2c_info = {0};
	struct fwcmd_mru_grp_upd *para;

	h2c_info.agg_en = 0;
	h2c_info.content_len = sizeof(struct fwcmd_mru_grp_upd);
	h2c_info.h2c_cat = FWCMD_H2C_CAT_MAC;
	h2c_info.h2c_class = FWCMD_H2C_CL_MEDIA_RPT;
	h2c_info.h2c_func = FWCMD_H2C_FUNC_MRU_GRP_UPD;
	h2c_info.rec_ack = 0;
	h2c_info.done_ack = 0;

	para = (struct fwcmd_mru_grp_upd *)PLTFM_MALLOC(h2c_info.content_len);
	if (!para)
		return MACNPTR;
	PLTFM_MEMSET(para, 0, sizeof(struct fwcmd_mru_grp_upd));

	para->dword0 |=
		cpu_to_le32((grp_info->dl_ul ? FWCMD_H2C_MRU_GRP_UPD_UL_DL : 0) |
			    (grp_info->grp_vld ? FWCMD_H2C_MRU_GRP_UPD_GRP_VALID : 0) |
			    (grp_info->is_hw_grp ? FWCMD_H2C_MRU_GRP_UPD_IS_HWGRP : 0) |
			    (grp_info->mru ? FWCMD_H2C_MRU_GRP_UPD_MRU : 0));

	para->dword0 |=
		cpu_to_le32(SET_WORD(grp_info->band, FWCMD_H2C_MRU_GRP_UPD_BAND) |
			    SET_WORD(grp_info->sta_cnt, FWCMD_H2C_MRU_GRP_UPD_STA_CNT) |
			    SET_WORD(grp_info->grp_id, FWCMD_H2C_MRU_GRP_UPD_GPR_ID));

	para->dword1 |=
		cpu_to_le32(SET_WORD(grp_info->macid[0], FWCMD_H2C_MRU_GRP_UPD_MACID_U0) |
			    SET_WORD(grp_info->macid[1], FWCMD_H2C_MRU_GRP_UPD_MACID_U1));

	para->dword2 |=
		cpu_to_le32(SET_WORD(grp_info->macid[2], FWCMD_H2C_MRU_GRP_UPD_MACID_U2) |
			    SET_WORD(grp_info->macid[3], FWCMD_H2C_MRU_GRP_UPD_MACID_U3));

	para->dword3 |=
		cpu_to_le32(SET_WORD(grp_info->macid[4], FWCMD_H2C_MRU_GRP_UPD_MACID_U4) |
			    SET_WORD(grp_info->macid[5], FWCMD_H2C_MRU_GRP_UPD_MACID_U5));

	para->dword4 |=
		cpu_to_le32(SET_WORD(grp_info->macid[6], FWCMD_H2C_MRU_GRP_UPD_MACID_U6) |
			    SET_WORD(grp_info->macid[7], FWCMD_H2C_MRU_GRP_UPD_MACID_U7));

	para->dword5 |=
		cpu_to_le32(SET_WORD(grp_info->macid[8], FWCMD_H2C_MRU_GRP_UPD_MACID_U8) |
			    SET_WORD(grp_info->macid[9], FWCMD_H2C_MRU_GRP_UPD_MACID_U9));

	para->dword6 |=
		cpu_to_le32(SET_WORD(grp_info->macid[10], FWCMD_H2C_MRU_GRP_UPD_MACID_U10) |
			    SET_WORD(grp_info->macid[11], FWCMD_H2C_MRU_GRP_UPD_MACID_U11));

	para->dword7 |=
		cpu_to_le32(SET_WORD(grp_info->macid[12], FWCMD_H2C_MRU_GRP_UPD_MACID_U12) |
			    SET_WORD(grp_info->macid[13], FWCMD_H2C_MRU_GRP_UPD_MACID_U13));

	para->dword8 |=
		cpu_to_le32(SET_WORD(grp_info->macid[14], FWCMD_H2C_MRU_GRP_UPD_MACID_U14) |
			    SET_WORD(grp_info->macid[15], FWCMD_H2C_MRU_GRP_UPD_MACID_U15));

	for (ac = MAC_AX_CMAC_AC_SEL_BE ; ac < MAC_AX_CMAC_AC_SEL_MAX; ac++) {
		for (index = 0 ; index < grp_info->sta_cnt ; index++) {
			if (grp_info->ul_bsr_sel[index][ac] == 1)
				ac_bsr_sel[ac] |= BIT(index);
		}
	}

	para->dword9 |= (u32)ac_bsr_sel[MAC_AX_CMAC_AC_SEL_BE];
	para->dword9 |= (u32)ac_bsr_sel[MAC_AX_CMAC_AC_SEL_BK] << 16;
	para->dword10 |= (u32)ac_bsr_sel[MAC_AX_CMAC_AC_SEL_VI];
	para->dword10 |= (u32)ac_bsr_sel[MAC_AX_CMAC_AC_SEL_VO] << 16;

	para->dword11 |=
		cpu_to_le32(SET_WORD(grp_info->trigger_len, FWCMD_H2C_MRU_GRP_UPD_TRIGGER_LEN) |
			    SET_WORD(grp_info->trigger_threshold,
				     FWCMD_H2C_MRU_GRP_UPD_TRIGGER_UL_THRESHOLD));
