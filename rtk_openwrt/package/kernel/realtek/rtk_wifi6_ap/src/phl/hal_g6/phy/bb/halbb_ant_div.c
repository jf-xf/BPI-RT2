...
void halbb_antdiv_set_ant(struct bb_info *bb, u8 ant)
{
	...
	if ((bb_ant_div->pre_target_ant != ant) || (bb_ant_div->training_ant != ant) ) {
		...
		if(ant == MAIN_ANT) {
			...
			} else if (bb->ic_sub_type == BB_IC_SUB_TYPE_8192XB_8192XB && dev->rfe_type > 50) {
...
void halbb_antdiv_get_highest_mcs_new(struct bb_info *bb)
{
	...
	struct bb_rate_info *rate_i = &cmn_rpt->bb_rate_i;
	struct bb_antdiv_rate_info *bb_rate_i = &bb_ant_div->bb_rate_i;
	enum channel_width main_max_bw;
	enum channel_width aux_max_bw;

	u32 main_max_tp_160 = 0, main_max_tp_80 = 0, main_max_tp_40 = 0, main_max_tp_20 = 0;
	u32 aux_max_tp_160 = 0, aux_max_tp_80 = 0, aux_max_tp_40 = 0, aux_max_tp_20 = 0;
	u16 main_max_cnt_160 = 1, main_max_cnt_80 = 1, main_max_cnt_40 = 1, main_max_cnt_20 = 1;
	u16 aux_max_cnt_160 = 1, aux_max_cnt_80 = 1, aux_max_cnt_40 = 1, aux_max_cnt_20 = 1;
	u16 main_max_idx_160 = 0, main_max_idx_80 = 0, main_max_idx_40 = 0, main_max_idx_20 = 0;
	u16 aux_max_idx_160 = 0, aux_max_idx_80 = 0, aux_max_idx_40 = 0, aux_max_idx_20 = 0;
	u16 main_max_cnt = 1;
	u16 aux_max_cnt = 1;
	u16 main_max_idx = 0;
	u16 aux_max_idx = 0;
	...
	enum channel_width CHANNEL_WIDTH[4] = {CHANNEL_WIDTH_20, CHANNEL_WIDTH_40, CHANNEL_WIDTH_80, CHANNEL_WIDTH_160};
	...
	Cnt_info cnt_info[4] = {
		{&main_max_tp_20, &aux_max_tp_20, &main_max_cnt_20, &aux_max_cnt_20, &main_max_idx_20, &aux_max_idx_20},
		{&main_max_tp_40, &aux_max_tp_40, &main_max_cnt_40, &aux_max_cnt_40, &main_max_idx_40, &aux_max_idx_40},
		{&main_max_tp_80, &aux_max_tp_80, &main_max_cnt_80, &aux_max_cnt_80, &main_max_idx_80, &aux_max_idx_80},
		{&main_max_tp_160, &aux_max_tp_160, &main_max_cnt_160, &aux_max_cnt_160, &main_max_idx_160, &aux_max_idx_160}
	};
	...
	if ((bb_rate_i->main_he_pkt_not_zero == true) || (bb_rate_i->aux_he_pkt_not_zero == true)) {
		for (i = 0; i < HE_RATE_NUM; i++) {
			COUNT_MAIN_HE[0][i] = bb_rate_i->main_pkt_cnt_he_sc20[i];
			COUNT_MAIN_HE[1][i] = bb_rate_i->main_pkt_cnt_he_sc40[i];
			COUNT_MAIN_HE[2][i] = bb_rate_i->main_pkt_cnt_he_sc80[i];
			COUNT_MAIN_HE[3][i] = bb_rate_i->main_pkt_cnt_he[i];
			COUNT_AUX_HE[0][i] = bb_rate_i->aux_pkt_cnt_he_sc20[i];
			COUNT_AUX_HE[1][i] = bb_rate_i->aux_pkt_cnt_he_sc40[i];
			COUNT_AUX_HE[2][i] = bb_rate_i->aux_pkt_cnt_he_sc80[i];
			COUNT_AUX_HE[3][i] = bb_rate_i->aux_pkt_cnt_he[i];

			if (bb_ant_div->tp_decision_method == TP_MAX_DOMINATION) {
				for(j = 0; j < 4; j++) {
					if (COUNT_MAIN_HE[j][i] >= main_max_cnt) {
						main_max_cnt = COUNT_MAIN_HE[j][i];
						main_max_idx = i;
						main_max_bw = CHANNEL_WIDTH[j];
					}
					if (COUNT_AUX_HE[j][i] >= aux_max_cnt) {
						aux_max_cnt = COUNT_AUX_HE[j][i];
						aux_max_idx = i;
						aux_max_bw = CHANNEL_WIDTH[j];
					}
				}
				BB_DBG(bb, DBG_ANT_DIV,
				...
				for(j = 0; j < 4; j++) {
					if (COUNT_MAIN_HE[j][i] >= bb_ant_div->tp_lb) {
						*(cnt_info[j].sub_main_max_cnt) = COUNT_MAIN_HE[j][i];
						*(cnt_info[j].sub_main_max_idx) = i;
						*(cnt_info[j].sub_main_max_tp) = 
							MULTIPLIERS_HE[j] * bb_phy_rate_table[i + LEGACY_RATE_NUM];
						main_max_bw = CHANNEL_WIDTH[j];
					}
					if (COUNT_AUX_HE[j][i] >= bb_ant_div->tp_lb) {
						*(cnt_info[j].sub_aux_max_cnt) = COUNT_AUX_HE[j][i];
		...
		for (i = 0; i < VHT_RATE_NUM; i++) {
			COUNT_MAIN_VHT[0][i] = bb_rate_i->main_pkt_cnt_vht_sc20[i];
			COUNT_MAIN_VHT[1][i] = bb_rate_i->main_pkt_cnt_vht_sc40[i];
			COUNT_MAIN_VHT[2][i] = bb_rate_i->main_pkt_cnt_vht_sc80[i];
			COUNT_MAIN_VHT[3][i] = bb_rate_i->main_pkt_cnt_vht[i];
			COUNT_AUX_VHT[0][i] = bb_rate_i->aux_pkt_cnt_vht_sc20[i];
			COUNT_AUX_VHT[1][i] = bb_rate_i->aux_pkt_cnt_vht_sc40[i];
			COUNT_AUX_VHT[2][i] = bb_rate_i->aux_pkt_cnt_vht_sc80[i];
			COUNT_AUX_VHT[3][i] = bb_rate_i->aux_pkt_cnt_vht[i];

			if (bb_ant_div->tp_decision_method == TP_MAX_DOMINATION) {
				for(j = 0; j < 4; j++) {
					if (COUNT_MAIN_VHT[j][i] >= main_max_cnt) {
						main_max_cnt = COUNT_MAIN_VHT[j][i];
						main_max_idx = i;
						main_max_bw = CHANNEL_WIDTH[j];
					}
					if (COUNT_AUX_VHT[j][i] >= aux_max_cnt) {
						aux_max_cnt = COUNT_AUX_VHT[j][i];
						aux_max_idx = i;
						aux_max_bw = CHANNEL_WIDTH[j];
					}
				}
				...
				for(j = 0; j < 4; j++) {
					if (COUNT_MAIN_VHT[j][i] >= bb_ant_div->tp_lb) {
						*(cnt_info[j].sub_main_max_cnt) = COUNT_MAIN_VHT[j][i];
						*(cnt_info[j].sub_main_max_idx) = i;
						*(cnt_info[j].sub_main_max_tp) = MULTIPLIERS_VHT[j] * bb_phy_rate_table[i + LEGACY_RATE_NUM];
					}
					if (COUNT_AUX_VHT[j][i] >= bb_ant_div->tp_lb) {
						*(cnt_info[j].sub_aux_max_cnt) = COUNT_AUX_VHT[j][i];
						*(cnt_info[j].sub_aux_max_idx) = i;
		...
		for (i = 0; i < HT_RATE_NUM; i++) {
			COUNT_MAIN_HT[0][i] = bb_rate_i->main_pkt_cnt_ht_sc20[i];
			COUNT_MAIN_HT[1][i] = bb_rate_i->main_pkt_cnt_ht[i];
			COUNT_AUX_HT[0][i] = bb_rate_i->aux_pkt_cnt_ht_sc20[i];
			COUNT_AUX_HT[1][i] = bb_rate_i->aux_pkt_cnt_ht[i];

			if (bb_ant_div->tp_decision_method == TP_MAX_DOMINATION) {
				for(j = 0; j < 2; j++) {
					if (COUNT_MAIN_HT[j][i] >= main_max_cnt) {
						main_max_cnt = COUNT_MAIN_HT[j][i];
						main_max_idx = i;
						main_max_bw = CHANNEL_WIDTH[j];
					}
					if (COUNT_AUX_HT[j][i] >= aux_max_cnt) {
						aux_max_cnt = COUNT_AUX_HT[j][i];
						aux_max_idx = i;
						aux_max_bw = CHANNEL_WIDTH[j];
					}
				}
				...
				for(j = 0; j < 2; j++) {
					if (COUNT_MAIN_HT[j][i] >= bb_ant_div->tp_lb) {
						*(cnt_info[j].sub_main_max_cnt) = COUNT_MAIN_HT[j][i];
						*(cnt_info[j].sub_main_max_idx) = i;
						*(cnt_info[j].sub_main_max_tp) = MULTIPLIERS_HT[j] * bb_phy_rate_table[i + LEGACY_RATE_NUM];
					}
					if (COUNT_AUX_HT[j][i] >= bb_ant_div->tp_lb) {
						*(cnt_info[j].sub_aux_max_cnt) = COUNT_AUX_HT[j][i];
						*(cnt_info[j].sub_aux_max_idx) = i;
	...
	if (bb_ant_div->tp_decision_method == TP_HIGHEST_DOMINATION) {
		if((main_max_tp_160 >= main_max_tp_80) && (main_max_tp_160 >= main_max_tp_40) && (main_max_tp_160 >= main_max_tp_20)){
			main_max_cnt = main_max_cnt_160;
			main_max_idx = main_max_idx_160;
		}else if((main_max_tp_80 >= main_max_tp_40) && ((main_max_tp_80 >= main_max_tp_20))){
			main_max_cnt = main_max_cnt_80;
			main_max_idx = main_max_idx_80;
		}else if((main_max_tp_40 >= main_max_tp_20)){
			main_max_cnt = main_max_cnt_40;
			main_max_idx = main_max_idx_40;
		}else{
			main_max_cnt = main_max_cnt_20;
			main_max_idx = main_max_idx_20;
		}
		if((aux_max_tp_160 >= aux_max_tp_80) && (aux_max_tp_160 >= aux_max_tp_40) && (aux_max_tp_160 >= aux_max_tp_20)){
			aux_max_cnt = aux_max_cnt_160;
			aux_max_idx = aux_max_idx_160;
		}else if((aux_max_tp_80 >= aux_max_tp_40) && ((aux_max_tp_80 >= aux_max_tp_20))){
			aux_max_cnt = aux_max_cnt_80;
			aux_max_idx = aux_max_idx_80;
		}else if((aux_max_tp_40 >= aux_max_tp_20)){
			aux_max_cnt = aux_max_cnt_40;
			aux_max_idx = aux_max_idx_40;
		}else{
			aux_max_cnt = aux_max_cnt_20;
			aux_max_idx = aux_max_idx_20;
		}
	}
	/* Compute all throughput*/
	if (bb_ant_div->tp_decision_method == TP_AVG_DOMINATION) {
