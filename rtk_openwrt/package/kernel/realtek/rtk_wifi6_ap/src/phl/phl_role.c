				...
				enum wr_chg_id chg_id, void *chg_info)
{
	...
	break;
	case WR_CHG_BCN_IGNORE_EDCCA:
	{
		u8 enable = *(u8 *)chg_info;

		hstate = rtw_hal_set_beacon_ignore_edcca(phl_info->hal, wrole, enable);
		if (hstate == RTW_HAL_STATUS_SUCCESS)
			pstate = RTW_PHL_STATUS_SUCCESS;
	}
	break;
...
rtw_phl_mac_sifs_chk_cca_en(void *phl, struct rtw_wifi_role_t *wrole, u8 en)
{
	...
	if (wrole) {
		phl_status = rtw_hal_mac_sifs_chk_cca_en(hal_com, wrole->hw_band, en);
	}
...
rtw_phl_mac_resp_chk_cca(void *phl, struct rtw_wifi_role_t *wrole, u8 en)
{
	...
	if (wrole) {
		phl_status = rtw_hal_mac_resp_chk_cca(hal_com, wrole->hw_band, en);
	}
