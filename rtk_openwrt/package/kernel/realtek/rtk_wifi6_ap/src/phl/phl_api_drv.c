...
u8 rtw_phl_get_xtxr(void *phl_in){
	struct phl_info_t *phl = (struct phl_info_t *)phl_in;
	struct hal_info_t *hal_info = (struct hal_info_t *)phl->hal;
	struct rtw_hal_com_t *hal_com = rtw_hal_get_halcom(hal_info);
	return hal_com->def_rfpath_tx;
}
