				...
				struct rtw_rx_buf *rx_buf)
{
	...
	#if defined(PHL_DMA_NONCOHERENT) || defined(PLATFORM_ARM_98F_FT2)
	#if !defined(HAL_TO_NONCACHE_ADDR) || defined(PLATFORM_ARM_98F_FT2)
	/* Bidirectional $ WB to clear RX buf and invalidate $ */
	if (rx_buf->cache) {
		#if defined(CONFIG_RTW_VM_CACHE_HANDLING) && !defined(PLATFORM_ARM_98F_FT2)
...
_phl_alloc_txbd_pcie(struct phl_info_t *phl_info, u8 ch_num)
{
	...
	if (NULL != txbd) {
		for (i = 0; i < ch_num; i++) {
			...
			_os_spinlock_init(phl_to_drvpriv(phl_info), &txbd[i].txbd_lock);
...
static void _phl_tx_callback_pcie(void *context)
{
	...
	void *drvpriv = phl_to_drvpriv(phl_info);
	struct dvobj_priv *dvobj = (struct dvobj_priv *)drvpriv;
	_os_list sta_list;
	bool tx_pause = false;
	...
	if (true == tx_pause)
		goto end;
	else if (dvobj->ser_L2_inprogress == 1){
		PHL_PRINT("\n\n\n !!! SER L2 inprogress(%d), phl_sw_tx_sts:%d, chk_stop !!!\n\n\n",
			dvobj->ser_L2_inprogress, _os_atomic_read(drvpriv, &phl_info->phl_sw_tx_sts));
		goto end;
	}
	...
	/* for latency sta */
	if (true == phl_check_xmit_ring_resource(phl_info, &sta_list, 1)) {
	...
	/* for none-latency sta */
	if (true == phl_check_xmit_ring_resource(phl_info, &sta_list, 0)) {
		_phl_tx_flow_ctrl_pcie(phl_info, &sta_list);
...
#if defined(PLATFORM_LINUX) && defined(CONFIG_RTW_MEMPOOL)
extern void rtw_init_rxbuf_mempool(struct rtw_phl_com_t *phl_com);
extern void rtw_fini_rxbuf_mempool(struct rtw_phl_com_t *phl_com);
#endif
...
enum rtw_phl_status phl_trx_init_pcie(struct phl_info_t *phl_info)
{
	...
	do {
		...
		phl_info->fw_ptr = 0;
#endif
		/* Enable RX INT mitigation by default */
		#if defined(PCIE_TRX_MIT_EN)
		hci_info->fixed_mitigation = 0;
		#endif
							...
							struct rtw_phl_stainfo_t *phl_sta, struct rtw_phl_pkt_req *phl_pkt_req)
{
	...
	if ((tx_req->treq_type == RTW_PHL_TREQ_TYPE_CORE_TXSC)
		...
		if (!ptxsc->txsc_wd_cached || ptxsc->txsc_wd_seq_offset == 0) {
			PHL_INFO("[txsc][phl] phl_txsc re-update, txsc_wd_cached:%d, txsc_wd_seq_offset:%d\n",
...
bool  _phl_stbc_mapping(struct phl_info_t *phl_info, struct rtw_phl_stainfo_t *psta)
{
	...
	rt_i = &psta->hal_sta->ra_info.rpt_rt_i;
	wrole = psta->wrole;
	asoc_cap_i = &psta->asoc_cap;
	rate = psta->hal_sta->ra_info.rpt_rt_i.mcs_ss_idx;

	/* Driver wifi mode mapping */
	if (psta->wmode & WLAN_MD_11AX) {
		if (!wrole->proto_role_cap.stbc_he_tx){
			stbc_en = false;
		}else if ((asoc_cap_i->stbc_he_rx != 0 && rt_i->bw <= HAL_RATE_BW_80) ||
			(asoc_cap_i->stbc_rx_greater_80mhz!= 0 && rt_i->bw > HAL_RATE_BW_80)){

			if (psta->hal_sta->bf_entry){
				if(((rate + 0x180) >= RTW_DATA_RATE_HE_NSS1_MCS0) && ((rate + 0x180) <= RTW_DATA_RATE_HE_NSS1_MCS2)){
					stbc_en = true;
				}
			}else{
				stbc_en = true;
	...
	} else if (psta->wmode & WLAN_MD_11AC) {
		if (!wrole->proto_role_cap.stbc_vht_tx)
			stbc_en = 0;
		else if (asoc_cap_i->stbc_vht_rx){
			if (psta->hal_sta->bf_entry){
				if(((rate + 0x100) >= RTW_DATA_RATE_VHT_NSS1_MCS0) && ((rate + 0x100) <= RTW_DATA_RATE_VHT_NSS1_MCS2)){
					stbc_en = true;
				}
			}else{
				stbc_en = true;
	...
	} else if  (psta->wmode & WLAN_MD_11N) {
...
phl_prepare_tx_pcie(struct phl_info_t *phl_info, struct rtw_xmit_req *tx_req)
{
	...
	struct rtw_pkt_filter *pfilter = NULL;
#endif
	FUNCIN_WSTS(pstatus);
	do {
		...
		*/
		...
		if (phl_sta && (phl_pkt_req.tx_req->mdata.type == RTW_PHL_PKT_TYPE_DATA)){

			phl_pkt_req.tx_req->mdata.f_stbc = _phl_stbc_mapping(phl_info, phl_sta);

			if ((phl_sta->hal_sta->stbc_info.stbc_disable == true) && phl_pkt_req.tx_req->mdata.f_stbc)
				phl_pkt_req.tx_req->mdata.f_stbc = false;

			if (phl_pkt_req.tx_req->mdata.f_stbc)
				ATOMIC_INC(&phl_sta->wd_stbc_cnt);
		}
...
#endif
#ifdef RTW_CORE_PKT_TRACE
		if(tx_req->pktinfo && tx_req->pktinfo->adapter && tx_req->pktinfo->adapter->pkt_trace_enable)
		{
			pfilter = rtw_ptk_find_filter(tx_req->pktinfo->adapter, &tx_req->pktinfo[i]);
			if(pfilter)
			{
				_dbgdump("[%s %d]: TX:\n",__FUNCTION__,__LINE__);
				rtw_pkt_trace_dump(pfilter, tx_req->pktinfo);
				_dbgdump("### %pX %uB: #################\n", tx_req->pktinfo->ptr, tx_req->pktinfo->pkt_len);
				debug_dump_buf(phl_pkt_req.wd_page, (u16)phl_pkt_req.wd_len, "dump wd page");
				_dbgdump("#############################\n");
			}
		}
...
static void phl_record_dmach_per_sta(void *drv_priv,
				...
				u8 ch, bool increase)
{
	...
	if (map_record != 0xff) {
		if (increase) { _os_atomic_inc(drv_priv, &psta->cur_dma_wd[map_record]); }
		else { _os_atomic_dec(drv_priv, &psta->cur_dma_wd[map_record]); }
	}
}
				...
				u16 txcnt, u8 ch)
{
	...
	if (wrole && wrole->active) {
		...
		phl_list_for_loop_safe(psta, n, struct rtw_phl_stainfo_t,
			...
			for (j = 0; j < 6; j++) {
				wd_ptr = _os_atomic_read(drv_priv, &psta->cur_dma_wd[j]);
				psta->cur_dma_wd_record[j] = wd_ptr;
			}
		}
	...
	while (txcnt > cnt) {
		...
		if (RTW_HAL_STATUS_SUCCESS == hstatus) {
			for (j = 0; j < wd_num; j++) {
				...
				#endif
				phl_record_dmach_per_sta(drv_priv, phl_sta, ch, _TRUE);
				...
				if (ch == 9) {
					if (phl_sta == NULL
						...
						ptr = wd_ring->wp_tag[wd->wp_seq].ptr;
						treq = (struct rtw_xmit_req *)ptr;
		...
		if (RTW_HAL_STATUS_SUCCESS != hstatus) {
			...
			if (wrole && wrole->active) {
				...
				phl_list_for_loop_safe(psta, n, struct rtw_phl_stainfo_t,
					...
					for (j = 0; j < 6; j++) {
						_os_atomic_set(drv_priv, &psta->cur_dma_wd[j], psta->cur_dma_wd_record[j]);
					}
				}
				_os_spinunlock(drv_priv, &wrole->assoc_sta_queue.lock, _irq, &sp_flags);
			}
...
static enum rtw_phl_status phl_tx_pcie(struct phl_info_t *phl_info)
{
	...
	for (ch = 0; ch < hci_info->total_txch_num; ch++) {
		...
		if (0 == hw_res) {
			...
			if ((ch == 3) && txcnt < phl_info->vi_lowest_pkt_thr)
				continue;
#endif
					 ...
					 struct rtw_phl_rx_pkt **pphl_rx)
{
	...
	do {
		...
		#endif /* CONFIG_RTW_HW_RX_AMSDU_CUT */
		{
			...
			if (RTW_HAL_STATUS_SUCCESS != hstatus) {
				if (ser_m0_print_once == 1) {
					PHL_ERR("\n\n!!! %s: hdl_rx_buf sw_rx_sts:%u sts:%u\n\n", __FUNCTION__,
						_os_atomic_read(drvpriv,&phl_info->phl_sw_rx_sts), hstatus);
					ser_m0_print_once++;
				}
				goto drop;
			} else {
				if (ser_m0_print_once > 1) {
					PHL_ERR("\n\n!!! %s: hdl_rx_buf has print %d times, now reset\n\n",
						__FUNCTION__, ser_m0_print_once);
					ser_m0_print_once = 1;
				}
			}
		}
			 ...
			 u8 txsts, u8 code)
{
	...
	if (dma_ch == 9) {
		...
		if (phl_sta != NULL
			...
			busy_hiq_cnt = _os_atomic_read(drv_priv, &wrole->hiq_busy_cnt);
			if (busy_hiq_cnt > 0)
			...
			if (wrole->dropping_hiq) {
				if (wrole->dropping_hiq-- > 1) {
					PHL_PRINT("HiQ RC M%u S%u C%u L%u DP%u BzC%u\n", wrole->hw_mbssid,
					          txsts, code, treq->mdata.pktlen, wrole->dropping_hiq, busy_hiq_cnt);
				}
...
#ifdef RTW_CORE_PKT_TRACE
	if(treq && treq->pktinfo && treq->pktinfo->adapter && treq->pktinfo->adapter->pkt_trace_enable)
	{
		for(i = 0; i < treq->trace_pkt_cnt; i++)
		{
			pfilter = rtw_ptk_find_filter(treq->pktinfo->adapter, &treq->pktinfo[i]);
			if(pfilter)
			{
				_dbgdump("[%s %d]: TX status %d\n",__FUNCTION__,__LINE__,txsts);
				rtw_pkt_trace_dump(pfilter, &treq->pktinfo[i]);
				if(treq->pktinfo->adapter->pkt_trace_level == RTW_PKT_DUMP_HEADER)
				{
					rtw_pkt_dump_header(&treq->pktinfo[i]);
				} else if(treq->pktinfo->adapter->pkt_trace_level == RTW_PKT_DUMP_ALL)
				{
					rtw_pkt_dump_all(&treq->pktinfo[i]);
				}
			}
		}
	}
	...
	if (RTW_PHL_TREQ_TYPE_TEST_PATTERN == treq->treq_type) {
		...
		if (phl_sta && (treq->mdata.type == RTW_PHL_PKT_TYPE_DATA) && treq->mdata.f_stbc && ATOMIC_READ(&phl_sta->wd_stbc_cnt)){
			ATOMIC_DEC(&phl_sta->wd_stbc_cnt);

			if ((phl_sta->hal_sta->stbc_info.stbc_disable == true) && ( ATOMIC_READ(&phl_sta->wd_stbc_cnt) == 0)){
				rtw_phl_bb_ra_stbc_status(phl_info, phl_sta);
			}
		}
		if (NULL == ops->tx_recycle)
			goto end;
	...
	if (RTW_PHL_STATUS_SUCCESS != sts) {
		...
		#endif
		phl_record_dmach_per_sta(drv_priv, phl_sta, dma_ch, _FALSE);
		_os_spinlock(phl_to_drvpriv(phl_info),
			     &wd_ring[dma_ch].wp_tag_lock,
			     _bh, NULL);
...
void phl_get_dmach_wd_per_sta(void *phl, void *m) {
	struct phl_info_t *phl_info = (struct phl_info_t *)phl;
	void *drv_priv = phl_to_drvpriv(phl_info);
	...
	struct rtw_phl_stainfo_t *psta, *n;
	_os_spinlockfg sp_flags;
	u16 dma_ch[6] = {0};

	RTW_PRINT_SEL(m, "======= Dump STA dma_ch wd cnt =======\n");
	_os_spinlock(drv_priv, &wrole->assoc_sta_queue.lock, _irq, &sp_flags);
	phl_list_for_loop_safe(psta, n, struct rtw_phl_stainfo_t,
					&wrole->assoc_sta_queue.queue, list) {
		dma_ch[0] = _os_atomic_read(drv_priv, &psta->cur_dma_wd[0]);
		dma_ch[1] = _os_atomic_read(drv_priv, &psta->cur_dma_wd[1]);
		dma_ch[2] = _os_atomic_read(drv_priv, &psta->cur_dma_wd[2]);
		dma_ch[3] = _os_atomic_read(drv_priv, &psta->cur_dma_wd[3]);
		dma_ch[4] = _os_atomic_read(drv_priv, &psta->cur_dma_wd[4]);
		dma_ch[5] = _os_atomic_read(drv_priv, &psta->cur_dma_wd[5]);
		if (psta->macid) {
			RTW_PRINT_SEL(m, "      macid[%d] {ACH0, ACH1, ACH2, ACH3, ACH8, ACH9} = {%d, %d, %d, %d, %d, %d}\n",
				psta->macid, dma_ch[0], dma_ch[1], dma_ch[2],dma_ch[3], dma_ch[4], dma_ch[5]);
		}
	}
	_os_spinunlock(drv_priv, &wrole->assoc_sta_queue.lock, _irq, &sp_flags);
