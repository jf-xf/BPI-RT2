...
void rtw_phl_reset_hw_cnt_tx(void *phl)
{
	...
	if (rpt_sts) {
		for(i = 0; i < txch_num; i++) {
			memset(&rpt_sts[i], 0, sizeof(struct rtw_wp_rpt_stats));
		}

		for (i = 0; i < HW_BAND_MAX; i++) {
			mgmt_ch = rtw_hal_tx_chnl_mapping(phl_info->hal, 0,
				RTW_PHL_RING_CAT_MGNT, i);
			memset(&rpt_sts[mgmt_ch], 0, sizeof(struct rtw_wp_rpt_stats));
		}
	}
	hal_com->trx_stat.wp_polluted = 0;
...
void rtw_phl_set_manual_giltf(void *phl, struct rtw_phl_stainfo_t *phl_sta, u8 ax_sgi)
{
	...
	hal_sta = phl_sta->hal_sta;
	if (!hal_sta) {
		PHL_ERR("Error GILTF setup !!! Pointer is NULL !!!\n");
		return;
	}

	if(ax_sgi > 5) {
		PHL_ERR("Error GILTF setup !!! GI out of range !!!\n");
		return;
	}

	hal_sta->ra_info.fix_giltf_en = 1;
	hal_sta->ra_info.cal_giltf = ax_sgi;
}
...
{
	struct phl_info_t *phl_info = (struct phl_info_t *)phl;
	if (rtw_hal_update_sta_entry(phl_info->hal, phl_sta, true) != RTW_HAL_STATUS_SUCCESS)
		PHL_WARN("rtw_hal_update_sta_entry failed!\n");
}
#endif
...
u8 rtw_phl_set_fw_latency_flow_ctrl(void *phl, u16 macid, u8 en, u32 max_int)
{
	...
	sts = rtw_hal_mac_set_fw_flow_ctrl(phl_info->hal, macid, en, max_int);
