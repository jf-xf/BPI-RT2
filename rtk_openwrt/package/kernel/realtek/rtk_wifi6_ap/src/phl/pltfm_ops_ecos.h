 ...
 *
 *****************************************************************************/
#ifndef _PLTFM_OPS_ECOS_H_
#define _PLTFM_OPS_ECOS_H_
#include "drv_types.h"
#include <stdarg.h>
...
static inline void __os_cache_inv(void *d, u32 *bus_addr_l, u32 *bus_addr_h,
...
static inline void __os_cache_wback(void *d, u32 *bus_addr_l, u32 *bus_addr_h,
                                   u32 buf_sz, u8 direction)
{
	struct dvobj_priv *pobj = (struct dvobj_priv *)d;
	PPCI_DATA pci_data = dvobj_to_pci(pobj);
...
#define _os_cache_inv(d, bus_addr_l, bus_addr_h, buf_sz, direction) \
	({ \
		barrier(); \
		__os_cache_inv(d, bus_addr_l, bus_addr_h, buf_sz, direction); \
		barrier(); \
	})

#define _os_cache_wback(d, bus_addr_l, bus_addr_h, buf_sz, direction) \
	({ \
		barrier(); \
		__os_cache_wback(d, bus_addr_l, bus_addr_h, buf_sz, direction); \
		barrier(); \
	})
					   ...
					   void **data_2)
{
	...
	rxb_info->type = 1;
	#ifdef CONFIG_PCI_HCI
	*bus_addr_l_1 = pci_map_single(pdev, skb_1->data, rxb_info->rx_buf_sz,
	                               PCI_DMA_FROMDEVICE);
	#ifdef PHL_DMA_ADDR_64
	...
	*os_priv_1 = skb_1;
	...
	rxb_info->type = 2;
	#ifdef CONFIG_PCI_HCI
	*bus_addr_l_2 = pci_map_single(pdev, skb_2->data, rxb_info->rx_buf_sz,
	                               PCI_DMA_FROMDEVICE);
	#ifdef PHL_DMA_ADDR_64
...
{
	_thread_hdl_ nt = NULL;

	if (thread && thread->thread_handler) {
		nt = (_thread_hdl_)(thread->thread_handler);
	}
	if (nt && nt->thread_handle) {
		cyg_thread_set_priority(nt->thread_handle, PRIO_THREAD_RT);
	} else {
		printk("[%s] set_priority fail!!\n", __func__);
	}
...
{
	extern void __force_update_heartbeat(void);
	__force_update_heartbeat();
}
...
static inline void _os_smp_mb(void)
{
	RTW_SMP_MB();
}

static inline void _os_smp_rmb(void)
{
	RTW_SMP_RMB();
}

static inline void _os_smp_wmb(void)
{
	RTW_SMP_WMB();
}

#endif /*_PLTFM_OPS_ECOS_H_*/
