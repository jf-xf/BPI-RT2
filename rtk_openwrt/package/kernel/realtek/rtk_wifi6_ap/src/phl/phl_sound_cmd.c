...
_phl_snd_cmd_bfer_cmd_proc(struct phl_info_t *phl, struct snd_cmd_bfer *snd_cmd)
{
	...
	u8 role_idx = 0, tmp_reg = 0, success_cnt = 0;
	...
	case SND_CMD_BFER_PRECFG:
	{
		...
		sta_info = rtw_phl_get_stainfo_by_macid(phl, snd_grp->sta[0].macid);

		if ( sta_info  && (ATOMIC_READ(&sta_info->already_inform_txbf_to_fw))) {
			/* avoid to su sounding sts */
			mstatus = MDL_RET_FAIL;
			break;
		}
		...
		_phl_snd_get_available_f2p_idx(phl, snd_grp);
		mstatus = MDL_RET_SUCCESS;
	}
	break;
	...
	struct snd_cmd_bfer *cmd_buf)
{
	...
	case SND_CMD_BFER_POSTCFG:
	{
		sta_info = rtw_phl_get_stainfo_by_macid(phl_info, cmd_buf->snd_grp->sta[0].macid);
		if (snd_obj->is_terminated) {
		...
		} else if(cmd_buf->event == SND_CMD_BFER_PRECFG && sta_info && ATOMIC_READ(&sta_info->already_inform_txbf_to_fw) ) {
			phl_snd_cmd_sound_evt(phl_info, cmd_buf->wrole,
					      cmd_buf->snd_grp,
					      SND_CMD_BFER_CHECK_NEXT);
