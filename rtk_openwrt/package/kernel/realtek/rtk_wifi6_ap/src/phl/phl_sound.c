 ...
 **/
enum rtw_phl_status
rtw_phl_sound_start(void *phl, u8 wrole_idx, u8 st_dlg_tkn, u8 period, u16 test_flag)
{
	enum rtw_phl_status status = RTW_PHL_STATUS_SUCCESS;
	struct phl_info_t *phl_info = (struct phl_info_t *)phl;
	struct phl_sound_obj *snd = (struct phl_sound_obj *)phl_info->snd_obj;
...
enum rtw_phl_status
rtw_phl_check_bf_mu_entry(void *phl, struct rtw_phl_stainfo_t *phl_sta)
{
	struct phl_info_t *phl_info = (struct phl_info_t *)phl;
	enum rtw_phl_status pstatus = RTW_PHL_STATUS_FAILURE;

	if(phl_sta->hal_sta->bf_entry)
		if (rtw_hal_check_bf_mu_entry(phl_info->hal, phl_sta->hal_sta->bf_entry) == RTW_HAL_STATUS_SUCCESS)
...
phl_snd_func_snd_init(struct phl_info_t *phl_info)
{
	...
	u8 mu_grp_id[MAX_SND_MU_STA_NUM][MAX_SND_MU_STA_NUM] = {
		{0xff, 1, 2, 3, 4, 5},
		{16, 0xff,6, 7, 8, 9},
		{17, 21, 0xff, 10, 11, 12},
		{18, 22, 25, 0xff, 13, 14},
	...
	u8 mu_grp_bitmap[MAX_SND_MU_STA_NUM][MAX_SND_MU_STA_NUM] = {
		{0xff, BIT0, BIT1, BIT2, BIT3, BIT4},
		{BIT0, 0xff, BIT1, BIT2, BIT3, BIT4},
		{BIT0, BIT1, 0xff, BIT2, BIT3, BIT4},
		{BIT0, BIT1, BIT2, 0xff, BIT3, BIT4},
		{BIT0, BIT1, BIT2, BIT3, 0xff, BIT4},
		{BIT0, BIT1, BIT2, BIT3, BIT4, 0xff}
	};
	...
	_os_mem_cpy(d, snd->snd_param.fix_param.f_ru_tbl_160, f_ru_tbl_160m,
		    MAX_SND_HE_BFRP_USER_NUM * MAX_SND_HE_BFRP_USER_NUM);
	_os_mem_cpy(d, snd->snd_param.fix_param.mu_grp_id, mu_grp_id,
		    MAX_SND_MU_STA_NUM * MAX_SND_MU_STA_NUM);
	_os_mem_cpy(d, snd->snd_param.fix_param.mu_grp_bitmap, mu_grp_bitmap,
		    MAX_SND_MU_STA_NUM * MAX_SND_MU_STA_NUM);
...
u8 _phl_snd_check_mu_tx_rate(struct rtw_phl_stainfo_t *phl_sta)
{
	...
	switch (phl_sta->hal_sta->ra_info.rpt_rt_i.mode) {
		...
		case HAL_VHT_MODE:
			...
			if((rate + 0x100) == RTW_DATA_RATE_VHT_NSS1_MCS9)
				ret = true;
			break;
		case HAL_HE_MODE:
			//if(((rate + 0x180) >= RTW_DATA_RATE_HE_NSS1_MCS7) && ((rate + 0x180) <= RTW_DATA_RATE_HE_NSS1_MCS11))
			if((rate + 0x180) == RTW_DATA_RATE_HE_NSS1_MCS11)
				ret = true;
			break;
		default:
...
u8 _phl_snd_check_tx_rate(struct rtw_phl_stainfo_t *phl_sta, enum snd_check_type check_type)
{
	u8 ret = false;
	u16 rate;
	...
	rate = phl_sta->hal_sta->ra_info.rpt_rt_i.mcs_ss_idx;
          if (check_type == PHL_SND_CHECK_ADD){
		switch (phl_sta->hal_sta->ra_info.rpt_rt_i.mode) {
			case HAL_LEGACY_MODE:
				ret = false;
				break;
			case HAL_HT_MODE:
				...
				break;
			case HAL_VHT_MODE:
				if(((rate + 0x100) >= RTW_DATA_RATE_VHT_NSS1_MCS0) && ((rate + 0x100) <= RTW_DATA_RATE_VHT_NSS2_MCS7))
					ret = true;
				break;
			case HAL_HE_MODE:
				if(((rate + 0x180) >= RTW_DATA_RATE_HE_NSS1_MCS0) && ((rate + 0x180) <= RTW_DATA_RATE_HE_NSS2_MCS7))
					ret = true;
				break;
			default:
          ...
          }else if (check_type == PHL_SND_CHECK_DEL){
		...
		switch (phl_sta->hal_sta->ra_info.rpt_rt_i.mode) {
			...
			case HAL_HT_MODE:
				...
				break;
			case HAL_VHT_MODE:
				if(((rate + 0x100) >= RTW_DATA_RATE_VHT_NSS1_MCS0) && ((rate + 0x100) <= RTW_DATA_RATE_VHT_NSS2_MCS8))
					ret = true;
				break;
			case HAL_HE_MODE:
				if(((rate + 0x180) >= RTW_DATA_RATE_HE_NSS1_MCS0) && ((rate + 0x180) <= RTW_DATA_RATE_HE_NSS2_MCS8))
					ret = true;
				break;
			default:
...
phl_snd_func_remove_grp_check(struct phl_info_t *phl_info, struct phl_snd_grp *grp)
{
	...
	if(0) {// (PHL_RTYPE_STATION == wrole->type)
	...
	else
	{
		...
		for (idx = 0; idx < grp->num_sta; idx++) {
			...
			{
				if((snd_param->test_flag & PHL_SND_TEST_F_FORCE_TXBF_MU_1SS) && !_phl_snd_check_tx_rate(sta, PHL_SND_CHECK_DEL)){
				...
				if((grp->num_sta != 1) &&  (snd_param->test_flag & PHL_SND_TEST_F_ENABLE_MU_SND)  &&  ((snd_param->test_flag & PHL_SND_TEST_F_GRP_EN_BF_FIX) == 0) &&((rtw_phl_get_sta_rssi(sta) < (phl_info->phl_com->txbf_mu_rssi_thres - 10)) || !_phl_snd_check_mu_tx_rate(sta)))
					{

					PHL_TRACE(COMP_PHL_SOUND, _PHL_ERR_, "%s %d: macid=%d , rssi %d , _phl_snd_check_mu_tx_rate_check == %d\n", __func__,__LINE__,sta->macid,rtw_phl_get_sta_rssi(sta),_phl_snd_check_mu_tx_rate(sta));
					pstatus = RTW_PHL_STATUS_FAILURE;
					goto out;
					}
...
phl_snd_func_remove_grp(struct phl_info_t *phl_info, struct phl_snd_grp *grp)
{
	...
	if (snd_param->grp_used_map & BIT(grp->gidx)) {
		...
		/* Clear F2P index BIT */
		psta = rtw_phl_get_stainfo_by_macid(phl_info, grp->sta[0].macid);

		_phl_snd_remove_f2p_idx(phl_info, psta);
		if (grp->num_sta == 2) {
			pstatus = rtw_phl_mac_mu_del_dl_grp_info(phl_info, grp);
			if (pstatus != RTW_PHL_STATUS_SUCCESS) {
				PHL_TRACE(COMP_PHL_SOUND, _PHL_INFO_, "rtw_phl_mac_mu_del_dl_grp_info fail\n");
			}
		}
...
/**
 * phl_snd_func_remove_su_grp_to_mu()
 * 	remove the su sounding grp if mu condition satisfied;
 */
void
phl_snd_func_remove_su_grp_to_mu(struct phl_info_t *phl_info)
{
	enum rtw_phl_status pstatus = RTW_PHL_STATUS_SUCCESS;
	struct phl_snd_grp *grp = NULL;
	struct phl_sound_obj *snd = (struct phl_sound_obj *)phl_info->snd_obj;
	...
	bool ac_gidx_satisfy_mu_condition[MAX_SND_GRP_NUM];
	bool ax_gidx_satisfy_mu_condition[MAX_SND_GRP_NUM];
	u8 ac_gidx_satisfy_mu_cnt = 0, ax_gidx_satisfy_mu_cnt = 0;

	memset(ac_gidx_satisfy_mu_condition, false, MAX_SND_GRP_NUM);
	memset(ax_gidx_satisfy_mu_condition, false, MAX_SND_GRP_NUM);

	if ((snd_param->test_flag & PHL_SND_TEST_F_ENABLE_MU_SND) && ((snd_param->test_flag & PHL_SND_TEST_F_GRP_EN_BF_FIX) == 0)){

		for(idx = 0; idx < MAX_SND_GRP_NUM; idx++) {
			grp = phl_snd_get_grp_byidx(phl_info, idx);
			if ((grp != NULL) && (grp->num_sta ==1)) {
				sta = rtw_phl_get_stainfo_by_macid(phl_info, grp->sta[0].macid);
				if (grp->snd_type >= PHL_SND_TYPE_HE_HW) {
					if ((ax_gidx_satisfy_mu_cnt < 2) && (rtw_phl_get_sta_rssi(sta) >= phl_info->phl_com->txbf_mu_rssi_thres) && _phl_snd_check_mu_tx_rate(sta)){
						ax_gidx_satisfy_mu_condition[idx] = true;
						ax_gidx_satisfy_mu_cnt++;

					}
				}else {
					if ((ac_gidx_satisfy_mu_cnt < 2) && (rtw_phl_get_sta_rssi(sta) >= phl_info->phl_com->txbf_mu_rssi_thres) && _phl_snd_check_mu_tx_rate(sta)){
						ac_gidx_satisfy_mu_condition[idx] = true;
						ac_gidx_satisfy_mu_cnt++;
					}
				}
			}
		}

		if (ax_gidx_satisfy_mu_cnt == 2){
			for (idx = 0; idx < MAX_SND_GRP_NUM; idx++){
				if (ax_gidx_satisfy_mu_condition[idx] == false)
					continue;
				grp = phl_snd_get_grp_byidx(phl_info, idx);

				pstatus = phl_snd_func_remove_grp(phl_info, grp);
				if (pstatus != RTW_PHL_STATUS_SUCCESS) {
					PHL_TRACE(COMP_PHL_SOUND, _PHL_INFO_, "Remove SND GRP[%d] Fail\n", ax_gidx_satisfy_mu_condition[idx]);
				}
			}
		}

		if (ac_gidx_satisfy_mu_cnt == 2){
			for (idx = 0; idx < MAX_SND_GRP_NUM; idx++){
				if (ac_gidx_satisfy_mu_condition[idx] == false)
					continue;
				grp = phl_snd_get_grp_byidx(phl_info, idx);

				pstatus = phl_snd_func_remove_grp(phl_info, grp);
				if (pstatus != RTW_PHL_STATUS_SUCCESS) {
					PHL_TRACE(COMP_PHL_SOUND, _PHL_INFO_, "Remove SND GRP[%d] Fail\n", ac_gidx_satisfy_mu_condition[idx]);
				}
			}
		}
	}
...
void _phl_snd_get_available_f2p_idx(struct phl_info_t *phl_info, struct phl_snd_grp *grp)
{
	struct phl_sound_obj *snd = (struct phl_sound_obj *)phl_info->snd_obj;
	struct phl_sound_param *param = &snd->snd_param;
	u8 i=0, f2p_idx = MAX_SND_GRP_NUM;
	struct rtw_phl_stainfo_t *psta= NULL;

	if (grp->num_sta == 1){
		for (f2p_idx = 0; f2p_idx < MAX_BW_SU_SND_NUM; f2p_idx++) {
			if (!(param->f2p_used_map & BIT(f2p_idx))) {
				param->f2p_used_map |= BIT(f2p_idx);
				break;
			}
		}
		if (f2p_idx >= MAX_BW_SU_SND_NUM){
			PHL_TRACE(COMP_PHL_SOUND, _PHL_INFO_, "Get SND Grp f2p_idx Fail : f2p_idx >= MAX_BW_SU_SND_NUM\n");
		}
		psta = rtw_phl_get_stainfo_by_macid(phl_info, grp->sta[0].macid);
if (psta)
		psta->hal_sta->f2p_index = f2p_idx;
	}else{
		for (f2p_idx = MAX_BW_SU_SND_NUM; f2p_idx < MAX_SND_GRP_NUM; f2p_idx++) {
			if (!(param->f2p_used_map & BIT(f2p_idx))) {
				param->f2p_used_map |= BIT(f2p_idx);
				break;
			}
		...
		if (f2p_idx >= MAX_SND_GRP_NUM){
			PHL_TRACE(COMP_PHL_SOUND, _PHL_INFO_, "Get SND Grp f2p_idx Fail : f2p_idx >= MAX_SND_GRP_NUM\n");
		}

		for (i = 0; i < grp->num_sta; i++) {
			psta = rtw_phl_get_stainfo_by_macid(phl_info, grp->sta[i].macid);
if (psta)
			psta->hal_sta->f2p_index = f2p_idx;
		}
...
void _phl_snd_remove_f2p_idx(struct phl_info_t *phl_info, struct rtw_phl_stainfo_t *psta)
{
	struct phl_sound_obj *snd = (struct phl_sound_obj *)phl_info->snd_obj;
	struct phl_sound_param *snd_param = &snd->snd_param;
	...
	if (snd_param->f2p_used_map & BIT(psta->hal_sta->f2p_index)) {

		snd_param->f2p_used_map &= ~BIT(psta->hal_sta->f2p_index);
...
phl_snd_func_grouping(struct phl_info_t *phl_info, u8 wroleidx)
{
	...
	if (rtw_phl_role_is_client_category(wrole) && wrole->mstate==MLME_LINKED ) {
		...
		if(!self->hal_sta->ra_info.ra_registered)
		{
			return RTW_PHL_STATUS_FAILURE;
		}

		if((self->asoc_cap.vht_su_bfme==0) && (self->asoc_cap.he_su_bfme==0))  //the associated ap don't support bfee
		{
			return RTW_PHL_STATUS_FAILURE;
		}
		if((snd_param->test_flag & PHL_SND_TEST_F_FORCE_TXBF_MU_1SS) && !_phl_snd_check_tx_rate(self, PHL_SND_CHECK_ADD))
		{
			return RTW_PHL_STATUS_FAILURE;
		}
		if(!(snd_param->test_flag & PHL_SND_TEST_F_DISABLE_TP_LIMIT) && ((self->stats.tx_tp_kbits >> 10) < MIN_SND_TP))
		{
			return RTW_PHL_STATUS_FAILURE;
		}

		//STA already have bf_entry
		if(self->hal_sta->bf_entry != NULL){
			return RTW_PHL_STATUS_SUCCESS;
		}
		...
		phl_list_for_loop(sta, struct rtw_phl_stainfo_t,
			...
			if((snd_param->test_flag & PHL_SND_TEST_F_FORCE_TXBF_MU_1SS) && !_phl_snd_check_tx_rate(sta, PHL_SND_CHECK_ADD))
			{
				continue;
			}
			if(!(snd_param->test_flag & PHL_SND_TEST_F_DISABLE_TP_LIMIT) && ((sta->stats.tx_tp_kbits >> 10) < MIN_SND_TP))
			...
			if (snd_param->test_flag & PHL_SND_TEST_F_ENABLE_MU_SND)
			{
				...
				if(mu_grp_cnt < MAX_SND_MU_GRP_NUM)
				{
					...
					{
						grp = &snd_param->snd_grp[gidx];
						if((grp->num_sta == 1) &&(wrole->id== grp->wrole_idx) )
						{
							...
							{
								if ((snd_param->test_flag & PHL_SND_TEST_F_GRP_EN_BF_FIX) == 0) {
									if ( ((rtw_phl_get_sta_rssi(sta) >= phl_info->phl_com->txbf_mu_rssi_thres) && _phl_snd_check_mu_tx_rate(sta))
										&& ((rtw_phl_get_sta_rssi(p_sta) >= phl_info->phl_com->txbf_mu_rssi_thres) && _phl_snd_check_mu_tx_rate(p_sta))){
											cnt = 1;
											break;
									}
								}else{
			...
			if (is_sta_exist_grp == false){
				if (cnt == 0) {//TXBF or MU first
					#ifdef TX_BEAMFORMING
					if (sta->is_zld_exist_csi  && (snd_param->is_zld_exist_csi_sta_cnt % 2)){
						continue;
						}
					#endif
					gidx = _phl_snd_get_available_gidx(snd);
					...
					if ((snd_param->test_flag & PHL_SND_TEST_F_GRP_EN_BF_FIX)  && (gidx < MAX_SND_GRP_NUM)){
	...
	for (idx = 0; idx < grp->num_sta; idx++) {
		...
		if (sta->hal_sta->bf_entry != NULL) {
			...
			if (RTW_HAL_STATUS_FAILURE == hstatus) {
				...
				_phl_snd_remove_f2p_idx(phl_info, sta);
				qry_bf = true;
		...
		if (true == qry_bf) {
			hstatus = rtw_hal_snd_query_proc_sta_res(
					...
					grp->en_swap_mode);
			if (hstatus != RTW_HAL_STATUS_SUCCESS) {
				PHL_INFO("_phl_snd_proc_get_res_bf: macid 0x%x query sounding resource FAIL \n",
				snd_sta->macid);
				if (grp->en_swap_mode) {
...
phl_snd_cal_mu_grp_bitmap(struct phl_info_t *phl_info, struct phl_snd_grp *grp)
{
	...
	for (i = 0; i < MAX_NUM_STA_SND_GRP; i++) {
		...
		for (j = 0; j < MAX_NUM_STA_SND_GRP; j++) {
			...
			#endif
			psta_info->hal_sta->mugrp_bmp = snd->snd_param.fix_param.mu_grp_bitmap[bfmu_idx][bfmu_idx_tmp];
		}
			       ...
			       struct phl_snd_grp *grp)
{
	...
	u8 *snd_cmd = NULL;
	u8 i = 0;
	struct hal_ax_fwcmd_snd *cmd = NULL;
	...
	do {
		...
		case PHL_SND_TYPE_VHT_SW:
		{
			...
			snd_cmd = rtw_hal_snd_prepare_snd_cmd(phl_info->hal);

			if (snd_cmd == NULL)
				break;

			cmd = (struct hal_ax_fwcmd_snd *)snd_cmd;
			...
			hstatus = rtw_hal_snd_send_fw_cmd(phl_info->hal, (void *)cmd);
			if (hstatus != RTW_HAL_STATUS_SUCCESS) {
				PHL_TRACE(COMP_PHL_SOUND, _PHL_INFO_,
					  "ERROR: rtw_hal_snd_send_fw_cmd Fail!!!!\n");
			}
			/* free cmd buf at last !!! */
			hstatus = rtw_hal_snd_release_snd_cmd(phl_info->hal, (void *)cmd);
		}
		break;
...
#ifdef TX_BEAMFORMING
					|| (sta_info->asoc_cap.trig_su_bfm_fb && (sta_info->chandef.bw > CHANNEL_WIDTH_40))
#endif
					)

						_phl_snd_proc_fw_cmd_he_tb_1sta(phl_info, grp, (void *)cmd);
					else
			       ...
			       struct phl_snd_grp *grp)
{
	...
	u8 mu_entry_idx_0 = 0 , mu_entry_idx_1 = 0;
	struct phl_sound_obj *snd = (struct phl_sound_obj *)phl_info->snd_obj;
	...
	mu_entry_idx_0 = hal_get_sta_mu_entry_idx(phl_info->hal, rtw_phl_get_stainfo_by_macid(phl_info, grp->sta[0].macid));
	mu_entry_idx_1 = hal_get_sta_mu_entry_idx(phl_info->hal, rtw_phl_get_stainfo_by_macid(phl_info, grp->sta[1].macid));

	if ((mu_entry_idx_0 < MAX_SND_MU_STA_NUM) && (mu_entry_idx_1 < MAX_SND_MU_STA_NUM))
	info.grp_id = snd->snd_param.fix_param.mu_grp_id[mu_entry_idx_0][mu_entry_idx_1];

	grp->mu_grp_id = info.grp_id;
...
enum rtw_phl_status
rtw_phl_mac_mu_del_dl_grp_info(void *phl,
			       struct phl_snd_grp *grp)
{
	struct phl_info_t *phl_info = (struct phl_info_t *)phl;
	enum rtw_phl_status pstatus = RTW_PHL_STATUS_FAILURE;
