...
{
	PHL_PRINT("[h2c_stats] idle cmd %d, idle data %d, idle ldata %d, busy h2c %d.\n",
...
void phl_dump_hiq_stuck_stats(struct rtw_phl_com_t *phl_com)
{
	u32 role_idx, busy_hiq_num;
	u8 band_idx = 0;
	...
	for (role_idx = 0; role_idx < MAX_WIFI_ROLE_NUMBER; role_idx++) {
		...
		PHL_PRINT("WROLE[%u][act:%d] bz_hiq_num:%d last_hiq_wp_time:%u dropping_hiq:%u\n",
			role_idx, wrole->active, busy_hiq_num, wrole->last_hiq_wp_time, wrole->dropping_hiq);
	}
}
...
void phl_free_deferred_tx_ring(struct phl_info_t *phl_info)
{
	void *drv_priv = phl_to_drvpriv(phl_info);
	struct rtw_phl_tring_list *phl_tring_list = NULL, *t;
	_os_list *free_list = NULL;
	_os_list ring_list;

	free_list = &phl_info->t_ring_free_list;
	...
	_os_spinlock(drv_priv, &phl_info->t_ring_free_list_lock, _bh, NULL);
	list_splice_init(free_list, &ring_list);
	_os_spinunlock(drv_priv, &phl_info->t_ring_free_list_lock, _bh, NULL);
...
u8 phl_check_xmit_ring_resource(struct phl_info_t *phl_info, _os_list *sta_list, u8 latency)
{
	...
	phl_list_for_loop_safe(tring_list, t, struct rtw_phl_tring_list,
		...
		if(psta){
			if(latency && !psta->is_latency_sta){
				continue;
			}
