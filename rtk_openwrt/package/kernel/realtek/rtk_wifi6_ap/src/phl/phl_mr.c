			       ...
			       struct phl_msg *msg)
{
	...
	if (chctx_num == 1) {
		if (hal_ch != op_chdef.chan) {
			...
			phl_set_ch_bw(phl_info, role->hw_band, &op_chdef, RFK_TYPE_DBCC_EN);
		}
	}
			       ...
			       struct phl_msg *msg)
{
	...
	if (chctx_num == 1) {
		if (hal_ch != op_chdef.chan) {
			PHL_ERR("hw chan(%d) not in op-chan(%d)\n", hal_ch, op_chdef.chan);
			phl_set_ch_bw(phl_info, band_idx, &op_chdef, RFK_TYPE_FORCE_NOT_DO);
		}
	}
			 ...
			 struct phl_msg *msg)
{
	...
	switch(MSG_EVT_ID_FIELD(msg->msg_id)) {
		...
		case MSG_EVT_SCAN_END:
			...
			phl_set_ch_bw(phl_info, role->hw_band, &chandef, RFK_TYPE_SCAN_BK_OP);
