			 ...
			 u32 input_num, char *output, u32 out_len)
{
	...
	do {
		...
		rtw_hal_set_ch_bw(phl_info->hal, (u8)band, &chdef, RFK_TYPE_FORCE_NOT_DO);
		      ...
		      u32 input_num, char *output, u32 out_len)
{
	...
	if (1 == ctrl) {
		sounding_flag = PHL_SND_TEST_F_ONE_TIME|PHL_SND_TEST_F_PASS_STS_CHK|PHL_SND_TEST_F_DISABLE_TP_LIMIT|PHL_SND_TEST_F_ENABLE_MU_SND;
		...
		sounding_flag = PHL_SND_TEST_F_PASS_STS_CHK|PHL_SND_TEST_F_DISABLE_TP_LIMIT|PHL_SND_TEST_F_ENABLE_MU_SND;
		 if (phl_info->phl_com->txbf_mu_fix_mode)
		 	sounding_flag |= PHL_SND_TEST_F_GRP_EN_BF_FIX;
		psts = rtw_phl_sound_start(phl_info, (u8)role_idx, 0, 30,
					   sounding_flag);
		...
		role = rtw_phl_get_wrole_by_ridx(phl_info->phl_com, (u8)role_idx);
		if (NULL != role) {
			sounding_flag = PHL_SND_TEST_F_PASS_STS_CHK;
		...
		if (NULL != role) {
			...
			psts = rtw_phl_snd_add_grp(phl_info, role, 0, (u16 *)&macid, 2, true, true);
			psts = rtw_phl_snd_add_grp(phl_info, role, 1, (u16 *)&macid, 2, true, true);
			sounding_flag = PHL_SND_TEST_F_ONE_TIME|PHL_SND_TEST_F_PASS_STS_CHK;
...
static void _phl_set_trx_check_flag(struct phl_info_t *phl_info, u32 bitmap)
{
	...
	phl_com = phl_info->phl_com;
	/*clean, then set*/
	phl_com->check_flag.rxq_stuck_check = 0;
	phl_com->check_flag.achq_stuck_check = 0;
	phl_com->check_flag.mgntq_stuck_check = 0;
	phl_com->check_flag.hiq_stuck_check = 0;
	phl_com->check_flag.h2cq_stuck_check = 0;
	phl_com->check_flag.rx_cck_ofdm_cnt_check = 0;
	phl_com->check_flag.rx_data_cnt_check = 0;
	phl_com->check_flag.tx_cck_ofdm_cnt_check = 0;
