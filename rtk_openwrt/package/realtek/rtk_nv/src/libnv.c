...
#if defined(CONFIG_MTD_NAND) && !defined(CONFIG_MTD_NAND_RTK)
#define CONFIG_MTD_NAND_RTK
#endif

#if defined(CONFIG_MTD_SPI_NAND)
#define CONFIG_MTD_SPI_NAND_RTK
#endif
...
#ifdef HAVE_REDUND_ENV
#define HaveRedundEnv (1)
#else
#define HaveRedundEnv (0)
#endif
...
int
mtd_is_bad(int fd, mtd_info_t *mtd_info, uint32_t offset )
{
	int ret = -1;
	loff_t seek;
	if (offset > mtd_info->size) {
		printf("Check offset error! MTD size is %x, offset is %x\n", mtd_info->size, offset);
		return ret;
	}
	seek = offset;
...
static int
mtd_read_partition(int fd, mtd_info_t *mtd_info, unsigned char *buf, unsigned int size)
{
	unsigned int start = 0;
	ssize_t rlen = 0;
	int rc = 0;
	for ( start = 0; start < mtd_info->size && rlen < size; start += mtd_info->erasesize ) {
		rc = mtd_is_bad(fd, mtd_info, start);
		if ( rc > 0) {
			printf("INFO: Reading and Skip bad block in %x\n", start);
			rc = lseek(fd, start+mtd_info->erasesize, SEEK_SET);
			continue;
		} else if (rc < 0) {
			// ioctrl error or offset error
			return 0;
		}
		rlen += read(fd, buf, (size-start) > mtd_info->erasesize ? mtd_info->erasesize:(size-start));
	}
	return rlen;
}

static int
mtd_erase_partition(int fd, mtd_info_t *mtd_info)
{
	int ret = 0;
	erase_info_t ei;
	...
	for(ei.start = 0; ei.start < mtd_info->size; ei.start += mtd_info->erasesize) {
		ret = mtd_is_bad(fd, mtd_info, ei.start);
		if ( ret > 0) {
			printf("INFO: Erasing and Skip bad block in %x\n", ei.start);
			continue;
		} else if (ret < 0) {
			// ioctrl error or offset error
			return ret;
...
static int mtd_write_partition(int fd, mtd_info_t *mtd_info, const void *buf, unsigned int w_size)
{
	unsigned int start = 0;
	ssize_t wlen;
	ssize_t want_len;
	...
	for ( start = 0; start < mtd_info->size && write_len < w_size; start += mtd_info->erasesize ) {
		rc = mtd_is_bad(fd, mtd_info, start);
		if ( rc > 0) {
			printf("INFO: Writing and Skip bad block in %x\n", start);
			rc = lseek(fd, start+mtd_info->erasesize, SEEK_SET);
			continue;
		} else if ( rc < 0) {
			// ioctrl error or offset error
		...
		want_len = w_size - write_len >  mtd_info->erasesize ? mtd_info->erasesize : (w_size - write_len);
		wlen = write(fd, buf, want_len);
		if(wlen != want_len) {
			fprintf(stderr, "Try writing %ld bytes but only %ld is written\n", want_len, wlen);
			rc = -2;
			break;
		}
		write_len += wlen;
...
#if defined(CONFIG_MTD_NAND_RTK) || defined(CONFIG_MTD_SPI_NAND_RTK) || defined (CONFIG_MTD_SPI_NOR)
		rc = mtd_write(env_mtd_fd, &mtd_info, environment.image, uboot_env_size, 0);
#else
		rc = mtd_write_partition(env_mtd_fd, &mtd_info, environment.image, uboot_env_size);
...
#else
		rc = mtd_read_partition(env_mtd_fd, &mtd_info, environment.image, CONFIG_ENV_SIZE);
#endif /* CONFIG_MTD_NAND_RTK */
...
#if defined(HAVE_REDUND_ENV) && !defined(CONFIG_MTD_NAND_RTK) && !defined(CONFIG_MTD_SPI_NAND_RTK) && !defined (CONFIG_MTD_SPI_NOR)
...
#ifdef G3_ENV_SIZE
#define NAND_UBOOT_ENV_SIZE G3_ENV_SIZE
#elif  defined (CONFIG_MTD_SPI_NOR)
#ifdef CONFIG_RTK_SOC_RTL8198D
#define NAND_UBOOT_ENV_SIZE (64*1024)
#else
#define NAND_UBOOT_ENV_SIZE (8*1024)
#endif
#elif defined(CONFIG_MTD_NAND_RTK) || defined(CONFIG_MTD_SPI_NAND_RTK)
#ifdef CONFIG_RTK_SOC_RTL8198D
#define NAND_UBOOT_ENV_SIZE (128*1024)
#elif OTTO_ENV_SIZE
...
static int env_relocate_spec(void)
{
  ...
  #if defined(HAVE_REDUND_ENV)
	env_name = NV_ENV2_MTD;
  #else
	env_name = NV_ENV_MTD;
  #endif
	env_mtd_fd = mtd_get_partition_fd(env_name);
	if(env_mtd_fd < 0) {
		fprintf(stderr, "Error: cannot find partition named as %s\n", env_name);
		return -EIO;
	}
	...
	NV_DBG("CONFIG_ENV_SIZE=%u, ENV_SIZE=%u\n", CONFIG_ENV_SIZE, ENV_SIZE);
	if (!tmp_env1) {
	...
	crc_value = crc32(0, (uint8_t *)tmp_env1->data, ENV_SIZE);
	NV_DBG("crc_value =%u, %u, ENV_SIZE=%u\n", crc_value, tmp_env1->crc, ENV_SIZE);
	if ( crc_value == tmp_env1->crc)
		crc1_ok = 1;
