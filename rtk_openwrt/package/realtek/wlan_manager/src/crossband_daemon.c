...
static int crossband_set_mib(const char *interfacename, const char *mibname, char value)
{
    ...
    if(mib_chain_get_wlan((unsigned char *)interfacename,&WlanEnable_Entry,&vwlanidx)!=0)
    {
        close(skfd);
        return -1;
    }
...
static int get_env_info(const char *ifname, struct envinfo_data* pointer)
{
    int sock, cmd_id, vwlanidx=0;
    struct iwreq wrq;
    int ret = -1;
    int err;
	MIB_CE_MBSSIB_T WlanEnable_Entry;
	...
	memset(&WlanEnable_Entry,0,sizeof(MIB_CE_MBSSIB_T));
    if(mib_chain_get_wlan((unsigned char *)ifname,&WlanEnable_Entry,&vwlanidx)!=0)
    {
        close(sock);
        return -1;
    }
...
static int retrieve_drivermibValues( const char* interfacename, struct driver_mib_info* pointer)
{
    if(crossband_get_mib(interfacename, crossband_enable_mib, &(pointer->enable), 1) == -1 )
        goto error;
    if(crossband_get_mib(interfacename, pathready_mib, &(pointer->pathReady), 1) == -1 )
        goto error;
    if(crossband_get_mib(interfacename, assoc_mib, &(pointer->assoc), 1) == -1 )
        goto error;
    if(crossband_get_mib(interfacename, preferband_mib, &(pointer->preferband), 1) == -1 )
        goto error;
...
static int rtk_wlan_get_wlan_sta_num( char *interface, int *num )
{
    int skfd=0, cmd_id, vwlanidx=0;
    ...
    memset(&WlanEnable_Entry,0,sizeof(MIB_CE_MBSSIB_T));
    if(mib_chain_get_wlan((unsigned char *)interface,&WlanEnable_Entry,&vwlanidx)!=0)
    {
        close(skfd);
        return -1;
    }
...
//			retrieve_mibValues(wlan1_ifName, &mibValueOf5G);
		...
		rtk_wlan_get_wlan_sta_num(wlan0_vxd_ifName,&sta_num);
		rtk_wlan_get_wlan_sta_num(wlan1_vxd_ifName,&sta_num1);
		...
		retrieve_drivermibValues(wlan0_ifName, &wlan0_mib);
		retrieve_drivermibValues(wlan1_ifName, &wlan1_mib);
