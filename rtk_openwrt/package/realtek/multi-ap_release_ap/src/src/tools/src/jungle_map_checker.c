...
const char MAP_DHCPC_PID_FILE[] = "/var/udhcpc/udhcpc-br0.pid";

const char MAP_BRIF[] = "br0";
const char SYSTEMD_PID[] = "/var/run/systemd.pid";
...
pid_t find_pid_by_name(char *pidName)
{
	...
	while ((next = readdir(dir)) != NULL) {
		...
		sprintf(filename, "/proc/%d/status", pid_num);
...
uint8_t is_eth_connected(char *eth_interface_name)
{
	...
	if (-1 == access(file_name_buf, F_OK)) {
...
void append_br0_interfaces(char *interfaces)
{
	char buf_tmp[512];
...
void get_eth_interfaces(struct eth_interface_info *eth_interface)
{
	...
	append_br0_interfaces(interfaces);
...
int get_current_backhaul_radio (int *backhaul_sta_radio_index)
{
	...
	while(fgets(buf2,sizeof(buf2),fp)!=NULL)
	{
		...
		}else if(strstr(buf2,"network_type") && (strstr(buf2,"128") || strstr(buf2,"64"))){
			line_backhaul = line;
...
int dynamic_debug_enable()
{
	int  ret = 0;
	char debug_level = '0';
	if (-1 != access(DYNAMIC_DEBUG_FLAG, F_OK)) {
...
void trigger_dhcp_renew(){
	...
	snprintf(pid_file, sizeof(pid_file), "%s", (char*)MAP_DHCPC_PID_FILE);
...
void check_controller_ip_renew()
{
	...
	if(is_valid_ip(tmp_controller_ip)){
		if(!is_valid_ip(controller_ip)){
			...
			snprintf(controller_ip, sizeof(controller_ip), tmp_controller_ip);
		}else{
...
static void send_br_arp(){
	...
	char tmp2[20]={0};
	char tmp3[20]={0};
	_getInAddr(ifname, IP_ADDR, (void *)&inAddr);
	_getInAddr(ifname, SUBNET_MASK, (void *)&inAddr1);
	int i=0,j=0;
	int backhaul_sta_radio_index = -1;
	...
	if(0 == get_current_backhaul_radio(&backhaul_sta_radio_index)){
		...
		MAP_CHECKER_DBG("EZMESH[%s %d],backhaul_sta_radio_index:%d, ifname:%s\n",__FUNCTION__,__LINE__,backhaul_sta_radio_index,backhaul_ifname);
	}

	if(0 == getMacAddr(backhaul_ifname, backhaul_hwaddr)){
		memcpy(&sMAC[3], &backhaul_hwaddr[3], 3);
		MAP_CHECKER_DBG("EZMESH[%s %d],backhaul_hwaddr:%02x%02x%02x%02x%02x%02x, sMAC:%02x%02x%02x%02x%02x%02x\n",
			__FUNCTION__,__LINE__,
	...
	strcpy(tmp1, inet_ntoa(inAddr));
	strcpy(tmp2, inet_ntoa(inAddr1));
	strcpy(tmp3, inet_ntoa(requested_ip));
	...
	for(j=0; j<eth_info.eth_interface_nr; j++){
		...
		if(is_eth_connected(eth_info.eth_interface_names[j])){
			MAP_CHECKER_DBG("EZMESH[%s %d]: send arp packet on if:%s\n",__FUNCTION__,__LINE__,eth_info.eth_interface_names[j]);
			for(i=0; i<3; i++){
				_ezmesh_send_arp(requested_ip.s_addr, inAddr.s_addr, sMAC, eth_info.eth_interface_names[j]);
				usleep(100*1000);
			}
		}else{
			for(i=0; i<2; i++){
...
/*
	for(j =2; j<5; j++){
		...
		MAP_CHECKER_DBG("EZMESH[%s %d]: send arp packet on if:%s,sMAC:%02x%02x%02x%02x%02x%02x\n",__FUNCTION__,__LINE__,ifname_s,
			sMAC[0],sMAC[1],sMAC[2],sMAC[3],sMAC[4],sMAC[5]);
		for(i=0; i<3; i++){
...
int get_connected_eth_ifname(int eth_nr, char **eth_names, char *linked_eth_ifname)
{
	...
	for (i = 0; i < eth_nr; i++) {
		...
		if (is_connected) {
			strncat(linked_eth_ifname, (const char*)eth_names[i], strlen(eth_names[i]));
			ret = 1;
		}
	}
#if 0
...
int backhaul_link_detect(int *backhaul_link)
{
	...
	if (-1 != access(BACKHAUL_LINK_INFO, F_OK)) {
		...
		printf("ERROR (%s)%d access %s file error.\n", __FUNCTION__, __LINE__, BACKHAUL_LINK_INFO);
		ret = -1;
	}
	return ret;
}
...
int main(int argc, char *argv[])
{
	...
	while ((c = getopt(argc, argv, "vs:i:l:d:p:")) != -1) {
		switch (c) {
			case 'v': {
				char verbose_char = 'v';
				// Each time this flag appears, the verbose_level is incremented.
				strncat(log_level_string, &verbose_char, 1);
				break;
			}
			case 'p': {
				if (optarg) {
	...
	printf("[%s %d]:map_checker_debug on : %d\n",__FUNCTION__,__LINE__,map_checker_debug);
	...
	while (1) {
		...
		printf("ps for %s found, pid is %d \n", map_name, map_pid);
		map_alive = 1;

		// Enable bh bss after vlan is fully configured & daemon start
		if (strcmp(map_name, "map_controller") == 0){
			if(-1 != access("/tmp/map_bh_bss_off", F_OK)) {
				printf("re-enable backhaul bss after vlan configured!\n");
				sprintf(cmd_buf, "iwpriv wlan%d-vap0 set_mib func_off=0\n", detect_control.backhaul_sta_radio_index);
				printf("%s",cmd_buf);
				system(cmd_buf);
				remove("/tmp/map_bh_bss_off");
			}
		}
			...
			if (strcmp(map_name, "map_agent") == 0){
				...
				if(detect_control.backhaul_switch != 0){
					if (check_count % detect_control.check_interval == 0){
						backhaul_link_switch(eth_info, detect_control, &backhaul_info);
					}
				}
				if(map_checker_conf.auto_renew_ip == 1){
					if(check_count % ip_check_interval == 0){
						check_controller_ip_renew();
					}
				}
		...
		if (-1 != access("/tmp/map_checker_end.txt", F_OK)) {
			remove("/tmp/map_checker_end.txt");
			break;
		}
		...
		char cmd[64] = { 0 };
		sprintf(cmd, "%s -d%s -p%d", map_name, log_level_string, multiap_profile);
		system(cmd);
	}
