...
uint8_t is_interface_up(char *interface)
{
	...
	if (-1 == skfd) {
		printf("[openwrt_map_vlan_config][is_interface_up] socket() error!!");
		return 0;
	}
...
int obtain_vxd_interface_name(char *bridge_name, char *vxd_interface_name, size_t size)
{
	char  buf_tmp[512];
	...
	if (brctl) {
		while (fgets(buf_tmp, 512, brctl) != 0) {
			// check bridge name, only br-lan will set flag to 1
			ptr = buf_tmp;
			...
			if ((ptr = strstr(buf_tmp, "-vxd"))) {
				...
				printf("wlan%c-vxd found under bridge interface br-lan \n", *(ptr + 4));
				pclose(brctl);
				return atoi(ptr + 4);
			}
		}

		printf("No vxd interface found under bridge interface br-lan \n");
		pclose(brctl);
		return -1;
	}
...
int obtain_parent_profile(char *bridge_name)
{
	FILE *sta_info;
	char line[128];
	int parent_profile;
	...
	int vxd_number = obtain_vxd_interface_name(bridge_name, vxd_interface_name, sizeof(vxd_interface_name));
	...
	snprintf(sta_info_path, sizeof(sta_info_path), "/proc/net/rtk_wifi6/wlan%d-vxd/sta_info", vxd_number);
...
int main()
{
	...
	if (fp == NULL) {
		printf("[openwrt_map_vlan][main] Read VLAN_CONFIG_FILE fail!!\n");
		exit(1);
	}
	...
	parent_profile = obtain_parent_profile(map_brif);
	...
	for (i = 0; i < up_interface_nr; i++) {
		if (_rtk_get_is_ax_support(up_interfaces[i]))
			continue;
	...
	if (fp == NULL) {
		printf("[openwrt_map_vlan][main] Read VLAN_RESET_FILE fail!!\n");
		exit(1);
	}
	...
	fprintf(fp, "ebtables -F STP_BR_OUT_CHECK\n");
	fprintf(fp, "ebtables -D OUTPUT -d 01:80:C2:00:00:00 -j STP_BR_OUT_CHECK\n");
	...
	system("ebtables -F STP_BR_OUT_CHECK");
	system("ebtables -D OUTPUT -d 01:80:C2:00:00:00 -j STP_BR_OUT_CHECK");
	...
	if (1 == parent_profile) {
		...
		obtain_vxd_interface_name(map_brif, vxd_interface, sizeof(vxd_interface));
	...
	// Avoid stp blocking secondary vlan group
	printf("\n----- Drop secondary vlan group BPDU packets to avoid STP blocking interfaces -----\n\n");
	system("ebtables -A INPUT -d 01:80:C2:00:00:00 -j STP_BR_CHECK");
	system("ebtables -A OUTPUT -d 01:80:C2:00:00:00 -j STP_BR_OUT_CHECK");
	for (i = 0; i < global_vlan.vlan_group_nr; i++) {
		char               vlan_interface[64];
		for (j = 0; j < backhaul_interface_nr; j++) {
			if (global_vlan.primary_vid != global_vlan.vlan_groups[i].vid) {
				...
				printf(cmd_buf);
				system(cmd_buf);
				sprintf(cmd_buf, "ebtables -A STP_BR_OUT_CHECK --logical-out %s -o %s -j DROP\n", map_brif, vlan_interface);
				printf(cmd_buf);
				system(cmd_buf);
			}
