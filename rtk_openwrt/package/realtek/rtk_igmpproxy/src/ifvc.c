...
*/
{
	...
	if( (Sock = socket( AF_INET, SOCK_DGRAM, 0 )) < 0 )
		igmpproxy_log( LOG_ERR, errno, "RAW socket open" );
	 ...
	 */
	{
		...
		if( ioctl( Sock, SIOCGIFCONF, &IoCtlReq ) < 0 )
			igmpproxy_log( LOG_ERR, errno, "ioctl SIOCGIFCONF" );
			...
			*/
			{
				...
				if( ioctl( Sock, SIOCGIFFLAGS, &IfReq ) < 0 )
					igmpproxy_log( LOG_ERR, errno, "ioctl SIOCGIFFLAGS" );
				
				IfDescEp->Flags = IfReq.ifr_flags;
			}
				
			igmpproxy_log( LOG_DEBUG, 0, "buildIfVc: Interface %s Addr: %s, Flags: 0x%04x",
			IfDescEp->Name,
...
struct IfDesc *getIfByVif( const int vif )
/*
** Returns a pointer to the IfDesc of the interface 'vif'
**
** returns: - pointer to the IfDesc of the requested interface
**          - NULL if no interface 'vif' exists
**
*/
{
  struct IfDesc *Dp;
