...
#define ENABLE_WLAN0_MP 1
#else
#define ENABLE_WLAN1_MP 1
#endif
#else
#if defined(CPTCFG_RTL8192CD_MODULE) && defined(BAND_5G_ON_WLAN0)
#define ENABLE_WLAN1_MP 1
#else
#define ENABLE_WLAN0_MP 1
#endif
...
#if (defined(CONFIG_YUEME) || defined(CONFIG_CU_BASEON_YUEME)) && defined(CONFIG_DUALBAND_CONCURRENT)
           			...
           			struct iwreq *pwrq)    /* Fixed part of the request */
{
	memset(pwrq->ifr_name, 0, IFNAMSIZ);
  	strncpy(pwrq->ifr_name, ifname, IFNAMSIZ-1);	/* Set device name */
...
int valid_char_check(char * str){
	...
	for( i=0; i < strlen(str); i++){
		if( !((str[i] >= '0' && str[i] <= '9') || 
			...
			/* consider "||" and ">>" are invalid*/
			if( (str[i] == '|' && str[i+1] == '|') || 
				(str[i] == '>' && str[i+1] == '>')){
				invalid_idx = i;
				//printf("|| and >> are invalid\n");
				break;
			}

			/* there is a '|' in str but "grep" is not right after '|', consider that it is invalid at '|'.*/
			if( str[i] == '|' ){
				/* There would be some ' ' between | and "grep" */
				int j=i+1;
				while(str[j]==' ' && j < strlen(str)){
					j++;
				}
				if(memcmp(str+j, "grep", 4)){
					invalid_idx = i;
					break;
				}
	...
	//printf("[valid_char_check] invalid_idx: %d\n",invalid_idx);
...
int main(void) {
	...
	printf("MP AUTOMATION daemon (ver %s)\n", MP_DAEMON_VERSION);
	//Self reading flash!!!
	#if 	0
	if(!ReadSinguture())
	{
	...
	while (1) {
		...
		isshortmsg=0;
		invalid_idx = -1;
		// length should be - strlen(" > /tmp/MP.txt"), becuase buf would be strcat with " > /tmp/MP.txt"
		
		if ((numbytes = recvfrom(sockfd, buf, BUFLEN-100, 0,
		...
		//truncate the string after invalid char in buf[] 
		invalid_idx = valid_char_check(buf);		
		if(invalid_idx != -1){		
			memset(buf+invalid_idx, 0, strlen(buf+invalid_idx));
			//printf("invalid_idx: %d\n", invalid_idx);
		}
		...
		if (0){}
			...
			if ( (!memcmp(buf, "flash read", 10)) ){
				...
				if (fp) {
					...
					{	//fix read_reg bug
						...
						char strreadsyn[]="wlan0     read_syn:\n";
						...
						char strreadsyn1[]="wlan1     read_syn:\n";
						...
						if( strncmp(buf,strreadrf,strlen(strreadrf))==0 )
							get_read_reg_value( fp, buf, BUFLEN );
						if( strncmp(buf,strreadsyn,strlen(strreadsyn))==0 )
							get_read_reg_value( fp, buf, BUFLEN );
						...
						if( strncmp(buf,strreadsyn1,strlen(strreadsyn1))==0 ) {
							get_read_reg_value( fp, buf, BUFLEN );
						}
			...
			//ack to the client
				...
				snprintf(pre_result,BUFLEN, "%s > /tmp/MP.txt ok", buf);
			else {
...
#ifdef SWAP_WLAN_MIB_INDEX
			if(!memcmp(buf+10, "HW_WLAN", 7)){
				if(buf[17]=='0') buf[17] = '1';
				else if (buf[17]=='1') buf[17] = '0';
			}
#endif
			if(strlen(buf) > BUFLEN-12-16){ //the max strlen of %s is size - "flash gethw " - " > /tmp/MP.txt"
				printf("Recved command is too long to be copied, so it would be truncated\n");
			}
			snprintf(cmdWrap, BUFLEN-12-16, "flash gethw %s", buf+10); // " > /tmp/MP.txt" would occupy some space
			rwHW = 1;
			////strcat(buf, " > /tmp/MP.txt");
...
#endif
			if(strlen(buf) > BUFLEN-12){ //the max strlen of %s is size - "flash sethw "
				printf("Recved command is too long to be copied, so it would be truncated\n");
			}
			snprintf(cmdWrap, BUFLEN-12, "flash sethw %s", buf+10);// " > /tmp/MP.txt" would occupy some space
			rwHW = 1;
		...
		if(rwHW == 1){
			...
			if(!memcmp(buf, "flash get", 9) || !memcmp(buf, "flash set", 9) || !memcmp(buf, "flash read", 10) || !memcmp(buf, "iwpriv wlan", 11) || !memcmp(buf, "ifconfig wlan", 13) || !memcmp(buf, "cat", 3)) {
				system(buf);
				//for reset btm
				...
				printf("Unknown command for 11N_UDPserver!\n\n");
			}
		}
#endif				
		...
		if (memcmp(buf, "flash set", 9) != 0) {
			...
			#ifdef CONFIG_NO_WLAN_DRIVER
			char *offset2;
			if((offset2 = strstr(buf, "wlan1")) != NULL)
				offset2[4] = '0';
			#endif
