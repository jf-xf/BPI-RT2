...
return view.extend({
	...
	renderRules: function(data) {
		...
		m = new form.Map('firewall', _('Firewall - IP/Port Filtering'),
			_('IP/Port Filtering define policies for packets traveling between different zones, for example to reject traffic between certain hosts or to open WAN ports on the router.'));
		...
		s.filter = function(section_id) {
			return (uci.get('firewall', section_id, 'rtk_firewall') == 'ipport');
		};
		...
		s.handleAdd = function(ev) {
			...
			uci.set(config_name, section_id, 'rtk_firewall', 'ipport');
		...
		o = s.taboption('general', form.ListValue, 'dir', _('Direction'));
		o.modalonly = true;
		o.value('out', _('Outgoing'));
		o.value('in', _('Incoming'));
		o.write = function(section_id, value) {
			var config_name = this.uciconfig || this.map.config;

			if (value == 'out') {
				uci.set(config_name, section_id, 'dir', 'out');
				uci.set(config_name, section_id, 'src', 'lan');
				uci.set(config_name, section_id, 'dest', 'wan');
			}
			if (value == 'in') {
				uci.set(config_name, section_id, 'dir', 'in');
