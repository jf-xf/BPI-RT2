...
return view.extend({
	...
	renderRules: function(data) {
		...
		m = new form.Map('firewall', _('Firewall - ACL Rules'),
			_('ACL rules define policies for packets traveling between different zones, for example to reject traffic between certain hosts or to open WAN ports on the router.'));
		...
		s.filter = function(section_id) {			
			return ( uci.get('firewall', section_id, 'rtk_firewall') == 'acl');
		};
		...
		o = s.taboption('general', form.ListValue, 'proto', _('Service'));
		o.modalonly = true;
		o.value('ftp', _('FTP'));
		o.value('http', _('HTTP'));
		o.value('ssh', _('SSH'));
		...
		o.write = function(section_id, value) {
			...
			if (value == 'ftp') {
				uci.set(config_name, section_id, 'proto', 'tcp');
				uci.set(config_name, section_id, 'dest_port', '21');
			}
			if (value == 'http') {
				uci.set(config_name, section_id, 'proto', 'tcp');
				uci.set(config_name, section_id, 'dest_port', '80');
			}
			if (value == 'ssh') {
				uci.set(config_name, section_id, 'proto', 'tcp');
				uci.set(config_name, section_id, 'dest_port', '22');
			}
			if (value == 'icmp') {
				uci.set(config_name, section_id, 'proto', 'icmp');
				uci.set(config_name, section_id, 'dest_port', null);
			}
			uci.set(config_name, section_id, 'rtk_firewall', 'acl');
			//return true;
		};
		o.cfgvalue = function(section_id) {
			var config_name = this.uciconfig || this.map.config;
			var type, target;
			type = uci.get(config_name, section_id, 'dest_port');
			target = uci.get(config_name, section_id, 'target');
			...
			if (type == '21')
				...
				return 'ssh';
			else if (target) // Because Click "Add", option dest_port is null. It should return null. When click "Edit", option target have value but option dest_port is null. It should return value with "icmp".
				return 'icmp';
		};
		...
		o.nocreate = true;
		o.rmempty = false;
		//o.hideZone = 'INTERNET TR069 Voice nat';
		o.displayZone = 'lan wan';
		//o.optional = false
		//o.allowany = true;
		//o.allowlocal = 'src';
