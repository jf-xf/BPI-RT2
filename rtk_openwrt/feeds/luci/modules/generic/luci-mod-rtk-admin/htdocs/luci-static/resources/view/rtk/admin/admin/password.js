...
var callSessionDestroy = rpc.declare({
	object: 'session',
	method: 'destroy'
});
...
return view.extend({
	...
	render: function() {
		...
		o = s.option(form.ListValue, 'user', _('UserName'));
		o.value('root', 'root');
		o.value('user', 'user');
	...
	handleSave: function() {
		...
		return dom.callClassMethod(map, 'save').then(function() {
			...
			fs.exec('/etc/samba/smb.sh', [ formData.password.user,  formData.password.pw1 ]);
			return callSetPassword(formData.password.user, formData.password.pw1).then(function(success) {
				if (success) {
					ui.addNotification(null, E('p', _('The system password has been successfully changed.')), 'info');
					if (formData.password.user == 'root') {
						callSessionDestroy();
						location.reload();
					}
				}
