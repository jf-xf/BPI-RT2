...
return view.extend({
	...
	handleSaveLog: function(ev) {
		return fs.exec('/etc/admin/log.sh').then(function(res0) {
			return fs.exec('/bin/chmod', [ '666', '/www/log.tar.gz' ]).then(function(res1) {
				var form = E('form', {
					method: 'post',
					action: '/log.tar.gz',  // www/log.tar.gz
	...
	handleClearLog: function(ev) {
		if (!confirm(_('Do you really want to clear all system and kernel Log?')))
			return;

		fs.exec('/bin/dd', [ 'if=/dev/null', 'of=/www/system.log' ]);
		fs.exec('/bin/dmesg', [ '-c' ]);
	},
	...
	render: function(rpc_replies) {
		...
		*/
		...
		o = s.taboption('logging', form.Flag, 'logenabled', _('Enable'));
		o.modalonly = false;
		o.default = o.enabled;
		o.editable = true;
		...
		*/
		...
		o = s.taboption('logging', form.ListValue, 'displaylevel', _('Display Level'))
		o.value(8, _('Debug'))
		o.value(7, _('Info'))
		o.value(6, _('Notice'))
		o.value(5, _('Warning'))
		...
		o = s.taboption('logging', form.Button, 'save_log', _('Save Log to File'));
		o.inputstyle = 'action important';
		o.inputtitle = _('Save');
		o.onclick = this.handleSaveLog;

		o = s.taboption('logging', form.Button, 'clear_log', _('Clear Log'));
		o.inputstyle = 'action important';
		o.inputtitle = _('Reset');
		o.onclick = this.handleClearLog;
		...
		if (L.hasSystemFeature('sysntpd')) {
			...
			o = s.taboption('timesync', rtkwidgets.NetworkSelect, 'interface', _('WAN Interface'), _(''));
			o.ucisection = 'ntp';
			o.valueType="IPv4ORv6";
			//o.ipmodefilter="IPv4";
			o.ignoreBridge=1;
			o.nocreate = true;
