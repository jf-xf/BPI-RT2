...
return view.extend({
	...
	render: function(rpc_replies) {
		...
		m = new form.JSONMap(mapdata, _('Firmware Upgrade'), _('Upload image here to replace the running firmware.'));
		...
		if (has_sysupgrade) {
			o = s.option(form.Button, 'img', _('Image'));
			o.inputstyle = 'action important';
			o.inputtitle = _('Flash image...');
			o.onclick = L.bind(this.handleSysupgrade, this, storage_size, has_rootfs_data);
		}
		else {
			o = s.option(form.SectionValue, 'actions', form.NamedSection, 'actions', 'actions', _('Flash new firmware image error'),
