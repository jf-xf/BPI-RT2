...
return view.extend({
	...
	parseNeigh: function(s, networks, rtkWans, v6) {
		...
		for (var i = 0; i < lines.length; i++) {
			...
			if(net){
				...
				for(var idx=0; idx<rtkWans.length; idx++){
					var interfaces=L.toArray(rtkWans[idx].config.interface);
				...
				res.push([
					...
					found ? rtkNetwork.renderIfaceBadge(rtkWans[idx], null) : E('span', { 'class': 'ifacebadge' }, [ '%s'.format(flags.dev) ])
				]);
	...
	parseRoute: function(s, networks, rtkWans, v6) {
		var lines = s.trim().split(/\n/),
		    res = [];
	...
	*/
	...
	render: function(data) {
		...
		*/
		...
		cbi_update_table(neigh4tbl, this.parseNeigh(ip4neigh, networks, rtkWans, false),
			E('em', _('No entries available'))
		);
		cbi_update_table(route4tbl, this.parseRoute(ip4route, networks, rtkWans, false),
			E('em', _('No entries available'))
		);
		/*
		...
		*/
		cbi_update_table(neigh6tbl, this.parseNeigh(ip6neigh, networks, rtkWans, true),
			E('em', _('No entries available'))
		);
		cbi_update_table(route6tbl, this.parseRoute(ip6route, networks, rtkWans, true),
			E('em', _('No entries available'))
		);
		/*
		...
		*/
		var view = E([], [
			...
			E('div', {}, [
				E('div', { 'data-tab': 'ipv4routing', 'data-tab-title': _('IPv4') }, [
					E('h3', {}, [ _('IPv4 Neighbours(ARP)') ]),
					neigh4tbl,
				...
				E('div', { 'data-tab': 'ipv6routing', 'data-tab-title': _('IPv6') }, [
