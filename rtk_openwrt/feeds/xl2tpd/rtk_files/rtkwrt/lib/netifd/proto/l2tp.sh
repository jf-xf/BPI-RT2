...
proto_l2tp_setup() {
	local interface="$1"
	local optfile="/etc/ppp/options.$interface.xl2tpd"
...
EOF
	...
	xl2tpd-control add-lac ${interface} pppoptfile=${optfile} lns=${server} || {
		echo "xl2tpd-control: Add l2tp-$interface failed" >&2
		proto_setup_failed "$interface"
		exit 1
	}
	xl2tpd-control connect-lac ${interface} || {
...
proto_l2tp_teardown() {
	...
	if [ -p /var/run/xl2tpd/l2tp-control ]; then
		xl2tpd-control remove-lac ${interface} || {
