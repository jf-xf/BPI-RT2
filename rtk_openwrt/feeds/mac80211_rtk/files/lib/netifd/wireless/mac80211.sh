...
mac80211_hostapd_setup_base() {
	...
	[ "$auto_channel" -gt 0 ] && acs_num_scans=1
...
${acs_num_scans:+acs_num_scans=$acs_num_scans}
...
mac80211_hostapd_setup_bss() {
	...
	rtk_mac80211_fc_add_iface "$phy" "$ifname" "$band"
...
mac80211_setup_supplicant() {
	...
	[ "$enable" = 0 ] && {
		rtk_mac80211_fc_update_wlan_wanportmask "$ifname" "$enable" "$is_lan" "$is_wwan"
	...
	json_for_each_item check_network_item network
	rtk_mac80211_fc_update_wlan_wanportmask "$ifname" "$enable" "$is_lan" "$is_wwan"
...
mac80211_interface_cleanup() {
	...
	rtk_mac80211_fc_del_alliface "$phy" "${primary_ap}"
...
drv_mac80211_setup() {
	...
	coex_2g_40m_file="/proc/net/rtk_wifi6/$primary_ap/coex_2g_40m"
	echo "$auto_channel" > "$coex_2g_40m_file"
	[ -n "$hostapd_ctrl" ] && {
		local no_reload=1
	...
	rtk_mac80211_phy_oob_config $primary_ap
	for_each_interface "ap" rtk_mac80211_interface_oob_config "$phy"
	for_each_interface "ap" rtk_hostapd_wps_led_control_setup "$phy"
	for_each_interface "ap" rtk_hostapd_event_action_setup

	NEWSPLIST=
	NEWUMLIST=

	for_each_interface "sta adhoc mesh monitor" mac80211_setup_vif
	for_each_interface "sta" rtk_wpas_wps_led_control_setup "$phy"
	for_each_interface "sta" rtk_wpa_event_action_setup
	for_each_interface "sta" rtk_multiap_sta_set_bss_type
