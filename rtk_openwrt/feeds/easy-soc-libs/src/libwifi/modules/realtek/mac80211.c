	...
	if (fp) {
		while (fgets(line, sizeof(line),fp)) {
			...
			if (!strcmp(token, "tx_bytes")) {
				s->tx_bytes = strtoull(value, NULL, 10);
			}
			else if (!strcmp(token, "rx_bytes")) {
				s->rx_bytes = strtoull(value, NULL, 10);
			}
			else if (!strcmp(token, "tx_packets")) {
				s->tx_pkts = strtoull(value, NULL, 10);
			}
			else if (!strcmp(token, "rx_packets")) {
				s->rx_pkts = strtoull(value, NULL, 10);
			}
			else if (!strcmp(token, "rx_errors")) {
				s->rx_err_pkts = strtoull(value, NULL, 10);
			}
			else if (!strcmp(token, "tx_uc_packets")) {
				s->tx_ucast_pkts = strtoull(value, NULL, 10);
			}
			else if (!strcmp(token, "rx_uc_packets")) {
				s->rx_ucast_pkts = strtoull(value, NULL, 10);
			}
			else if (!strcmp(token, "tx_drops")) {
				s->tx_dropped_pkts = strtoull(value, NULL, 10);
			}
			else if (!strcmp(token, "rx_drops")) {
				s->rx_dropped_pkts = strtoull(value, NULL, 10);
			}
			else if (!strcmp(token, "tx_mc_packets")) {
				s->tx_mcast_pkts = strtoull(value, NULL, 10);
			}
			else if (!strcmp(token, "rx_mc_packets")) {
				s->rx_mcast_pkts = strtoull(value, NULL, 10);
			}
			else if (!strcmp(token, "tx_bc_packets")) {
				s->tx_bcast_pkts = strtoull(value, NULL, 10);
			}
			else if (!strcmp(token, "rx_bc_packets")) {
				s->rx_bcast_pkts = strtoull(value, NULL, 10);
			}
		}
		fclose(fp);
	}
...
static int iface_req_btm(const char *ifname, unsigned char *sta,
			 int bsss_nr, uint8_t *bsss, struct wifi_btmreq *b)
{
	struct bss_transition_params params = { 0 };
...
const struct wifi_driver rtk_driver = {
	.name = "wlan,phy",
