...
static int browseRoutingRouteInformationInterfaceSettingInst(struct dmctx *dmctx, DMNODE *parent_node, void *prev_data, char *prev_instance)
{
	...
	uci_foreach_sections("network", "interface", s) {
		char *proto = NULL, *ip6prefix = NULL;

		dmuci_get_value_by_section_string(s, "proto", &proto);
		dmuci_get_value_by_section_string(s, "ip6prefix", &ip6prefix);
		if ((proto && DM_LSTRCMP(proto, "dhcpv6") == 0) || (ip6prefix && ip6prefix[0] != '\0')) {
			json_object *res = NULL, *prefix_obj = NULL, *arrobj = NULL;

			char *if_name = section_name(s);
			dmubus_call("network.interface", "status", UBUS_ARGS{{"interface", if_name, String}}, 1, &res);
			dmjson_foreach_obj_in_array(res, arrobj, prefix_obj, i, 1, "ipv6-prefix") {
				inst = handle_instance_without_section(dmctx, parent_node, ++id);
				if (DM_LINK_INST_OBJ(dmctx, parent_node, (void *)prefix_obj, inst) == DM_STOP)
					break;
			}
		}
	}

	uci_foreach_sections("network", "globals", s) {
		char *ula_prefix = NULL;
		dmuci_get_value_by_section_string(s, "ula_prefix", &ula_prefix);
		if(ula_prefix && ula_prefix[0] != '\0'){
			char *pch = NULL, *spch = NULL;
			struct json_object *prefix = json_object_new_object();
			pch = strtok_r(ula_prefix, "/", &spch);
			if(pch){
				json_object_object_add(prefix, "address", json_object_new_string(pch));
				pch = strtok_r(NULL, "/", &spch);
				if(pch){
					json_object_object_add(prefix, "mask", json_object_new_int(DM_STRTOL(pch)));
				}
			}
			json_object_object_add(prefix, "class", json_object_new_string("lan"));

			inst = handle_instance_without_section(dmctx, parent_node, ++id);
			if (DM_LINK_INST_OBJ(dmctx, parent_node, (void *)prefix, inst) == DM_STOP)
				break;
		}
	}
...
static int get_RoutingRouteInformation_InterfaceSettingNumberOfEntries(char *refparam, struct dmctx *ctx, void *data, char *instance, char **value)
{
	...
	uci_foreach_sections("network", "interface", s) {
		...
		dmuci_get_value_by_section_string(s, "ip6prefix", &ip6prefix);
		if ((proto && DM_LSTRCMP(proto, "dhcpv6") == 0) || (ip6prefix && ip6prefix[0] != '\0')) {
			json_object *res = NULL, *prefixs = NULL;

			char *if_name = section_name(s);
			dmubus_call("network.interface", "status", UBUS_ARGS{{"interface", if_name, String}}, 1, &res);
			DM_ASSERT(res, *value = "0");
			json_object_object_get_ex(res, "ipv6-prefix", &prefixs);
			nbre_prefixs += (prefixs) ? json_object_array_length(prefixs) : 0;
		}
	}
	...
	dmasprintf(value, "%d", nbre_prefixs);
	return 0;
}
...
{
	char *prefix;
	json_object *res = NULL, *active_prefix_obj = NULL, *arrobj = NULL;
	int i = 0;
	char *if_name="lan";

	*value = "NoForwardingEntry";
	prefix = dmjson_get_value((struct json_object *)data, 1, "address");

	dmubus_call("network.interface", "status", UBUS_ARGS{{"interface", if_name, String}}, 1, &res);
	dmjson_foreach_obj_in_array(res, arrobj, active_prefix_obj, i, 1, "ipv6-prefix-assignment") {
		char *active_prefix = dmjson_get_value(active_prefix_obj, 1, "address");
		if(!DM_STRCASECMP(active_prefix, prefix)){
...
{
	char *iface = dmjson_get_value((struct json_object *)data, 1, "class");
	if(iface && iface[0] != '\0'){
...
{
	*value = dmjson_get_value((struct json_object *)data, 1, "address");
	return 0;
}
