...
static int set_user_password(char *refparam, struct dmctx *ctx, void *data, char *instance, char *value, int action)
{
	switch (action) {
		...
		case VALUESET:
			...
			if(strcmp(section_name(((struct dmmap_dup *)data)->config_section), "root") == 0) {
				char cmd_str[256]={0};
				int ret;

				sprintf(cmd_str, "/etc/samba/smb.sh %s %s", "root", value);
				ret = system(cmd_str);
			}
			break;
	}
...
{"User", &DMWRITE, NULL, NULL, NULL, browseUserInst, NULL, NULL, NULL, tUsersUserParams, NULL, BBFDM_BOTH, LIST_KEY{"Username", "Alias", NULL}, "2.0"},
{0}
};
...
{"Enable", &DMWRITE, DMT_BOOL, get_user_enable, NULL, BBFDM_BOTH, "2.0"},
//{"Username", &DMWRITE, DMT_STRING, get_user_username, set_user_username, BBFDM_BOTH, "2.0"},
{"Username", &DMWRITE, DMT_STRING, get_user_username, NULL, BBFDM_BOTH, "2.0"},
