...
int browseInterfaceStackInst(struct dmctx *dmctx, DMNODE *parent_node, void *prev_data, char *prev_instance)
{
	...
	uci_path_foreach_sections(bbfdm, DMMAP, "link", s) {
		...
		// The lower layer is Device.Optical.Interface.{i}.
		if (value == NULL && DM_STRNCMP(linker, "veip0", DM_STRLEN("veip0")) == 0) {
			char *vid = DM_STRCHR(linker, '.');
			if (vid) *vid = '\0';
			adm_entry_get_linker_param(dmctx, "Device.Optical.Interface.", linker, &value);
		}
	...
	uci_path_foreach_sections(bbfdm, "dmmap_bridge", "device", s) {
		...
		uci_path_foreach_option_eq(bbfdm, "dmmap_bridge_port", "bridge_port", "br_inst", br_inst, sd) {
			...
			// The lower layer is Device.Optical.Interface.{i}.
			if (!found && value == NULL && DM_STRNCMP(device, "veip0", DM_STRLEN("veip0")) == 0) {
				char *tag = DM_STRCHR(device, '.');
				if (tag) *tag = '\0';
				adm_entry_get_linker_param(dmctx, "Device.Optical.Interface.", device, &value);
			}

			if (!found && value != NULL) {
				DM_STRNCPY(package, "ports", sizeof(package));
				struct uci_section *optical_s = NULL;
				uci_path_foreach_option_eq(bbfdm, "dmmap", "optical", "name", "nas0", optical_s) {
					loweralias = get_alias_by_section("dmmap", "optical", optical_s, "optical_alias");
					dmuci_get_value_by_section_string(optical_s, "optical_instance", &bridge_port_inst);
					break;
				}
				found = 1;
			}
