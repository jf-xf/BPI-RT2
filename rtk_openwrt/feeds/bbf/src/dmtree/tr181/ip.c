	...
	uci_foreach_sections_safe("network", "interface", int_stmp, int_ss) {
		...
		if (DM_LSTRNCMP(proto, "ppp", 3) == 0) {
			...
			if (ppp_s) {
				...
				dmuci_get_value_by_section_string(ppp_s, "added_by_controller", &added_by_controller);

				if (added_by_controller && DM_LSTRCMP(added_by_controller, "1") == 0) {
		...
		/* Remove list interface related to this "IP.Interface." object */
		struct uci_section *device_s = get_dup_section_in_config_opt("network", "device", "name", device);
		if (device_s) {
			struct uci_list *uci_list = NULL;

			dmuci_get_value_by_section_list(device_s, "interface", &uci_list);
			if (value_exists_in_uci_list(uci_list, int_sec_name))
				dmuci_del_list_value_by_section(device_s, "interface", int_sec_name);
		}
	...
	switch (action)	{
		...
		case VALUESET:
			...
			if (DM_STRNCMP(value, eth_vlan_term, DM_STRLEN(eth_vlan_term)) == 0) {
				...
				char ipif_name[32] = {0};
				snprintf(ipif_name, sizeof(ipif_name), "%s_v4", section_name(vlan_ter_s));
				if (strcmp(section_name((struct uci_section *)data), ipif_name) != 0) {
					dmuci_rename_section_by_section((struct uci_section *)data, ipif_name);
				}

				struct uci_list *uci_list = NULL;
				dmuci_get_value_by_section_list(vlan_ter_s, "interface", &uci_list);
				if (!value_exists_in_uci_list(uci_list, ipif_name)) {
					dmuci_add_list_value_by_section(vlan_ter_s, "interface", ipif_name);
				}
				...
				dmuci_get_value_by_section_string(ppp_s, "device", &ppp_device);

				if (ppp_device && *ppp_device != '\0') {
					struct uci_section *s = NULL;
					uci_foreach_option_eq("network", "device", "name", ppp_device, s) {
						char ipif_name[32] = {0};
						snprintf(ipif_name, sizeof(ipif_name), "%s_v4", section_name(s));
						if (strcmp(section_name((struct uci_section *)data), ipif_name) != 0) {
							dmuci_rename_section_by_section((struct uci_section *)data, ipif_name);
						}

						struct uci_list *uci_list = NULL;
						dmuci_get_value_by_section_list(s, "interface", &uci_list);
						if (!value_exists_in_uci_list(uci_list, ipif_name)) {
							dmuci_add_list_value_by_section(s, "interface", ipif_name);
						}

						dmuci_set_value_by_section(s, "rtk_chmode", "PPPoE");

						// To avoid ppp interface name too long
						char pppname[32] = {0};
						snprintf(pppname, sizeof(pppname), "ppp%s", section_name(s));
						dmuci_set_value_by_section((struct uci_section *)data, "pppname", pppname);
						dmuci_set_value_by_section((struct uci_section *)data, "rtk_ppp_authmode", "auto");
						dmuci_set_value_by_section_bbfdm(ppp_s, "name", pppname);
...
static int get_IPInterface_Type(char *refparam, struct dmctx *ctx, void *data, char *instance, char **value)
{
	char *proto=NULL;
	dmuci_get_value_by_section_string(((struct uci_section *)data), "proto", &proto);
	if (DM_LSTRCMP(proto, "6in4") == 0
	|| DM_LSTRCMP(proto, "6rd") == 0
	|| DM_LSTRCMP(proto, "l2tp") == 0 || DM_LSTRCMP(proto, "l2tpv2") == 0
	|| DM_LSTRCMP(proto, "6to4") == 0
	|| DM_LSTRCMP(proto, "dslite") == 0
	) {
		*value = "Tunnel";
	}else{
