...
static int browseInterfaceSettingInst(struct dmctx *dmctx, DMNODE *parent_node, void *prev_data, char *prev_instance)
{
	...
	struct uci_section *zone_wan_s = get_dup_section_in_config_opt("firewall", "zone", "name", "wan");
	struct uci_section *zone_nat_s = get_dup_section_in_config_opt("firewall", "zone", "name", "nat");
	if (zone_wan_s && zone_nat_s) {
		struct uci_list *zone_wan_network_l = NULL;

		dmuci_get_value_by_section_list(zone_wan_s, "network", &zone_wan_network_l);

		if (zone_wan_network_l) {
			struct uci_element *zone_wan_network_e;

			uci_foreach_element(zone_wan_network_l, zone_wan_network_e) {
				struct uci_section *dmmap_section = NULL;
				if ((dmmap_section = get_dup_section_in_dmmap_opt("dmmap_firewall", "nat", "iface_name", zone_wan_network_e->name)) == NULL) {
					dmuci_add_section_bbfdm("dmmap_firewall", "nat", &dmmap_section);
					dmuci_set_value_by_section_bbfdm(dmmap_section, "iface_name", zone_wan_network_e->name);
				}

				/*
				 * Add system and dmmap sections to the list
				 */
				add_dmmap_config_dup_list(&dup_list, zone_nat_s, dmmap_section);
			}
		}
		 ...
		 */
		uci_path_foreach_sections_safe(bbfdm, "dmmap_firewall", "nat", stmp, s) {
			dmuci_get_value_by_section_string(s, "iface_name", &v);
			if (zone_wan_network_l) {
				if (!value_exists_in_uci_list(zone_wan_network_l, v)) {
...
static int browsePortMappingInst(struct dmctx *dmctx, DMNODE *parent_node, void *prev_data, char *prev_instance)
{
	...
	list_for_each_entry(p, &dup_list, list) {
		...
		dmuci_get_value_by_section_string(p->config_section, "name", &target);
		if (*target != '\0' && DM_LSTRCMP(target, "DMZ") == 0)
			continue;
...
{
	char *val;
	struct uci_list *zone_nat_network_l = NULL;

	dmuci_get_value_by_section_list(((struct dmmap_dup *)data)->config_section, "network", &zone_nat_network_l);
	dmuci_get_value_by_section_string(((struct dmmap_dup *)data)->dmmap_section, "iface_name", &val);
	if (val && value_exists_in_uci_list(zone_nat_network_l, val)) {
		*value = "1";
	}
	else
		*value = "0";
...
static int set_nat_interface_setting_enable(char *refparam, struct dmctx *ctx, void *data, char *instance, char *value, int action)
{
	...
	switch (action) {
		...
		case VALUESET:
			...
			if (b) {
				if (val && !value_exists_in_uci_list(zone_nat_network_l, val))
					dmuci_add_list_value_by_section(((struct dmmap_dup *)data)->config_section, "network", val);
			}
			else {
				if (val && value_exists_in_uci_list(zone_nat_network_l, val))
					dmuci_del_list_value_by_section(((struct dmmap_dup *)data)->config_section, "network", val);
			}
			return 0;
	}
	return 0;
...
static int get_nat_interface_setting_interface(char *refparam, struct dmctx *ctx, void *data, char *instance, char **value)
{
	char *val;
	dmuci_get_value_by_section_string(((struct dmmap_dup *)data)->dmmap_section, "iface_name", &val);
	adm_entry_get_linker_param(ctx, "Device.IP.Interface.", val, value);
	return 0;
}
...
{"InterfaceSetting", &DMWRITE, /*add_NAT_InterfaceSetting*/NULL, /*delete_NAT_InterfaceSetting*/NULL, NULL, browseInterfaceSettingInst, NULL, NULL, NULL, tNATInterfaceSettingParams, NULL, BBFDM_BOTH, LIST_KEY{"Interface", "Alias", NULL}, "2.0"},
...
{"Interface", &DMWRITE, DMT_STRING, get_nat_interface_setting_interface, /*set_nat_interface_setting_interface*/NULL, BBFDM_BOTH, "2.0"},
{0}
};
