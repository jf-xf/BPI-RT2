...
static int browseTemperatureSensorEntriesInst(struct dmctx *dmctx, DMNODE *parent_node, void *prev_data, char *prev_instance)
{
	json_object *res = NULL, *temp_obj = NULL, *arrobj = NULL;
	char *inst = NULL;
	int id = 0, i = 0;

	dmubus_call("rtk-rpc", "board_info", UBUS_ARGS{0}, 0, &res);
	dmjson_foreach_obj_in_array(res, arrobj, temp_obj, i, 1, "temperature") {
		inst = handle_instance_without_section(dmctx, parent_node, ++id);
		if (DM_LINK_INST_OBJ(dmctx, parent_node, (void *)temp_obj, inst) == DM_STOP)
			break;
	}
	return 0;
}
...
static int get_device_manufacturer(char *refparam, struct dmctx *ctx, void *data, char *instance, char **value)
{
	...
	dmubus_call("system", "board", UBUS_ARGS{0}, 0, &res);
	*value = dmjson_get_value(res, 2, "release", "menufaturer");
	return 0;
}
...
static int get_device_manufactureroui(char *refparam, struct dmctx *ctx, void *data, char *instance, char **value)
{
	...
	char *macAddr = dmmib_get_value("ELAN_MAC_ADDR");
	dmasprintf(value, "%.6s", macAddr);
#else
...
static int get_device_productclass(char *refparam, struct dmctx *ctx, void *data, char *instance, char **value)
{
	...
	dmubus_call("system", "board", UBUS_ARGS{0}, 0, &res);
	*value = dmjson_get_value(res, 2, "release", "product");
	return 0;
}
...
{
#ifdef BBF_VENDOR_REALTEK
	*value = dmmib_get_value("HW_SERIAL_NO");
#else
...
static int get_device_specversion(char *refparam, struct dmctx *ctx, void *data, char *instance, char **value)
{
	*value = "0";
#ifdef BBF_VENDOR_REALTEK
	*value = dmmib_get_value("RTK_DEVINFO_SPECVER");
#endif
	return 0;
}
...
static int get_device_softwareversion(char *refparam, struct dmctx *ctx, void *data, char *instance, char **value)
{
	...
	dmubus_call("system", "board", UBUS_ARGS{0}, 0, &res);
	*value = dmjson_get_value(res, 2, "release", "revision");
	return 0;
}
...
static int get_device_hardwareversion(char *refparam, struct dmctx *ctx, void *data, char *instance, char **value)
{
	...
	dmubus_call("system", "board", UBUS_ARGS{0}, 0, &res);
	*value = dmjson_get_value(res, 2, "release", "hwrevision");
	return 0;
}
...
static int get_device_modelname(char *refparam, struct dmctx *ctx, void *data, char *instance, char **value)
{
	...
	dmubus_call("system", "board", UBUS_ARGS{0}, 0, &res);
	*value = dmjson_get_value(res, 1, "model");
	return 0;
}
...
static int get_device_description(char *refparam, struct dmctx *ctx, void *data, char *instance, char **value)
{
	...
	dmubus_call("system", "board", UBUS_ARGS{0}, 0, &res);
	*value = dmjson_get_value(res, 2, "release", "description");
	return 0;
}
...
{
	json_object *res = NULL, *mem_obj = NULL;
	dmubus_call("system", "info", UBUS_ARGS{{}}, 0, &res);
	DM_ASSERT(res, *value = "0");
        mem_obj = dmjson_get_obj(res, 1, "memory");
        if (!mem_obj) return 0;
	dmasprintf(value, "%d", (DM_STRTOL(dmjson_get_value(mem_obj, 1, "total"))/1024));
	return 0;
}
...
static int get_memory_status_free(char* refparam, struct dmctx *ctx, void *data, char *instance, char **value)
{
	...
	DM_ASSERT(res, *value = "0");
        mem_obj = dmjson_get_obj(res, 1, "memory");
        if (!mem_obj) return 0;
	dmasprintf(value, "%d", (DM_STRTOL(dmjson_get_value(mem_obj, 1, "free"))/1024));
	return 0;
}
...
static int get_TemperatureSensor_number_of_entries(char* refparam, struct dmctx *ctx, void *data, char *instance, char **value)
{
	int cnt = get_number_of_entries(ctx, data, instance, browseTemperatureSensorEntriesInst);
	dmasprintf(value, "%d", cnt);
	return 0;
}

static int get_TempSensor_enable(char* refparam, struct dmctx *ctx, void *data, char *instance, char **value)
{
	*value = "1";
	return 0;
}

static int get_TempSensor_status(char* refparam, struct dmctx *ctx, void *data, char *instance, char **value)
{
	*value = "Enabled";
	return 0;
}

static int get_TempSensor_name(char* refparam, struct dmctx *ctx, void *data, char *instance, char **value)
{
	*value = dmjson_get_value((json_object *)data, 1, "description");
	return 0;
}

static int get_TempSensor_value(char* refparam, struct dmctx *ctx, void *data, char *instance, char **value)
{
	double temp = 0;
	int output = 0;
	char *tempStr = dmjson_get_value((json_object *)data, 1, "temperature");
	if(tempStr && tempStr[0] != '\0'){
		sscanf(tempStr, "%lf", &temp);
		output = temp;
	}
	dmasprintf(value, "%d", output);
	return 0;
}
...
{"TemperatureStatus", &DMREAD, NULL, NULL, NULL, NULL, NULL, NULL, tDeviceInfoTemperatureStatusObj, tDeviceInfoTemperatureStatusParams, NULL, BBFDM_BOTH, NULL, "2.0"},
...
{"SpecVersion", &DMREAD, DMT_STRING, get_device_specversion, NULL, BBFDM_BOTH, "2.0"},
...
/* OBJ, permission, addobj, delobj, checkdep, browseinstobj, nextdynamicobj, dynamicleaf, nextobj, leaf, linker, bbfdm_type, uniqueKeys, version*/
{"TemperatureSensor", &DMREAD, NULL, NULL, NULL, browseTemperatureSensorEntriesInst, NULL, NULL, NULL, tDeviceInfoTemperatureStatusTemperatureSensorParams, NULL, BBFDM_BOTH, LIST_KEY{"Name", NULL}, "2.0"},
{0}
};

DMLEAF tDeviceInfoTemperatureStatusParams[] = {
/* PARAM, permission, type, getvalue, setvalue, bbfdm_type, version*/
{"TemperatureSensorNumberOfEntries", &DMREAD, DMT_UNINT, get_TemperatureSensor_number_of_entries, NULL, BBFDM_BOTH, "2.0"},
{0}
};
...
/* PARAM, permission, type, getvalue, setvalue, bbfdm_type, version*/
{"Enable", &DMREAD, DMT_BOOL, get_TempSensor_enable, NULL, BBFDM_BOTH, "2.0"},
{"Status", &DMREAD, DMT_STRING, get_TempSensor_status, NULL, BBFDM_BOTH, "2.0"},
{"Name", &DMREAD, DMT_STRING, get_TempSensor_name, NULL, BBFDM_BOTH, "2.0"},
{"Value", &DMREAD, DMT_INT, get_TempSensor_value, NULL, BBFDM_BOTH, "2.0"},
{0}
};
