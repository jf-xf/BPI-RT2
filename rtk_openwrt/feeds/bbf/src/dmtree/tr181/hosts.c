...
/*#Device.Hosts.Host.{i}.AddressSource!UBUS:topology/hosts//hosts[@i-1].address_source*/
static int get_HostsHost_AddressSource(char *refparam, struct dmctx *ctx, void *data, char *instance, char **value)
{
	*value = dmjson_get_value((json_object *)data, 1, "address_source");
	return 0;
}

static int get_HostsHost_AssociatedDevice(char *refparam, struct dmctx *ctx, void *data, char *instance, char **value)
{
	char *device = dmjson_get_value((json_object *)data, 1, "device");
	adm_entry_get_linker_param(ctx, "Device.WiFi.AccessPoint.", device, value);
	if (!(*value) || (*value)[0] == 0){
		adm_entry_get_linker_param(ctx, "Device.WiFi.DataElements.Network.Device.", device, value);
	}else{
		char *assoc_mac = dmjson_get_value((json_object *)data, 1, "link_macaddr");
		adm_entry_get_linker_param(ctx, *value, assoc_mac, value);
	}
	return 0;
}
...
static int get_HostsHost_Active(char *refparam, struct dmctx *ctx, void *data, char *instance, char **value)
{
	...
	*value = (DM_LSTRCMP(*value, "false") == 0) ? "0" : "1";
	return 0;
}
...
{"AddressSource", &DMREAD, DMT_STRING, get_HostsHost_AddressSource, NULL, BBFDM_BOTH, "2.0"},
