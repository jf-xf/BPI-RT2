...
**************************************************************/
#ifdef USE_DNSMASQ
char *getInterfaceIPv4AddressBySectionName(char *name){
	...
	get_config_section_of_dmmap_section("network", "interface", name, &intf_s);
	if(intf_s){
		dmuci_get_value_by_section_string(intf_s, "proto", &proto);
		if (DM_LSTRCMP(proto, "none") == 0)
			return NULL;

		if_name = section_name(intf_s);
		if(DM_LSTRCMP(if_name, name) == 0){
...
char *getInterfaceSectionNameByIPv4Address(char *addr){
	struct uci_section *intf_s=NULL;
	char *ipaddr, *proto;
	char *if_name = NULL;
	json_object *res = NULL, *ipv4_obj = NULL;
...
#ifdef USE_DNSMASQ
static bool is_dhcp_relay_section_exist(char *dmmap_file, char *sec_name)
{
	struct uci_section *s = NULL;

	uci_path_foreach_option_eq(bbfdm, dmmap_file, "relay", "relay_name", sec_name, s) {
		return true;
	}
...
static void dmmap_synchronizeDHCPv4RelayForwarding(struct dmctx *dmctx, DMNODE *parent_node, void *prev_data, char *prev_instance)
{
	struct uci_section *s = NULL, *stmp = NULL;
#ifdef USE_DNSMASQ
	uci_path_foreach_sections_safe(bbfdm, "dmmap_dhcp_relay", "relay", stmp, s) {
		char *added_by_controller = NULL;
		struct uci_section *relay_s = NULL;
		char *relay_name = NULL;
		dmuci_get_value_by_section_string(s, "added_by_controller", &added_by_controller);
		//if (DM_LSTRCMP(added_by_controller, "1") == 0)
		//	continue;

		dmuci_get_value_by_section_string(s, "relay_name", &relay_name);
		if (DM_STRLEN(relay_name))
			get_config_section_of_dmmap_section("dhcp", "relay", relay_name, &relay_s);

		if (!relay_s)
			dmuci_delete_by_section(s, NULL, NULL);
	}

	uci_foreach_sections("dhcp", "relay", s) {
		struct uci_section *dmmap_s = NULL;
		char *local_addr = NULL;

		if (is_dhcp_relay_section_exist("dmmap_dhcp_relay", section_name(s)))
			continue;

		dmuci_add_section_bbfdm("dmmap_dhcp_relay", "relay", &dmmap_s);
		dmuci_set_value_by_section_bbfdm(dmmap_s, "relay_name", section_name(s));
		dmuci_get_value_by_section_string(s, "local_addr", &local_addr);
		if (DM_STRLEN(local_addr)){
			char *iface_name=getInterfaceSectionNameByIPv4Address(local_addr);
			if (DM_STRLEN(iface_name))
				dmuci_set_value_by_section_bbfdm(dmmap_s, "iface_name", iface_name);
			else
				dmuci_set_value_by_section_bbfdm(dmmap_s, "iface_name", "");
		}
	}
#else
...
static int get_DHCPv4ServerPool_Option_Value(struct uci_section *s, const char *option, char **value)
{
	...
	char *target_opt[3]={"dhcp_option", "dhcp_option_force", NULL};
	int i=0;

	for(i=0; target_opt[i] != NULL; i++){
		dmuci_get_value_by_section_list(s, target_opt[i], &dhcp_option);
		if (dhcp_option == NULL)
			continue;
...
#ifdef USE_DNSMASQ
	uci_path_foreach_sections(bbfdm, "dmmap_dhcp_relay", "relay", dmmap_s) {
		struct uci_section *relay_s = NULL;
		char *relay_name = NULL, *local_addr=NULL;

		dmuci_get_value_by_section_string(dmmap_s, "relay_name", &relay_name);
		if (DM_STRLEN(relay_name)){
			get_config_section_of_dmmap_section("dhcp", "relay", relay_name, &relay_s);
			if(relay_s){
				dmuci_get_value_by_section_string(relay_s, "local_addr", &local_addr);
				if (DM_STRLEN(local_addr)){
					char *iface_name=getInterfaceSectionNameByIPv4Address(local_addr);
					dmuci_set_value_by_section_bbfdm(dmmap_s, "iface_name", iface_name?iface_name:"");
				}else
					dmuci_set_value_by_section_bbfdm(dmmap_s, "iface_name", "");
			}
		}

		curr_dhcp_relay_args.iface_s = relay_s;
...
#ifdef USE_DNSMASQ
	struct uci_section *uci_sect = NULL;
	dmuci_add_section("dhcp", "relay", &uci_sect);
	dmuci_set_value_by_section(uci_sect, "disabled", "1");

	dmuci_add_section_bbfdm("dmmap_dhcp_relay", "relay", &dmmap_sect);
	dmuci_set_value_by_section(dmmap_sect, "disabled", "1");
	dmuci_set_value_by_section(dmmap_sect, "relay_name", section_name(uci_sect));
	dmuci_set_value_by_section(dmmap_sect, "iface_name", "lan");
	dmuci_set_value_by_section(dmmap_sect, "added_by_controller", "1");
	...
	switch (del_action) {
		...
		case DEL_ALL:
			...
			uci_path_foreach_sections_safe(bbfdm, "dmmap_dhcp_relay", "relay", stmp, s) {
				...
				if (relay_s)
					dmuci_delete_by_section(relay_s, NULL, NULL);
			}
#else
			uci_path_foreach_sections_safe(bbfdm, "dmmap_dhcp_relay", "interface", stmp, s) {
				struct uci_section *iface_s = NULL;
...
static int get_DHCPv4ServerPool_Enable(char *refparam, struct dmctx *ctx, void *data, char *instance, char **value)
{
	...
	*value = (*value && DM_LSTRCMP(*value, "server") == 0) ? "1" : "0";
	return 0;
}
...
{
	dmuci_get_value_by_section_string((((struct dhcp_args *)data)->sections)->config_section, "dhcpv4", value);
	*value = (*value && DM_LSTRCMP(*value, "server") == 0) ? "Enabled" : "Disabled";
	return 0;
}
...
/*#Device.DHCPv4.Server.Enable!UCI:dhcp/dnsmasq,@dnsmasq[0]/dhcpv4mode*/
static int get_DHCPv4Server_Enable(char *refparam, struct dmctx *ctx, void *data, char *instance, char **value)
{
	*value = dmuci_get_option_value_fallback_def("dhcp", "@dnsmasq[0]", "dhcpv4mode", "server");
	*value = (DM_LSTRCMP(*value, "server") == 0) ? "Enabled" : "Disabled";
	return 0;
}
...
static int set_DHCPv4Server_Enable(char *refparam, struct dmctx *ctx, void *data, char *instance, char *value, int action)
{
	...
	switch (action) {
		...
		case VALUESET:
			string_to_bool(value, &b);
			dmuci_set_value("dhcp", "@dnsmasq[0]", "dhcpv4mode", b ? "server" : "disabled");
			return 0;
	}
	return 0;
}
...
{
#ifdef USE_DNSMASQ
	/*#Device.DHCPv4.Relay.Enable!UCI:dhcp/dnsmasq,@dnsmasq[0]/dhcpv4mode*/
	*value = dmuci_get_option_value_fallback_def("dhcp", "@dnsmasq[0]", "dhcpv4mode", "server");
	*value = (DM_LSTRCMP(*value, "relay") == 0) ? "Enabled" : "Disabled";
#else
...
static int set_DHCPv4Relay_Enable(char *refparam, struct dmctx *ctx, void *data, char *instance, char *value, int action)
{
	...
	switch (action)	{
		...
		case VALUESET:
			string_to_bool(value, &b);
#ifdef USE_DNSMASQ
			dmuci_set_value("dhcp", "@dnsmasq[0]", "dhcpv4mode", b ? "relay" : "disabled");
#else
...
{
#ifdef USE_DNSMASQ
	get_DHCPv4Relay_Enable(refparam, ctx, data, instance, value);
	*value = (DM_LSTRCMP(*value, "1") == 0) ? "Enabled" : "Disabled";
#else
	char *path = "/etc/rc.d/*relayd";
	if (check_file(path))
...
#ifdef USE_DNSMASQ
/*#Device.DHCPv4.Relay.Forwarding.{i}.Enable!UCI:dhcp/relay,@i-1/disabled*/
#else
...
#ifdef USE_DNSMASQ
/*#Device.DHCPv4.Relay.Forwarding.{i}.Status!UCI:dhcp/relay,@i-1/disabled*/
#else
	...
	switch (action)	{
		...
		case VALUESET:
			...
			if (!linker || *linker == 0) {
				...
				if(dhcp_relay->iface_s)
					dmuci_set_value_by_section(dhcp_relay->iface_s, "local_addr", "");
			} else {
				char *ipaddr=getInterfaceIPv4AddressBySectionName(linker);
				dmuci_set_value_by_section(dhcp_relay->iface_s, "local_addr", ipaddr?ipaddr:"");
...
#ifdef USE_DNSMASQ
/*#Device.DHCPv4.Relay.Forwarding.{i}.DHCPServerIPAddress!UCI:dhcp/relay,@i-1/server_addr*/
static int get_DHCPv4RelayForwarding_DHCPServerIPAddress(char *refparam, struct dmctx *ctx, void *data, char *instance, char **value)
{
	...
	if (dhcp_relay_s) {
		dmuci_get_value_by_section_string(dhcp_relay_s, "server_addr", value);
	}
	return 0;
}
...
static int set_DHCPv4RelayForwarding_DHCPServerIPAddress(char *refparam, struct dmctx *ctx, void *data, char *instance, char *value, int action)
{
	switch (action)	{
		...
		case VALUESET:
			...
			if (dhcp_relay_s) {
				dmuci_set_value_by_section(dhcp_relay_s, "server_addr", value);
			}
			break;
	}
	return 0;
