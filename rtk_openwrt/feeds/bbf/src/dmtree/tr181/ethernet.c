...
* COMMON FUNCTIONS
**************************************************************/
static int get_new_device_veip_id(void)
{
	...
	uci_foreach_sections("network", "device", s) {
		s_name = section_name(s);
		if (DM_STRNCMP(s_name, "veip0_", DM_STRLEN("veip0_")) == 0) {
			int veip_id = 0;
			sscanf(s_name, "veip0_%d", &veip_id);
			if (veip_id > max) {
				max = veip_id;
			}
		}
...
static void add_new_dmmap_section(char *macaddr, char *iface_device, char *iface_name)
{
	...
	dmuci_set_value_by_section(dmmap, "is_eth", (!DM_LSTRNCMP(iface_device, "atm", 3) || !DM_LSTRNCMP(iface_device, "ptm", 3) || !DM_LSTRNCMP(iface_device, "veip", 4)) ? "0" : "1");
...
static void create_link(char *sec_name, char *mac_addr, char *iface_device)
{
...
#if 1 // interface name is veip0.x
	char *has_vid = NULL;
#else
	...
	uci_path_foreach_sections(bbfdm, DMMAP, "link", s) {
		...
		if (DM_STRCMP(iface_dev, link_device) == 0) {
			...
			dmuci_set_value_by_section(s, "section_name", sec_name);
			return;
		}
	}
...
static int dmmap_synchronizeEthernetLink(struct dmctx *dmctx, DMNODE *parent_node, void *prev_data, char *prev_instance)
{
	 ...
	 */
	uci_path_foreach_sections_safe(bbfdm, DMMAP, "link", stmp, s) {
		struct uci_section *ss = NULL;
		bool found = false;
		char *linker;
		char *dev;

		dmuci_get_value_by_section_string(s, "linker", &linker);
		// Create by add object method
		if (*linker != 0)
			continue;
		...
		uci_foreach_sections("network", "interface", ss) {
			...
			// Skip this interface section if its name is equal to loopback
			if (strcmp(section_name(ss), "loopback") == 0)
				continue;
...
static int addObjEthernetVLANTermination(char *refparam, struct dmctx *ctx, void *data, char **instance)
{
	...
	snprintf(device_name, sizeof(device_name), "veip0_%d", get_new_device_veip_id());

	// Add device section
	dmuci_add_section("network", "device", &s);
	dmuci_rename_section_by_section(s, device_name);
	dmuci_set_value_by_section(s, "type", "8021q");
	dmuci_set_value_by_section(s, "ipv4", "1");
	dmuci_set_value_by_section(s, "rtk_ipmode", "IPv4");
	dmuci_set_value_by_section(s, "rtk_chmode", "IPoE");
	dmuci_set_value_by_section(s, "ifname", "veip0");
	dmuci_set_value_by_section(s, "name", "veip0.1");
...
static int delObjEthernetVLANTermination(char *refparam, struct dmctx *ctx, void *data, char *instance, unsigned char del_action)
{
	...
	char *name, *type, *is_vlan, *ifname;

	switch (del_action) {
	case DEL_INST:
		// Update dmmap link device option
		...
		if (*ifname != '\0') {
			...
			uci_path_foreach_sections(bbfdm, "dmmap", "link", s_dev) {
				char *link_device;
				dmuci_get_value_by_section_string(s_dev, "device", &link_device);

				if (DM_STRCMP(link_device, old_name) == 0) {
					dmuci_set_value_by_section(s_dev, "device", ifname);
					break;
				}
			}

			// Remove device option in interface section
			uci_foreach_sections("network", "interface", s_dev) {
				char *link_device;
				dmuci_get_value_by_section_string(s_dev, "device", &link_device);
				if (DM_STRCMP(link_device, old_name) == 0) {
					dmuci_set_value_by_section(s_dev, "device", "");
					break;
				}
			}
		}
...
#if 1
	char out[2048] = {0};
	int i = 0, instance_int = atoi(instance);

	FILE *fp = popen("/sbin/diag rt_port get status port all", "r");
	if (fp == NULL) {
		*value = "0";
		return 0;
	}
	...
	while (fgets(out, sizeof(out) - 1, fp) != NULL) {
		...
		while (p) {
			...
			if (i == 0) {
				...
				if ((*endval != 0) || i_val != (instance_int - 1))
					break;
			}

			if (i == 1) {
				if (DM_LSTRCMP(p, "Down") == 0) {
					*value = "0";
					break;
				}
			}
			...
			if (i == 2) {
				if (DM_LSTRCMP(p, "10M") == 0) {
					*value = "10";
				}
				else if (DM_LSTRCMP(p, "100M") == 0) {
					*value = "100";
				}
				else if (DM_LSTRCMP(p, "1000M") == 0) {
					*value = "1000";
				}
				else {
					*value = "0";
...
static int get_EthernetLink_LowerLayers(char *refparam, struct dmctx *ctx, void *data, char *instance, char **value)
{
	...
	if ((*value)[0] == '\0') {
		...
		if (DM_STRNCMP(linker, "veip0", DM_STRLEN("veip0")) == 0) {
			char *vid = DM_STRCHR(linker, '.');
			if (vid) *vid = '\0';
			adm_entry_get_linker_param(ctx, "Device.Optical.Interface.", linker, value);
			if (*value != NULL && (*value)[0] != 0)
				return 0;
		}
...
#if 0 // LAN port's interface name is eth0.x
			char *vid = DM_STRCHR(linker, '.');
			if (vid) *vid = '\0';
#endif
...
static int set_EthernetLink_LowerLayers(char *refparam, struct dmctx *ctx, void *data, char *instance, char *value, int action)
{
	...
	char optical_link[32] = "Device.Optical.Interface.";
		...
		case VALUESET:
			...
			if (!link_linker || link_linker[0] == 0) {
				dmuci_set_value_by_section((struct uci_section *)data, "device", "");
					   ...
					   DM_STRNCMP(value, optical_link, DM_STRLEN(optical_link)) == 0) {
		...
		case VALUESET:
			...
			if (!vlan_linker || vlan_linker[0] == 0) {
				...
				if ((DM_LSTRCMP(type, "macvlan") == 0)) {
					...
					if (vid && *vid != '\0')
						snprintf(new_name, sizeof(new_name), "%s.%s", "veip0", vid);
					else
						snprintf(new_name, sizeof(new_name), "%s.1", "veip0");

					if (vid && *vid != '\0' && ethernet___name_exists_in_devices(new_name)) {
						// Skip the same section
						if (old_name && *old_name != 0 && DM_STRCMP(old_name, new_name) != 0)
							return -1;
					}
					...
					/* Write to the dmmap file */
					char link_inst[8] = {0};
					char *tok = strstr(value, eth_link);
					if (tok) {
						tok = tok + strlen(eth_link);
						char *tok_end = strchr(tok, '.');
						if (tok_end) *tok_end = '\0';
						snprintf(link_inst, sizeof(link_inst), "%s", tok);

						get_dmmap_section_of_config_section_eq("dmmap", "link", "link_instance", link_inst, &dmmap_s);
						dmuci_set_value_by_section_bbfdm(dmmap_s, "device", new_name);
					}
				}
