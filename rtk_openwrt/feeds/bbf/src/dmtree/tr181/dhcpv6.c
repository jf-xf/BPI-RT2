...
static int get_value_in_date_time_format(json_object *json_obj, char *option_name, char **value)
{
	*value = "0001-01-01T00:00:00Z";
	...
	if (option_value && *option_value != '\0' && DM_STRTOL(option_value) > 0) {
		...
		time_t curr_value = time(NULL);
		time_t sum = curr_value + time_value;
		char s_now[sizeof "AAAA-MM-JJTHH:MM:SSZ"];
		if (strftime(s_now, sizeof s_now, "%Y-%m-%dT%H:%M:%SZ", gmtime(&sum)) == 0)
			return -1;
...
static int browseDHCPv6ServerPoolClientInst(struct dmctx *dmctx, DMNODE *parent_node, void *prev_data, char *prev_instance)
{
	...
	json_object *res = NULL, *res1 = NULL, *dev_obj = NULL, *net_obj = NULL;
	...
	json_object *lease_arr = dmjson_get_obj(net_obj, 1, "leases");
	unsigned int lease_cnt = json_object_array_length(lease_arr);
	int idx=0,idx2=0, idx3=0;
	if(!lease_cnt) return 0;
	json_object *client_arr[lease_cnt];
	for(idx=0; idx < lease_cnt; idx++)
		client_arr[idx]=NULL;

	/* collect same duid to same json_object */
	for ( idx=0 ; idx < json_object_array_length(lease_arr) ; idx++ ) {
		int add=false;
		json_object *lease_obj = json_object_array_get_idx(lease_arr, idx);
		char *duid_target = dmjson_get_value(lease_obj, 1, "duid");
		for(idx2=0; idx2 < lease_cnt; idx2++){
			if(!client_arr[idx2])
				continue;
			for ( idx3=0 ; idx3 < json_object_array_length(client_arr[idx2]) ; idx3++ ) {
				json_object *cur_lease_obj = json_object_array_get_idx(client_arr[idx2], idx3);
				char *duid_cur = dmjson_get_value(cur_lease_obj, 1, "duid");
				if (DM_STRCASECMP(duid_target, duid_cur) == 0) {
					json_object_array_add(client_arr[idx2], lease_obj);
					add = true;
					break;
				}
			}
		...
		if(!add){
			for(idx2=0; idx2 < lease_cnt; idx2++){
				if(client_arr[idx2])
					continue;
				client_arr[idx2]=json_object_new_array();
				json_object_array_add(client_arr[idx2], lease_obj);
				break;
			}
		}
	...
	for(i=0; i < lease_cnt; i++){
		if(!client_arr[i])
			break;
		init_dhcpv6_client_args(&curr_dhcp_client_args, client_arr[i], NULL, i);
...
static int browseDHCPv6ServerPoolClientIPv6AddressInst(struct dmctx *dmctx, DMNODE *parent_node, void *prev_data, char *prev_instance)
{
	...
	char *inst = NULL;
	int i = 0, idx = 0;
	json_object *lease_arr = (json_object *)dhcpv6_serv_pool_client->client;

	for ( idx=0 ; idx < json_object_array_length(lease_arr) ; idx++ ) {
		json_object *lease_obj = json_object_array_get_idx(lease_arr, idx);
		while (1) {
			address_obj = dmjson_select_obj_in_array_idx(lease_obj, i, 1, "ipv6-addr");
			if (address_obj == NULL)
				break;
			init_dhcpv6_client_args(&curr_dhcv6_address_args, lease_obj, address_obj, i);
			i++;
...
static int browseDHCPv6ServerPoolClientIPv6PrefixInst(struct dmctx *dmctx, DMNODE *parent_node, void *prev_data, char *prev_instance)
{
	...
	for ( idx=0 ; idx < json_object_array_length(lease_arr) ; idx++ ) {
		json_object *lease_obj = json_object_array_get_idx(lease_arr, idx);
		while (1) {
			address_obj = dmjson_select_obj_in_array_idx(lease_obj, i, 1, "ipv6-prefix");
			if (address_obj == NULL)
				break;
			init_dhcpv6_client_args(&curr_dhcv6_address_args, lease_obj, address_obj, i);
			i++;
...
static int set_DHCPv6Server_Enable(char *refparam, struct dmctx *ctx, void *data, char *instance, char *value, int action)
{
	...
	switch (action) {
		...
		case VALUESET:
			...
			dmcmd("/etc/init.d/odhcpd", 1, b ? "start" : "stop");
			break;
	}
    return 0;
}
