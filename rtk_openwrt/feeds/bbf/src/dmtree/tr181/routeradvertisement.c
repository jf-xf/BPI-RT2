...
static int browseRouterAdvertisementInterfaceSettingOptionInst(struct dmctx *dmctx, DMNODE *parent_node, void *prev_data, char *prev_instance)
{
	...
	struct uci_section *dhcp_dmmap_s = NULL, *stmp = NULL;
	...
	uci_path_foreach_sections_safe(bbfdm, "dmmap_radv", "radv_option", stmp, dhcp_dmmap_s) {
		char *section_name = NULL;
		bool found=false;
		dmuci_get_value_by_section_string(dhcp_dmmap_s, "section_name", &section_name);
		if (DM_STRCMP(section_name, (section_name(dhcp_s))) != 0)
			continue;
		...
		if (dns_list != NULL) {
			struct uci_element *e = NULL;
			uci_foreach_element(dns_list, e) {
				if (DM_STRCMP(option_value, e->name) == 0){
					found=true;
					break;
				}
			}
		...
		if(!found){
			dmuci_delete_by_section(dhcp_dmmap_s, NULL, NULL);
		}
	}
...
static int get_RouterAdvertisementInterfaceSetting_Prefixes(char *refparam, struct dmctx *ctx, void *data, char *instance, char **value)
{
	...
	dmuci_get_value_by_section_string(((struct dmmap_dup *)data)->dmmap_section, "radv_intf_instance", &ip_inst);
